...
你可以直接在 Agent 构造函数中提供一些 [LLM 参数](../llm-parameters.md#llm-parameter-reference)，以自定义 LLM 的行为。
...
首先，通过使用 [`@Tool`](https://api.koog.ai/agents/agents-tools/ai.koog.agents.core.tools.annotations/-tool/index.html) 注解标记函数来创建工具：
...
在这个示例中，`askUser` 是一个工具，它通过向控制台输出内容并从中读取内容，帮助 Agent 与用户进行对话。
...
!!! tip
    ...
    你也可以定义这些参数并将其作为单独的配置对象进行传递，而不是将模型、温度、最大迭代次数和其他参数直接传递给 Agent 构造函数。
...
有关 Koog Agent 功能的更多信息，请参阅[功能概览](../features-overview.md)。
