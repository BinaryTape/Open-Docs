...
Koog 提供了将 agent 跟踪导出到 [Langfuse](https://langfuse.com/) 的内置支持，Langfuse 是一个用于 AI 应用程序可观测性与分析的平台。
通过 Langfuse 集成，您可以可视化、分析及调试 Koog agent 与 LLM、API 和其他组件的交互方式。
...
1. 创建 Langfuse 项目。按照 [在 Langfuse 中创建新项目](https://langfuse.com/docs/get-started#create-new-project-in-langfuse) 中的设置指南进行操作。
2. 获取 API 凭据。按照 [Langfuse API 密钥在哪里？](https://langfuse.com/faq/all/where-are-langfuse-api-keys) 中的说明检索您的 Langfuse `public key`（公钥）和 `secret key`（密钥）。
3. 将 Langfuse 主机、公钥和密钥传递给 Langfuse 导出器。
   可以通过将它们作为参数提供给 `addLangfuseExporter()` 函数，或者通过设置如下所示的环境变量来完成：
...
该导出器底层使用 `OtlpHttpSpanExporter` 将跟踪发送到 Langfuse 的 OpenTelemetry 端点。

### 示例：带有 Langfuse 跟踪功能的 agent
...
Langfuse 使用跟踪级属性来通过会话、环境、标签和其他元数据等功能增强可观测性。
`addLangfuseExporter` 函数支持 `traceAttributes` 参数，该参数接受 `CustomAttribute` 对象列表。

这些属性被添加到每条跟踪的根 `InvokeAgentSpan` span 中，从而启用 Langfuse 的高级功能。您可以传递 Langfuse 支持的任何属性 - 请参阅 [Langfuse OpenTelemetry 文档中的完整列表](https://langfuse.com/integrations/native/opentelemetry#trace-level-attributes)。

常用属性：
- **会话 (Sessions)** (`langfuse.session.id`)：将相关跟踪分组，以便进行聚合指标分析、成本分析和评分。
- **环境 (Environments)**：将生产环境跟踪与开发和暂存环境隔离，以便进行更清晰的分析。
...
启用后，Langfuse 导出器会捕获与 Koog 的常规 OpenTelemetry 集成相同的 span，包括：
...
Koog 还会捕获 Langfuse 显示 [Agent 图表](https://langfuse.com/docs/observability/features/agent-graphs) 所需的 span 属性。

出于安全原因，默认情况下 OpenTelemetry span 的某些内容会被掩码处理。
要使内容在 Langfuse 中可用，请使用 OpenTelemetry 配置中的 [setVerbose](opentelemetry-support.md#setverbose) 方法，并将其 `verbose` 参数设置为 `true`，如下所示：
...
### Langfuse 中没有出现跟踪
- 请仔细检查您的环境中是否设置了 `LANGFUSE_HOST`、`LANGFUSE_PUBLIC_KEY` 和 `LANGFUSE_SECRET_KEY`。
- 如果运行在私有化部署的 Langfuse 上，请确认您的应用程序环境可以访问 `LANGFUSE_HOST`。
- 验证公钥/密钥对是否属于正确的项目。