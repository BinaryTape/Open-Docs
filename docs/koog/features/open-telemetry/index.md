...
如果没有注册指标导出器，Koog 将回退到控制台 `LoggingMetricExporter`，以便在本地开发期间可以看到指标。
...
- 对于 `execute_tool` 操作，该值为 `koog`，因为工具执行是在进程内发生的，而不是针对第三方提供商执行。MCP 工具执行保留此值，并由于对应 span 上的单独 `mcp.*` 属性来呈现 MCP 特有细节，从而使工具指标保持低基数。
...
- `AIAgentError` 的子类 — 用于工具执行失败和工具验证失败。
- LLM 客户端或智能体运行时抛出的任何 `Throwable` — 用于通过智能体级失败挂钩呈现的 `text_completion` 失败。
