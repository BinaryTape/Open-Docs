...
| `categories`     | Map&lt;ModerationCategory, Boolean&gt;               | 是      |            | 审核类别到布尔值的映射，指示哪些类别被标记。 |
| `categoryScores` | Map&lt;ModerationCategory, Double&gt;                | 否       | emptyMap() | 审核类别到置信度分数（0.0 到 1.0）的映射。                          |
| `categoryAppliedInputTypes` | Map&lt;ModerationCategory, List&lt;InputType&gt;&gt; | 否       | emptyMap()           | 指示哪些输入类型（`TEXT` 或 `IMAGE`）触发了每个类别的映射。                |
...
Ollama 的审核格式方法与 OpenAI 的方法有很大不同。
