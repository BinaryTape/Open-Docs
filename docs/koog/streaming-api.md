...
Koog 的**流式传输 API** 允许您以 Kotlin 中的 `Flow<StreamFrame>` 或 Java 中的 `Flow.Publisher<StreamFrame>` 形式**增量地消耗 LLM 输出**。
您的代码无需等待完整响应，而是可以：
...
工具调用: " + toolCall.getName()
                    ...
                    System.out.println("
[END] 原因=" + end.getFinishReason());
                }
                // 处理其他类型的帧（TextComplete、ToolCallDelta 等）
            }
