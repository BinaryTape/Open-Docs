...
??? tip "Nightly 构建"
    ...
    来自 develop 分支的 Nightly 构建发布在 [JetBrains Grazie Maven](https://packages.jetbrains.team/maven/p/grazi/grazie-platform-public) 仓库中。
    
    要使用 Nightly 构建，请将以下仓库添加到您的构建配置中：
    `https://packages.jetbrains.team/maven/p/grazi/grazie-platform-public`。
    
    然后将您的 Koog 依赖项更新为所需的 Nightly 版本。Nightly 版本遵循以下模式：
    `[next-major-version]-develop-[date]-[time]`。
    
    您可以在[此处](https://packages.jetbrains.team/maven/p/grazi/grazie-platform-public/ai/koog/koog-agents/)浏览可用的 Nightly 构建。

## 设置 API 密钥

Koog 需要来自[受支持的 LLM 提供商](../llm-providers.md)的 API 密钥或本地运行的 LLM。
...
## 创建您的第一个 Koog 智能体 (Agent)

=== "OpenAI"

    以下示例通过 OpenAI API 使用 [`GPT-4o`](https://platform.openai.com/docs/models/gpt-4o) 模型创建并运行一个简单的 Koog 智能体。
...
=== "Anthropic"
    ...
    以下示例通过 Anthropic API 使用 [`Claude Opus 4.1`](https://www.anthropic.com/news/claude-opus-4-1) 模型创建并运行一个简单的 Koog 智能体。
...
=== "Google"
    ...
    以下示例通过 Gemini API 使用 [`Gemini 2.5 Pro`](https://cloud.google.com/vertex-ai/generative-ai/docs/models/gemini/2-5-pro) 模型创建并运行一个简单的 Koog 智能体。
...
=== "OpenRouter"
    ...
    以下示例通过 OpenRouter API 使用 [`GPT-4o`](https://openrouter.ai/openai/gpt-4o) 模型创建并运行一个简单的 Koog 智能体。
...
=== "Bedrock"
    ...
    以下示例通过 Bedrock API 使用 [`Claude Sonnet 4.5`](https://www.anthropic.com/news/claude-sonnet-4-5) 模型创建并运行一个简单的 Koog 智能体。
...
=== "Mistral"
    ...
    以下示例通过 Mistral AI API 使用 [`Mistral Medium 3.1`](https://docs.mistral.ai/models/mistral-medium-3-1-25-08) 模型创建并运行一个简单的 Koog 智能体。
...
- [构建具有记忆功能的聊天智能体](chat-agent.md) —— 创建一个能记住先前消息的对话智能体
- 详细了解如何[构建智能体](../agents/index.md)