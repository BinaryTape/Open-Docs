...
Koog 提供了一种使用 `AIAgentStorage` 存储和传递数据的方式。这是一个键值存储系统，旨在作为一种类型安全的方式在不同节点或子图之间传递数据。
...
该存储使用类型化的键系统，以确保在存储和检索数据时的类型安全性：

- `AIAgentStorageKey<T>`：一个代表用于标识和访问数据的存储键的数据类。以下是 `AIAgentStorageKey` 类的主要特性：
    - 泛型类型形参 `T` 指定了与此键关联的数据类型，从而确保类型安全性。
    - 每个键都有一个 `name` 属性，这是一个唯一代表该存储键的字符串标识符。
