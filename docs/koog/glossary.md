...
- **提示词 (Prompt)**：提供给 LLM 的对话历史记录，由来自用户、助手和系统的消息组成。

- **系统提示词 (System prompt)**：提供给智能体的指令，用于引导其行为、定义其角色并提供执行任务所需的关键信息。

- **上下文 (Context)**：LLM 发生交互的环境，可以访问对话历史记录和工具。

- **LLM 会话 (LLM session)**：与 LLM 交互的一种结构化方式，包括对话历史记录、可用工具以及发出请求的方法。
...
- **子图 (Subgraph)**：智能体策略中一个自包含的处理单元，拥有自己的一组工具、上下文和职责。有关子图操作的信息既可以封装在子图内部，也可以使用 AgentMemory 功能在子图之间传输。

## 工具 (Tools)

- **工具 (Tool)**：智能体可以用来执行特定任务或访问外部系统的函数。智能体了解可用工具及其参数，但不知道其实现细节。

- **工具调用 (Tool call)**：来自 LLM 的请求，旨在使用提供的参数运行特定工具。其功能类似于函数调用。

- **工具描述符 (Tool descriptor)**：工具的元数据，包括其名称、描述和参数。
