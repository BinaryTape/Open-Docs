...
* 使用 **LLM 客户端**直接与特定提供商交互。
  每个客户端都实现了 `LLMClient` 接口，负责处理提供商的认证、
  请求格式化和响应解析。
  ...
  * 使用 **提示词执行器**，它是一个更高级别的抽象，封装了一个或多个 LLM 客户端，
    ...
    关于详细信息，请参见 [提示词执行器](prompts/llm-clients.md)。

提示词执行器在单个或多个 LLM 客户端之上提供了一个更高级别的层。
它管理客户端的生命周期，并公开一个跨提供商的统一接口。
在多提供商设置中，它可以在提供商之间路由请求，并可在核心请求需要时，选择回退到指定的客户端。您可以创建自己的执行器，也可以使用预定义的执行器——包括单提供商和多提供商选项。
