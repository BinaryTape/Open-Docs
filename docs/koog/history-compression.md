...
要了解更多信息和具体示例，请参阅[历史压缩节点](nodes-and-components.md#history-compression-node)。
...
=== "Java"
    ...
    // 收到助手消息时从 sendToolResult 到 finish 的边
    graph.edge(AIAgentEdge.builder()
        .from(sendToolResult)
        .to(graph.nodeFinish)
        .onIsInstance(Message.Assistant.class)
...
这是默认策略，它将整个历史记录压缩为一条总结目前已取得成就的 TLDR 消息。
