...
策略图是 Koog 框架中智能体工作流的核心。它们定义了智能体如何处理输入、与工具交互以及生成输出。策略图由通过边连接的节点组成，执行流程由条件决定。
...
| onTextMessage       | 当 LLM 以文本消息形式响应时匹配的条件。 |
...
对于需要并行执行多个工具的工作流，可以使用 `nodeExecuteToolsAndGetResults` 节点，并设置 `parallel = true`：
