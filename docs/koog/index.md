...
Koog 是一个开源的 JetBrains 框架，用于构建专门为 JVM 生态系统设计的 AI 智能体。
它为 Kotlin 和 Java 开发者提供了一流的开发体验，具有符合语言习惯的、类型安全的 Kotlin DSL 以及流式构建器风格的 Java API。
...
-   :material-shield-check-outline:{ .lg .middle } [**模块版本管理**](module-versioning.md)
...
-   :material-list-status:{ .lg .middle } [**规划者智能体**](agents/planner-agents/index.md) <span class="beta-badge" title="Beta — API may change">beta</span>
...
-   :material-tools:{ .lg .middle } [**工具 (Tools)**](tools/index.md)

    ---

    使智能体能够与外部数据源和服务进行交互

-   :material-toy-brick-outline:{ .lg .middle } [**功能 (Features)**](features/index.md)
...
-   :material-database-search:{ .lg .middle } [**知识检索**](embeddings.md) <span class="beta-badge" title="Beta — API may change">beta</span>

    ---

    使用[向量嵌入](embeddings.md)和 [RAG](retrieval-augmented-generation.md) 在对话中保留和检索知识

-   :material-timeline-text:{ .lg .middle } [**跟踪 (Tracing)**](features/tracing.md)

    ---

    通过详细的、可配置的跟踪来调试和监控智能体执行

-   :material-timeline-text:{ .lg .middle } [**长期记忆**](features/long-term-memory.md) <span class="beta-badge" title="Beta — API may change">beta</span>
...
-   :material-puzzle:{ .lg .middle } [**模型上下文协议 (MCP)**](model-context-protocol.md) <span class="beta-badge" title="Beta — API may change">beta</span>
...
-   :material-lan:{ .lg .middle } [**A2A 协议**](a2a/index.md) <span class="beta-badge" title="Beta — API may change">beta</span>
