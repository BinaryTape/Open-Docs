...
LLM 客户端旨在与 LLM 提供商直接交互。每个客户端都实现了 [`LLMClient`](https://api.koog.ai/prompt/prompt-executor/prompt-executor-clients/ai.koog.prompt.executor.clients/-l-l-m-client/index.html) 接口，该接口提供了用于执行 prompt 和流式传输响应的方法。
...
| [Amazon Bedrock](https://aws.amazon.com/bedrock/)   | [BedrockLLMClient](https://api.koog.ai/prompt/prompt-executor/prompt-executor-clients/prompt-executor-bedrock-client/ai.koog.prompt.executor.clients.bedrock/-bedrock-l-l-m-client/index.html)              | ✓                | ✓         | -                | ✓    | ✓[^2]      | -                                       | 仅限 JVM 的 AWS SDK 客户端，支持多种模型系列。                                                                              |
| [Mistral](https://mistral.ai/)                      | [MistralAILLMClient](https://api.koog.ai/prompt/prompt-executor/prompt-executor-clients/prompt-executor-mistralai-client/ai.koog.prompt.executor.clients.mistralai/-mistral-a-i-l-l-m-client/index.html)    | ✓                | ✓         | ✓                | ✓          | ✓[^3]      | ✓                                       | 与 OpenAI 兼容的客户端。                                                                                                    |
...
你可以通过使用 `executeMultipleChoices()` 方法，在单次调用中请求模型的多个替代响应。它还需要在要执行的 prompt 中额外指定 [`numberOfChoices`](prompt-creation/index.md#prompt-parameters) LLM 参数。
...
!!! tip
    你也可以通过向 prompt 中添加 [`numberOfChoices`](prompt-creation/index.md#prompt-parameters) LLM 参数来请求多重选择。
