...
### 自动配置附件 {id="auto-configured-attachments"}
...
在 Kotlin 中，`+` 运算符可将文本内容与附件一起添加到用户消息中。在 Java 中，请使用 `ContentPartsBuilder` 的 `text()` 方法。

### 自定义配置附件 {id="custom-configured-attachments"}
