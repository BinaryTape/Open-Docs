...
KSP는 Kotlin 컴파일러 플러그인이므로 Kotlin 컴파일러와 함께 실행해야 합니다. 파일들을 다운로드하고 압축을 해제하세요.
...
이는 Gradle로 실행할 때 `kotlin-compiler-embeddable`과 함께 사용하도록 설계된 `symbol-processing-%kspSupportedKotlinVersion%-%kspVersion%.jar`와는 다릅니다.
커맨드 라인 `kotlinc`에는 `symbol-processing-cmdline-%kspSupportedKotlinVersion%-%kspVersion%.jar`가 필요합니다.
