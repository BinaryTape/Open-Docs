...
`ksp(...)` 구성은 프로세서를 애플리케이션 소스에만 적용합니다. 테스트 소스를 처리하려면 `kspTest(...)` 구성을 사용하여 프로세서를 추가하세요.

> `ksp(...)` 구성은 단일 플랫폼 프로젝트에서만 사용할 수 있습니다.
> 개별 Kotlin 멀티플랫폼(Kotlin Multiplatform) 타겟 및 컴파일에 대해 프로세서를 구성하는 방법을 알아보려면 
> [KSP와 Kotlin 멀티플랫폼](ksp-multiplatform.md)을 참조하세요.
> 
{style="note"}
