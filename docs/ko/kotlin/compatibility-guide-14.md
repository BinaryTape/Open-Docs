...
### infix 연산자와 ConcurrentHashMap의 예기치 않은 동작 {id="unexpected-behavior-with-in-infix-operator-and-concurrenthashmap"}
...
### 공개 인라인 멤버 내에서 protected 멤버에 대한 접근 금지 {id="prohibit-access-to-protected-members-inside-public-inline-members"}
...
### 암시적 수신 객체가 있는 호출에서의 계약 (Contracts) {id="contracts-on-calls-with-implicit-receivers"}
...
### 부동 소수점 수 비교의 일관성 없는 동작 {id="inconsistent-behavior-of-floating-point-number-comparisons"}
...
### 제네릭 람다의 마지막 표현식에 스마트 캐스트가 적용되지 않음 {id="no-smart-cast-on-the-last-expression-in-a-generic-lambda"}
...
### 결과값을 Unit으로 강제 변환할 때 람다 인자의 순서에 의존하지 않음 {id="do-not-depend-on-the-order-of-lambda-arguments-to-coerce-result-to-unit"}
...
### 로우 타입과 정수 리터럴 타입 사이의 잘못된 공통 상위 타입으로 인한 불안정한 코드 {id="wrong-common-supertype-between-raw-and-integer-literal-type-leads-to-unsound-code"}
...
### 여러 동일한 타입 변수가 서로 다른 타입으로 인스턴스화되어 발생하는 타입 안전성 문제 {id="type-safety-problem-because-several-equal-type-variables-are-instantiated-with-a-different-types"}
...
### 교차 타입에 대한 잘못된 하위 타입 지정으로 인한 타입 안전성 문제 {id="type-safety-problem-because-of-incorrect-subtyping-for-intersection-types"}
...
### 람다 내부의 빈 when 표현식에 타입 불일치 에러가 발생하지 않음 {id="no-type-mismatch-with-an-empty-when-expression-inside-lambda"}
...
### 가능한 반환 값 중 하나에 정수 리터럴이 포함된 조기 반환 람다에 대해 반환 타입 Any가 추론됨 {id="return-type-any-inferred-for-lambda-with-early-return-with-integer-literal-in-one-of-possible-return-values"}
...
### 재귀 타입을 사용하는 스타 프로젝션의 적절한 캡처 {id="proper-capturing-of-star-projections-with-recursive-types"}
...
### 불완전한 타입과 유연한 타입이 섞인 공통 상위 타입 계산 시 잘못된 결과 발생 {id="common-supertype-calculation-with-non-proper-type-and-flexible-one-leads-to-incorrect-results"}
...
### nullable 타입 인자에 대한 캡처 변환 부족으로 인한 타입 안전성 문제 {id="type-safety-problem-because-of-lack-of-captured-conversion-against-nullable-type-argument"}
...
### unchecked 캐스트 후 공변 타입에 대한 교차 타입 유지 {id="preserve-intersection-type-for-covariant-types-after-unchecked-cast"}
...
### this 표현식 사용으로 인해 빌더 추론에서 타입 변수가 유출됨 {id="type-variable-leaks-from-builder-inference-because-of-using-this-expression"}
...
### nullable 타입 인자를 가진 반공변 타입에 대한 잘못된 오버로드 해소 {id="wrong-overload-resolution-for-contravariant-types-with-nullable-type-arguments"}
...
### 중첩되지 않은 재귀적 제약 조건이 있는 빌더 추론 {id="builder-inference-with-non-nested-recursive-constraints"}
...
### 성급한 타입 변수 고정으로 인한 모순된 제약 시스템 발생 {id="eager-type-variable-fixation-leads-to-a-contradictory-constraint-system"}
...
### open 함수에서 tailrec 수정자 사용 금지 {id="prohibit-tailrec-modifier-on-open-functions"}
...
### 컴패니언 객체의 INSTANCE 필드가 컴패니언 객체 클래스 자체보다 더 높은 가시성을 가짐 {id="the-instance-field-of-a-companion-object-more-visible-than-the-companion-object-class-itself"}
...
### return 이전에 삽입된 외부 finally 블록이 finally가 없는 내부 try 블록의 catch 간격에서 제외되지 않음 {id="outer-finally-block-inserted-before-return-is-not-excluded-from-the-catch-interval-of-the-inner-try-block-without-finally"}
...
### 공변 및 제네릭 특화 오버라이드의 반환 타입 위치에서 인라인 클래스의 박싱된 버전 사용 {id="use-the-boxed-version-of-an-inline-class-in-return-type-position-for-covariant-and-generic-specialized-overrides"}
...
### Kotlin 인터페이스로의 위임 사용 시 JVM 바이트코드에서 체크 예외를 선언하지 않음 {id="do-not-declare-checked-exceptions-in-jvm-bytecode-when-using-delegation-to-kotlin-interfaces"}
...
### 단일 가변 인자 매개변수를 가진 메서드에 대한 시그니처 다형성 호출의 동작 변경 (인자를 다른 배열로 래핑하지 않도록 함) {id="changed-behavior-of-signature-polymorphic-calls-to-methods-with-a-single-vararg-parameter-to-avoid-wrapping-the-argument-into-another-array"}
...
### KClass가 제네릭 매개변수로 사용될 때 애너테이션 내의 잘못된 제네릭 시그니처 수정 {id="incorrect-generic-signature-in-annotations-when-kclass-is-used-as-a-generic-parameter"}
...
### 시그니처 다형성 호출에서 스프레드 연산자 금지 {id="forbid-spread-operator-in-signature-polymorphic-calls"}
...
### tail-recursive 최적화 함수의 기본값 초기화 순서 변경 {id="change-initialization-order-of-default-values-for-tail-recursive-optimized-functions"}
...
### non-const val에 대해 ConstantValue 속성을 생성하지 않음 {id="do-not-generate-constantvalue-attribute-for-non-const-vals"}
...
### open 메서드에서 @JvmOverloads로 생성된 오버로드 함수는 final이어야 함 {id="generated-overloads-for-jvmoverloads-on-open-methods-should-be-final"}
...
### kotlin.Result를 반환하는 람다가 이제 언박싱된 값 대신 박싱된 값을 반환함 {id="lambdas-returning-kotlin-result-now-return-boxed-value-instead-of-unboxed"}
...
### null 체크 예외 통합 {id="unify-exceptions-from-null-checks"}
...
### 배열/리스트 연산(contains, indexOf, lastIndexOf)에서의 부동 소수점 비교: IEEE 754 또는 전체 순서 (total order) {id="comparing-floating-point-values-in-array-list-operations-contains-indexof-lastindexof-ieee-754-or-total-order"}
...
### appendln을 지양하고 appendLine을 권장 {id="deprecate-appendln-in-favor-of-appendline"}
...
### 부동 소수점 타입을 Short 및 Byte로 변환하는 기능 지원 중단 {id="deprecate-conversions-of-floating-point-types-to-short-and-byte"}
...
### Regex.findAll에서 잘못된 startIndex 입력 시 즉시 실패 (Fail fast) {id="fail-fast-in-regex-findall-on-an-invalid-startindex"}
...
### 지원 중단된 kotlin.coroutines.experimental 제거 {id="remove-deprecated-kotlin-coroutines-experimental"}
...
### 지원 중단된 mod 연산자 제거 {id="remove-deprecated-mod-operator"}
...
### Throwable.addSuppressed 멤버를 숨기고 확장 함수를 우선함 {id="hide-throwable-addsuppressed-member-and-prefer-extension-instead"}
...
### capitalize가 이중음자(digraphs)를 타이틀 케이스로 변환해야 함 {id="capitalize-should-convert-digraphs-to-title-case"}
...
### Windows에서 구분 기호 문자가 포함된 컴파일러 인자는 큰따옴표로 전달해야 함 {id="compiler-arguments-with-delimiter-characters-must-be-passed-in-double-quotes-on-windows"}
...
### KAPT: 프로퍼티에 대한 합성 $annotations() 메서드의 이름이 변경됨 {id="kapt-names-of-synthetic-annotations-methods-for-properties-have-changed"}
