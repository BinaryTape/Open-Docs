...
### Kotlin/JS 유스케이스 {id="kotlin-js-use-cases"}
...
#### JavaScript/TypeScript 코드베이스와 비즈니스 로직 공유 {id="share-business-logic-with-a-javascript-typescript-codebase"}
...
#### 코드를 공유하지 않고 코틀린으로 웹 앱 구축 {id="build-web-apps-with-kotlin-without-sharing-the-code"}
...
### Kotlin/Wasm 유스케이스 {id="kotlin-wasm-use-cases"}

로직과 UI를 여러 플랫폼에서 모두 공유하려는 경우 Kotlin/Wasm을 사용하세요.

#### Compose 멀티플랫폼으로 크로스 플랫폼 앱 구축 {id="build-cross-platform-apps-with-compose-multiplatform"}
...
## 웹 접근 방식 선택 {id="choose-your-web-approach"}
...
## 웹 타겟을 위한 호환성 모드 {id="compatibility-mode-for-web-targets"}
