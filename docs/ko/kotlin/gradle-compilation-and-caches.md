...
* [Kotlin 컴파일러 폴백(Fallback) 전략](#kotlin-compiler-fallback-strategy)
...
_Kotlin 컴파일러 실행 전략_은 Kotlin 컴파일러가 실행되는 위치와 각 경우에 증분 컴파일이 지원되는지 여부를 정의합니다.

세 가지 컴파일러 실행 전략이 있습니다:

| 전략 | Kotlin 컴파일러 실행 위치 | 증분 컴파일 | 기타 특징 및 참고 사항 |
|----------------|--------------------------------------------|-------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Kotlin daemon  | 자체 데몬 프로세스 내부 | 예 | _기본 전략이며 가장 빠름_. 서로 다른 Gradle 데몬과 여러 병렬 컴파일 간에 공유될 수 있음. |
| In process     | Gradle 데몬 프로세스 내부 | 아니요 | Gradle 데몬과 힙(heap)을 공유할 수 있음. "In process" 실행 전략은 "Daemon" 실행 전략보다 _느림_. 각 [워커(worker)](https://docs.gradle.org/current/userguide/worker_api.html)는 각 컴파일에 대해 별도의 Kotlin 컴파일러 클래스로더를 생성함. |
| Out of process | 각 컴파일에 대한 별도 프로세스 내부 | 아니요 | 가장 느린 실행 전략. "In process"와 유사하지만, 각 컴파일에 대해 Gradle 워커 내에 별도의 Java 프로세스를 추가로 생성함. |

Kotlin 컴파일러 실행 전략을 정의하기 위해 다음 프로퍼티 중 하나를 사용할 수 있습니다:
* `kotlin.compiler.execution.strategy` Gradle 프로퍼티.
* `compilerExecutionStrategy` 컴파일 태스크 프로퍼티.

태스크 프로퍼티인 `compilerExecutionStrategy`가 Gradle 프로퍼티인 `kotlin.compiler.execution.strategy`보다 우선순위가 높습니다.

`kotlin.compiler.execution.strategy` 프로퍼티에 사용할 수 있는 값은 다음과 같습니다:
1. `daemon` (기본값)
2. `in-process`
3. `out-of-process`

`gradle.properties`에서 `kotlin.compiler.execution.strategy` Gradle 프로퍼티를 사용하세요:
...
빌드 스크립트에서 `compilerExecutionStrategy` 태스크 프로퍼티를 사용하세요:
...
Kotlin 컴파일러의 폴백(Fallback) 전략은 데몬이 어떤 이유로 실패할 경우 Kotlin 데몬 외부에서 컴파일을 실행하는 것입니다.
Gradle 데몬이 켜져 있으면 컴파일러는 ["In process" 전략](#defining-kotlin-compiler-execution-strategy)을 사용합니다.
Gradle 데몬이 꺼져 있으면 컴파일러는 "Out of process" 전략을 사용합니다.

이러한 폴백이 발생하면 Gradle 빌드 출력에 다음과 같은 경고 줄이 표시됩니다:
...
하지만 다른 전략으로의 조용한 폴백은 많은 시스템 리소스를 소모하거나 비결정적인(non-deterministic) 빌드로 이어질 수 있습니다.
이에 대한 자세한 내용은 [YouTrack 이슈](https://youtrack.jetbrains.com/issue/KT-48843/Add-ability-to-disable-Kotlin-daemon-fallback-strategy)를 참조하세요.
이를 방지하기 위해 `kotlin.daemon.useFallbackStrategy`라는 Gradle 프로퍼티가 있으며, 기본값은 `true`입니다.
이 값이 `false`이면 데몬의 시작 또는 통신 문제 발생 시 빌드가 실패합니다. `gradle.properties`에 이 프로퍼티를 선언하세요:

```none
kotlin.daemon.useFallbackStrategy=false
```

Kotlin 컴파일 태스크에도 `useDaemonFallbackStrategy` 프로퍼티가 있으며, 두 프로퍼티를 모두 사용할 경우 태스크 프로퍼티가 Gradle 프로퍼티보다 우선합니다.
...
컴파일을 실행하기에 메모리가 부족한 경우 로그에서 관련 메시지를 확인할 수 있습니다.
...
빌드 보고서에는 프로젝트에 사용된 Kotlin 버전도 표시됩니다. 또한 Kotlin 1.9.0부터는 [Gradle 빌드 스캔](https://scans.gradle.org/)에서 코드를 컴파일하는 데 어떤 컴파일러가 사용되었는지 확인할 수 있습니다.
