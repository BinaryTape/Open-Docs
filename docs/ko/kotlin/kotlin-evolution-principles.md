...
## 실용적인 진화의 원칙 {id="principles-of-pragmatic-evolution"}
...
### 컴파일러 버그 처리하기 {id="dealing-with-compiler-bugs"}
...
## 언어 기능 제공 {id="language-feature-delivery"}
...
### 안정화 전 기능 (Pre-stable features) {id="pre-stable-features"}
...
* **KEEP 토론 (KEEP discussion)**. 기능이 언어에 추가되어야 한다는 점이 상당히 확실합니다. 우리는 _KEEP_(Kotlin Evolution and Enhancement Process)이라고 불리는 문서에 동기, 유스케이스, 디자인 및 기타 중요한 세부 사항을 제공하는 것을 목표로 합니다. 사용자들이 KEEP에 제공된 모든 정보를 논의하는 데 집중하여 피드백을 주기를 기대합니다.
...
### 다양한 컴포넌트의 상태 {id="status-of-different-components"}
...
### 호환성 옵션 {id="compatibility-options"}
...
### 바이너리 형식의 진화 {id="evolving-the-binary-format"}
...
#### Kotlin klib 바이너리 {id="kotlin-klib-binaries"}
