...
ABI 덤프가 `maven-publish` 플러그인에 의해 발행된 아티팩트를 정확하게 반영하도록 하려면, `build.gradle.kts` 파일에 다음 내용을 추가하세요.
...
> Kotlin/Android 프로젝트 및 Android 타겟이 있는 멀티플랫폼 프로젝트는 JAR 파일을 발행하지 않으므로, 이 기능은 해당 프로젝트에 적용되지 않습니다.
> 
{style="warning"}