...
Kotlin 2.4.0부터 JVM용 Kotlin 표준 라이브러리는 각 릴리스 라인에 대해 18개월의 지원 기간을 갖습니다. 언어 릴리스(2._x_._0_) 및 이어지는 툴링 릴리스(2._x_._20_)는 동일한 릴리스 라인(2._x_)에 속합니다. JVM용 표준 라이브러리에서 보안 취약점을 처리하는 방법에 대한 자세한 내용은 [표준 라이브러리 보안 지원](releases.md#standard-library-security-support)을 참고하세요.
