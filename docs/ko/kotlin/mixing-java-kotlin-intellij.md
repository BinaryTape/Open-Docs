...
3. `<build><plugins>` 섹션에 Kotlin 플러그인을 추가합니다:
    ...
    * `<extensions>true</extensions>`는 Maven이 Kotlin 플러그인을 빌드 수명 주기(lifecycle)에 통합할 수 있게 합니다.
    * 사용자 정의 실행 단계(Custom execution phases)를 통해 Kotlin 플러그인이 Kotlin을 먼저 컴파일한 다음 Java를 컴파일하도록 합니다.
    * 구성된 `sourceDirs` 디렉터리를 통해 Kotlin과 Java 코드가 서로를 참조할 수 있습니다.
    * 확장이 포함된 Kotlin Maven 플러그인을 사용할 때는 `<build><pluginManagement>` 섹션에 별도의 `maven-compiler-plugin`이 필요하지 않습니다.
...
Kotlin 플러그인에는 Java 파일을 Kotlin으로 자동으로 변환해 주는 Java to Kotlin 변환기(*J2K*)가 포함되어 있습니다. 파일에서 J2K를 사용하려면, 해당 파일의 컨텍스트 메뉴 또는 IntelliJ IDEA의 **Code** 메뉴에서 **Convert Java File to Kotlin File**을 클릭하세요.
