...
### 데이터 클래스 copy 함수의 가시성을 생성자와 동일하게 변경 {id="data-class-copy-function-to-have-the-same-visibility-as-constructor"}
...
### 컨텍스트 리시버를 컨텍스트 파라미터로 단계적 교체 {id="phased-replacement-of-context-receivers-with-context-parameters"}
...
### 기본 타겟 계층 구조에서 소스 세트용 정적 접근자 제공 {id="static-accessors-for-source-sets-from-the-default-target-hierarchy"}
...
### Kotlin 멀티플랫폼 Gradle 플러그인과 Gradle Java 플러그인 간의 호환성 중단 예고 {id="deprecated-compatibility-with-kotlin-multiplatform-gradle-plugin-and-gradle-java-plugins"}
...
### 가비지 컬렉터의 동시 마킹(Concurrent marking) {id="concurrent-marking-in-garbage-collector"}
...
### 비트코드 임베딩(bitcode embedding) 지원 제거 {id="support-for-bitcode-embedding-removed"}
...
### signposts를 이용한 GC 성능 모니터링 변경 사항 {id="changes-to-gc-performance-monitoring-with-signposts"}
...
### 메인 스레드 이외의 스레드에서 Swift/Objective-C로부터 Kotlin 서스펜딩 함수 호출 가능 {id="ability-to-call-kotlin-suspending-functions-from-swift-objective-c-on-non-main-threads"}
...
### 기본 내보내기(default export) 사용 시 에러 발생 {id="error-in-default-export-usage"}
...
### ExperimentalWasmDsl 어노테이션의 새로운 위치 {id="new-location-of-experimentalwasmdsl-annotation"}
...
### JavaScript에서 Kotlin 정적 멤버 사용 지원 {id="support-for-using-kotlin-static-members-in-javascript"}
...
### JavaScript에서 Kotlin 컬렉션 생성 기능 {id="ability-to-create-kotlin-collections-from-javascript"}
...
### JVM 히스토리 파일 기반의 증분 컴파일 지원 중단 {id="deprecated-incremental-compilation-based-on-jvm-history-files"}
...
### 프로젝트 간 JVM 아티팩트를 클래스 파일로 공유하는 옵션 {id="option-to-share-jvm-artifacts-between-projects-as-class-files"}
...
### Kotlin Gradle 플러그인과 java-test-fixtures 플러그인의 의존성 동작 일치 {id="aligned-dependency-behavior-of-kotlin-gradle-plugin-with-java-test-fixtures-plugin"}
...
### 컴파일 태스크에 아티팩트 의존성이 누락된 희귀 사례에 대한 태스크 의존성 추가 {id="added-task-dependency-for-rare-cases-when-the-compile-task-lacks-one-on-an-artifact"}
...
### 2.0.0에서 도입된 불필요한 리컴포지션 문제 수정 {id="fix-for-the-unnecessary-recompositions-issue-introduced-in-2-0-0"}
...
### 컴파일러 옵션 설정의 새로운 방식 {id="new-way-to-configure-compiler-options"}
...
### 강력한 건너뛰기 모드(Strong skipping mode) 기본 활성화 {id="strong-skipping-mode-enabled-by-default"}
...
### 컴포지션 추적 마커(Composition trace markers) 기본 활성화 {id="composition-trace-markers-enabled-by-default"}
...
### 비건너뛰기 그룹 최적화(Non-skipping group optimizations) {id="non-skipping-group-optimizations"}
...
### 추상 컴포저블 함수의 기본 파라미터 지원 {id="support-for-default-parameters-in-abstract-composable-functions"}
...
### 공통 Kotlin 표준 라이브러리의 UUID 지원 {id="support-for-uuids-in-the-common-kotlin-standard-library"}
...
### HexFormat의 minLength 지원 {id="support-for-minlength-in-hexformat"}
...
### Base64 디코더 동작 변경 {id="changes-to-the-base64-s-decoder-behavior"}
...
#### Base64 디코더에 패딩 필수 적용 {id="the-base64-decoder-now-requires-padding"}

이제 Base64 인코더는 기본적으로 패딩을 추가하며, 디코더는 패딩을 요구하고 디코딩 시 0이 아닌 패드 비트를 금지합니다.

#### 패딩 설정을 위한 withPadding 함수 {id="withpadding-function-for-padding-configuration"}
...
## Kotlin 2.0.20 설치 {id="install-kotlin-2-0-20"}
