...
Java 선언의 타입은 Kotlin에서 특정한 방식으로 처리되며 이를 *플랫폼 타입(platform types)*이라고 부릅니다. 이러한 타입에 대해서는 null 체크가 완화되므로, 안전성 보장은 Java와 동일합니다([아래](#mapped-types)를 참조하세요).
...
`strict`, `warn`, `ignore` 값은 `MigrationStatus`의 값과 동일한 의미를 가지며, `strict` 모드만이 Kotlin에서 보이는 어노테이션된 선언의 타입에 영향을 줍니다.
