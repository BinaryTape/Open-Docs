...
모든 것이 예상대로 작동하는지 확인하려면, [IDE에서](native-get-started.md#build-and-run-the-application) `runDebugExecutable<YourTargetName>` Gradle 태스크를 실행하거나 터미널에서 콘솔 명령을 사용하세요. 이 예시의 경우 다음과 같습니다:
