...
* 소스 호환성을 깨지 않고도 바이너리 호환성을 깨뜨릴 수 있으며, 그 반대의 경우도 가능합니다.
...
바이너리 호환성을 유지하기 위해, 함수에 새 파라미터를 추가하려는 경우, 기본 인자를 받는 단일 함수 대신 여러 오버로드를 수동으로 생성해야 합니다. 위 예시에서, 이는 `Int` 파라미터를 받기를 원하는 경우 별도의 `fib` 함수를 생성하는 것을 의미합니다.
...
JVM용 Kotlin 코드를 작성할 때, 기본 인자를 가진 [`@JvmOverloads`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.jvm/-jvm-overloads/) 어노테이션이 지정된 함수에 파라미터를 추가할 때 주의하십시오. 이 어노테이션은 바이너리 호환성을 유지하지 않으므로, 여전히 수동 오버로드를 추가해야 합니다.
