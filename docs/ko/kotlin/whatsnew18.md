...
### TYPE_USE 및 TYPE_PARAMETER 어노테이션 타겟 생성 안 함 기능 {id="ability-to-not-generate-typeuse-and-typeparameter-annotation-targets"}
...
### 최적화 비활성화를 위한 새로운 컴파일러 옵션 {id="a-new-compiler-option-for-disabling-optimizations"}
...
### 이전 백엔드 삭제 {id="removal-of-the-old-backend"}
...
### Lombok의 @Builder 어노테이션 지원 {id="support-for-lombok-s-builder-annotation"}
...
### Xcode 14.1 지원 {id="support-for-xcode-14-1"}
...
### Objective-C/Swift 상호운용성 개선 {id="improved-objective-c-swift-interoperability"}
...
### CocoaPods Gradle 플러그인에서 동적 프레임워크가 기본값으로 설정됨 {id="dynamic-frameworks-by-default-in-the-cocoapods-gradle-plugin"}
...
## Kotlin Multiplatform: 새로운 Android 소스 세트 레이아웃 {id="kotlin-multiplatform-a-new-android-source-set-layout"}
...
#### KotlinSourceSet 명명 스키마 {id="kotlinsourceset-naming-schema"}
...
#### 소스 디렉토리 (SourceDirectories) {id="sourcedirectories"}
...
#### AndroidManifest.xml 파일의 위치 {id="the-location-of-the-androidmanifest-xml-file"}
...
#### Android 테스트와 공통(common) 테스트 간의 관계 {id="the-relation-between-android-and-common-tests"}
...
#### Android flavor 지원 {id="support-for-android-flavors"}
...
### 안정화된 JS IR 컴파일러 백엔드 {id="stable-js-ir-compiler-backend"}
...
### yarn.lock 업데이트 보고를 위한 새로운 설정 {id="new-settings-for-reporting-that-yarn-lock-has-been-updated"}
...
### Gradle 속성을 통해 브라우저용 테스트 타겟 추가 {id="add-test-targets-for-browsers-via-gradle-properties"}
...
### 프로젝트에 CSS 지원을 추가하는 새로운 방식 {id="new-approach-to-adding-css-support-to-your-project"}
...
### Kotlin 컴파일러 옵션을 Gradle 지연 속성으로 노출 {id="exposing-kotlin-compiler-options-as-gradle-lazy-properties"}
...
### 최소 지원 버전 상향 {id="bumping-the-minimum-supported-versions"}
...
### Kotlin 데몬 폴백 전략 비활성화 기능 {id="ability-to-disable-the-kotlin-daemon-fallback-strategy"}
...
### 전이 의존성에서 최신 kotlin-stdlib 버전 사용 {id="usage-of-the-latest-kotlin-stdlib-version-in-transitive-dependencies"}
...
### 관련 Kotlin 및 Java 컴파일 태스크의 JVM 타겟 호환성 확인 필수화 {id="obligatory-check-for-jvm-targets-of-related-kotlin-and-java-compile-tasks"}
...
### Kotlin Gradle 플러그인의 전이 의존성 확인 {id="resolution-of-kotlin-gradle-plugins-transitive-dependencies"}
...
### 지원 중단 및 삭제 사항 {id="deprecations-and-removals"}
...
### JVM 컴파일 타겟 업데이트 {id="updated-jvm-compilation-target"}
...
### Java와 Kotlin 간의 TimeUnit 변환 {id="timeunit-conversion-between-java-and-kotlin"}
...
### 비교 및 뺄셈 가능한 TimeMarks {id="comparable-and-subtractable-timemarks"}
...
### 디렉토리 재귀 복사 또는 삭제 {id="recursive-copying-or-deletion-of-directories"}
...
#### 커스텀 복사 동작 {id="custom-copying-action"}
...
### Java Optionals 확장 함수 {id="java-optionals-extension-functions"}
...
### kotlin-reflect 성능 향상 {id="improved-kotlin-reflect-performance"}
...
## Kotlin 1.8.0 설치하기 {id="install-kotlin-1-8-0"}
...
## Kotlin 1.8.0 호환성 가이드 {id="compatibility-guide-for-kotlin-1-8-0"}
