...
## 명시적인 조합성 선호 {id="prefer-explicit-composability"}
...
예를 들어, 코루틴 Flow API에서는 [버퍼링과 합류(conflation)](coroutines-flow-operators.md#concurrent-processing-operators)가 모두 별도의 함수로 구현되어 있습니다.
...
## 확장 함수 및 프로퍼티 사용 {id="use-extension-functions-and-properties"}
...
## 불리언 타입을 인자로 사용하지 않기 {id="avoid-using-the-boolean-type-as-an-argument"}
...
## 숫자 타입을 적절하게 사용 {id="use-numeric-types-appropriately"}
