...
3. IDE에서 `linkDebugSharedNative` Gradle 태스크를 실행하거나 터미널에서 다음 콘솔 명령을 사용하여 라이브러리를 빌드합니다.

   ```bash
   ./gradlew linkDebugSharedNative
   ```

빌드 결과 라이브러리는 `build/bin/native/debugShared` 디렉터리에 다음 파일들과 함께 생성됩니다.
...
余libnative_KBoolean (*getNonNullValueOfBoolean)(libnative_kref_kotlin_Boolean);
...
이제 API가 어떻게 생성되는지 알았으므로, `libnative_ExportedSymbols` 구조체의 초기화가 시작점이 됩니다. `libnative_api.h`의 마지막 부분을 살펴보겠습니다.
