...
1. 사용 가능한 `run` Gradle 태스크 중 하나를 실행하여 프로젝트를 실행하세요. 예를 들어, 멀티플랫폼 프로젝트에서는 `browserDevelopmentRun` 또는 `jsBrowserDevelopmentRun`을 실행합니다.
...
[IntelliJ IDEA Ultimate](https://www.jetbrains.com/idea/)는 개발 중에 코드를 디버깅할 수 있는 강력한 도구 세트를 제공합니다.
