...
Kotlin 1.6.0은 이러한 제한을 제거합니다. 이제 `@kotlin.annotation.Repeatable`은 모든 유지를 허용하며 Kotlin과 Java 모두에서 애노테이션을 반복 가능하게 만듭니다. 이제 Java 측의 반복 가능한 애노테이션도 Kotlin 쪽에서 지원됩니다.
...
Kotlin 1.6.0은 [기능 릴리스](kotlin-evolution-principles.md#language-and-tooling-releases)이므로 이전 버전의 언어로 작성된 코드와 호환되지 않는 변경 사항이 발생할 수 있습니다. 이러한 변경 사항에 대한 자세한 목록은 [Kotlin 1.6 호환성 가이드](compatibility-guide-16.md)에서 확인하세요.