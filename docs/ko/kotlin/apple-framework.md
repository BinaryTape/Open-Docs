...
2. `build.gradle(.kts)` Gradle 빌드 파일을 다음과 같이 업데이트합니다:
    ...
    Kotlin/Native는 iOS의 경우 `iosArm64`, `iosX64`, `iosSimulatorArm64` 타겟을 지원하며, macOS의 경우 `macosArm64` 및 `macosX64` 타겟을 지원합니다. 따라서 `iosArm64()`를 해당 타겟 플랫폼에 맞는 Gradle 함수로 교체할 수 있습니다:

    | 타겟 플랫폼/디바이스 | Gradle 함수 |
    |------------------------|-----------------------|
...
3. IDE에서 `linkDebugFrameworkNative` Gradle 태스크를 실행하거나 터미널에서 다음 콘솔 명령을 사용하여 프레임워크를 빌드합니다:

   ```bash
   ./gradlew linkDebugFrameworkNative
   ```
    
빌드가 완료되면 `build/bin/native/debugFramework` 디렉토리에 프레임워크가 생성됩니다.

> `linkNative` Gradle 태스크를 사용하여 프레임워크의 `debug` 및 `release` 변리언트를 모두 생성할 수도 있습니다.
>
{style="tip"}
...
`build/bin/native/debugFramework/Demo.framework/Headers` 디렉토리에서 `Demo.h` 헤더 파일을 엽니다.
