...
### Java 15 타겟 {id="java-15-target"}

이제 Java 15를 Kotlin/JVM 타겟으로 사용할 수 있습니다.

### invokedynamic 문자열 결합 {id="invokedynamic-string-concatenation"}
...
### Gradle DSL 변경 사항 {id="gradle-dsl-changes"}
...
#### webpack 구성을 위한 단일 설정 지점 {id="single-point-for-webpack-configuration"}
...
#### Gradle에서 package.json 커스터마이징 {id="package-json-customization-from-gradle"}
...
#### 선택적 yarn 의존성 해결 {id="selective-yarn-dependency-resolutions"}
...
#### 세분화된 워크스페이스 비활성화 {id="disabling-granular-workspaces"}
...
### 새로운 위저드 템플릿 {id="new-wizard-templates"}
...
### IR 컴파일러를 사용한 컴파일 오류 무시 {id="ignoring-compilation-errors-with-ir-compiler"}
...
### 성능 개선 및 버그 수정 {id="performance-improvements-and-bug-fixes"}
...
### Objective-C 예외 래핑 옵트인 {id="opt-in-wrapping-of-objective-c-exceptions"}
...
### CocoaPods 플러그인 개선 사항 {id="cocoapods-plugin-improvements"}
...
#### 개선된 작업 실행 {id="improved-task-execution"}
...
#### Xcode와의 업데이트된 통합 {id="updated-integration-with-xcode"}
...
### Xcode 12 라이브러리 지원 {id="support-for-xcode-12-libraries"}
    
Xcode 12와 함께 제공되는 새로운 라이브러리에 대한 지원을 추가했습니다. 이제 Kotlin 코드에서 이를 사용할 수 있습니다.

## Kotlin Multiplatform {id="kotlin-multiplatform"}

### 멀티플랫폼 라이브러리 배포 구조 업데이트 {id="updated-structure-of-multiplatform-library-publications"}
...
#### 이전 버전과의 호환성 {id="compatibility-with-earlier-versions"}
...
### java.nio.file.Path를 위한 확장 {id="extensions-for-java-nio-file-path"}
...
### String.replace 함수 성능 개선 {id="improved-string-replace-function-performance"}
...
### Synthetics 뷰 지원 중단 {id="deprecation-of-synthetic-views"}
...
### Parcelable 구현 생성기를 위한 새로운 플러그인 {id="new-plugin-for-parcelable-implementation-generator"}
