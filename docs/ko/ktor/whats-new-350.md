...
### 변경된 경우에만 세션 쿠키 전송 {id="session-cookies"}

Ktor 3.5.0은 세션 데이터가 변경된 경우에만 전송하는(예: 쿠키 기반 세션의 `Set-Cookie` 헤더) [Sessions](server-sessions.md) 플러그인의 새로운 옵션을 도입했습니다.

기본적으로 세션 데이터는 기존 동작을 유지하기 위해 모든 응답에서 전송됩니다. 변경된 경우에만 전송하려면 세션 쿠키 구성에서 `sendOnlyIfModified` 플래그를 활성화하십시오:
