...
### 에러 처리를 위한 OAuth 폴백(fallback) {id="oauth-fallback-for-error-handling"}
...
### 정적 OAuth 프로바이더 설정 {id="static-oauth-provider-settings"}
...
### Zstd 압축 지원 {id="zstd-compression-support"}
...
### 설정 파일에서의 SSL 트러스트 스토어(trust store) 설정 {id="ssl-trust-store-settings-in-a-configuration-file"}
...
### 부분 응답을 위한 HTML 프래그먼트(fragment) {id="html-fragments-for-partial-responses"}
...
### HTTP 요청 라이프사이클 {id="http-request-lifecycle"}
...
### 리소스로 응답하는 새로운 메서드 {id="new-method-to-respond-with-a-resource"}
...
### 런타임 OpenAPI 라우트 어노테이션 {id="runtime-openapi-route-annotations"}
...
### API Key 인증 {id="api-key-authentication"}
...
### 다중 헤더 파싱 {id="multiple-header-parsing"}
...
### 인증 토큰 캐시 제어 {id="authentication-token-cache-control"}
...
#### 인증 토큰 액세스 및 삭제 {id="accessing-and-clearing-authentication-tokens"}
...
#### 토큰 캐시 동작 구성 {id="configuring-token-cache-behavior"}
...
### OkHttp를 위한 듀플렉스 스트리밍(Duplex streaming) {id="duplex-streaming-for-okhttp"}
...
### Apache5 연결 관리자 구성 {id="apache5-connection-manager-configuration"}
...
### 네이티브 클라이언트 엔진을 위한 디스패처(Dispatcher) 구성 {id="dispatcher-configuration-for-native-client-engines"}
...
### 플러그인 및 기본 요청 구성 대체 {id="plugin-and-default-request-configuration-replacement"}

Ktor 클라이언트 구성은 이제 런타임에 기존 설정을 대체하는 기능에 대해 더 많은 제어권을 제공합니다.

#### 플러그인 구성 대체 {id="replace-plugin-configuration"}
...
#### 기본 요청 구성 대체 {id="replace-default-request-configuration"}
...
### `js` 및 `wasmJs` 타겟을 위한 공유 소스 세트 지원 {id="shared-source-set-support-for-js-and-wasmjs-targets"}
...
### `ByteReadChannel`에서 `RawSink`로 바이트 스트리밍 {id="stream-bytes-from-a-bytereadchannel-to-a-rawsink"}
...
### OpenAPI 컴파일러 확장 {id="openapi-compiler-extension"}
