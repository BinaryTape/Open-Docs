...
[Ktor HTTP 클라이언트](client-create-and-configure.md)는 JVM, [Android](https://kotlinlang.org/docs/android-overview.html), [JavaScript](https://kotlinlang.org/docs/js-overview.html), [Native](https://kotlinlang.org/docs/native-overview.html)를 포함한 다양한 플랫폼에서 사용할 수 있습니다. 특정 플랫폼에는 네트워크 요청을 처리하는 특정 엔진이 필요할 수 있습니다. 예를 들어, JVM 애플리케이션에는 `Apache` 또는 `Jetty`를, Android에는 `OkHttp` 또는 `Android`를, Kotlin/Native를 대상으로 하는 데스크톱 애플리케이션에는 `Curl` 등을 사용할 수 있습니다. 각 엔진은 특정 기능을 가지며 다양한 구성 옵션을 제공합니다.
...
_* 이전 Android 버전에서 CIO 엔진을 사용하려면 [Java 8 API 디슈가링](https://developer.android.com/studio/write/java8-support)을 활성화해야 합니다._
...
- 엔진이 HTTP/2를 지원하는 경우, 엔진 구성을 사용자 지정하여 활성화할 수 있습니다([](#java) 엔진의 예시를 참조하세요).
- Ktor 클라이언트에서 [SSL](client-ssl.md)을 구성하려면 선택한 엔진의 구성을 사용자 지정해야 합니다.
- 일부 엔진은 [프록시](client-proxy.md#supported_engines)를 지원하지 않습니다.
- [Logging](client-logging.md) 플러그인은 다양한 플랫폼에 대해 다른 로거 유형을 제공합니다.
...
> 다양한 엔진에 대해 Ktor는 `-jvm` 또는 `-js`와 같은 접미사가 붙은 플랫폼별 아티팩트(예: `ktor-client-cio-jvm`)를 제공합니다. Gradle은 주어진 플랫폼에 적합한 아티팩트를 해결하지만, Maven은 이 기능을 지원하지 않습니다. 이는 Maven의 경우 플랫폼별 접미사를 수동으로 추가해야 함을 의미합니다.
>
{type="note"}

## 지정된 엔진으로 클라이언트 생성 {id="create"}
특정 엔진으로 HTTP 클라이언트를 생성하려면 [`HttpClient`](https://api.ktor.io/ktor-client/ktor-client-core/io.ktor.client/-http-client/index.html) 생성자에 엔진 클래스를 인수로 전달합니다. 예를 들어, 다음과 같이 `CIO` 엔진으로 클라이언트를 생성할 수 있습니다.
```kotlin
```
{src="snippets/_misc_client/CioCreate.kt"}

### 기본 엔진 {id="default"}
`HttpClient` 생성자를 인수 없이 호출하면, 클라이언트는 [빌드 스크립트에 추가된](#dependencies) 아티팩트에 따라 엔진을 자동으로 선택합니다.
```kotlin
```
{src="snippets/_misc_client/DefaultEngineCreate.kt"}

이는 멀티플랫폼 프로젝트에 유용할 수 있습니다. 예를 들어, [Android와 iOS](client-create-multiplatform-application.md)를 모두 대상으로 하는 프로젝트의 경우, `androidMain` 소스 세트에 [Android](#jvm-android) 종속성을, `iosMain` 소스 세트에 [Darwin](#darwin) 종속성을 추가할 수 있습니다. 필요한 종속성은 컴파일 시점에 선택됩니다.

## 엔진 구성 {id="configure"}
`engine` 메서드를 사용하여 엔진을 구성할 수 있습니다. 모든 엔진은 [`HttpClientEngineConfig`](https://api.ktor.io/ktor-client/ktor-client-core/io.ktor.client.engine/-http-client-engine-config/index.html)에 의해 노출되는 몇 가지 공통 속성을 공유합니다. 예를 들어:

```kotlin
```
{src="snippets/_misc_client/BasicEngineConfigExample.kt" interpolate-variables="true" disable-links="false"}

특정 엔진을 구성하는 방법을 알아보려면 아래 해당 섹션을 참조하세요.

## JVM {id="jvm"}
이 섹션에서는 JVM에서 사용 가능한 엔진을 살펴보겠습니다.

### Apache {id="apache"}
`Apache` 엔진은 HTTP/1.1을 지원하며 여러 구성 옵션을 제공합니다. HTTP/2 지원이 필요한 경우 `Apache5` 엔진을 사용할 수도 있으며, 이 엔진은 기본적으로 HTTP/2가 활성화되어 있습니다.
...
### Java {id="java"}
`Java` 엔진은 Java 11에 도입된 [Java HTTP 클라이언트](https://openjdk.java.net/groups/net/httpclient/intro.html)를 사용합니다. 이를 사용하려면 아래 단계를 따르세요.
...
이 섹션에서는 JVM/Android에서 사용 가능한 엔진과 해당 구성을 살펴보겠습니다.
...
### OkHttp {id="okhttp"}
OkHttp를 기반으로 하는 `OkHttp` 엔진은 다음과 같이 구성할 수 있습니다.
...
### Darwin {id="darwin"}
`Darwin` 엔진은 [Darwin 기반](https://en.wikipedia.org/wiki/Darwin_(operating_system)) 운영 체제(macOS, iOS, tvOS 등)를 대상으로 하며 내부적으로 [`NSURLSession`](https://developer.apple.com/documentation/foundation/nsurlsession)을 사용합니다. `Darwin` 엔진을 사용하려면 아래 단계를 따르세요.
...
3. 엔진을 구성하려면 [`DarwinClientEngineConfig`](https://api.ktor.io/ktor-client/ktor-client-darwin/io.ktor.client.engine.darwin/-darwin-client-engine-config/index.html)에 의해 노출되는 설정을 `engine` 함수에 전달합니다.
   ...
   전체 예제는 여기에서 찾을 수 있습니다: [client-engine-darwin](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-engine-darwin).

### WinHttp {id="winhttp"}
`WinHttp` 엔진은 Windows 기반 운영 체제를 대상으로 합니다. `WinHttp` 엔진을 사용하려면 아래 단계를 따르세요.
...
3. 엔진을 구성하려면 [`WinHttpClientEngineConfig`](https://api.ktor.io/ktor-client/ktor-client-winhttp/io.ktor.client.engine.winhttp/-winhttp-client-engine-config/index.html)에 의해 노출되는 설정을 `engine` 함수에 전달합니다.
   ...
   전체 예제는 여기에서 찾을 수 있습니다: [client-engine-winhttp](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-engine-winhttp).

### Curl {id="curl"}

데스크톱 플랫폼의 경우 Ktor는 `Curl` 엔진도 제공합니다. 이 엔진은 `linuxX64`, `macosX64`, `macosArm64`, `mingwX64` 플랫폼에서 지원됩니다. `Curl` 엔진을 사용하려면 아래 단계를 따르세요.
...
4. 엔진을 구성하려면 `CurlClientEngineConfig`에 의해 노출되는 설정을 `engine` 메서드에 전달합니다.
   ...
   전체 예제는 여기에서 찾을 수 있습니다: [client-engine-curl](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-engine-curl).

## JVM, Android 및 Native {id="jvm-android-native"}

### CIO {id="cio"}
CIO는 JVM, Android, Native 플랫폼에서 사용할 수 있는 완전 비동기 코루틴 기반 엔진입니다. 현재는 HTTP/1.x만 지원합니다. 이를 사용하려면 아래 단계를 따르세요.
...
`Js` 엔진은 [JavaScript 프로젝트](https://kotlinlang.org/docs/js-overview.html)에 사용할 수 있습니다. 이 엔진은 브라우저 애플리케이션의 경우 [fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API)를, Node.js의 경우 `node-fetch`를 사용합니다. 이를 사용하려면 아래 단계를 따르세요.
...
전체 예제는 여기에서 찾을 수 있습니다: [client-engine-js](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-engine-js).
...
[](client-create-multiplatform-application.md) 튜토리얼에서 생성된 프로젝트를 사용하여 이를 달성하는 방법을 시연해 보겠습니다.
...
2. `shared/src/androidMain/kotlin/com/example/kmmktor/Platform.kt`를 열고 Android 모듈에 대한 `httpClient` 함수의 `actual` 선언을 추가합니다.
   ```kotlin
   import io.ktor.client.*
...
3. `shared/src/iosMain/kotlin/com/example/kmmktor/Platform.kt`를 열고 iOS 모듈에 대한 `httpClient` 함수의 `actual` 선언을 추가합니다.
   ```kotlin
   import io.ktor.client.*
   import io.ktor.client.engine.darwin.*
