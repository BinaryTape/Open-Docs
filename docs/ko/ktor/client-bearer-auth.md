...
> 서버 측에서 Ktor는 Bearer 인증을 처리하기 위한 [Authentication](server-bearer-auth.md) 플러그인을 제공합니다.
...
* `HttpRequestBuilder.markAsRefreshTokenRequest()` 함수는 해당 요청을 토큰 갱신 요청으로 표시합니다. 이렇게 표시된 요청은 인증 재시도 메커니즘에서 제외됩니다. 이는 리프레시 요청 자체가 `401 Unauthorized`로 실패할 경우 클라이언트가 토큰을 다시 갱신하려고 시도하는 것을 방지하여 무한 갱신 루프를 피할 수 있게 합니다.
