...
Koog은 A2A 프로토콜과의 원활한 통합을 제공하여, Koog 에이전트를 A2A 서버로 노출하거나 Koog 에이전트를 다른 A2A 호환 에이전트에 연결할 수 있도록 합니다.

## 의존성 {id="dependencies"}

A2A Koog 통합을 위해서는 사용 사례에 따라 특정 기능 모듈이 필요합니다.

### Koog 에이전트를 A2A 서버로 노출하는 경우 {id="for-exposing-koog-agents-as-a2a-servers"}
...
### Koog 에이전트를 A2A 에이전트에 연결하는 경우 {id="for-connecting-koog-agents-to-a2a-agents"}
...
1. **Koog 에이전트를 A2A 서버로 노출** - Koog 에이전트를 A2A 프로토콜을 통해 검색 및 접근 가능하게 만듭니다.
2. **Koog 에이전트를 A2A 에이전트에 연결** - Koog 에이전트가 다른 A2A 호환 에이전트와 통신할 수 있도록 합니다.

## Koog 에이전트를 A2A 서버로 노출하기 {id="exposing-koog-agents-as-a2a-servers"}

### A2A 기능이 포함된 Koog 에이전트 정의하기 {id="define-koog-agent-with-a2a-feature"}

먼저 Koog 에이전트를 정의해 보겠습니다. 에이전트의 로직은 다양할 수 있지만, 여기서는 도구(tools)를 사용하는 기본적인 싱글 런(single run) 에이전트의 예시를 보여줍니다.
이 에이전트는 사용자로부터 메시지를 받아 LLM으로 전달합니다.
LLM 응답에 도구 호출(tool call)이 포함되어 있으면 에이전트는 도구를 실행하고 결과를 LLM에 전달합니다.
LLM 응답에 어시스턴트 메시지가 포함되어 있으면 에이전트는 해당 메시지를 사용자에게 보내고 종료합니다.

입력이 들어오면(on input resize), 에이전트는 입력 메시지와 함께 작업 제출됨(task submitted) 이벤트를 A2A 클라이언트에 보냅니다.
각 도구 호출 시, 에이전트는 도구 호출 및 결과와 함께 작업 진행 중(task working) 이벤트를 A2A 클라이언트에 보냅니다.
어시스턴트 메시지가 발생하면, 에이전트는 어시스턴트 메시지와 함께 작업 완료(task complete) 이벤트를 A2A 클라이언트에 보냅니다.
...
private fun createAgent(
    ...
    strategy = strategy<A2AMessage, Unit>("test") {
        ...
        // LLM에서 도구 호출이 반환되면 도구 처리 노드로 전달한 후 다시 LLM으로 전달
...
## A2AAgentServer 기능 메커니즘 {id="a2aagentserver-feature-mechanism"}

`A2AAgentServer`는 Koog 에이전트와 A2A 프로토콜 간의 원활한 통합을 가능하게 하는 Koog 에이전트 기능(feature)입니다.
`A2AAgentServer` 기능은 Koog 에이전트 내부에서 A2A 클라이언트와 통신하는 데 사용되는 `RequestContext` 및 `SessionEventProcessor` 엔티티에 대한 접근을 제공합니다.

기능을 설치하려면 에이전트에서 `install` 함수를 호출하고 `A2AAgentServer` 기능과 함께 `RequestContext` 및 `SessionEventProcessor`를 전달하세요:
```kotlin
// 기능 설치
install(A2AAgentServer) {
    this.context = context
...
Koog 에이전트 전략에서 이러한 엔티티에 접근할 수 있도록, 이 기능은 `withA2AAgentServer` 함수를 제공합니다. 이 함수를 통해 에이전트 노드는 실행 컨텍스트 내에서 A2A 서버 기능을 사용할 수 있습니다.
이 함수는 설치된 `A2AAgentServer` 기능을 가져와 액션 블록의 수신객체(receiver)로 제공합니다.

```kotlin
// 에이전트 노드 내에서의 사용
withA2AAgentServer {
    // 이제 'this'는 A2AAgentServer 인스턴스입니다.
    eventProcessor.sendTaskUpdate("Processing your request...", TaskState.Working)
}
```

### A2A 서버 시작하기 {id="start-a2a-server"}
서버를 실행하면 Koog 에이전트는 A2A 프로토콜을 통해 검색 및 접근이 가능해집니다.
...
## Koog 에이전트를 A2A 에이전트에 연결하기 {id="connecting-koog-agents-to-a2a-agents"}

### A2A 클라이언트 생성 및 A2A 서버 연결 {id="create-a2a-client-and-connect-to-the-a2a-server"}
...
### Koog 에이전트 생성 및 A2AAgentClient 기능에 A2A 클라이언트 추가 {id="create-koog-agent-and-add-a2a-client-to-a2aagentclient-feature"}
Koog 에이전트에서 A2A 에이전트에 연결하려면 A2A 에이전트 연결을 위한 클라이언트 API를 제공하는 `A2AAgentClient` 기능을 사용할 수 있습니다.
클라이언트의 원칙은 서버와 동일합니다. 기능을 설치하고 `A2AAgentClient` 기능과 함께 `RequestContext` 및 `SessionEventProcessor`를 전달합니다.
