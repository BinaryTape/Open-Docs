...
## Koog 설치하기 {id="install-koog"}

--8<-- "quickstart-snippets.md:dependencies"

??? tip "모듈 버전 관리 (Module Versioning)"

    Koog는 유의적 버전(Semantic Versioning, `X.Y.Z`)을 따릅니다. 안정적인 모듈(예: `1.0.0`)은 보장된 API를 가지며, 베타 모듈(예: `1.0.0-beta`)은 실험적이며 릴리스 간에 변경될 수 있습니다.

    자세한 내용은 [모듈 버전 관리](module-versioning.md)를 참조하세요.
...
## API 키 설정하기 {id="set-up-an-api-key"}
...
## 첫 번째 Koog 에이전트 만들기 {id="create-your-first-koog-agent"}
...
=== "OpenAI"
    ...
    도움이 필요한 것이 있다면 말씀해 주세요. 제가 여기 있습니다!
    ```
...
- [에이전트 유형](agents/index.md)에 대해 자세히 알아보기