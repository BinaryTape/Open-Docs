...
[그래프 기반 에이전트](graph-based-agents.md)에서는
기본 에이전트가 사용하는 미리 정의된 전략 그래프를 다시 만드는 방법을 확인할 수 있습니다.
...
가장 기본적인 에이전트를 생성하려면, [`AIAgent`](https://api.koog.ai/agents/agents-core/ai.koog.agents.core.agent/-a-i-agent/index.html)를 인스턴스화하고
[언어 모델](../model-capabilities.md#creating-a-model-llmodel-configuration)이 포함된 [프롬프트 실행기(prompt executor)](../prompts/prompt-executors.md)를 제공하세요.
...
LLM의 동작을 커스텀하기 위해 에이전트 생성자에 몇 가지 [LLM 파라미터](../llm-parameters.md#llm-parameter-reference)를 직접 제공할 수 있습니다.
...
먼저, 함수에 [`@Tool`](https://api.koog.ai/agents/agents-tools/ai.koog.agents.core.tools.annotations/-tool/index.html) 어노테이션을 달아 도구를 생성합니다.
...
에이전트가 사용자에게 질문하기로 결정하면, 
`println()`을 통해 `stdout`에 쓰고 `readln()`을 통해 `stdin`으로부터 읽는 이 도구를 호출할 수 있습니다.
...
!!! tip
    ...
    모델, temperature, 최대 반복 횟수 및 기타 파라미터들을 에이전트 생성자에 직접 전달하는 대신, 별도의 구성 객체로 정의하여 전달할 수도 있습니다.
    자세한 내용은 [에이전트 구성](index.md#agent-configuration)을 참조하세요.

## 에이전트 실행 중 이벤트 처리하기

테스트 및 디버깅을 돕고 체인으로 연결된 에이전트 상호작용을 위한 후크(hook)를 만들기 위해,
Koog는 [EventHandler](https://api.koog.ai/agents/agents-features/agents-features-event-handler/ai.koog.agents.features.eventHandler.feature/-event-handler/index.html) 기능을 제공합니다.
...
Koog 에이전트 기능에 대한 자세한 내용은 [기능 개요](../features-overview.md)를 참조하세요.
