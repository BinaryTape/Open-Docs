...
전략 그래프(Strategy graphs)는 Koog 프레임워크에서 에이전트 워크플로의 중추입니다. 이는 에이전트가 입력을 처리하고, 도구와 상호작용하며, 출력을 생성하는 방식을 정의합니다. 전략 그래프는 에지(edge)로 연결된 노드(node)들로 구성되며, 실행 흐름은 조건에 따라 결정됩니다.
