...
## 대화 메모리 활성화 {id="enable-chat-memory"}
...
## 세션 ID (Session IDs) {id="session-ids"}
...
## 이력 제공자 (History providers) {id="history-providers"}
...
### 전처리기 순서 {id="order-of-preprocessors"}
...
### 커스텀 전처리기 {id="custom-preprocessors"}
...
## 대화 메모리 vs 에이전트 지속성 (Chat memory vs agent persistence) {id="chat-memory-vs-agent-persistence"}
