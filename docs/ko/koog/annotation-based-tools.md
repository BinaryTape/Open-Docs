...
애노테이션 기반 도구는 Kotlin과 Java 모두에서 대규모 언어 모델(LLM)에 함수와 메서드를 도구로 노출하는 선언적인 방법을 제공합니다.
애노테이션을 사용하면 모든 함수나 메서드를 LLM이 이해하고 사용할 수 있는 도구로 변환할 수 있습니다.

이 방식은 도구 설명을 수동으로 작성하지 않고 Kotlin 또는 Java의 기존 기능을 LLM에 노출해야 할 때 유용합니다.

!!! note
    애노테이션 기반 도구는 JVM 전용이며 다른 플랫폼에서는 사용할 수 없습니다. 멀티플랫폼 지원이 필요한 경우 [클래스 기반 도구 API](class-based-tools.md)를 사용하세요.

## 주요 애노테이션 {id="key-annotations"}
...
## @Tool 애노테이션 {id="tool-annotation"}

`@Tool` 애노테이션은 LLM에 도구로 노출해야 하는 함수(Kotlin) 또는 메서드(Java)를 표시하는 데 사용됩니다.
`@Tool`이 지정된 함수와 메서드는 `ToolSet` 인터페이스를 구현하는 객체에서 리플렉션(reflection)을 통해 수집됩니다. 자세한 내용은 [ToolSet 인터페이스 구현](#1-toolset-인터페이스-구현)을 참조하세요.
...
함수나 메서드를 도구로 표시하려면 `ToolSet` 인터페이스를 구현하는 클래스 내의 해당 함수나 메서드에 `@Tool` 애노테이션을 적용하세요.
...
## @LLMDescription 애노테이션 {id="llmdescription-annotation"}

`@LLMDescription` 애노테이션은 코드 요소(클래스, 함수, 메서드, 매개변수 등)에 대한 설명 정보를 LLM에 제공합니다.
...
=== "Java"
    ...
    **/
    ...
    public String processTool(
            ...
            @LLMDescription(description = "선택적 구성 매개변수") String config
    ) {
        // 함수 구현
        return "Processed: " + input + " with config: " + config;
    }
...
클래스에 함수나 메서드를 추가하고 `@Tool` 애노테이션을 달아 도구로 노출합니다.
...
=== "Java"
    ...
    **/
    ...
    public class DiagnosticToolSet implements ToolSet {
        ...
        @Tool
        @LLMDescription(description = "장치에 대한 진단을 실행하여 상태를 확인하고 문제를 식별합니다")
        ...
        @Tool
        @LLMDescription(description = "오류 코드를 분석하여 의미와 가능한 해결책을 결정합니다")
        public String analyzeError(
                @LLMDescription(description = "분석할 오류 코드 (예: 'E1001')") String errorCode
        ) {
            // 구현
            return "Analysis of error code " + errorCode;
        }
...
* **기본값 문서화**: 매개변수에 기본값(Kotlin)이나 오버로드(Java)가 있는 경우 설명에 이를 문서화하세요.
...
### 도구가 인식되지 않는 경우 {id="tools-not-being-recognized"}

에이전트가 도구를 인식하지 못하는 경우 다음 사항을 확인하세요.

- 클래스가 `ToolSet` 인터페이스를 구현하고 있는지 확인합니다.
- 모든 도구 함수나 메서드에 `@Tool` 애노테이션이 달려 있는지 확인합니다.
- 도구 함수나 메서드가 적절한 반환 유형을 가지고 있는지 확인합니다 (단순성을 위해 `String`을 권장합니다).
- 도구가 에이전트에 올바르게 등록되었는지 확인합니다.

### 도구 설명이 불분명한 경우 {id="unclear-tool-descriptions"}

LLM이 도구를 올바르게 사용하지 못하거나 목적을 오해하는 경우 다음을 시도해 보세요.

- 가능한 경우 기본 매개변수 유형(`Kotlin: String, Boolean, Int` 또는 `Java: String, boolean, int`)을 사용합니다.
- 매개변수 설명에 예상 형식을 명확하게 기술합니다.
- 복잡한 유형의 경우 특정 형식을 가진 `String` 매개변수를 사용하고 도구 내에서 이를 파싱하는 것을 고려하세요.
- 매개변수 설명에 유효한 입력의 예시를 포함합니다.
- Java는 기본 매개변수(default parameters)를 지원하지 않으므로 대신 메서드 오버로딩을 사용해야 함에 유의하세요.

### 매개변수 유형 문제 {id="parameter-type-issues"}
