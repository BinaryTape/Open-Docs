...
1. 커스텀 도구를 만들거나 기본 제공 도구 중 하나를 사용합니다.
...
- 에이전트와 사용자 간의 상호 작용 및 대화 관리를 위한 기능을 제공하는 기본 제공 도구. 자세한 내용은 [Built-in tools](built-in-tools.md)를 참고하세요.
...
- 도구 구성.
- 여러 도구 레지스트리 병합 지원.
- 이름 또는 유형별로 도구를 가져오는 메서드 제공.

자세한 내용은 [ToolRegistry](api:agents-tools::ai.koog.agents.core.tools.ToolRegistry)를 참고하세요.
...
=== "Java"
    ...
    **/
    ...
    // ToolRegistry를 빌드하고 ToolSet에서 도구를 등록합니다.
...
!!! tip
    에이전트 실패를 방지하려면 도구에 적절한 [에러 처리](../features/agent-event-handlers.md)를 구현했는지 확인하세요.

도구는 `AIAgentLLMWriteSession`으로 표현되는 특정 세션 컨텍스트 내에서 호출됩니다.
이 세션 컨텍스트는 다음과 같은 도구 호출 메서드를 제공합니다:

- 주어진 인자로 도구 호출.
- 이름과 주어진 인자로 도구 호출.
- 제공된 도구 클래스와 인자로 도구 호출.
- 특정 유형의 도구를 주어진 인자로 호출.
...
=== "Kotlin"
    ...
    class BookTool() : SimpleTool<Book>(
        ...
        name = NAME,
        description = "Markdown에서 책 정보를 파싱하는 도구"
    ) {
        companion object {
            const val NAME = "book"
        }
...
* **nodeExecuteTool**: 단일 도구 호출을 수행하고 그 결과를 반환합니다. 자세한 내용은 [API 레퍼런스](api:agents-core::ai.koog.agents.core.dsl.extension.nodeExecuteTool)를 참고하세요.

* **nodeExecuteSingleTool**: 제공된 인자로 특정 도구를 호출합니다. 자세한 내용은 [API 레퍼런스](api:agents-core::ai.koog.agents.core.dsl.extension.nodeExecuteSingleTool)를 참고하세요.

* **nodeExecuteMultipleTools**: 여러 도구 호출을 수행하고 그 결과를 반환합니다. 자세한 내용은 [API 레퍼런스](api:agents-core::ai.koog.agents.core.dsl.extension.nodeExecuteMultipleTools)를 참고하세요.

* **nodeLLMSendToolResult**: 도구 결과를 LLM에 보내고 응답을 받습니다. 자세한 내용은 [API 레퍼런스](api:agents-core::ai.koog.agents.core.dsl.extension.nodeLLMSendToolResult)를 참고하세요.
...
=== "Kotlin"
    ...
    -->
    ```kotlin
    // 금융 분석 에이전트 생성을 담당하는 전문 에이전트 서비스를 생성합니다.
    ...
    // 호출 시 금융 분석 에이전트를 실행하는 도구를 생성합니다.
    val analysisAgentTool = analysisAgentService.createAgentTool(
        agentName = "analyzeTransactions",
        agentDescription = "금융 트랜잭션 분석 수행",
        inputDescription = "트랜잭션 분석 요청",
...
=== "Kotlin"
    ...
    val coordinatorAgent = AIAgent(
        promptExecutor = simpleOpenAIExecutor(apiKey),
        llmModel = OpenAIModels.Chat.GPT4o,
        systemPrompt = "당신은 다양한 전문 서비스를 조정합니다.",
        toolRegistry = ToolRegistry {
...
1. 인자가 입력 디스크립터(input descriptor)에 따라 역직렬화됩니다.
...
- **관심사 분리(Separation of concerns)**: 각 에이전트는 특정 도메인에만 집중할 수 있습니다.