...
ACP 지원은 기본적으로 Koog에서 제공되지 않는 선택적 [기능(feature)](features-overview.md)입니다.
...
*   **`eventsProducer`**: ACP 이벤트가 전송되는 `kotlinx.coroutines.channels.ProducerScope<Event>`입니다. 자세한 내용은 [이벤트 스트리밍](#event-streaming)을 참조하세요.
*   **`setDefaultNotifications`**: 에이전트 수명 주기 이벤트에 대해 기본 알림 핸들러를 등록할지 여부입니다. 자세한 내용은 [에이전트 알림 처리](#handling-agent-notifications)를 참조하세요.
...
- [`AgentSupport`](https://github.com/agentclientprotocol/kotlin-sdk/blob/master/acp/src/commonMain/kotlin/com/agentclientprotocol/agent/AgentSupport.kt): 에이전트의 식별 정보, 기능 및 세션 수명 주기(세션 생성 또는 로드)를 관리합니다.
- [`AgentSession`](https://github.com/agentclientprotocol/kotlin-sdk/blob/master/acp/src/commonMain/kotlin/com/agentclientprotocol/agent/AgentSession.kt): 단일 대화 세션을 관리하고, `prompt` 실행을 처리하며, 취소를 관리합니다.
