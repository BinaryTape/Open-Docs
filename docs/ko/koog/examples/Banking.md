...
이 튜토리얼에서는 Kotlin에서 **Koog** 에이전트를 사용하여 소형 뱅킹 어시스턴트를 구축해 보겠습니다.
다음 내용을 배우게 됩니다:
- 도메인 모델 및 샘플 데이터 정의
- **송금** 및 **거래 분석**을 위한 기능 중심 도구 노출
- 사용자 의도 분류 (송금 vs 분석)
- 두 가지 스타일의 호출 오케스트레이션:
  1) 그래프/서브그래프 전략
  2) “도구로서의 에이전트(agents as tools)”
...
// Maven Central의 koog를 사용하려면 이 주석을 해제하세요
// %use koog
```
...
두 가지 "소프트 계약(soft contracts)"을 모델링합니다:
- `chooseRecipient`는 모호함이 감지될 때 *후보군*을 반환합니다.
- `sendMoney`는 `confirmed` 플래그를 지원합니다. `false`인 경우, 에이전트에게 사용자에게 확인을 요청하도록 요구합니다.
...
val transferAgentService = AIAgentService(
    ...
    temperature = 0.0,  // 금융 작업을 위해 결정론적 응답 사용
...
class TransactionAnalysisTools : ToolSet {
    ...
    fun getTransactions(
        ...
        // userId 검증 (프로덕션 환경에서는 실제 데이터베이스를 쿼리함)
...
val strategy = strategy<String, String>("banking assistant") {
    ...
    val classifyRequest by subgraph<String, ClassifiedBankRequest>(
        ...
        // 적절한 분류를 보장하기 위해 구조화된 출력(structured output) 사용
    ...
    val transferMoney by subgraphWithTask<ClassifiedBankRequest, String>(
        ...
        llmModel = OpenAIModels.Chat.GPT4o  // 송금에는 더 뛰어난 모델 사용
    ) { request ->
        """
...
$transactionAnalysisPrompt")
    ...
    maxAgentIterations = 50  // 복잡한 멀티스텝 작업 허용
)
...
2. 특정 작업을 수행하기 위해 LLM과 도구를 결합한 단일 목적 에이전트 구축
3. 복잡한 워크플로우를 위해 전략과 서브그래프를 사용하는 그래프 에이전트 구현
...
3. 에러 핸들링: 항상 입력을 검증하고 의미 있는 에러 메시지를 제공하세요.
4. 사용자 경험: 송금과 같은 중요한 작업에는 사용자 확인 단계를 포함하세요.
5. 모듈성: 유지보수성을 높이기 위해 관심사를 서로 다른 도구와 에이전트로 분리하세요.