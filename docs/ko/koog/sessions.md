...
1.  `requestLLM()`: 현재 프롬프트와 도구를 사용하여 LLM에 요청을 보내고, 단일 응답을 반환합니다.

2.  `requestLLMMultiple()`: 현재 프롬프트와 도구를 사용하여 LLM에 요청을 보내고, 여러 응답을 반환합니다.

3.  `requestLLMWithoutTools()`: 도구 없이 현재 프롬프트만 사용하여 LLM에 요청을 보내고, 단일 응답을 반환합니다.

4.  `requestLLMForceOneTool`: 현재 프롬프트와 도구를 사용하여 LLM에 요청을 보내며, 하나의 도구 사용을 강제합니다.

5.  `requestLLMOnlyCallingTools`: 도구만 사용하여 처리되어야 하는 요청을 LLM에 보냅니다.
...
4.  **암시적 요청 없음**: 시스템은 암시적인 요청을 하지 않습니다. 요청 메서드를 명시적으로 호출해야 합니다.
...
더 고급 사용 사례를 위해 플랫폼은 구조화된 요청 및 스트리밍 요청을 위한 메서드를 제공합니다:
...
3.  `requestLLMStreaming()`: LLM에 스트리밍 요청을 보내고 응답 청크(chunk)의 플로우(flow)를 반환합니다.
...
전략 그래프의 `nodeLLMCompressHistory` 노드를 사용하여 특정 시점에서 이력을 압축할 수도 있습니다.
