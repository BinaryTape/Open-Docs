...
테스트 기능은 Koog 프레임워크에서 **AI 에이전트 파이프라인(AI agent pipelines)**, **서브그래프(subgraphs)**, **도구 상호작용(tool interactions)**을 테스트하기 위한 포괄적인 프레임워크를 제공합니다. 개발자는 이를 통해 **모의 LLM(거대 언어 모델) 실행기(mock LLM (Large Language Model) executors)**, **도구 레지스트리(tool registries)**, **에이전트 환경(agent environments)**을 갖춘 제어된 테스트 환경을 구축할 수 있습니다.

### 목적

이 기능의 주요 목적은 다음을 통해 **에이전트 기반 AI 기능(agent-based AI features)**의 테스트를 용이하게 하는 것입니다:
