...
프롬프트 실행기(prompt executor)를 통해 실행하는 반복적인 요청의 경우, 성능을 최적화하고 비용을 절감하기 위해 LLM 응답을 캐싱할 수 있습니다.
...
캐싱된 프롬프트 실행기를 생성하려면 다음 단계를 수행하세요:
...
!!!note
    * 캐싱된 프롬프트 실행기에서 `executeStreaming()`을 호출하면 응답이 단일 청크(single chunk)로 생성됩니다.
    * 캐싱된 프롬프트 실행기에서 `moderate()`를 호출하면 요청을 내포된(nested) 프롬프트 실행기로 전달하며 캐시를 사용하지 않습니다.
    * 객관식 응답(`executeMultipleChoice()`)의 캐싱은 지원되지 않습니다.