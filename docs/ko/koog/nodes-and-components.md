...
각 노드는 본질적으로 특정 유형의 입력을 받아 특정 유형의 출력을 반환하는 함수(Kotlin) 또는 액션(Java)입니다.
...
graph LR
    ...
    subgraph node ["구조화된 응답 LLM 요청 노드"]
        execute(구조화된 LLM 요청)
    end
...
graph LR
    ...
    subgraph node ["스트리밍 응답 LLM 요청 노드"]
        execute(스트리밍 LLM 요청)
    end
...
LLM 프롬프트에 사용자 메시지를 추가하고 도구 호출이 활성화된 상태에서 여러 LLM 응답을 받는 노드입니다. 자세한 내용은 Kotlin의 경우 [nodeLLMRequestMultiple](api:agents-core::ai.koog.agents.core.dsl.extension.nodeLLMRequestMultiple)을, Java의 경우 [AIAgentNode.llmRequestMultiple()](api:agents-core::ai.koog.agents.core.agent.entity.AIAgentNode.Companion.llmRequestMultiple)을 참조하세요.
...
graph LR
    ...
    subgraph node ["다중 도구 결과 후속 조치 노드"]
        execute(여러 응답을 예상하는 LLM 요청)
    end
...
- [runMode](api:agents-core::ai.koog.agents.core.agent.entity.SubgraphWithTaskBuilder.runMode): 작업 중 도구 호출이 실행되는 방식 제어(기본값은 순차적). 기본 모델/실행기에서 지원하는 경우 다른 도구 실행 전략으로 전환하는 데 사용합니다.
- [assistantResponseRepeatMax](api:agents-core::ai.koog.agents.core.agent.entity.SubgraphWithTaskBuilder.assistantResponseRepeatMax): 작업을 완료할 수 없다고 결론 내리기 전까지 허용되는 어시스턴트 응답 횟수를 제한합니다(제공되지 않으면 안전한 내부 제한값으로 설정됨).
...
이 서브그래프는 워크플로의 끝에서 LLM이 검증 도구를 호출하여 작업이 성공적으로 완료되었는지 확인하도록 보장합니다. 이 검증이 마지막 단계로 수행되도록 보장하며, 작업의 성공 여부와 상세 피드백을 나타내는 [CriticResult](api:agents-core::ai.koog.agents.ext.agent.CriticResult)를 반환합니다.
예시는 다음과 같습니다:
