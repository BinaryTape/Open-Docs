...
## 히스토리 압축 시점 {id="when-to-compress-history"}
...
## 히스토리 압축 구현 {id="history-compression-implementation"}
...
### 전략 그래프에서의 히스토리 압축 {id="history-compression-in-a-strategy-graph"}
...
### 커스텀 노드에서의 히스토리 압축 {id="history-compression-in-a-custom-node"}
...
## 히스토리 압축 전략 {id="history-compression-strategies"}

선택적인 `strategy` 매개변수를 사용하여 압축 프로세스를 커스터마이징할 수 있습니다:

* **Kotlin**: 전략을 `nodeLLMCompressHistory(strategy=...)` 또는 `replaceHistoryWithTLDR(strategy=...)`에 전달합니다.
* **Java**: `.compressionStrategy()` 빌더 메서드를 사용합니다.

프레임워크는 여러 내장 전략을 제공합니다.

### WholeHistory (기본값) {id="wholehistory-default"}
...
## 커스텀 히스토리 압축 전략 구현 {id="custom-history-compression-strategy-implementation"}
...
## 압축 중 메모리 보존 {id="memory-preservation-during-compression"}
