...
* 히스토리가 너무 길어질 때 히스토리를 압축하려면, 헬퍼 함수를 정의하고 다음 로직을 사용하여 전략 그래프에 `nodeLLMCompressHistory` 노드를 추가할 수 있습니다:
...
`nodeLLMCompressHistory(strategy=...)` 또는 `replaceHistoryWithTLDR(strategy=...)`에 선택적인 `strategy` 매개변수를 전달하여 압축 프로세스를 커스터마이징할 수 있습니다.
...
모든 히스토리 압축 방법에는 압축 중에 메모리 관련 메시지를 보존할지 여부를 결정하는 `preserveMemory` 매개변수가 있습니다.
이러한 메시지는 메모리에서 검색된 사실을 포함하거나 메모리 기능이 활성화되지 않았음을 나타내는 메시지입니다.

다음과 같이 `preserveMemory` 매개변수를 사용할 수 있습니다:
