...
AI 에이전트는 사용자 메시지, 어시스턴트 응답, 도구 호출, 도구 응답을 포함하는 메시지 히스토리를 유지합니다.
...
전체 메시지 히스토리를 고정된 크기의 청크(chunk)로 분할하고 각 청크를 독립적으로 TLDR 메시지로 압축하는 전략입니다.
...
제공된 개념 목록과 관련된 특정 사실을 히스토리에서 검색하여 검색합니다.
