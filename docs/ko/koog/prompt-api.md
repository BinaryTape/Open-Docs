...
- LLM 및 해당 라이프사이클 관리를 위한 더 높은 수준의 추상화가 필요하거나, 여러 제공자 간에 일관된 API로 프롬프트를 실행하고 동적으로 전환하고 싶다면 프롬프트 실행기를 사용하세요. 자세한 내용은 [프롬프트 실행기로 프롬프트 실행](#running-prompts-with-prompt-executors)을 참조하세요.
