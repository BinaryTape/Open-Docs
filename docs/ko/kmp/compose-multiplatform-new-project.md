...
## 기반 다지기 {id="lay-the-foundation"}

시작하기 위해 새로운 `App()` 컴포저블을 구현합니다.

1. `shared/src/commonMain/kotlin` 폴더에서 `App.kt` 파일을 열고 코드를 다음 `App()` 컴포저블로 교체합니다.
   ...
   * 두 컴포저블은 `timeAtLocation` 속성이라는 단일 공유 상태로 연결됩니다. `Text` 컴포저블은 이 상태를 관찰(observer)합니다.
...
6. 앱이 자동으로 업데이트되는 것을 확인하려면 수정된 파일을 저장(<shortcut>⌘ S</shortcut> / <shortcut>Ctrl+S</shortcut>)하세요. 모양이 개선된 것을 볼 수 있습니다.
   ...
   ![데스크톱에서 크기가 조정된 Compose Multiplatform 앱의 창](first-compose-project-on-desktop-4.png){width=350}
...
3. 타겟팅하는 각 플랫폼에서 애플리케이션을 실행합니다. 표시되는 시간은 여전히 하드코딩되어 있지만, 이제 텍스트 필드에 타임존을 입력할 수 있습니다.
...
<Tabs>
    ...
    <TabItem id="web-user-input" title="웹">
        <img src="first-compose-project-on-web-3.png" alt="웹용 Compose Multiplatform 앱의 사용자 입력" width="500"/>
    </TabItem>
</Tabs>

## 시간 계산하기 {id="calculate-time"}

다음 단계는 입력된 값을 사용하여 시간을 계산하는 것입니다. 이를 위해 `currentTimeAt()` 함수를 만듭니다.

1. `shared/src/commonMain/kotlin/compose.project.demo/App.kt` 파일로 돌아가 다음 함수를 추가합니다.
   ...
   fun currentTimeAt(location: String): String? {
    ...
    > [kotlinx-datetime](https://github.com/Kotlin/kotlinx-datetime) 라이브러리가 아직 프로젝트에 추가되지 않았다면, [새 의존성 추가](compose-multiplatform-modify-project.md#add-a-new-dependency) 섹션의 안내를 따르세요.
    >
    {style="note"}
...
<Tabs>
    ...
    <TabItem id="web-time-display" title="웹">
        <img src="first-compose-project-on-web-4.png" alt="웹용 Compose Multiplatform 앱의 시간 표시" width="500"/>
    </TabItem>
</Tabs>

## 스타일 개선하기 {id="improve-the-style"}
...
1. 이러한 문제를 해결하기 위해 다음 버전의 `App` 컴포저블을 사용합니다.
    ...
    * `Text` 컴포저블은 사용 가능한 가로 공간을 모두 채우고 콘텐츠를 중앙에 정렬합니다.
...
<Tabs>
    ...
    <TabItem id="web-improved-style" title="웹">
        <img src="first-compose-project-on-web-5.png" alt="웹용 Compose Multiplatform 앱의 개선된 스타일" width="500"/>
    </TabItem>
</Tabs>

## 디자인 리팩터링하기 {id="refactor-the-ui"}

애플리케이션은 잘 작동하지만 오타에 취약합니다. 예를 들어 사용자가 "France" 대신 "Franse"를 입력하면 앱이 이를 처리할 수 없습니다. 사용자가 미리 정의된 목록에서 국가를 선택하도록 하는 것이 더 편리합니다.

1. 이를 위해 `App()` 컴포저블과 `currentTimeAt()` 함수를 업데이트하고 보조 데이터 클래스를 추가합니다.
...
2. IDE의 안내에 따라 누락된 의존성을 임포트합니다.
   `Row()`를 임포트할 때는 `@Composable` 버전을 선택하세요.
...
<Tabs>
   ...
   <TabItem id="web-country-list" title="웹">
        <img src="first-compose-project-on-web-6.png" alt="웹용 Compose Multiplatform 앱의 국가 목록" width="500"/>
    </TabItem>
</Tabs>
...
국가 이름 목록도 작동하지만 사용자 경험 측면에서 아주 좋지는 않습니다. 국가 이름 옆에 국기 이미지를 추가하여 목록을 개선할 수 있습니다.

Compose Multiplatform은 모든 플랫폼의 공통 코드를 통해 리소스에 접근할 수 있는 라이브러리를 제공합니다. Kotlin Multiplatform 마법사가 이미 이 라이브러리를 추가하고 구성했으므로, 바로 리소스를 로드할 수 있습니다.
...
3. 애플리케이션을 빌드하거나 실행하여 추가된 리소스에 대한 접근자(accessors)가 포함된 `Res` 클래스를 생성합니다.
...
<Tabs>
   ...
   <TabItem id="web-flags" title="웹">
        <img src="first-compose-project-on-web-7.png" alt="웹용 Compose Multiplatform 앱의 국기 표시" width="500"/>
    </TabItem>
</Tabs>
