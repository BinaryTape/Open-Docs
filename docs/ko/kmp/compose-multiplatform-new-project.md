...
4. 이를 해결하려면 `composeApp/src/desktopMain/kotlin`에서 `main.kt` 파일을 다음과 같이 업데이트하세요.
   ...
   fun main() = application {
    ...
    > 2. **Compose Hot Reload (Alpha)로 'main [desktop]' 실행**을 선택하세요.
    ...
    > 
    > Compose Hot Reload는 현재 [알파](https://kotlinlang.org/components-stability.html#stability-levels-explained) 단계이므로 기능이 변경될 수 있습니다.
    >
    {style="tip"}
