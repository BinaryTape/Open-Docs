...
## Compose Multiplatform 테스트와 Jetpack Compose의 차이점 {id="how-compose-multiplatform-testing-is-different-from-jetpack-compose"}
...
## Compose Multiplatform에서 테스트 작성 및 실행하기 {id="writing-and-running-tests-with-compose-multiplatform"}

먼저, 모듈에 테스트를 위한 소스 세트와 필요한 의존성을 추가합니다. 그런 다음 예제 테스트를 작성 및 실행하고 필요에 맞게 수정해 보십시오.

### 테스트 소스 세트 생성 및 테스트 라이브러리 의존성 추가 {id="create-the-test-source-set-and-add-the-testing-library-to-dependencies"}

구체적인 예를 제공하기 위해, 이 페이지의 지침은 [Kotlin Multiplatform 마법사](https://kmp.jetbrains.com/)에서 생성된 프로젝트 구조를 따릅니다. 기존 프로젝트에 테스트를 추가하는 경우, 경로와 명령에서 `shared`를 테스트하려는 모듈 이름으로 바꿔야 할 수도 있습니다.

Kotlin Multiplatform 마법사에서 **Include tests** 옵션을 선택하면 `*Test` 소스 세트를 포함한 테스트를 위한 기본 구조가 생성됩니다.

UI 테스트 라이브러리 설정:
`shared/build.gradle.kts` 파일의 `commonTest` 소스 세트 의존성에 UI 테스트 라이브러리를 추가합니다.
...
Android용 인스트루먼티드(에뮬레이터) 테스트를 실행해야 하는 경우, 다음과 같이 Gradle 구성을 수정합니다:
   
1. `shared/build.gradle.kts` 파일의 `androidLibrary {}` 블록에 다음 코드를 추가하여 인스트루먼티드 테스트 소스 세트가 공통 테스트 소스 세트에 의존하도록 구성합니다.
...
2. `androidApp/build.gradle.kts` 파일의 `android.defaultConfig {}` 블록에 다음 코드를 추가하여 Android 테스트 인스트루먼테이션 러너를 구성합니다:
...
3. `androidApp/build.gradle.kts` 파일의 루트 `dependencies {}` 블록에 필요한 의존성을 추가합니다:
...
5. 인스트루먼티드 테스트를 위한 `shared/src/androidDeviceTest` 소스 세트를 생성해야 함을 잊지 마십시오. 대부분의 테스트가 `commonTest`에 있더라도, `androidDeviceTest` 디렉토리에는 테스트에 사용할 일반 액티비티를 가리키는 `AndroidManifest.xml` 파일이 최소한 하나는 있어야 합니다:
...
### 공통 테스트 작성 및 실행 {id="write-and-run-common-tests"}

`shared/src/commonTest/kotlin/<package>` 디렉토리에 `ExampleTest.kt`라는 이름의 파일을 생성하고 다음 코드를 복사해 넣습니다:
...
두 가지 옵션이 있습니다:
* IntelliJ IDEA에서 `myTest()` 함수 옆의 거터(gutter)에 있는 초록색 실행 아이콘을 클릭하고, **Run | ExampleTest.myTest**를 선택한 다음 테스트를 위한 iOS 타겟을 선택합니다.
* 터미널에서 다음 명령을 실행합니다:
