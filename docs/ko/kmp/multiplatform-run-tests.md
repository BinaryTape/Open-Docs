...
1. `commonMain/kotlin` 디렉터리에 `org.kmp.testing` 패키지를 새로 만듭니다.
2. 이 패키지에 `CurrentRuntime.kt` 파일을 생성하고 다음 구현으로 업데이트합니다:
...
3. `sharedLogic/src` 디렉터리 내에 테스트용 디렉터리를 생성합니다:
   ...
   1. `sharedLogic/src` 디렉터리를 마우스 오른쪽 버튼으로 클릭하고 **New | Directory**를 선택합니다. IDE에 옵션 목록이 표시됩니다.
   2. `androidHostTest/kotlin` 경로를 입력하여 선택 범위를 좁힌 다음, 목록에서 선택합니다:

      ![Android 테스트 디렉터리 생성](create-android-test-dir.png){width=350}

4. `androidHostTest/kotlin` 디렉터리에 `org.kmp.testing` 패키지를 새로 만듭니다.
5. 이 패키지에 `AndroidRuntimeTest.kt` 파일을 생성하고 다음 Android 테스트로 업데이트합니다. 테스트를 통과시키려면 런타임의 실제 이름과 버전을 설정해야 합니다(하지만 테스트가 어떻게 실패하는지 확인하는 것도 유용합니다):
...
테스트를 실행하면 IDE의 출력 외에도 HTML 보고서가 생성됩니다. `sharedLogic/build/reports/tests` 디렉터리에서 찾을 수 있습니다:
