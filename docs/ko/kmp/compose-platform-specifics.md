...
이 페이지에서는 컴포즈 멀티플랫폼으로 다양한 플랫폼을 위한 공유 UI 코드를 작성할 때 예상해야 할
...
1.  프로젝트에 Android 타겟이 없다면 추가합니다 (프리뷰 메커니즘은 Android 라이브러리를 사용합니다).
    > 컴포즈 멀티플랫폼 [1.10.0](whats-new-compose-110.md#unified-preview-annotation)부터는 `commonMain` 소스 세트의 모든 타겟 플랫폼에 `@Preview` 어노테이션을 사용할 수 있습니다.
    >
    >{style="note"}
