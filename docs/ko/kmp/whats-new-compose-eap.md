...
### Kotlin 2.0.0을 위해 새로운 Compose 컴파일러 Gradle 플러그인이 필요합니다. {id="new-compose-compiler-gradle-plugin-is-required-for-kotlin-2-0-0"}
...
### 리소스(Resources) {id="resources"}

#### 안정적인 리소스 라이브러리 {id="stable-resource-library"}

[리소스 라이브러리 API](compose-multiplatform-resources.md)의 대부분이 이제 안정화(Stable)된 것으로 간주됩니다.

#### Compose Multiplatform 리소스를 사용하는 멀티모듈 프로젝트 지원 {id="support-for-multimodule-projects-with-compose-multiplatform-resources"}
...
#### 멀티플랫폼 리소스를 위한 구성 DSL {id="configuration-dsl-for-multiplatform-resources"}
...
#### 리소스 URI 생성을 위한 공개 함수 {id="public-function-for-generating-resource-uri"}
...
#### 이미지 및 폰트를 위한 실험적 바이트 배열 함수 {id="experimental-byte-array-functions-for-images-and-fonts"}
...
### 실험적 내비게이션 라이브러리 {id="experimental-navigation-library"}
...
### Lifecycle 라이브러리 {id="lifecycle-library"}
...
### Kotlin 2.0.0 지원 {id="support-for-kotlin-2-0-0"}
...
### BasicTextField2 기본 지원 {id="basic-support-of-basictextfield2"}
...
### DialogWindow를 위한 alwaysOnTop 플래그 {id="alwaysontop-flag-for-dialogwindow"}
...
### 접근성(Accessibility) 지원 개선 {id="accessibility-support-improvements"}
...
### iOS 17 이상을 위한 선택 컨테이너 돋보기 {id="selection-container-magnifier-for-ios-17-and-higher"}
...
### 대화 상자 중앙 정렬을 위한 소프트웨어 키보드 인셋 {id="software-keyboard-inset-for-dialog-centering"}
...
### 기본적인 IME 키보드 지원 {id="basic-ime-keyboard-support"}

Compose Multiplatform의 웹 타겟에서 이제 가상(IME) 키보드를 기본적으로 지원합니다.

## Gradle 플러그인 {id="gradle-plugin"}

### macOS 최소 버전 수정 가능 {id="possibility-to-modify-the-macos-minimum-version"}
...
### Proguard 지원을 포함한 uber JAR 생성 옵션 {id="option-to-create-uber-jars-with-proguard-support"}
...
### MissingResourceException {id="missingresourceexception"}
...
### NativeCodeGeneratorException {id="nativecodegeneratorexception"}
