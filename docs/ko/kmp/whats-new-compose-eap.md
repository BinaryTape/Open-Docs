...
*   [Compose Multiplatform 리소스가 포함된 다중 모듈 프로젝트 지원](#support-for-multimodule-projects-with-compose-multiplatform-resources)
...
*   Lifecycle 라이브러리 `org.jetbrains.androidx.lifecycle:lifecycle-*:2.8.0-rc02`. [Jetpack Lifecycle 2.8.0-rc01](https://developer.android.com/jetpack/androidx/releases/lifecycle#2.8.0-rc01) 기반.
...
Kotlin 2.0.0-RC2부터 Compose Multiplatform은 새로운 Compose 컴파일러 Gradle 플러그인이 필요합니다.
...
#### Compose Multiplatform 리소스가 포함된 다중 모듈 프로젝트 지원

Compose Multiplatform 1.6.10-beta01부터
어떤 Gradle 모듈과 소스 세트에도 리소스를 저장할 수 있으며, 리소스가 포함된 프로젝트와 라이브러리를 게시할 수 있습니다.

다중 모듈 지원을 활성화하려면 프로젝트를 Kotlin 버전 2.0.0 이상 및 Gradle 7.6 이상으로 업데이트하세요.

#### 다중 플랫폼 리소스용 구성 DSL

이제 프로젝트에서 `Res` 클래스 생성을 세부 조정할 수 있습니다: 클래스의 모달리티와 할당된 패키지를 변경하고, 항상, 절대, 또는 리소스 라이브러리에 명시적 종속성이 있을 때만 생성하는 조건을 선택할 수 있습니다.

자세한 내용은 [문서 섹션](compose-multiplatform-resources.md#configuration)을 참조하세요.

#### 리소스 URI 생성을 위한 공개 함수

새로운 `getUri()` 함수를 사용하면 리소스의 플랫폼 종속 URI를 외부 라이브러리에 전달하여
해당 라이브러리가 파일에 직접 액세스할 수 있습니다.
자세한 내용은 [문서](compose-multiplatform-resources.md#accessing-multiplatform-resources-from-external-libraries)를 참조하세요.

#### 문자열 리소스의 복수형

이제 다른 다중 플랫폼 문자열 리소스와 함께 복수형(수량 문자열)을 정의할 수 있습니다.
자세한 내용은 [문서](compose-multiplatform-resources.md#plurals)를 참조하세요.

#### 3자리 로케일 지원

[언어 한정자](compose-multiplatform-resources.md#language-and-regional-qualifiers)는 이제 로케일에 대해 alpha-3 (ISO 639-2) 코드를 지원합니다.

#### 이미지 및 폰트용 실험적 바이트 배열 함수

폰트와 이미지를 바이트 배열로 가져올 수 있는 두 가지 함수인 `getDrawableResourceBytes` 및 `getFontResourceBytes`를 사용해 볼 수 있습니다.
이 함수들은 서드파티 라이브러리에서 다중 플랫폼 리소스에 액세스하는 데 도움이 되도록 고안되었습니다.
...
이 버전의 주요 제한 사항:
*   [딥 링크](https://developer.android.com/guide/navigation/design/deep-link)(처리 또는 따라가기)는 아직 지원되지 않습니다.
*   [BackHandler](https://developer.android.com/develop/ui/compose/libraries#handling_the_system_back_button) 함수와
    [예측형 뒤로 가기 제스처](https://developer.android.com/guide/navigation/custom-back/predictive-back-gesture)는
    Android에서만 지원됩니다.

### Lifecycle 라이브러리

Jetpack lifecycle 기반의 공통 라이프사이클 라이브러리가 이제 사용 가능합니다. 자세한 내용은 [문서](compose-lifecycle.md)를 참조하세요.

이 라이브러리는 주로 공통 내비게이션 기능을 지원하기 위해 도입되었지만, 실험적인 교차 플랫폼 `ViewModel` 구현도 제공하며, 프로젝트에 구현할 수 있는 공통 `LifecycleOwner` 인터페이스도 포함합니다.
...
최신 컴파일러 버전으로 Compose Multiplatform을 사용하려면 프로젝트의 모듈에 플러그인을 적용하세요(자세한 내용은 [마이그레이션 가이드](compose-compiler.md#migrating-a-compose-multiplatform-project) 참조).
...
프로젝트에서 반드시 필요하거나 테스트 목적으로 사용하세요. 하지만 다루지 않은 엣지 케이스가 있을 수 있음을 유념하세요.
예를 들어, `BasicTextField2`는 현재 IME 이벤트를 지원하지 않으므로, 중국어, 일본어 또는 한국어용 가상 키보드를 사용할 수 없습니다.

컴포넌트의 전체 지원과 다른 플랫폼에 대한 지원은 Compose Multiplatform 1.7.0 릴리스에 예정되어 있습니다.

### DialogWindow의 alwaysOnTop 플래그
...
*   대화 상자와 팝업이 접근성 기능과 제대로 통합되었습니다.
*   `UIKitView` 및 `UIKitViewController`를 사용하여 생성된 상호 운용 뷰가 이제 접근성 서비스에 의해 접근 가능합니다.
...
### iOS 17 이상에서 선택 컨테이너 확대기

iOS의 Compose Multiplatform 선택 컨테이너는 이제 네이티브 확대 도구를 에뮬레이트합니다.

![텍스트 확대기가 활성화된 iPhone 채팅 앱 스크린샷](compose-1610-ios-magnifier.png){width=390}

### 대화 상자 중앙 정렬을 위한 소프트 키보드 인셋

`Dialog` 컴포저블의 동작이 이제 Android와 정렬됩니다. 소프트웨어 키보드가 화면에 나타나면, 대화 상자가 애플리케이션 창의 유효 높이를 고려하여 중앙에 정렬됩니다.
`DialogProperties.useSoftwareKeyboardInset` 속성을 사용하여 이를 비활성화할 수 있는 옵션이 있습니다.
...
### Proguard 지원과 함께 uber JAR 생성 옵션

이제 ProGuard Gradle 태스크를 사용하여 uber JAR(애플리케이션과 모든 종속성의 JAR를 포함하는 복합 패키지)을 생성할 수 있습니다.
...
이를 해결하려면 프로젝트의 `build` 디렉터리(프로젝트의 루트 및 모듈 폴더에 있는 폴더 포함)를 삭제하세요.
...
자세한 내용은 [GitHub 이슈](https://github.com/JetBrains/compose-multiplatform/issues/4809)를 팔로우하세요.