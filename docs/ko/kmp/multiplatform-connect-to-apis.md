...
Android에서는 `AndroidPlatform`의 인스턴스를 생성하고 이를 `application()` 함수에 전달해야 하며, iOS에서는 유사하게 `IOSPlatform`의 인스턴스를 생성하고 전달해야 합니다. 이러한 진입점은 애플리케이션의 진입점이 될 필요는 없지만, 이곳에서 공유 모듈의 특정 기능을 호출할 수 있습니다.
