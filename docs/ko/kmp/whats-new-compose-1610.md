...
## 중대한 변경 사항: 새로운 Compose 컴파일러 Gradle 플러그인 {id="breaking-change-new-compose-compiler-gradle-plugin"}
...
#### 문자열 리소스의 복수형(Plurals) {id="plurals-for-string-resources"}
...
#### 3자리 로케일 지원 {id="support-three-letter-locales"}
...
### Dialog 중앙 정렬을 위한 소프트웨어 키보드 인셋 {id="software-keyboard-inset-for-dialog-centering"}
...
### 알파 단계의 Kotlin/Wasm 지원 {id="kotlin-wasm-support-in-alpha"}
...
### 기본 IME 키보드 지원 {id="basic-ime-keyboard-support"}

Compose Multiplatform의 웹 타겟에서 이제 가상(IME) 키보드를 기본적으로 지원합니다.

## Gradle 플러그인 {id="gradle-plugin"}

### macOS 최소 버전 수정 기능 {id="possibility-to-modify-the-macos-minimum-version"}
...
### ProGuard 지원이 포함된 uber JAR 생성 옵션 {id="option-to-create-uber-jars-with-proguard-support"}
...
### 알려진 문제: MissingResourceException {id="known-issue-missingresourceexception"}
