...
오픈소스 UI 소프트웨어 프레임워크인 React Native는 Meta Platforms(이전 Facebook)에 의해 2015년(Flutter보다 약간 일찍)에 개발되었습니다. 이는 Facebook의 `JavaScript` 라이브러리인 `React`를 기반으로 하며, 개발자가 네이티브로 렌더링되는 크로스 플랫폼 모바일 앱을 구축할 수 있도록 합니다.
