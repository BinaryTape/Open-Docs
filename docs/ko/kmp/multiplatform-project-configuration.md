...
* [iOS 앱에서 Kotlin Multiplatform 모듈이 생성한 iOS 프레임워크를 어떻게 사용하나요?](#connect-a-kotlin-multiplatform-module-to-an-ios-app)
...
iOS 앱에서 Kotlin Multiplatform 공유 모듈을 사용하려면 먼저 이 공유 모듈에서
[iOS 프레임워크](https://developer.apple.com/library/archive/documentation/MacOSX/Conceptual/BPFrameworks/Concepts/WhatAreFrameworks.html)를
생성해야 합니다. 그런 다음, 이를 iOS 프로젝트에 종속성으로 추가해야 합니다.

일반적으로 구현 방식이 다른 두 가지 옵션이 있습니다:

* 로컬 종속성. Kotlin 빌드는 iOS 빌드와 직접 상호작용합니다.
* 원격 종속성. Kotlin 빌드는 iOS 프레임워크를 생성하며, 이를 패키지 관리자를 사용하여 iOS 프로젝트에 연결합니다.

iOS 통합을 위한 모든 사용 가능한 옵션을 검토하려면 [iOS 통합 방법](multiplatform-ios-integration-overview.md)을 참조하세요.

## 모듈 구성

Kotlin Multiplatform 프로젝트에서 사용할 수 있는 모듈 구성 옵션은 단일 모듈 또는 여러 공유 모듈 두 가지가 있습니다.
...
Android 앱은 Kotlin Multiplatform 공유 모듈에 일반 Kotlin 모듈처럼 종속될 수 있습니다. 그러나 iOS는 Kotlin을 직접 사용할 수 없으므로, iOS 앱은 Kotlin Multiplatform 모듈이 생성한 iOS 프레임워크에 종속되어야 합니다.
...
> 엄브렐러 프레임워크 번들에는 프로젝트의 모든 공유 모듈이 포함되며 iOS 앱으로 가져와집니다.
>
{style="tip"}
...
엄브렐러 프레임워크로 일부 모듈만 내보낼 수 있습니다. 일반적으로 프레임워크 아티팩트가 원격 종속성으로 사용될 때 이러한 방식을 취합니다. 주요 이유는 자동 생성된 코드를 제외하여 최종 아티팩트의 크기를 줄이는 것입니다.
...
새로운 Kotlin Multiplatform 프로젝트와 기존 Kotlin Multiplatform 프로젝트에서 사용할 수 있는 다양한 저장소 구성 옵션이 있으며, 이는 단일 저장소를 사용하거나 여러 저장소를 조합하는 방식입니다.
...
이 워크플로는 일반적으로 Android 및 iOS 팀 구성원 모두 공유 Kotlin Multiplatform 코드를 편집하려는 경우에 사용됩니다. iOS 개발자는 IntelliJ IDEA 또는 Android Studio를 설치하고 Kotlin 및 Gradle에 대한 기본적인 지식이 있어야 합니다.
...
이 워크플로는 모든 팀 구성원이 전체 프로젝트의 코드를 편집할 준비가 되어 있을 때 가장 효과적입니다. 여기에는 공통 부분에 변경 사항을 적용한 후 Android 및 iOS 부분 모두에 대한 작업이 포함됩니다. 이상적으로는 모든 팀 구성원이 공통 코드에 변경 사항을 적용한 후 두 앱을 모두 열고 실행할 수 있도록 IntelliJ IDEA/Android Studio와 Xcode가 설치되어 있어야 합니다.
...
<td>
  <list>
    ...
    <li>iOS 개발자는 IntelliJ IDEA 또는 Android Studio와 Gradle 사용법을 배워야 합니다.</li>
...
원격 배포는 프레임워크 아티팩트가 Swift Package Manager를 사용하거나 CocoaPod으로 게시되어 iOS 앱에서 사용되는 것을 의미합니다. Android 앱은 바이너리 종속성을 로컬 또는 원격으로 사용할 수 있습니다.
...
<tr>
<td>프로젝트에 참여하지 않는 iOS 팀 구성원은 Kotlin 코드를 작성하거나 IntelliJ IDEA/Android Studio 또는 Gradle과 같은 도구 사용법을 배울 필요가 없습니다.
      이는 팀의 진입 장벽을 크게 낮춥니다.</td>
<td>
  <list>
...
원격 배포 워크플로의 경우 Swift Package Manager를 사용합니다. 로컬 배포 워크플로의 경우 프레임워크를 직접 통합합니다.
