...
이 문서는 KMP 애플리케이션을 빌드, 테스트 및 배포하기 위해 [TeamCity](https://www.jetbrains.com/teamcity/)를 설정하는 방법을 설명합니다. 
...
3. **Create**를 클릭하면 TeamCity에서 실제 빌드 작업을 수행할 객체의 유형(빌드 구성 또는 파이프라인)을 선택하라는 메시지가 표시됩니다.
   ...
   <deflist type="medium">
   <def title="빌드 구성 (Build configuration)">
...
YAML 파이프라인 에이전트로 전환하고 다음 마크업을 붙여넣어 첫 번째 잡을 설정합니다.
