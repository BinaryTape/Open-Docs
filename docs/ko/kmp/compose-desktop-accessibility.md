...
Compose Multiplatform은 [Jetpack Compose](https://developer.android.com/jetpack/compose)를 기반으로 구축되어, 대부분의 접근성 기능을 모든 플랫폼의 공통 코드에서 사용할 수 있게 해줍니다. 현재 데스크톱의 접근성 지원 상태는 다음과 같습니다:
...
접근성 기능이 포함된 네이티브 배포판을 만들려면 `modules` DSL 메서드를 사용하여 `jdk.accessibility` 모듈을 추가하세요:
...
macOS에서 애플리케이션 요소의 접근성 정보를 테스트하려면 [Accessibility Inspector](https://developer.apple.com/documentation/accessibility/accessibility-inspector)
(**Xcode** | **Open Developer Tool** | **Accessibility Inspector**)를 사용할 수 있습니다:
