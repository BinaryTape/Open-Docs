...
* _androidApp_은 Android 애플리케이션으로 빌드되는 Kotlin 모듈입니다. Gradle을 빌드 시스템으로 사용합니다. composeApp 모듈은 shared 모듈에 의존하며, 이를 일반적인 Android 라이브러리처럼 사용합니다.
* _iosApp_은 iOS 애플리케이션으로 빌드되는 Xcode 프로젝트입니다.
  `sharedLogic` 모듈에 의존하며, 이는 iOS 프레임워크(framework)로 내보내집니다.
  IDE 마법사로 생성된 Kotlin Multiplatform 프로젝트는 [직접 통합(direct integration)](multiplatform-direct-integration.md)을 통해 일반 프레임워크 의존성을 사용합니다.
* _sharedLogic_은 Android와 iOS 애플리케이션 모두에 공통적인 로직을 포함하는 멀티플랫폼 모듈입니다.
* _sharedUI_는 Compose Multiplatform UI 코드가 포함된 모듈입니다. 이 프로젝트에서는 Android 앱에서만 사용되지만, 준비가 되면 언제든지 다른 타겟에서도 사용할 수 있는 멀티플랫폼 모듈입니다.

`iosApp`을 제외한 모든 모듈은 [Gradle](https://kotlinlang.org/docs/gradle.html)을 빌드 시스템으로 사용합니다.
...
`commonMain` 소스 세트는 공유 Kotlin 코드를 포함하며, 플랫폼 소스 세트는 각 타겟에 특정한 Kotlin 코드를 사용합니다.
...
2. 인터페이스를 구현하는 플랫폼 전용 클래스들을 `androidMain` 및 `iosMain` 소스 세트에서 찾을 수 있습니다.
    ...
    * `AndroidPlatform` 클래스의 `name` 프로퍼티 구현은 Android 전용 코드인 `android.os.Build` 의존성을 사용합니다. 이 코드는 Kotlin/JVM으로 해석됩니다. 여기서 `java.util.Random`과 같은 JVM 전용 클래스에 접근하려고 하면 정상적으로 컴파일됩니다.
    * `IOSPlatform` 클래스의 `name` 프로퍼티 구현은 iOS 전용 코드인 `platform.UIKit.UIDevice` 의존성을 사용합니다. 이는 Kotlin/Native로 해석되며, 즉 Kotlin에서 iOS 선언을 참조할 수 있음을 의미합니다. 이 코드는 iOS 프레임워크의 일부가 되며, `iosApp` 모듈의 Swift 코드에서 임포트됩니다.

3. 각 소스 세트에는 `getPlatform()` 함수가 포함되어 있습니다. 기대 선언에는 본문(body)이 없으며, 실제 구현은 플랫폼 코드에서 제공됩니다.
...
특정 플랫폼을 위한 코드를 생성하는 동안 Kotlin 컴파일러는 기대 선언과 실제 선언을 올바른 구현이 포함된 단일 `getPlatform()` 함수로 병합합니다.
