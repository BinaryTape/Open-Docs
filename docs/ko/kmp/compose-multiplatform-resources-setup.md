...
스크립트나 다른 BCP 47 서브태그를 지정하려면 `b+` 접두사를 사용하고 이후의 모든 서브태그 하이픈(`-`)을 `+` 기호로 대체하세요. 예를 들어 `values-b+zh+Hans` 및 `values-b+zh+Hant`와 같이 작성합니다.
...
이러한 모호함을 피하려면 스크립트 전용 리소스가 있는 모든 언어에 대해 스크립트가 없는 디렉터리를 추가하세요:

```
commonMain/composeResources/
├── values/                 # 기본 리소스
├── values-zh/              # 스크립트가 없는 `zh`를 위한 리소스
├── values-b+zh+Hans/
└── values-b+zh+Hant/
```
