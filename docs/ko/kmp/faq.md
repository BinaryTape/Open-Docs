...
### Kotlin Multiplatform으로 UI를 공유할 수 있나요?
...
Kotlin Multiplatform 프로젝트 작업에는 Android Studio IDE를 사용하는 것을 권장합니다. 사용 가능한 다른 대안에 대해서는 [권장 IDE 및 코드 에디터](recommended-ides.md)를 읽어보세요.
...
[안드로이드 애플리케이션을 iOS에서 동작하게 만들기](multiplatform-integrate-in-existing-app.md) 단계별 튜토리얼에서 기존 안드로이드 애플리케이션을 네이티브 UI와 함께 iOS에서 동작하게 만드는 방법을 설명합니다. UI까지 Compose Multiplatform으로 공유하고 싶다면 [관련 답변](#jetpack-compose를-사용하는-기존-안드로이드-애플리케이션이-있습니다-다른-플랫폼으로-마이그레이션하려면-어떻게-해야-하나요)을 확인하세요.
...
자세한 내용은 [권장 IDE](recommended-ides.md)를 참고하세요.
...
사용 방법은 [iOS와 안드로이드 간 더 많은 로직 공유하기](multiplatform-upgrade-app.md)를 참고하세요.
...
우리는 여러분이 인기 있는 플랫폼인 안드로이드, iOS, 데스크톱(Linux, macOS, Windows), 웹(Wasm 기반)의 어떤 조합 간에도 UI를 공유할 수 있기를 바랍니다. 현재 Compose Multiplatform은 안드로이드, iOS, 데스크톱에서만 Stable(안정화) 단계입니다. 자세한 내용은 [지원되는 플랫폼](supported-platforms.md)을 참고하세요.

### 프로덕션에서 Compose Multiplatform을 사용할 수 있나요?

Compose Multiplatform의 안드로이드, iOS, 데스크톱 타겟은 Stable 단계입니다. 프로덕션 환경에서 사용할 수 있습니다.

WebAssembly 기반의 Compose Multiplatform for Web 버전은 Beta 단계이며, 이는 거의 완성되었음을 의미합니다. 사용할 수는 있지만 마이그레이션 이슈가 여전히 발생할 수 있습니다. iOS, 안드로이드, 데스크톱용 Compose Multiplatform과 동일한 UI를 가집니다.
...
Android Studio IDE 사용을 권장합니다. 자세한 내용은 [권장 IDE 및 코드 에디터](recommended-ides.md)를 참고하세요.
...
* 안드로이드 전용 의존성을 사용하는 코드를 멀티플랫폼 라이브러리를 사용하도록 다시 작성할 수 있습니다. 일부 라이브러리는 이미 Kotlin Multiplatform을 지원할 수 있으므로 변경이 필요 없을 수도 있습니다. [KMP-awesome](https://github.com/terrakok/kmp-awesome) 라이브러리 목록을 확인해 보세요.
...
[우리의 로드맵을 확인해 보세요](https://blog.jetbrains.com/kotlin/2024/10/kotlin-multiplatform-development-roadmap-for-2025/).

### Compose Multiplatform은 언제 Stable이 되나요?

Compose Multiplatform은 안드로이드, iOS, 데스크톱에서 Stable 단계이며, 웹 플랫폼 지원은 Beta 단계입니다. 웹 플랫폼의 Stable 릴리스를 위해 노력하고 있으며, 정확한 날짜는 추후 발표될 예정입니다.
