...
저장소에 게시물이 중복되지 않도록 모든 아티팩트를 단일 호스트에서 게시하여 게시 중 발생할 수 있는 문제를 방지하세요. 예를 들어, Maven Central은 중복 게시물을 명시적으로 금지하며 프로세스를 실패시킵니다.

## Android 라이브러리 게시

Android 라이브러리를 게시하려면 추가 구성을 제공해야 합니다. 기본적으로 Android 라이브러리의 아티팩트는 게시되지 않습니다.

> 이 섹션은 Android Gradle 라이브러리 플러그인을 사용한다고 가정합니다.
> 플러그인 설정 또는 레거시 `com.android.library` 플러그인에서 마이그레이션하는 방법에 대한 가이드는 Android 문서의 [Android Gradle 라이브러리 플러그인 설정](https://developer.android.com/kotlin/multiplatform/plugin#migrate) 페이지를 참조하세요.
>
{style="note"}

아티팩트를 게시하려면 `shared/build.gradle.kts` 파일에 `androidLibrary {}` 블록을 추가하고 KMP DSL을 사용하여 게시를 구성합니다.
예를 들어:
...
Android Gradle 라이브러리 플러그인은 제품 플레이버 및 빌드 베리언트를 지원하지 않아 구성을 간소화합니다. 결과적으로 테스트 소스 세트 및 구성을 생성하려면 옵트인해야 합니다. 예를 들어:
...
이전에는 GitHub 액션으로 테스트를 실행할 때, 예를 들어 디버그 및 릴리스 베리언트를 별도로 지정해야 했습니다:
...
Android Gradle 라이브러리 플러그인을 사용하면 소스 세트 이름과 함께 일반 타겟만 지정하면 됩니다:
...
귀하의 라이브러리는 [JetBrains 멀티플랫폼 라이브러리 카탈로그](https://klibs.io/)에 소개될 수 있습니다.
이 플랫폼은 타겟 플랫폼을 기반으로 Kotlin Multiplatform 라이브러리를 쉽게 찾을 수 있도록 설계되었습니다.

기준을 충족하는 라이브러리는 자동으로 추가됩니다. 라이브러리를 카탈로그에 표시하는 방법에 대한 자세한 내용은 [FAQ](https://klibs.io/faq)를 참조하세요.
