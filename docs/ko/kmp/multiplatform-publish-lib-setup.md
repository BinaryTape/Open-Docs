...
필요한 모든 아티팩트를 한 번에 배포하려면 `publishAllPublicationsTo<MavenRepositoryName>` 통합 태스크를 사용하세요. 예시:
...
kotlin {
    androidLibrary {
        ...
        // 호스트 측(유닛) 테스트 활성화 및 구성
        withHostTestBuilder {}.configure {}

        // 소스 세트 이름을 지정하여 기기 테스트 활성화
...
Kotlin 2.0.0부터, Kotlin 멀티플랫폼 라이브러리의 JVM 변량과 안드로이드 변량을 구분하는 데 도움이 되도록 Gradle 속성 [`org.gradle.jvm.environment`](https://docs.gradle.org/current/userguide/variant_attributes.html#sub:jvm_default_attributes)가 모든 Kotlin 변량과 함께 자동으로 배포됩니다. 이 속성은 어떤 라이브러리 변량이 어떤 JVM 환경에 적합한지 나타내며, Gradle은 이 정보를 사용하여 프로젝트의 의존성 확인을 돕습니다. 타겟 환경은 "android", "standard-jvm" 또는 "no-jvm"이 될 수 있습니다.

`gradle.properties` 파일에 다음 Gradle 속성을 추가하여 이 속성의 배포를 비활성화할 수 있습니다:
