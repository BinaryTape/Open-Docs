...
## 플랫폼별 로캘 테스트 {id="testing-locales-on-different-platforms"}
...
## 주요 테스트 시나리오 {id="key-testing-scenarios"}

### 커스텀 로캘 (Custom locale) {id="custom-locale"}
...
### 기본 리소스 (Default resources) {id="default-resources"}
...
### 로캘별 사례 (Locale-specific cases) {id="locale-specific-cases"}
