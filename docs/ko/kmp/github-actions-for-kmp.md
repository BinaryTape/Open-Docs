[//]: # (title: Kotlin Multiplatform 애플리케이션의 지속적 통합을 위한 GitHub Actions 설정)

<web-summary>GitHub Actions를 사용하여 Kotlin Multiplatform(KMP) 앱에 대한 지속적 통합(CI/CD)을 설정하는 방법을 알아봅니다. CI를 사용하여 공통 테스트를 실행하고 iOS, Android 및 데스크톱용 아티팩트를 빌드하세요.</web-summary>

이 가이드는 GitHub Actions로 설정된 Kotlin Multiplatform 애플리케이션의 지속적 통합(CI, Continuous Integration) 예시를 보여줍니다.
`main` 브랜치에 대한 모든 푸시(push) 또는 풀 리퀘스트(pull request) 발생 시 공통 테스트를 실행하고 Android, iOS 및 데스크톱용 아티팩트(artifact)를 빌드하는 워크플로를 설정하게 됩니다.

이 가이드는 [Jetcaster KMP 샘플](https://github.com/kotlin-hands-on/jetcaster-kmp-migration/)을 기반으로 합니다.
[레포지토리에서 액션 및 워크플로 설정](https://github.com/kotlin-hands-on/jetcaster-kmp-migration/tree/main/.github)을 확인하거나, 아래의 단계별 설명을 따라갈 수 있습니다.
...
* Gradle 옵션은 Gradle 데몬(daemon)을 비활성화하고 캐싱을 포함한 병렬 실행을 활성화해야 합니다.
...
1. 테스트를 실행할 레포지토리를 체크아웃(check out)합니다.
2. 앞서 준비한 컴포지트 `gradle-setup` 액션을 사용하여 Java와 Gradle을 설정합니다.
3. `./gradlew` 명령으로 테스트를 실행합니다.
4. 테스트 리포트를 `**/build/reports/tests/` 디렉터리에 아티팩트로 업로드합니다.
...
1. 패키지를 빌드할 레포지토리를 체크아웃합니다.
2. 앞서 준비한 컴포지트 `gradle-setup` 액션을 사용하여 Java와 Gradle을 설정합니다.
3. `./gradlew` 명령으로 APK를 빌드합니다.
4. `mobile/build/outputs/apk/debug/` 디렉터리에서 빌드된 패키지를 업로드합니다.
...
1. 애플리케이션을 빌드할 레포지토리를 체크아웃합니다.
2. 앞서 준비한 컴포지트 `gradle-setup` 액션을 사용하여 Java와 Gradle을 설정합니다.
3. `xcodebuild`를 사용하여 iOS 애플리케이션을 빌드합니다. 예시는 Jetcaster KMP 샘플에 사용된 옵션을 보여줍니다.
4. 빌드된 애플리케이션이 포함된 폴더(`build/Build/Products/Debug-iphonesimulator/*` 내부의 모든 항목)를 아티팩트로 업로드합니다.

iOS 애플리케이션은 `xcodebuild`가 포함된 macOS 러너(runner, `macos-latest`)에서 빌드됩니다.
...
GitHub Actions를 사용하여 앱 스토어에 애플리케이션을 게시하는 방법에 대한 안내는 [이 주제에 관한 Marco Gomiero의 포스트 시리즈](https://www.marcogomiero.com/posts/2024/kmp-ci-ios/)를 참조하세요.