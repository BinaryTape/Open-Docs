...
> `cocoapods {}` 블록에 다른 Pod에 대한 종속성이 있는 경우 CocoaPods 통합 방식을 계속 사용해야 합니다. 현재 멀티모달(multimodal) SPM 프로젝트에서는 Pod과 Kotlin 프레임워크 모두에 대한 종속성을 동시에 가질 수 없습니다.
>
{style="warning"}
...
5. 이제 로컬 Swift 패키지에 공유 모듈을 임포트하고 Kotlin 코드를 사용할 수 있습니다.
   ...
   ![SPM 사용 예시](xcode-spm-usage.png){width=700}
