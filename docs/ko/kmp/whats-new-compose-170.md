...
### 최소 AGP 버전 8.1.0으로 상향 {id="minimum-agp-version-raised-to-8-1-0"}
...
### Java 리소스 API가 멀티플랫폼 리소스 라이브러리로 대체되며 지원 중단됨 {id="java-resources-api-is-deprecated-in-favor-of-the-multiplatform-resource-library"}
...
### iOS 네이티브 요소의 터치 처리 기본 동작 변경 {id="new-default-behavior-for-processing-touch-in-ios-native-elements"}
...
### iOS에서 최소 프레임 지속 시간 비활성화가 필수 사항이 됨 {id="disabling-minimum-frame-duration-on-ios-is-mandatory"}
...
### 데스크톱의 Modifier.onExternalDrag 지원 중단 {id="deprecated-modifier-onexternaldrag-on-desktop"}
...
## 플랫폼 공통(Across platforms) {id="across-platforms"}
...
### 타입 안전한 내비게이션(Type-safe Navigation) {id="type-safe-navigation"}
...
### 멀티플랫폼 리소스 {id="multiplatform-resources"}

#### Android 에셋으로 패키징된 리소스 {id="resources-packed-into-android-assets"}
...
#### 멀티플랫폼 폰트 캐시 {id="multiplatform-font-cache"}

Compose Multiplatform은 Android의 폰트 캐시 기능을 다른 플랫폼으로 가져와 `Font` 리소스의 과도한 바이트 읽기를 제거했습니다.

#### 멀티플랫폼 테스트 리소스 지원 {id="support-for-multiplatform-test-resources"}
...
#### 빠른 접근을 위해 문자열 ID에 매핑된 리소스 {id="resources-mapped-to-string-ids-for-easy-access"}
...
#### 바이트 배열을 ImageBitmap 또는 ImageVector로 변환하는 함수 {id="functions-for-converting-byte-arrays-into-imagebitmap-or-imagevector"}
...
### 새로운 공통 모듈 {id="new-common-modules"}
...
### Skia Milestone 126으로 업데이트 {id="skia-updated-to-milestone-126"}
...
### GraphicsLayer – 새로운 그리기 API {id="graphicslayer-a-new-drawing-api"}
...
### LocalLifecycleOwner가 Compose UI 밖으로 이동 {id="locallifecycleowner-moved-out-of-compose-ui"}
...
### 네이티브 성능 개선 {id="native-performance-improvements"}
...
### BasicTextField2에서 이름이 변경된 BasicTextField 데스크톱 도입 {id="basictextfield-renamed-from-basictextfield2-adopted-on-desktop"}
...
### ComposePanel의 렌더링 설정 {id="render-settings-for-composepanel"}
...
### Kotlin/Wasm 애플리케이션에서 skiko.js 불필요 {id="skiko-js-is-redundant-for-kotlin-wasm-applications"}
