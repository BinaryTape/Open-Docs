...
### 추가
- [IDE 플러그인] 실행 중인 데이터베이스에 대해 SQLite, MySQL, PostgreSQL 명령 실행 지원을 추가했습니다 (#2718 by @aperfilyev)
- [IDE 플러그인] Android Studio DB 인스펙터 지원을 추가했습니다 (#3107 by @aperfilyev)
- [런타임] 비동기 드라이버 지원을 추가했습니다 (#3168 by @dellisd)
- [네이티브 드라이버] 새로운 Kotlin 네이티브 메모리 모델을 지원했습니다 (#3177 by @kpgalligan)
- [JS 드라이버] SqlJs 워커용 드라이버를 추가했습니다 (#3203 by @dellisd)
- [Gradle 플러그인] SQLDelight 작업에 대한 클래스패스를 노출했습니다
- [Gradle 플러그인] 마이그레이션 스쿼시를 위한 Gradle 작업을 추가했습니다
- [Gradle 플러그인] 마이그레이션 확인 중 스키마 정의를 무시하는 플래그를 추가했습니다
- [MySQL 다이얼렉트] MySQL에서 `FOR SHARE` 및 `FOR UPDATE`를 지원했습니다 (#3098)
- [MySQL 다이얼렉트] MySQL 인덱스 힌트를 지원했습니다 (#3099)
- [PostgreSQL 다이얼렉트] `date_trunc`를 추가했습니다 (#3295 by @hfhbd)
- [JSON 확장] JSON 테이블 함수를 지원했습니다 (#3090)

### 변경
- [런타임] 드라이버 없는 `AfterVersion` 타입을 제거했습니다 (#3091)
- [런타임] `Schema` 타입을 최상위 수준으로 이동했습니다
- [런타임] 서드파티 구현을 지원하기 위해 다이얼렉트 및 리졸버를 개방했습니다 (#3232 by @hfhbd)
- [컴파일러] 실패 보고서에 컴파일에 사용된 다이얼렉트를 포함했습니다 (#3086)
- [컴파일러] 사용하지 않는 어댑터를 건너뛰었습니다 (#3162 by @eygraber)
- [컴파일러] `PrepareStatement`에서 0-기반 인덱스를 사용했습니다 (#3269 by @hfhbd)
- [Gradle 플러그인] 다이얼렉트도 문자열 대신 적절한 Gradle 종속성으로 만들었습니다 (#3085)
- [Gradle 플러그인] Gradle 검증 작업: 데이터베이스 파일 누락 시 예외를 발생시켰습니다 (#3126 by @vanniktech)

### 수정
- [Gradle 플러그인] Gradle 플러그인의 사소한 정리 및 조정 (#3171 by @3flex)
- [Gradle 플러그인] 생성된 디렉토리에 AGP 문자열을 사용하지 않았습니다
- [Gradle 플러그인] AGP 네임스페이스 속성을 사용했습니다 (#3220)
- [Gradle 플러그인] Gradle 플러그인의 런타임 종속성으로 `kotlin-stdlib`를 추가하지 않았습니다 (#3245 by @mbonnin)
- [Gradle 플러그인] 멀티플랫폼 구성을 단순화했습니다 (#3246 by @mbonnin)
- [Gradle 플러그인] JS 전용 프로젝트를 지원했습니다 (#3310 by @hfhbd)
- [IDE 플러그인] Gradle 툴링 API에 Java 홈을 사용했습니다 (#3078)
- [IDE 플러그인] IDE 플러그인 내에서 올바른 클래스 로더에 JDBC 드라이버를 로드했습니다 (#3080)
- [IDE 플러그인] 이미 존재하는 PSI 변경 중 오류를 방지하기 위해 무효화하기 전에 파일 요소를 null로 표시했습니다 (#3082)
- [IDE 플러그인] `ALTER TABLE` 문에서 새 테이블 이름의 사용처를 찾을 때 크래시되지 않도록 했습니다 (#3106)
...
- [컴파일러] RETURNING 문이 있는 업데이트 및 삭제가 쿼리를 실행하도록 보장했습니다 (#3084)
- [컴파일러] 복합 SELECT 문에서 인수 타입을 올바르게 추론했습니다 (#3096)
- [컴파일러] 공통 테이블은 데이터 클래스를 생성하지 않으므로 반환하지 않도록 했습니다 (#3097)
- [컴파일러] 가장 상위 마이그레이션 파일을 더 빠르게 찾았습니다 (#3108)
- [컴파일러] 파이프 연산자에서 널 허용 여부(nullability)를 올바르게 상속했습니다
- [컴파일러] `iif` ANSI SQL 함수를 지원했습니다
- [컴파일러] 빈 쿼리 파일 생성을 금지했습니다 (#3300 by @hfhbd)
- [컴파일러] 물음표만 있는 어댑터를 수정했습니다 (#3314 by @hfhbd)
- [PostgreSQL 다이얼렉트] Postgres 기본 키 열은 항상 널이 아님을 확인했습니다 (#3092)
- [PostgreSQL 다이얼렉트] 여러 테이블에서 이름이 같은 복사를 수정했습니다 (#3297 by @hfhbd)
- [SQLite 3.35 다이얼렉트] 변경된 테이블에서 인덱싱된 열을 삭제할 때만 오류를 표시했습니다 (#3158 by @eygraber)
...
### 추가
- [컴파일러] 그룹화된 문 끝에 반환을 지원했습니다
- [컴파일러] 다이얼렉트 모듈을 통한 컴파일러 확장 지원 및 SQLite JSON 확장을 추가했습니다 (#1379, #2087)
- [컴파일러] 값을 반환하는 PRAGMA 문을 지원했습니다 (#1106)
- [컴파일러] 표시된 열에 대한 값 타입 생성을 지원했습니다
- [컴파일러] 낙관적 잠금 및 유효성 검사 지원을 추가했습니다 (#1952)
- [컴파일러] 다중 업데이트 문을 지원했습니다
- [PostgreSQL] postgres 반환 문을 지원했습니다
- [PostgreSQL] postgres 날짜 타입을 지원했습니다
- [PostgreSQL] pg 간격을 지원했습니다
- [PostgreSQL] PG 부울을 지원하고 변경 테이블에 대한 삽입을 수정했습니다
- [PostgreSQL] Postgres에서 선택적 제한을 지원했습니다
- [PostgreSQL] PG BYTEA 타입을 지원했습니다
- [PostgreSQL] postgres SERIAL에 대한 테스트를 추가했습니다
- [PostgreSQL] 업데이트 postgres 구문을 지원했습니다
- [PostgreSQL] PostgreSQL 배열 타입을 지원했습니다
- [PostgreSQL] PG에서 UUID 타입을 올바르게 저장/검색했습니다
- [PostgreSQL] PostgreSQL NUMERIC 타입을 지원했습니다 (#1882)
- [PostgreSQL] 공통 테이블 표현식 내에서 쿼리 반환을 지원했습니다 (#2471)
- [PostgreSQL] JSON 특정 연산자를 지원했습니다
- [PostgreSQL] Postgres 복사(Copy)를 추가했습니다 (by @hfhbd)
- [MySQL] MySQL Replace를 지원했습니다
- [MySQL] NUMERIC/BigDecimal MySQL 타입을 지원했습니다 (#2051)
- [MySQL] MySQL `TRUNCATE` 문을 지원했습니다
- [MySQL] MySQL에서 JSON 특정 연산자를 지원했습니다 (by @eygraber)
- [MySQL] MySQL INTERVAL을 지원했습니다 (#2969 by @eygraber)
- [HSQL] HSQL 윈도우 기능(Window functionality)을 추가했습니다
- [SQLite] WHERE 절에서 널 허용 매개변수에 대한 동등성 검사 대체 금지 (#1490 by @eygraber)
- [SQLite] Sqlite 3.35 반환 문을 지원했습니다 (#1490 by @eygraber)
- [SQLite] GENERATED 절을 지원했습니다
- [SQLite] Sqlite 3.38 다이얼렉트 지원을 추가했습니다 (by @eygraber)

### 변경
- [컴파일러] 생성된 코드를 약간 정리했습니다
- [컴파일러] 그룹화된 문에서 테이블 매개변수 사용을 금지했습니다 (#1822)
- [컴파일러] 그룹화된 쿼리를 트랜잭션 내에 배치했습니다 (#2785)
- [런타임] 드라이버의 실행 메서드에서 업데이트된 행 수를 반환했습니다
- [런타임] `SqlCursor`를 연결에 접근하는 임계 영역으로 제한했습니다 (#2123 by @andersio)
- [Gradle 플러그인] 마이그레이션에 대한 스키마 정의를 비교했습니다 (#841)
- [PostgreSQL] PG에 대한 이중 따옴표 사용을 허용하지 않았습니다
- [MySQL] MySQL에서 `==` 사용 시 오류를 발생시켰습니다 (#2673)

### 수정
- [컴파일러] 서로 다른 테이블의 동일한 어댑터 타입이 2.0 알파에서 컴파일 오류를 일으키는 문제를 수정했습니다
- [컴파일러] 업서트 문 컴파일 문제를 수정했습니다 (#2791)
- [컴파일러] 일치하는 항목이 여러 개 있을 경우 쿼리 결과는 SELECT 문의 테이블을 사용해야 함을 수정했습니다 (#1874, #2313)
- [컴파일러] `INSTEAD OF` 트리거가 있는 뷰 업데이트를 지원했습니다 (#1018)
- [컴파일러] 함수 이름에 `from` 및 `for`를 지원했습니다
- [컴파일러] 함수 표현식에서 `SEPARATOR` 키워드 사용을 허용했습니다
- [컴파일러] `ORDER BY`에서 별칭이 지정된 테이블의 ROWID에 접근할 수 없는 문제를 수정했습니다
- [컴파일러] MySQL의 HAVING 절에서 별칭이 지정된 열 이름이 인식되지 않는 문제를 수정했습니다
- [컴파일러] 잘못된 '여러 열 발견' 오류를 수정했습니다
- [컴파일러] `PRAGMA locking_mode = EXCLUSIVE;` 설정 불가 문제를 수정했습니다
- [PostgreSQL] postgresql 열 이름 변경을 수정했습니다
- [MySQL] UNIX_TIMESTAMP, TO_SECONDS, JSON_ARRAYAGG MySQL 함수 인식 불가 문제를 수정했습니다
- [SQLite] SQLite 윈도우 기능을 수정했습니다
- [IDE 플러그인] 빈 진행률 표시기에서 이동(goto) 핸들러를 실행했습니다 (#2990)
- [IDE 플러그인] 프로젝트가 구성되지 않은 경우 하이라이트 방문자가 실행되지 않도록 보장했습니다 (#2981, #2976)
- [IDE 플러그인] 전이적 생성 코드도 IDE에서 업데이트되도록 보장했습니다 (#1837)
...
이는 2.0의 첫 번째 알파 릴리스이며 몇 가지 주요 변경 사항이 있습니다. 더 많은 ABI 주요 변경 사항이 있을 것으로 예상되므로, 이 릴리스에 종속된 라이브러리를 게시하지 마십시오(애플리케이션은 괜찮습니다).
...
- IDE 플러그인은 2.X 버전으로 업데이트해야 하며, 이는 [알파 또는 eap 채널](https://plugins.jetbrains.com/plugin/8191-sqldelight/versions/alpha)에서 찾을 수 있습니다.
- 다이얼렉트가 이제 Gradle 내에서 지정할 수 있는 종속성입니다:
...
- 기본 타입은 이제 임포트되어야 합니다 (예: `INTEGER AS Boolean`인 경우 `import kotlin.Boolean`을 임포트해야 함). 이전에 지원되던 일부 타입은 이제 어댑터가 필요합니다. 대부분의 변환 (예: `Integer AS kotlin.Int`를 수행하는 `IntColumnAdapter`)을 위한 기본 어댑터는 `app.cash.sqldelight:primitive-adapters:2.0.0-alpha01`에서 사용할 수 있습니다.

### 추가
- [IDE 플러그인] 기본 마이그레이션 제안 (by @aperfilyev)
- [IDE 플러그인] 임포트 힌트 액션을 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] Kotlin 클래스 완성(completion)을 추가했습니다 (by @aperfilyev)
- [Gradle 플러그인] Gradle 타입 세이프 프로젝트 접근자에 대한 단축키를 추가했습니다 (by @hfhbd)
- [컴파일러] 다이얼렉트에 기반하여 코드 생성을 사용자 정의했습니다 (by @MariusVolkhart)
- [JDBC 드라이버] `JdbcDriver`에 공통 타입을 추가했습니다 (by @MariusVolkhart)
- [SQLite] SQLite 3.35 지원을 추가했습니다 (by @eygraber)
- [SQLite] ALTER TABLE DROP COLUMN 지원을 추가했습니다 (by @eygraber)
- [SQLite] Sqlite 3.30 다이얼렉트 지원을 추가했습니다 (by @eygraber)
- [SQLite] SQLite에서 `NULLS FIRST/LAST`를 지원했습니다 (by @eygraber)
- [HSQL] 생성된 절에 대한 HSQL 지원을 추가했습니다 (by @MariusVolkhart)
- [HSQL] HSQL에서 명명된 매개변수 지원을 추가했습니다 (by @MariusVolkhart)
- [HSQL] HSQL INSERT 쿼리를 사용자 정의했습니다 (by @MariusVolkhart)

### 변경
- [전체] 패키지 이름이 `com.squareup.sqldelight`에서 `app.cash.sqldelight`로 변경되었습니다.
- [런타임] 다이얼렉트를 자체적으로 격리된 Gradle 모듈로 이동했습니다
- [런타임] 드라이버가 구현한 쿼리 알림으로 전환했습니다.
- [런타임] 기본 열 어댑터를 별도 모듈로 추출했습니다 (#2056, #2060)
- [컴파일러] 각 모듈에서 다시 구현하는 대신 모듈이 쿼리 구현을 생성하도록 했습니다
- [컴파일러] 생성된 데이터 클래스의 사용자 정의 `toString` 생성을 제거했습니다 (by @PaulWoitaschek)
- [JS 드라이버] `sqljs-driver`에서 `sql.js` 종속성을 제거했습니다 (by @dellisd)
- [Paging] Android Paging 2 확장을 제거했습니다
- [IDE 플러그인] SQLDelight 동기화 중 에디터 배너를 추가했습니다 (#2511)
- [IDE 플러그인] 지원되는 최소 IntelliJ 버전은 2021.1입니다

### 수정
- [런타임] 할당 및 포인터 추적을 줄이기 위해 리스너 목록을 평탄화했습니다 (by @andersio)
- [IDE 플러그인] 오류로 이동할 수 있도록 오류 메시지를 수정했습니다 (by @hfhbd)
- [IDE 플러그인] 누락된 검사 설명을 추가했습니다 (#2768 by @aperfilyev)
- [IDE 플러그인] `GotoDeclarationHandler`에서 예외를 수정했습니다 (#2531, #2688, #2804 by @aperfilyev)
- [IDE 플러그인] 임포트 키워드를 강조했습니다 (by @aperfilyev)
- [IDE 플러그인] 해결되지 않은 Kotlin 타입을 수정했습니다 (#1678 by @aperfilyev)
- [IDE 플러그인] 해결되지 않은 패키지에 대한 강조를 수정했습니다 (#2543 by @aperfilyev)
- [IDE 플러그인] 프로젝트 인덱스가 아직 초기화되지 않은 경우 불일치 열을 검사하려고 시도하지 않도록 했습니다
- [IDE 플러그인] Gradle 동기화가 발생할 때까지 파일 인덱스를 초기화하지 않도록 했습니다
- [IDE 플러그인] Gradle 동기화가 시작되면 SQLDelight 임포트를 취소했습니다
- [IDE 플러그인] 실행 취소 작업이 수행되는 스레드 외부에서 데이터베이스를 재생성했습니다
- [IDE 플러그인] 참조를 해결할 수 없는 경우 빈 Java 타입을 사용했습니다
- [IDE 플러그인] 파일 파싱 중 메인 스레드에서 올바르게 이동하고, 쓰기 시에만 다시 메인 스레드로 이동했습니다
- [IDE 플러그인] 이전 IntelliJ 버전과의 호환성을 개선했습니다 (by @3flex)
- [IDE 플러그인] 더 빠른 어노테이션 API를 사용했습니다
- [Gradle 플러그인] 런타임 추가 시 js/android 플러그인을 명시적으로 지원했습니다 (by @ZacSweers)
- [Gradle 플러그인] 마이그레이션에서 스키마를 파생하지 않고 마이그레이션 출력 작업을 등록했습니다 (#2744 by @kevincianfarini)
- [Gradle 플러그인] 마이그레이션 작업이 크래시되면 실행 중 크래시된 파일을 출력했습니다
- [Gradle 플러그인] 동일한 출력을 보장하기 위해 코드 생성 시 파일을 정렬했습니다 (by @ZacSweers)
- [컴파일러] 파일 반복을 위해 더 빠른 API를 사용하고 전체 PSI 그래프를 탐색하지 않도록 했습니다
- [컴파일러] SELECT 함수 매개변수에 키워드 맨글링(mangling)을 추가했습니다 (#2759 by @aperfilyev)
- [컴파일러] 마이그레이션 어댑터에 대한 `packageName`을 수정했습니다 (by @hfhbd)
- [컴파일러] 타입 대신 속성에 어노테이션을 발행했습니다 (#2798 by @aperfilyev)
- [컴파일러] `Query` 서브타입으로 전달하기 전에 인수를 정렬했습니다 (#2379 by @aperfilyev)

## [1.5.3] - 2021-11-23
[1.5.3]: https://github.com/sqldelight/sqldelight/releases/tag/1.5.3

### 추가
- [JDBC 드라이버] 서드파티 드라이버 구현을 위해 `JdbcDriver`를 개방했습니다 (#2672 by @hfhbd)
- [MySQL 다이얼렉트] 시간 증분(time increments)에 대한 누락된 함수를 추가했습니다 (#2671 by @sdoward)
- [코루틴 확장] 코루틴 확장 기능에 M1 타겟을 추가했습니다 (by @PhilipDukhov)

### 변경
- [Paging3 확장] `sqldelight-android-paging3`를 AAR 대신 JAR로 배포했습니다 (#2634 by @julioromano)
- 소프트 키워드이기도 한 속성 이름은 이제 밑줄이 접미사로 붙습니다. 예를 들어, `value`는 `value_`로 노출됩니다.

### 수정
- [컴파일러] 중복 배열 매개변수에 대해 변수를 추출하지 않았습니다 (by @aperfilyev)
- [Gradle 플러그인] `kotlin.mpp.enableCompatibilityMetadataVariant`를 추가했습니다 (#2628 by @martinbonnin)
- [IDE 플러그인] 사용처 찾기(Find usages) 처리에 읽기 액션이 필요합니다
...
- [IDE 플러그인] NULL 비교 검사(inspection)를 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] 검사 억제기(suppressor)를 추가했습니다 (#2519 by @aperfilyev)
- [IDE 플러그인] 명명된 매개변수와 위치 매개변수 혼합 검사를 추가했습니다 (by @aperfilyev)
...
- [PostgreSQL 다이얼렉트] postgres 열 이름 변경 및 제거 (by @pabl0rg)
- [PostgreSQL 다이얼렉트] PostgreSQL 문법이 `CITEXT`를 인식하지 못하는 문제를 해결했습니다
- [PostgreSQL 다이얼렉트] `TIMESTAMP WITH TIME ZONE` 및 `TIMESTAMPTZ`를 포함했습니다
- [PostgreSQL 다이얼렉트] PostgreSQL GENERATED 열에 대한 문법을 추가했습니다
- [런타임] `AfterVersion`에 `SqlDriver`를 매개변수로 제공했습니다 (#2534, 2614 by @ahmedre)

### 변경
- [Gradle 플러그인] Gradle 7.0을 명시적으로 요구하도록 변경했습니다 (#2572 by @martinbonnin)
- [Gradle 플러그인] `VerifyMigrationTask`가 Gradle의 최신 상태 확인(up-to-date checks)을 지원하도록 변경했습니다 (#2533 by @3flex)
- [IDE 플러그인] 널 허용(nullable) 타입과 널 허용 안 함(non-nullable) 타입을 조인할 때 "Join compares two columns of different types" 경고가 발생하지 않도록 했습니다 (#2550 by @pchmielowski)
- [IDE 플러그인] 열 타입에서 소문자 'as'에 대한 오류를 명확히 했습니다 (by @aperfilyev)

### 수정
- [IDE 플러그인] 프로젝트가 이미 해제된 경우 새 다이얼렉트에서 다시 파싱하지 않도록 했습니다 (#2609)
- [IDE 플러그인] 연관된 가상 파일이 null인 경우 모듈도 null입니다 (#2607)
- [IDE 플러그인] 사용되지 않는 쿼리 검사 중 크래시를 피하도록 했습니다 (#2610)
- [IDE 플러그인] 데이터베이스 동기화 쓰기를 쓰기 액션(write action) 내에서 실행했습니다 (#2605)
- [IDE 플러그인] IDE가 SQLDelight 동기화를 스케줄링하도록 했습니다
- [IDE 플러그인] `JavaTypeMixin`의 NPE를 수정했습니다 (#2603 by @aperfilyev)
- [IDE 플러그인] `MismatchJoinColumnInspection`의 `IndexOutOfBoundsException`을 수정했습니다 (#2602 by @aperfilyev)
- [IDE 플러그인] `UnusedColumnInspection`에 대한 설명을 추가했습니다 (#2600 by @aperfilyev)
- [IDE 플러그인] `PsiElement.generatedVirtualFiles`를 읽기 액션(read action)으로 래핑했습니다 (#2599 by @aperfilyev)
- [IDE 플러그인] 불필요한 nonnull 캐스트를 제거했습니다 (#2596)
- [IDE 플러그인] 사용처 찾기에 대한 null을 올바르게 처리했습니다 (#2595)
- [IDE 플러그인] Android용으로 생성된 파일에 대한 IDE 자동 완성을 수정했습니다 (#2573 by @martinbonnin)
- [IDE 플러그인] `SqlDelightGotoDeclarationHandler`의 NPE를 수정했습니다 (by @aperfilyev)
- [IDE 플러그인] INSERT 문 내 인수에서 Kotlin 키워드를 맨글링했습니다 (#2433 by @aperfilyev)
- [IDE 플러그인] `SqlDelightFoldingBuilder`의 NPE를 수정했습니다 (#2382 by @aperfilyev)
- [IDE 플러그인] `CopyPasteProcessor`에서 `ClassCastException`을 캐치했습니다 (#2369 by @aperfilyev)
- [IDE 플러그인] 업데이트 라이브 템플릿을 수정했습니다 (by @IliasRedissi)
- [IDE 플러그인] 인텐션 액션에 설명을 추가했습니다 (#2489 by @aperfilyev)
- [IDE 플러그인] 테이블을 찾을 수 없는 경우 `CreateTriggerMixin`에서 예외를 수정했습니다 (by @aperfilyev)
- [컴파일러] 테이블 생성 문을 위상적으로 정렬했습니다
- [컴파일러] 디렉토리에 대해 `forDatabaseFiles` 콜백 호출을 중지했습니다 (#2532)
...
- [네이티브 드라이버] `watchosX64` 지원을 추가했습니다
- [IDE 플러그인] 매개변수 타입 및 어노테이션을 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] 'SELECT ALL' 쿼리 생성 액션을 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] 자동 완성(autocomplete)에서 열 타입을 표시했습니다 (by @aperfilyev)
- [IDE 플러그인] 자동 완성에 아이콘을 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] '기본 키(primary key)로 SELECT' 쿼리 생성 액션을 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] 'INSERT INTO' 쿼리 생성 액션을 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] 열 이름, 문 식별자, 함수 이름에 대한 강조(highlighting)를 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] 나머지 쿼리 생성 액션을 추가했습니다 (#489 by @aperfilyev)
- [IDE 플러그인] INSERT 문에서 매개변수 힌트를 표시했습니다 (by @aperfilyev)
- [IDE 플러그인] 테이블 별칭 인텐션 액션 (by @aperfilyev)
- [IDE 플러그인] 열 이름 한정(qualify) 인텐션 (by @aperfilyev)
- [IDE 플러그인] Kotlin 속성에 대한 선언으로 이동 (by @aperfilyev)

### 변경
- [네이티브 드라이버] 가능한 경우 고정(freezing) 및 공유 가능한 데이터 구조를 피하여 네이티브 트랜잭션 성능을 개선했습니다 (by @andersio)
- [Paging 3] Paging3 버전을 3.0.0 안정 버전으로 업데이트했습니다
- [JS 드라이버] `sql.js`를 1.5.0으로 업그레이드했습니다

### 수정
- [JDBC SQLite 드라이버] `ThreadLocal`을 지우기 전에 연결에서 `close()`를 호출했습니다 (#2444 by @hannesstruss)
- [RX 확장] 구독/해제 경합 누수(subscription / disposal race leak)를 수정했습니다 (#2403 by @pyricau)
...
- [IDE 플러그인] 임포트 옵티마이저를 수정했습니다 (#2350 by @aperfilyev)
- [IDE 플러그인] 사용되지 않는 열 검사(unused column inspection)를 수정했습니다 (by @aperfilyev)
- [IDE 플러그인] 임포트 검사 및 클래스 어노테이터에 중첩 클래스 지원을 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] `CopyPasteProcessor`의 NPE를 수정했습니다 (#2363 by @aperfilyev)
- [IDE 플러그인] `InlayParameterHintsProvider`의 크래시를 수정했습니다 (#2359 by @aperfilyev)
- [IDE 플러그인] `CREATE TABLE` 문에 어떤 텍스트를 복사-붙여넣기할 때 빈 줄 삽입을 수정했습니다 (#2431 by @aperfilyev)

## [1.5.0] - 2021-04-23
[1.5.0]: https://github.com/sqldelight/sqldelight/releases/tag/1.5.0

### 추가
- [SQLite 자바스크립트 드라이버] `sqljs-driver` 게시를 활성화했습니다 (#1667 by @dellisd)
- [Paging3 확장] Android Paging 3 라이브러리용 확장 (#1786 by @kevincianfarini)
- [MySQL 다이얼렉트] MySQL의 `ON DUPLICATE KEY UPDATE` 충돌 해결 지원을 추가했습니다 (by @rharter)
- [SQLite 다이얼렉트] SQLite `offsets()`에 대한 컴파일러 지원을 추가했습니다 (by @qjroberts)
- [IDE 플러그인] 알 수 없는 타입에 대한 임포트 빠른 수정(quick fix)을 추가했습니다 (#683 by @aperfilyev)
- [IDE 플러그인] 사용되지 않는 임포트 검사(unused import inspection)를 추가했습니다 (#1161 by @aperfilyev)
- [IDE 플러그인] 사용되지 않는 쿼리 검사(unused query inspection)를 추가했습니다 (by @aperfilyev)
- [IDE 플러그인] 사용되지 않는 열 검사(unused column inspection)를 추가했습니다 (#569 by @aperfilyev)
- [IDE 플러그인] 복사/붙여넣기 시 자동으로 임포트를 가져오도록 했습니다 (#684 by @aperfilyev)
- [IDE 플러그인] Gradle/IntelliJ 플러그인 버전 간에 호환성 문제가 있을 때 풍선 메시지를 표시했습니다
- [IDE 플러그인] `INSERT INTO ... VALUES(?)` 매개변수 힌트 (#506 by @aperfilyev)
- [IDE 플러그인] 인라인 매개변수 힌트 (by @aperfilyev)
- [런타임] 콜백을 사용하여 마이그레이션을 실행하는 런타임 API를 포함했습니다 (#1844)

### 변경
- [컴파일러] "IS NOT NULL" 쿼리를 스마트 캐스트했습니다 (#867)
- [컴파일러] 런타임에 실패할 키워드 사용을 금지했습니다 (#1471, #1629)
- [Gradle 플러그인] Gradle 플러그인 크기를 60MB에서 13MB로 줄였습니다.
- [Gradle 플러그인] Android 변형(variants)을 제대로 지원하고 KMM 타겟별 SQL 지원을 제거했습니다 (#1039)
- [Gradle 플러그인] `minSdk`에 기반하여 최소 SQLite 버전을 선택했습니다 (#1684)
- [네이티브 드라이버] 네이티브 드라이버 연결 풀 및 성능 업데이트

### 수정
- [컴파일러] 람다 앞의 `NBSP`를 수정했습니다 (by @oldergod)
- [컴파일러] 생성된 `bind*` 및 `cursor.get*` 문에서 호환되지 않는 타입을 수정했습니다
- [컴파일러] SQL 절이 적용된 타입을 유지하도록 했습니다 (#2067)
- [컴파일러] `NULL` 키워드만 있는 열은 널 허용(nullable)이어야 합니다
- [컴파일러] 타입 어노테이션이 있는 매퍼 람다를 생성하지 않도록 했습니다 (#1957)
- [컴파일러] 커스텀 쿼리가 충돌하는 경우, 파일 이름을 추가 패키지 접미사로 사용했습니다 (#1057, #1278)
- [컴파일러] 외래 키(foreign key) CASCADE가 쿼리 리스너에 알림을 보내도록 보장했습니다 (#1325, #1485)
- [컴파일러] 동일한 타입의 두 개를 UNION하는 경우, 테이블 타입을 반환하도록 했습니다 (#1342)
- [컴파일러] `ifnull` 및 `coalesce`의 매개변수가 널 허용(nullable)일 수 있도록 보장했습니다 (#1263)
- [컴파일러] 표현식에 대해 쿼리가 부과한 널 허용 여부(nullability)를 올바르게 사용했습니다
- [MySQL 다이얼렉트] MySQL IF 문을 지원했습니다
...
- [SQLite 드라이버] 인메모리(in memory)가 아닌 경우 `SqliteDriver`의 스레드에 여러 연결을 사용했습니다 (#1832)
- [JDBC 드라이버] JDBC 드라이버가 `autoCommit`이 `true`라고 가정하는 문제를 수정했습니다 (#2041)
- [JDBC 드라이버] 예외 발생 시 연결을 닫도록 보장했습니다 (#2306)
- [IDE 플러그인] 경로 구분자 버그로 인해 Windows에서 GoToDeclaration/FindUsages가 작동하지 않는 문제를 수정했습니다 (#2054 by @angusholder)
- [IDE 플러그인] IDE에서 Gradle 오류를 무시하는 대신 크래시되도록 했습니다.
- [IDE 플러그인] `sqldelight` 파일이 `sqldelight` 모듈이 아닌 곳으로 이동된 경우 코드 생성을 시도하지 않도록 했습니다
- [IDE 플러그인] IDE에서 코드 생성 오류를 무시했습니다
- [IDE 플러그인] 음수 서브스트링(substring)을 시도하지 않도록 보장했습니다 (#2068)
- [IDE 플러그인] Gradle 액션 실행 전에 프로젝트가 해제되지 않도록 보장했습니다 (#2155)
- [IDE 플러그인] 널 허용(nullable) 타입에 대한 산술 연산도 널 허용(nullable)이어야 합니다 (#1853)
...
- [IDE 플러그인] SQLDelight 인덱싱 중 IntelliJ에서 예외가 발생해도 크래시되지 않도록 했습니다
- [IDE 플러그인] IDE에서 코드 생성 전 오류 감지 시 발생하는 예외를 처리했습니다
- [IDE 플러그인] IDE 플러그인을 동적 플러그인(Dynamic Plugins)과 호환되도록 했습니다 (#1536)
- [Gradle 플러그인] `WorkerApi`를 사용하여 데이터베이스 생성 시 경합 조건(Race condition)을 수정했습니다 (#2062 by @stephanenicolas)
- [Gradle 플러그인] `classLoaderIsolation`이 커스텀 JDBC 사용을 방해하는 문제를 수정했습니다 (#2048 by @benasher44)
- [Gradle 플러그인] 누락된 `packageName` 오류 메시지를 개선했습니다 (by @vanniktech)
- [Gradle 플러그인] SQLDelight가 IntelliJ 종속성을 빌드스크립트 클래스 경로로 누출하는 문제를 수정했습니다 (#1998)
- [Gradle 플러그인] Gradle 빌드 캐싱을 수정했습니다 (#2075)
- [Gradle 플러그인] Gradle 플러그인에서 `kotlin-native-utils`에 의존하지 않도록 했습니다 (by @ilmat192)
- [Gradle 플러그인] 마이그레이션 파일만 있는 경우에도 데이터베이스를 작성했습니다 (#2094)
- [Gradle 플러그인] 최종 컴파일 유닛에서 다이아몬드 종속성(diamond dependencies)이 한 번만 선택되도록 보장했습니다 (#1455)
...
### 추가
- [PostgreSQL 다이얼렉트] `WITH` 문에서 데이터 수정 문(data-modifying statements)을 지원했습니다
- [PostgreSQL 다이얼렉트] `substring` 함수를 지원했습니다
- [Gradle 플러그인] SQLDelight 컴파일 중 마이그레이션을 검증하기 위한 `verifyMigrations` 플래그를 추가했습니다 (#1872)

### 변경
- [컴파일러] 비-SQLite 다이얼렉트에서 SQLite 특정 함수를 알 수 없는 함수로 플래그를 지정했습니다
- [Gradle 플러그인] `sqldelight` 플러그인이 적용되었지만 데이터베이스가 구성되지 않았을 때 경고를 제공했습니다 (#1421)

### 수정
- [컴파일러] `ORDER BY` 절에서 열 이름을 바인딩할 때 오류를 보고했습니다 (#1187 by @eygraber)
- [컴파일러] DB 인터페이스 생성 시 레지스트리 경고가 나타나는 문제를 수정했습니다 (#1792)
- [컴파일러] `CASE` 문에 대한 잘못된 타입 추론을 수정했습니다 (#1811)
- [컴파일러] 버전이 없는 마이그레이션 파일에 대한 더 나은 오류를 제공했습니다 (#2006)
- [컴파일러] 일부 데이터베이스 타입 `ColumnAdapter`에 대해 마샬링할 필수 데이터베이스 타입이 잘못된 문제를 수정했습니다 (#2012)
- [컴파일러] CAST의 널 허용 여부(nullability)를 수정했습니다 (#1261)
- [컴파일러] 쿼리 래퍼에서 이름 그림자(name shadowed) 경고가 많이 발생하는 문제를 수정했습니다 (#1946 by @eygraber)
- [컴파일러] 생성된 코드가 전체 한정자 이름을 사용하는 문제를 수정했습니다 (#1939)
- [IDE 플러그인] Gradle 동기화에서 `sqldelight` 코드 생성을 트리거했습니다
- [IDE 플러그인] `.sq` 파일 변경 시 플러그인이 데이터베이스 인터페이스를 재생성하지 않는 문제를 수정했습니다 (#1945)
- [IDE 플러그인] 파일을 새 패키지로 이동할 때의 문제를 수정했습니다 (#444)
- [IDE 플러그인] 커서를 이동할 곳이 없는 경우 크래시되는 대신 아무것도 하지 않도록 했습니다 (#1994)
- [IDE 플러그인] Gradle 프로젝트 외부 파일에 대해 빈 패키지 이름을 사용했습니다 (#1973)
- [IDE 플러그인] 잘못된 타입에 대해 정상적으로 실패하도록 했습니다 (#1943)
- [IDE 플러그인] 알 수 없는 표현식을 만났을 때 더 나은 오류 메시지를 발생시켰습니다 (#1958)
- [Gradle 플러그인] SQLDelight가 IntelliJ 종속성을 빌드스크립트 클래스 경로로 누출하는 문제를 수정했습니다 (#1998)
- [Gradle 플러그인] `*.sq` 파일에 메서드 doc을 추가할 때 "JavadocIntegrationKt not found" 컴파일 오류가 발생하는 문제를 수정했습니다 (#1982)
- [Gradle 플러그인] SqlDeslight Gradle 플러그인이 구성 캐싱(CoCa)을 지원하지 않는 문제를 수정했습니다 (#1947 by @stephanenicolas)
- [SQLite JDBC 드라이버] `SQLException: database in auto-commit mode`를 수정했습니다 (#1832)
- [코루틴 확장] 코루틴 확장 기능의 IR 백엔드를 수정했습니다 (#1918 by @dellisd)

## [1.4.3] - 2020-09-04
[1.4.3]: https://github.com/sqldelight/sqldelight/releases/tag/1.4.3

### 추가
- [MySQL 다이얼렉트] MySQL `last_insert_id` 함수 지원을 추가했습니다 (by @lawkai)
- [PostgreSQL 다이얼렉트] SERIAL 데이터 타입을 지원했습니다 (by @veyndan & @felipecsl)
- [PostgreSQL 다이얼렉트] PostgreSQL `RETURNING`을 지원했습니다 (by @veyndan)

### 수정
- [MySQL 다이얼렉트] MySQL `AUTO_INCREMENT`를 기본값을 가진 것으로 처리했습니다 (#1823)
- [컴파일러] Upsert 문 컴파일 오류를 수정했습니다 (#1809 by @eygraber)
- [컴파일러] 잘못된 Kotlin이 생성되는 문제를 수정했습니다 (#1925 by @eygraber)
- [컴파일러] 알 수 없는 함수에 대한 더 나은 오류 메시지를 제공했습니다 (#1843)
- [컴파일러] `instr`의 두 번째 매개변수에 대한 타입을 문자열로 노출했습니다
- [IDE 플러그인] IDE 플러그인의 데몬 비대화(bloat) 및 UI 스레드 멈춤(stalling) 현상을 수정했습니다 (#1916)
- [IDE 플러그인] Null 모듈 시나리오를 처리했습니다 (#1902)
- [IDE 플러그인] 구성되지 않은 `.sq` 파일에서 패키지 이름으로 빈 문자열을 반환했습니다 (#1920)
- [IDE 플러그인] 그룹화된 문을 수정하고 이에 대한 통합 테스트를 추가했습니다 (#1820)
- [IDE 플러그인] 요소에 대한 모듈을 찾기 위해 내장 `ModuleUtil`을 사용했습니다 (#1854)
- [IDE 플러그인] 유효한 요소만 조회에 추가했습니다 (#1909)
- [IDE 플러그인] 부모가 null일 수 있습니다 (#1857)

## [1.4.2] - 2020-08-27
[1.4.2]: https://github.com/sqldelight/sqldelight/releases/tag/1.4.2

### 추가
- [런타임] 새로운 JS IR 백엔드를 지원했습니다
- [Gradle 플러그인] `generateSqlDelightInterface` Gradle 작업을 추가했습니다 (by @vanniktech)
- [Gradle 플러그인] `verifySqlDelightMigration` Gradle 작업을 추가했습니다 (by @vanniktech)

### 수정
- [IDE 플러그인] Gradle 툴링 API를 사용하여 IDE와 Gradle 간의 데이터 공유를 용이하게 했습니다
- [IDE 플러그인] 스키마 파생에 대한 기본값을 false로 설정했습니다
- [IDE 플러그인] `commonMain` 소스 세트를 올바르게 검색했습니다
- [MySQL 다이얼렉트] `mySqlFunctionType()`에 `minute`를 추가했습니다 (by @maaxgr)

## [1.4.1] - 2020-08-21
[1.4.1]: https://github.com/sqldelight/sqldelight/releases/tag/1.4.1

### 추가
- [런타임] Kotlin 1.4.0을 지원했습니다 (#1859)

### 변경
- [Gradle 플러그인] AGP 종속성을 `compileOnly`로 변경했습니다 (#1362)

### 수정
- [컴파일러] 열 정의 규칙 및 테이블 인터페이스 생성기에 선택적 자바독(javadoc)을 추가했습니다 (#1224 by @endanke)
- [SQLite 다이얼렉트] sqlite `fts5` 보조 함수 `highlight`, `snippet`, `bm25` 지원을 추가했습니다 (by @drampelt)
- [MySQL 다이얼렉트] MySQL `BIT` 데이터 타입을 지원했습니다
- [MySQL 다이얼렉트] MySQL 이진 리터럴(binary literals)을 지원했습니다
- [PostgreSQL 다이얼렉트] `sql-psi`에서 `SERIAL`을 노출했습니다 (by @veyndan)
- [PostgreSQL 다이얼렉트] `BOOLEAN` 데이터 타입을 추가했습니다 (by @veyndan)
- [PostgreSQL 다이얼렉트] `NULL` 열 제약 조건(column constraint)을 추가했습니다 (by @veyndan)
- [HSQL 다이얼렉트] HSQL에 `AUTO_INCREMENT` 지원을 추가했습니다 (by @rharter)
...
- [IDE] 내장 SQL 라이브 템플릿을 모방하는 SQLDelight 라이브 템플릿을 추가했습니다 (#1154 by @veyndan)
- [IDE] 새로운 `SqlDelight` 파일 액션을 추가했습니다 (#42 by @romtsn)
- [런타임] 결과를 반환하는 트랜잭션을 위한 `transactionWithReturn` API
- [컴파일러] `.sq` 파일에서 여러 SQL 문을 함께 그룹화하는 구문
- [컴파일러] 마이그레이션 파일에서 스키마 생성을 지원했습니다
- [Gradle 플러그인] 마이그레이션 파일을 유효한 SQL로 출력하기 위한 작업을 추가했습니다

### 변경
- [문서] 문서 웹사이트를 대대적으로 개선했습니다 (by @saket)
- [Gradle 플러그인] 지원되지 않는 다이얼렉트 오류 메시지를 개선했습니다 (by @veyndan)
- [IDE] 다이얼렉트에 따라 파일 아이콘을 동적으로 변경했습니다 (by @veyndan)
- [JDBC 드라이버] `javax.sql.DataSource`에서 `JdbcDriver` 생성자를 노출했습니다 (#1614)

### 수정
- [컴파일러] 테이블에 대한 자바독(Javadoc)을 지원하고 한 파일에 여러 자바독이 있는 문제를 수정했습니다 (#1224)
- [컴파일러] 합성된 열(synthesized columns)에 대한 값 삽입을 활성화했습니다 (#1351)
- [컴파일러] 디렉토리 이름 정리의 불일치를 수정했습니다 (by @ZacSweers)
- [컴파일러] 합성된 열은 조인(join) 시 널 허용 여부(nullability)를 유지해야 합니다 (#1656)
- [컴파일러] DELETE 키워드에 DELETE 문을 고정했습니다 (#1643)
- [컴파일러] 인용(quoting)을 수정했습니다 (#1525 by @angusholder)
- [컴파일러] `BETWEEN` 연산자가 표현식으로 올바르게 재귀되도록 수정했습니다 (#1279)
- [컴파일러] 인덱스 생성 시 누락된 테이블/열에 대한 더 나은 오류를 제공했습니다 (#1372)
- [컴파일러] 조인 제약 조건에서 외부 쿼리(outer query)의 프로젝션(projection) 사용을 활성화했습니다 (#1346)
- [네이티브 드라이버] `execute`가 `transactionPool`을 사용하도록 했습니다 (by @benasher44)
- [JDBC 드라이버] SQLite 대신 JDBC 트랜잭션 API를 사용했습니다 (#1693)
- [IDE] `virtualFile` 참조가 항상 원본 파일이 되도록 수정했습니다 (#1782)
- [IDE] `Bugsnag`에 오류를 보고할 때 올바른 `throwable`을 사용했습니다 (#1262)
- [Paging 확장] `DataSource` 누수(leaky)를 수정했습니다 (#1628)
- [Gradle 플러그인] 스키마 생성 시 출력 DB 파일이 이미 존재하는 경우 삭제했습니다 (#1645)
- [Gradle 플러그인] 마이그레이션 유효성 검사에 공백이 있는 경우 실패하도록 했습니다
...
* 신규: [컴파일러] #1009 MySQL 다이얼렉트에 대한 실험적 지원.
* 신규: [컴파일러] #1436 SQLite 3.24 다이얼렉트 및 업서트(upsert) 지원.
* 신규: [JDBC 드라이버] SQLite JVM 드라이버에서 JDBC 드라이버를 분리.
* 수정: [컴파일러] #1199 모든 길이의 람다 지원.
* 수정: [컴파일러] #1610 `avg()`의 반환 타입이 널 허용(nullable)이 되도록 수정.
* 수정: [IntelliJ] #1594 Windows에서 경로 구분자 처리 오류로 인해 Goto 및 Find Usages가 작동하지 않는 문제 수정.

## [1.2.2] - 2020-01-22
[1.2.2]: https://github.com/sqldelight/sqldelight/releases/tag/1.2.2

* 신규: [런타임] Windows (mingW), tvOS, watchOS, macOS 아키텍처 지원.
* 수정: [컴파일러] `sum()`의 반환 타입이 널 허용(nullable)이어야 합니다.
* 수정: [Paging] 경합 조건을 피하기 위해 `Transacter`를 `QueryDataSourceFactory`로 전달.
* 수정: [IntelliJ 플러그인] 파일의 패키지 이름을 찾을 때 종속성을 검색하지 않도록 수정.
* 수정: [Gradle] #862 Gradle의 유효성 검사기 로그를 디버그 레벨로 변경.
* 개선: [Gradle] `GenerateSchemaTask`가 Gradle 워커(worker)를 사용하도록 변환.
* 참고: `sqldelight-runtime` 아티팩트가 `runtime`으로 이름 변경.
...
* 참고: 코루틴 확장 아티팩트는 빌드스크립트에 `kotlinx bintray maven` 저장소를 추가해야 합니다.
...
* 수정: [Gradle] #1380 클린 빌드가 때때로 실패합니다.
* 수정: [Gradle] #1348 검증 작업 실행 시 "Could not retrieve functions" 출력.
* 수정: [컴파일러] #1405 쿼리에 FTS 테이블 조인이 포함된 경우 프로젝트 빌드 불가.
* 수정: [Gradle] #1266 여러 데이터베이스 모듈이 있을 때 Gradle 빌드 실패가 산발적으로 발생합니다.
...
* 수정: [Gradle] #1243 Gradle `configure on demand`와 함께 SQLDelight를 사용하는 경우의 수정.
* 수정: [Gradle] #1385 점진적 어노테이션 처리(incremental annotation processing)와 함께 SQLDelight를 사용하는 경우의 수정.
* 수정: [Gradle] Gradle 작업이 캐시되도록 허용.
* 수정: [Gradle] #1274 Kotlin DSL과 함께 `sqldelight` 확장을 사용하도록 활성화.
* 수정: [컴파일러] 각 쿼리에 대해 결정적으로 고유 ID가 생성됩니다.
* 수정: [컴파일러] 트랜잭션이 완료될 때만 수신 중인 쿼리에 알립니다.
* 수정: [JVM 드라이버] #1370 `JdbcSqliteDriver` 사용자에게 DB URL을 제공하도록 강제.
...
* 신규: [런타임] 로깅 드라이버 데코레이터. (#1267)
* 수정: [컴파일러] 2^16 문자보다 긴 문자열 리터럴을 분할합니다. (#1254)
* 수정: [Gradle] 생성된 소스가 멀티플랫폼 프로젝트에서 iOS 소스로 인식됩니다. (#1260)
* 수정: [IDE] `kotlin.KotlinNullPointerException` in `CopyAsSqliteAction.kt:43`. (#1290)
* 수정: [Gradle] 최근 버전에서 `linkDebugFrameworkIos*` 작업 실행 실패. (#1268)

## [1.1.1] - 2019-03-01
[1.1.1]: https://github.com/sqldelight/sqldelight/releases/tag/1.1.1

* 수정: [Gradle] Android 프로젝트에 대한 모듈 종속성 컴파일을 수정했습니다.
* 수정: [Gradle] #1246 `afterEvaluate`에서 API 종속성을 설정했습니다.
* 수정: [컴파일러] 배열 타입이 올바르게 출력됩니다.

## [1.1.0] - 2019-02-27
[1.1.0]: https://github.com/sqldelight/sqldelight/releases/tag/1.1.0

* 신규: [Gradle] #502 스키마 모듈 종속성을 지정할 수 있도록 허용.
* 개선: [컴파일러] #1111 테이블 오류가 다른 오류보다 먼저 정렬됩니다.
* 수정: [컴파일러] #1225 `REAL` 리터럴에 대한 올바른 타입을 반환합니다.
* 수정: [컴파일러] #1218 `docid`가 트리거를 통해 전파됩니다.

## [1.0.3] - 2019-01-30
[1.0.3]: https://github.com/sqldelight/sqldelight/releases/tag/1.0.3

* 개선: [런타임] #1195 네이티브 드라이버/런타임 Arm32.
* 개선: [런타임] #1190 `Query` 타입에서 매퍼(mapper)를 노출합니다.

## [1.0.2] - 2019-01-26
[1.0.2]: https://github.com/sqldelight/sqldelight/releases/tag/1.0.2

* 수정: [Gradle 플러그인] Kotlin 1.3.20으로 업데이트.
* 수정: [런타임] 트랜잭션이 더 이상 예외를 삼키지 않습니다.

## [1.0.1] - 2019-01-21
[1.0.1]: https://github.com/sqldelight/sqldelight/releases/tag/1.0.1

* 개선: [네이티브 드라이버] `DatabaseConfiguration`에 디렉토리 이름을 전달할 수 있도록 허용.
* 개선: [컴파일러] #1173 패키지가 없는 파일은 컴파일에 실패합니다.
* 수정: [IDE] IDE 오류를 Square에 올바르게 보고합니다.
* 수정: [IDE] #1162 동일한 패키지의 타입이 오류로 표시되지만 정상적으로 작동합니다.
* 수정: [IDE] #1166 테이블 이름 변경 시 NPE로 실패합니다.
* 수정: [컴파일러] #1167 `UNION` 및 `SELECT`가 포함된 복잡한 SQL 문을 파싱하려고 할 때 예외 발생.

## [1.0.0] - 2019-01-08
[1.0.0]: https://github.com/sqldelight/sqldelight/releases/tag/1.0.0

* 신규: 생성된 코드가 Kotlin으로 완전히 재정비되었습니다.
* 신규: RxJava2 확장 아티팩트.
...
* 신규: 생성된 코드가 Support SQLite 라이브러리만 사용하도록 업데이트되었습니다. 이제 모든 쿼리가 원시 문자열 대신 문 객체를 생성합니다.
* 신규: IDE의 문(statement) 폴딩.
* 신규: 부울 타입이 이제 자동으로 처리됩니다.
* 수정: 코드 생성에서 사용되지 않는 마샬(marshals)을 제거했습니다.
* 수정: 'avg' SQL 함수 타입 매핑이 REAL이 되도록 수정했습니다.
* 수정: 'julianday' SQL 함수를 올바르게 감지했습니다.

## [0.6.1] - 2017-03-22
[0.6.1]: https://github.com/sqldelight/sqldelight/releases/tag/0.6.1

* 신규: 인수가 없는 DELETE, UPDATE, INSERT 문에 대해 컴파일된 문이 생성됩니다.
* 수정: 서브쿼리에서 사용되는 뷰 내의 `USING` 절이 오류를 발생시키지 않습니다.
* 수정: 생성된 Mapper에서 중복 타입이 제거되었습니다.
* 수정: 서브쿼리가 인수를 확인하는 표현식에서 사용될 수 있습니다.

## [0.6.0] - 2017-03-06
[0.6.0]: https://github.com/sqldelight/sqldelight/releases/tag/0.6.0

* 신규: SELECT 쿼리가 문자열 상수가 아닌 `SqlDelightStatement` 팩토리로 노출됩니다.
* 신규: 쿼리 자바독(Javadoc)이 이제 문 및 매퍼 팩토리로 복사됩니다.
* 신규: 뷰 이름에 대한 문자열 상수를 발행합니다.
* 수정: 팩토리가 필요한 뷰에 대한 쿼리가 해당 팩토리를 인수로 올바르게 요구하도록 수정했습니다.
* 수정: INSERT 문의 인수 개수가 지정된 열 개수와 일치하는지 유효성 검사.
* 수정: WHERE 절에 사용된 BLOB 리터럴을 올바르게 인코딩.
* 이 릴리스에는 Gradle 3.3 이상이 필요합니다.

## [0.5.1] - 2016-10-24
[0.5.1]: https://github.com/sqldelight/sqldelight/releases/tag/0.5.1

* 신규: 컴파일된 문은 추상 타입을 확장합니다.
* 수정: 매개변수의 기본 타입은 널 허용(nullable)인 경우 박싱됩니다.
* 수정: 바인딩 인수에 필요한 모든 팩토리가 팩토리 메서드에 존재합니다.
* 수정: 이스케이프된 열 이름이 올바르게 마샬링됩니다.

## [0.5.0] - 2016-10-19
[0.5.0]: https://github.com/sqldelight/sqldelight/releases/tag/0.5.0

* 신규: SQLite 인수가 팩토리를 통해 타입 안전하게 전달될 수 있습니다.
* 신규: IntelliJ 플러그인이 `.sq` 파일에 대해 포맷팅을 수행합니다.
* 신규: SQLite 타임스탬프 리터럴 지원.
* 수정: IntelliJ에서 매개변수화된 타입을 클릭하여 이동할 수 있습니다.
* 수정: 커서에서 가져온 이스케이프된 열 이름이 더 이상 `RuntimeException`을 발생시키지 않습니다.
* 수정: Gradle 플러그인이 예외 출력 시 크래시되지 않습니다.

## [0.4.4] - 2016-07-20
[0.4.4]: https://github.com/sqldelight/sqldelight/releases/tag/0.4.4

* 신규: `short`를 열 Java 타입으로 기본 지원.
* 신규: 생성된 매퍼 및 팩토리 메서드에 Javadoc.
* 수정: `group_concat` 및 `nullif` 함수의 널 허용 여부(nullability)가 올바릅니다.
* 수정: Android Studio 2.2-alpha와의 호환성.
* 수정: `WITH RECURSIVE`가 더 이상 플러그인을 크래시시키지 않습니다.

## [0.4.3] - 2016-07-07
[0.4.3]: https://github.com/sqldelight/sqldelight/releases/tag/0.4.3

* 신규: 컴파일 오류가 소스 파일로 연결됩니다.
* 신규: 마우스 오른쪽 클릭으로 SQLDelight 코드를 유효한 SQLite로 복사할 수 있습니다.
* 신규: 명명된 문(named statements)의 Javadoc이 생성된 문자열에 나타납니다.
* 수정: 생성된 뷰 모델에 널 허용 여부(nullability) 어노테이션이 포함됩니다.
* 수정: 유니온(unions)에서 생성된 코드가 가능한 모든 열을 지원하기 위해 올바른 타입과 널 허용 여부(nullability)를 가집니다.
* 수정: `sum` 및 `round` SQLite 함수의 생성된 코드 타입이 올바릅니다.
* 수정: `CAST` 및 내부 SELECT의 버그 수정.
...
* 신규: 뷰 및 별칭에 대해 자동 완성(Autocompletion) 및 사용처 찾기(find usages)가 이제 작동합니다.
* 수정: 컴파일 타임 유효성 검사가 함수를 SELECT에서 사용할 수 있도록 허용합니다.
* 수정: 기본값만 선언하는 INSERT 문을 지원합니다.
* 수정: SQLDelight를 사용하지 않는 프로젝트가 임포트될 때 플러그인이 크래시되지 않습니다.

## [0.3.1] - 2016-04-27
[0.3.1]: https://github.com/sqldelight/sqldelight/releases/tag/0.3.1

* 수정: 메서드 참조로 인한 `Illegal Access runtime` 예외를 피하기 위해 인터페이스 가시성을 다시 `public`으로 변경했습니다.
* 수정: 서브 표현식이 올바르게 평가됩니다.

## [0.3.0] - 2016-04-26
[0.3.0]: https://github.com/sqldelight/sqldelight/releases/tag/0.3.0

* 신규: 열 정의는 SQLite 타입을 사용하고 Java 타입을 지정하기 위해 추가 'AS' 제약 조건을 가질 수 있습니다.
* 신규: IDE에서 버그 리포트를 보낼 수 있습니다.
* 수정: 자동 완성 기능이 제대로 작동합니다.
* 수정: `.sq` 파일 편집 시 SQLDelight 모델 파일이 업데이트됩니다.
* 제거됨: 첨부된 데이터베이스는 더 이상 지원되지 않습니다.

## [0.2.2] - 2016-03-07
[0.2.2]: https://github.com/sqldelight/sqldelight/releases/tag/0.2.2

* 신규: INSERT, UPDATE, DELETE, INDEX, TRIGGER 문에서 사용되는 열에 대한 컴파일 타임 유효성 검사.
* 수정: 파일 이동/생성 시 IDE 플러그인이 크래시되지 않습니다.

## [0.2.1] - 2016-03-07
[0.2.1]: https://github.com/sqldelight/sqldelight/releases/tag/0.2.1

* 신규: Ctrl+`/` (OSX에서는 Cmd+`/`)로 선택된 줄의 주석을 토글합니다.
* 신규: SQL 쿼리에서 사용되는 열에 대한 컴파일 타임 유효성 검사.
* 수정: IDE 및 Gradle 플러그인 모두에서 Windows 경로를 지원합니다.

## [0.2.0] - 2016-02-29
[0.2.0]: https://github.com/sqldelight/sqldelight/releases/tag/0.2.0

* 신규: `Marshal` 클래스에 복사 생성자가 추가되었습니다.
* 신규: Kotlin 1.0 Final로 업데이트.
* 수정: 'sqldelight' 폴더 구조 문제를 실패하지 않는 방식으로 보고합니다.
* 수정: `table_name`으로 명명된 열을 금지합니다. 생성된 상수가 테이블 이름 상수와 충돌합니다.
* 수정: IDE 플러그인이 `.sq` 파일이 열렸는지 여부와 관계없이 모델 클래스를 즉시 생성하도록 보장합니다.
* 수정: IDE 및 Gradle 플러그인 모두에서 Windows 경로를 지원합니다.

## [0.1.2] - 2016-02-13
[0.1.2]: https://github.com/sqldelight/sqldelight/releases/tag/0.1.2

* 수정: 대부분의 프로젝트에서 Gradle 플러그인이 사용되는 것을 방지하던 코드를 제거했습니다.
* 수정: Antlr 런타임에 대한 누락된 컴파일러 종속성을 추가했습니다.

## [0.1.1] - 2016-02-12
[0.1.1]: https://github.com/sqldelight/sqldelight/releases/tag/0.1.1

* 수정: Gradle 플러그인이 런타임과 동일한 버전을 가리키도록 보장합니다.
