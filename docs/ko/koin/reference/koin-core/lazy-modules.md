...
## 지연 로딩 모듈이란 무엇인가요? {id="what-are-lazy-modules"}
...
## 지연 로딩 모듈 정의하기 {id="defining-lazy-modules"}
...
### 지연 로딩 모듈 구성하기 {id="composing-lazy-modules"}
...
## 지연 로딩 모듈 로드하기 {id="loading-lazy-modules"}

Koin 설정 내에서 `lazyModules()`를 사용하여 지연 로딩 모듈을 로드합니다.

### 기본 로딩 {id="basic-loading"}
...
### 병렬 로딩 (4.2.0+) {id="parallel-loading-4-2-0"}
...
### 완료 대기하기 {id="waiting-for-completion"}

#### 모든 플랫폼: `waitAllStartJobs()` {id="all-platforms-waitallstartjobs"}
...
#### JVM 전용: `runOnKoinStarted()` {id="jvm-only-runonkoinstarted"}
...
#### 중단(Suspending) 대안: `awaitAllStartJobs()` {id="suspending-alternative-awaitallstartjobs"}
...
## 커스텀 디스패처 {id="custom-dispatchers"}
...
## 실제 사용 예시 {id="real-world-example"}
...
## 중요한 제한 사항 {id="important-limitations"}

### 교차 의존성 피하기 {id="avoid-cross-dependencies"}
...
### 모범 사례: 로드 순서 {id="best-practice-load-order"}

1. **즉시 로드할 모듈** - 시작 시 필요한 필수 서비스
2. **지연 로딩 모듈** - 중요하지 않거나 지연 가능한 서비스
3. **필요한 경우 대기** - 지연 로딩 정의에 접근하기 전에 `waitAllStartJobs()` 사용

## 지연 로딩 모듈을 사용해야 하는 경우 {id="when-to-use-lazy-modules"}

### 좋은 사용 사례 {id="good-use-cases"}
