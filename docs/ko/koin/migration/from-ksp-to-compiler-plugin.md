...
컴파일러 플러그인은 Kotlin 2.3 이상이 필요합니다:

```kotlin
// build.gradle.kts
plugins {
    kotlin("jvm") version "2.3.20-Beta1" // 최소 2.3.20-Beta1 필요
}
```
...
```toml
[versions]
koin = "4.2.0-Beta4" // 또는 이후 버전
koin-plugin = "0.2.9" // 또는 이후 버전
...
이것이 주요 코드 변경 사항입니다. KSP 방식은 생성된 확장 함수를 사용하는 반면, 컴파일러 플러그인은 타입 기반(typed) API를 사용합니다.
...
- **[KSP 설정 (지원 중단)](/docs/setup/annotations-ksp)** - 기존 레퍼런스 (Legacy)