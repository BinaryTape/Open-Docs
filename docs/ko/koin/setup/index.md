...
### [KSP 애노테이션(Annotations) 설정](/docs/setup/annotations-ksp) (더 이상 사용되지 않음)

KSP 기반 애노테이션을 위한 기존 설정:
- ⚠️ 더 이상 사용되지 않음(Deprecated) - 컴파일러 플러그인으로 마이그레이션하세요.
- 마이그레이션 가이드 포함

## 버전 호환성

| Koin 버전 | Kotlin 버전 | 컴파일러 플러그인 |
|--------------|----------------|-----------------|
| 4.2.x | 2.3+ | ✅ 권장됨 |
| 4.1.x | 2.1/2.2+ | ⚠️ KSP 전용 |
| 4.0.x | 1.9/2.0+ | ⚠️ KSP 전용 |
| 3.5.x | 1.8+ | ❌ 이용 불가 |
