...
컴포넌트를 선언해 보겠습니다. `UserRepository`와 `UserService`의 싱글톤(singleton)을 만들고자 합니다:
...
> `UserService`는 `UserViewModel`의 생성자에서 참조됩니다.

Koin 모듈에 `UserViewModel`을 선언합니다. 메모리에 인스턴스를 유지하지 않도록(안드로이드 생명주기로 인한 누수 방지) `viewModel` 정의로 선언합니다:
...
**컴파일러 플러그인 DSL**(컴파일 타임 자동 와이어링)을 사용하는 경우:
...
:::tip
컴파일러 플러그인 DSL을 사용하려면 [Koin 컴파일러 플러그인](/docs/setup/compiler-plugin)이 필요합니다. 이는 컴파일 타임 의존성 해결(dependency resolution)과 더 깔끔한 구문을 제공합니다.
:::