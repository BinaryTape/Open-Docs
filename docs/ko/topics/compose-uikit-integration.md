...
Compose Multiplatform는 [UIKit](https://developer.apple.com/documentation/uikit) 프레임워크와 상호 운용 가능합니다.
Compose Multiplatform를 UIKit 애플리케이션 내에 포함시키거나, 네이티브 UIKit 컴포넌트를 Compose
Multiplatform 내에 포함시킬 수 있습니다. 이 페이지에서는 Compose Multiplatform를 UIKit 애플리케이션 내에서 사용하는 예제와
UIKit 컴포넌트를 Compose Multiplatform UI 내에 포함시키는 예제를 모두 제공합니다.

> SwiftUI 상호 운용성에 대해 알아보려면 [](compose-swiftui-integration.md) 문서를 참조하세요.
>
{style="tip"}

## UIKit 애플리케이션 내에서 Compose Multiplatform 사용하기

UIKit 애플리케이션 내에서 Compose Multiplatform를 사용하려면, Compose Multiplatform 코드를
모든 [컨테이너 뷰 컨트롤러](https://developer.apple.com/documentation/uikit/view_controllers)에 추가하세요.
이 예제는 `UITabBarController` 클래스 내에서 Compose Multiplatform를 사용합니다:
...
이 코드는 [샘플 프로젝트](https://github.com/JetBrains/compose-multiplatform/tree/master/examples/interop/ios-compose-in-uikit)에서 확인하세요.

## Compose Multiplatform 내에서 UIKit 사용하기

Compose Multiplatform 내에서 UIKit 요소를 사용하려면, 사용하려는 UIKit 요소를 Compose Multiplatform의
[UIKitView](https://github.com/JetBrains/compose-multiplatform-core/blob/47c012bfe2d4570fb08432253298b8e2b6e38ade/compose/ui/ui/src/uikitMain/kotlin/androidx/compose/ui/interop/UIKitView.uikit.kt)에 추가하세요.
이 코드는 Kotlin으로만 작성하거나 Swift도 사용할 수 있습니다.

### 맵 뷰

UIKit의 [`MKMapView`](https://developer.apple.com/documentation/mapkit/mkmapview) 컴포넌트를 사용하여 Compose Multiplatform에서 맵 뷰를 구현할 수 있습니다.
Compose Multiplatform의 `Modifier.size()` 또는 `Modifier.fillMaxSize()` 함수를 사용하여 컴포넌트 크기를 설정합니다:
...
이제 고급 예제를 살펴보겠습니다. 이 코드는 UIKit의 [`UITextField`](https://developer.apple.com/documentation/uikit/uitextfield/)를 Compose Multiplatform로 래핑합니다:
...
* `factory` 파라미터는 `editingChanged()` 함수와 `textField.addTarget()` 리스너를 포함하여 `UITextField`의 변경 사항을 감지합니다.
* `editingChanged()` 함수는 Objective-C 코드와 상호 운용되도록 `@ObjCAction`으로 어노테이션되어 있습니다.
* `addTarget()` 함수의 `action` 파라미터는 `editingChanged()` 함수의 이름을 전달하며, `UIControlEventEditingChanged` 이벤트에 대한 응답으로 이를 트리거합니다.
* `UIKitView()`의 `update` 파라미터는 관찰 가능한 메시지 상태의 값이 변경될 때 호출됩니다.
* 이 함수는 `UITextField`의 `text` 속성을 업데이트하여 사용자가 업데이트된 값을 볼 수 있도록 합니다.

이 예제의 코드는 [샘플 프로젝트](https://github.com/JetBrains/compose-multiplatform/tree/master/examples/interop/ios-uikit-in-compose)에서 확인하세요.
...
이를 통해 애플리케이션이 장치의 카메라에 액세스하고 실시간 미리보기를 표시할 수 있습니다.
...
이제 고급 예제를 살펴보겠습니다. 이 코드는 사진을 캡처하고, GPS 메타데이터를 첨부하고, 네이티브 `UIView`를 사용하여 실시간 미리보기를 표시합니다:
...
* 레이아웃 업데이트를 관리하고 미리보기 레이어를 내장하는 사용자 정의 `UIView` 서브클래스를 호스팅하는 `UIKitView`를 생성합니다.
* `AVCapturePhotoOutput`을 초기화하고 사진 캡처를 처리하기 위한 델리게이트를 구성합니다.
* `CLLocationManager`(`locationManager`를 통해)를 사용하여 캡처 시점의 GPS 좌표를 검색합니다.
...
이 예제의 전체 코드는 [ImageViewer 샘플 프로젝트](https://github.com/JetBrains/compose-multiplatform/tree/master/examples/imageviewer)에서 확인하세요.

### 웹 뷰

UIKit의 [`WKWebView`](https://developer.apple.com/documentation/webkit/wkwebview) 컴포넌트를 사용하여 Compose Multiplatform에서 웹 뷰를 구현할 수 있습니다.
이를 통해 애플리케이션이 UI 내에서 웹 콘텐츠를 표시하고 상호 작용할 수 있습니다.
Compose Multiplatform의 `Modifier.size()` 또는 `Modifier.fillMaxSize()` 함수를 사용하여 컴포넌트 크기를 설정합니다:
...
```
이제 고급 예제를 살펴보겠습니다. 이 코드는 내비게이션 델리게이트로 웹 뷰를 구성하고 Kotlin과 JavaScript 간의 통신을 허용합니다:
...
* `WKNavigationDelegateProtocol` 인터페이스를 구현하는 안정적인 델리게이트 객체를 생성합니다. 이 객체는 Compose의 `remember`를 사용하여 리컴포지션 간에 기억됩니다.
* `WKWebView`를 인스턴스화하고, `UIKitView`를 사용하여 포함시키고, 기억된 델리게이트를 할당하여 구성합니다.
...
Compose Multiplatform가 [SwiftUI 프레임워크와 통합될 수 있는 방법](compose-swiftui-integration.md)에 대해서도 살펴볼 수 있습니다.