[//]: # (title: Android 애플리케이션을 iOS에서 작동시키기 – 튜토리얼)

<secondary-label ref="IntelliJ IDEA"/>
<secondary-label ref="Android Studio"/>

<tldr>
<p>이 튜토리얼은 Android Studio를 사용하지만, IntelliJ IDEA에서도 따라 할 수 있습니다. <a href="quickstart.md">올바르게 설정되면</a>,
   두 IDE 모두 동일한 핵심 기능과 Kotlin Multiplatform 지원을 공유합니다.</p>
</tldr>

이 튜토리얼은 기존 Android 애플리케이션을 Android 및 iOS 모두에서 작동하도록 크로스 플랫폼으로 만드는 방법을 보여줍니다.
동일한 장소에서 Android와 iOS용 코드를 한 번에 작성할 수 있습니다.

이 튜토리얼은 사용자 이름과 비밀번호를 입력하는 단일 화면을 가진 [샘플 Android 애플리케이션](https://github.com/Kotlin/kmp-integration-sample)을 사용합니다.
자격 증명은 유효성을 검사하고 인메모리 데이터베이스에 저장됩니다.

애플리케이션이 iOS와 Android 모두에서 작동하도록 하려면,
먼저 코드 일부를 공유 모듈로 이동하여 코드를 크로스 플랫폼으로 만들 것입니다.
그 후 Android 애플리케이션에서 크로스 플랫폼 코드를 사용하고, 동일한 코드를 새 iOS 애플리케이션에서 사용할 것입니다.

> Kotlin Multiplatform에 익숙하지 않다면, 먼저 [처음부터 크로스 플랫폼 애플리케이션을 만드는 방법](quickstart.md)을
> 배우세요.
>
{style="tip"}
...
1. 퀵스타트에서 [Kotlin Multiplatform 개발 환경 설정](quickstart.md#set-up-the-environment) 지침을 완료합니다.
   ...
   > 이 튜토리얼의 특정 단계, 예를 들어 iOS 애플리케이션 실행을 완료하려면 macOS가 설치된 Mac이 필요합니다.
   > 이는 Apple 요구 사항 때문입니다.
   >
   {style="note"}

2. Android Studio에서 버전 제어로부터 새 프로젝트를 생성합니다:
...
1. [어떤 코드를 크로스 플랫폼으로 만들지 결정](#decide-what-code-to-make-cross-platform)
2. [크로스 플랫폼 코드를 위한 공유 모듈 생성](#create-a-shared-module-for-cross-platform-code)
3. [코드 공유 테스트](#add-code-to-the-shared-module)
4. [Android 애플리케이션에 공유 모듈 종속성 추가](#add-a-dependency-on-the-shared-module-to-your-android-application)
5. [비즈니스 로직을 크로스 플랫폼으로 만들기](#make-the-business-logic-cross-platform)
6. [Android에서 크로스 플랫폼 애플리케이션 실행](#run-your-cross-platform-application-on-android)

### 어떤 코드를 크로스 플랫폼으로 만들지 결정

Android 애플리케이션의 어떤 코드를 iOS와 공유하고 어떤 코드를 네이티브로 유지할지 결정합니다. 간단한 규칙은 다음과 같습니다:
가능한 한 많이 재사용하려는 것을 공유하십시오. 비즈니스 로직은 Android와 iOS 모두에서 동일한 경우가 많으므로,
재사용하기에 좋은 후보입니다.

샘플 Android 애플리케이션에서 비즈니스 로직은 `com.jetbrains.simplelogin.androidapp.data` 패키지에 저장되어 있습니다.
미래의 iOS 애플리케이션도 동일한 로직을 사용할 것이므로, 이 또한 크로스 플랫폼으로 만들어야 합니다.

![Business logic to share](business-logic-to-share.png){width=366}

### 크로스 플랫폼 코드를 위한 공유 모듈 생성

iOS와 Android 모두에 사용되는 크로스 플랫폼 코드는 공유 모듈에 저장됩니다.
Android Studio와 IntelliJ IDEA 모두 Kotlin Multiplatform용 공유 모듈을 생성하는 마법사를 제공합니다.

기존 Android 애플리케이션과 향후 iOS 애플리케이션을 모두 연결하는 공유 모듈을 생성합니다:

1. Android Studio에서 주 메뉴에서 **File** | **New** | **New Module**을 선택합니다.
...
3. **Finish**를 클릭합니다. 마법사가 공유 모듈을 생성하고, 그에 따라 빌드 스크립트를 변경하며, Gradle 동기화를 시작합니다.
4. 설정이 완료되면 `shared` 디렉터리에 다음 파일 구조가 표시됩니다:

   ![Final file structure inside the shared directory](shared-directory-structure.png){width="341"}

5. `shared/build.gradle.kts` 파일의 `kotlin.androidLibrary.minSdk` 속성이 `app/build.gradle.kts` 파일의 동일한 속성 값과 일치하는지 확인하십시오.

### 공유 모듈에 코드 추가

이제 공유 모듈이 있으니,
`commonMain/kotlin/com.jetbrains.simplelogin.shared` 디렉터리에 공유할 일부 공통 코드를 추가합니다:
...
[Kotlin Multiplatform 프로젝트 구조의 기본 사항](multiplatform-discover-project.md)을 참조하십시오.

### Android 애플리케이션에 공유 모듈 종속성 추가

Android 애플리케이션에서 크로스 플랫폼 코드를 사용하려면, 공유 모듈을 애플리케이션에 연결하고, 비즈니스 로직 코드를
그곳으로 이동하며, 이 코드를 크로스 플랫폼으로 만드십시오.

1. `app/build.gradle.kts` 파일에 공유 모듈에 대한 종속성을 추가합니다:
...
3. `app/src/main/java/` 디렉터리에서 `com.jetbrains.simplelogin.androidapp.ui.login`
   패키지 내의 `LoginActivity.kt` 파일을 엽니다.
4. 공유 모듈이 애플리케이션에 성공적으로 연결되었는지 확인하기 위해, `onCreate()` 메서드에 `Log.i()` 호출을 추가하여 `greet()` 함수
   결과를 로그에 덤프합니다:
...
이제 비즈니스 로직 코드를 Kotlin Multiplatform 공유 모듈로 추출하여 플랫폼 독립적으로 만들 수 있습니다.
이는 Android와 iOS 모두에서 코드를 재사용하는 데 필요합니다.

1. 비즈니스 로직 코드 `com.jetbrains.simplelogin.androidapp.data`를 `app` 디렉터리에서
   `shared/src/commonMain` 디렉터리 내의 `com.jetbrains.simplelogin.shared` 패키지로 이동합니다.

   ![Drag and drop the package with the business logic code](moving-business-logic.png){width=300}

2. Android Studio에서 무엇을 할지 물으면 패키지 이동을 선택한 다음 리팩토링을 승인합니다.

   ![Refactor the business logic package](refactor-business-logic-package.png){width=300}

3. 플랫폼 종속적인 코드에 대한 모든 경고를 무시하고 **Refactor Anyway**를 클릭합니다.

   ![Warnings about platform-dependent code](warnings-android-specific-code.png){width=450}

4. Android 고유 코드를 크로스 플랫폼 Kotlin 코드로 바꾸거나 [expect 및 actual 선언](multiplatform-connect-to-apis.md)을 사용하여 Android 고유 API에 연결하여 제거합니다. 자세한 내용은 다음 섹션을 참조하십시오:

   #### Android 고유 코드를 크로스 플랫폼 코드로 바꾸기 {initial-collapse-state="collapsed" collapsible="true"}
   
   코드가 Android와 iOS 모두에서 잘 작동하도록 하려면,
   가능한 한 이동된 `data` 디렉터리에서 모든 JVM 종속성을 Kotlin 종속성으로 바꿉니다.
       ...
       companion object {
           private val emailRegex = 
               ("[a-zA-Z0-9\\+\\.\\_\\%\\-\\+]{1,256}" +
                   ...
                   "(" +
                   "\\." +
                   "[a-zA-Z0-9][a-Z0-9\\-]{0,25}" +
                   ")+").toRegex()
       }
       ```
   
   2. `Patterns` 클래스에 대한 import 지시문을 제거합니다:
   
       ```kotlin
       import android.util.Patterns
       ```

   3. `LoginDataSource` 클래스에서 `login()` 함수에 있는 `IOException`을 `RuntimeException`으로 바꿉니다.
   ...
   4. `IOException`에 대한 import 지시문도 제거합니다:
   ...
   Kotlin 표준 라이브러리가 [UUID 생성을 위한 실험적 클래스](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin.uuid/-uuid/)를
   제공하지만, 이 경우 플랫폼별 기능을 사용하여 이를 연습해 봅시다.
   
   공유 코드에서 `randomUUID()` 함수에 대한 `expect` 선언을 제공하고
   해당 소스 세트의 각 플랫폼(Android 및 iOS)에 대한 `actual` 구현을 제공합니다.
   [플랫폼별 API에 연결](multiplatform-connect-to-apis.md)에 대해 자세히 알아볼 수 있습니다.
   
   1. `login()` 함수에서 `java.util.UUID.randomUUID()` 호출을 각 플랫폼에 대해 구현할 `randomUUID()` 호출로 변경합니다:
   ...
   3. `shared/src/androidMain` 디렉터리의 `com.jetbrains.simplelogin.shared` 패키지에 `Utils.android.kt` 파일을 생성하고
      Android에서 `randomUUID()`에 대한 `actual` 구현을 제공합니다:
...
이제 Kotlin은 Android 및 iOS에 대해 플랫폼별 UUID 구현을 사용할 것입니다.

### Android에서 크로스 플랫폼 애플리케이션 실행

Android용 크로스 플랫폼 애플리케이션을 실행하여 이전과 같이 작동하는지 확인하십시오.

![Android login application](android-login.png){width=300}

## 크로스 플랫폼 애플리케이션을 iOS에서 작동시키기

Android 애플리케이션을 크로스 플랫폼으로 만들었다면, iOS 애플리케이션을 생성하고
그 안에서 공유 비즈니스 로직을 재사용할 수 있습니다.
...
2. iOS 앱용 템플릿을 선택하고 **Next**를 클릭합니다.
...
4. 프로젝트 위치로 크로스 플랫폼 애플리케이션이 저장된 디렉터리, 예를 들어
   `kmp-integration-sample`을 선택합니다.

Android Studio에서는 다음과 같은 구조를 얻을 수 있습니다:
...
이렇게 하려면 Xcode를 닫은 다음 `simpleLoginIOS` 디렉터리 이름을 `iosApp`으로 변경하십시오.
...
이 방법에 대한 대안은 [iOS 통합 방법 개요](multiplatform-ios-integration-overview.md)에서 다루고 있지만,
이 튜토리얼의 범위를 벗어납니다.
...
2. Xcode에서 **Project** 네비게이터에서 프로젝트 이름을 두 번 클릭하여 iOS 프로젝트 설정을 엽니다.
...
5. **Based on dependency analysis** 옵션을 비활성화합니다.
   ...
   이렇게 하면 Xcode가 빌드할 때마다 스크립트를 실행하고, 매번 누락된 출력 종속성에 대해 경고하지 않도록 합니다.
...
7. **Build Settings** 탭에서 **Build Options** 아래의 **User Script Sandboxing** 옵션을 비활성화합니다:
   ...
   > 기본 `Debug` 또는 `Release`와 다른 사용자 정의 빌드 구성이 있는 경우, **Build Settings** 탭에서
   > **User-Defined** 아래에 `KOTLIN_FRAMEWORK_BUILD_TYPE` 설정을 추가하고 이를 `Debug` 또는 `Release`로 설정합니다.
   >
   {style="note"}

8. Xcode에서 프로젝트를 빌드합니다 (**Product** | **Build** in the main menu).
    모든 것이 올바르게 구성되었다면 프로젝트가 성공적으로 빌드되어야 합니다
    ("build phase will be run during every build" 경고는 무시해도 안전합니다).
   
    > **User Script Sandboxing** 옵션을 비활성화하기 전에 프로젝트를 빌드했다면 빌드가 실패할 수 있습니다:
    > Gradle 데몬 프로세스가 샌드박스 처리되어 다시 시작해야 할 수 있습니다.
    > 프로젝트 디렉터리(이 예에서는 `kmp-integration-sample`)에서 다음 명령을 실행하여 프로젝트를 다시 빌드하기 전에 중지하십시오:
    > 
    > ```shell
    > ./gradlew --stop
    > ```
...
Xcode가 올바르게 설정되었는지 확인했다면 Android Studio로 돌아갑니다:

1. 주 메뉴에서 **File | Sync Project with Gradle Files**를 선택합니다. Android Studio는 자동으로
   **simpleLoginIOS**라는 실행 구성을 생성합니다.

   Android Studio는 자동으로 **simpleLoginIOS**라는 실행 구성을 생성하고 `iosApp`
   디렉터리를 연결된 Xcode 프로젝트로 표시합니다.
...
`shared` 모듈의 `build.gradle.kts` 파일은
각 iOS 타겟에 대해 `binaries.framework.baseName` 속성을 `sharedKit`으로 정의합니다.
이것은 Kotlin Multiplatform이 iOS 앱이 사용할 수 있도록 빌드하는 프레임워크의 이름입니다.

통합을 테스트하려면 Swift 코드에서 공통 코드를 호출하는 것을 추가합니다:
...
2. 제대로 연결되었는지 확인하기 위해 `ContentView` 구조체를 크로스 플랫폼 앱의 공유 모듈에서
   `greet()` 함수를 사용하도록 변경합니다:
...
7. 사용자 이름으로 "Jane", 비밀번호로 "password"를 입력합니다.
...
## 결과를 즐기세요 – 로직은 한 번만 업데이트

이제 애플리케이션이 크로스 플랫폼이 되었습니다. `shared` 모듈에서 비즈니스 로직을 업데이트하고 Android와 iOS 모두에서 결과를 확인할 수 있습니다.

1. 사용자 비밀번호에 대한 유효성 검사 로직을 변경합니다: "password"는 유효한 옵션이 아니어야 합니다.
    이를 위해 `LoginDataValidator` 클래스의 `checkPassword()` 함수를 업데이트합니다
    (빠르게 찾으려면 <shortcut>Shift</shortcut>를 두 번 누르고 클래스 이름을 붙여넣은 다음 **Classes** 탭으로 전환합니다):
...
예를 들어, `ViewModel` 클래스 코드는 [Android](https://github.com/Kotlin/kmp-integration-sample/blob/final/app/src/main/java/com/jetbrains/simplelogin/androidapp/ui/login/LoginViewModel.kt)와
...
Android 애플리케이션을 크로스 플랫폼으로 만들었다면, 다음으로 진행할 수 있습니다:
...
* [Compose Multiplatform에서 사용 가능한 리소스 살펴보기](compose-multiplatform-resources.md)
* [공유 로직 및 UI로 앱 생성하기](compose-multiplatform-create-first-app.md)

또한 커뮤니티 리소스를 확인할 수 있습니다:

* [영상: Android 프로젝트를 Kotlin Multiplatform로 마이그레이션하는 방법](https://www.youtube.com/watch?v=vb-Pt8SdfEE&t=1s)
* [영상: Kotlin JVM 코드를 Kotlin Multiplatform에 준비하는 3가지 방법](https://www.youtube.com/watch?v=X6ckI1JWjqo)