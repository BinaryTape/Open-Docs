...
### @Singleton と @Inject {id="singleton-and-inject"}
...
### カスタム @Qualifier アノテーション {id="custom-qualifier-annotations"}
...
## Android ViewModel {id="android-viewmodel"}
...
## Hiltからの移行 {id="migration-from-hilt"}

### 移行前 (Hilt) {id="before-hilt"}
...
### 移行後 (Koin と JSR-330) {id="after-koin-with-jsr-330"}
...
## コンパイラプラグインのサポート {id="compiler-plugin-support"}
