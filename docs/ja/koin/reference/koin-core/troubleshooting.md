...
### 解決策 1: 遅延インジェクション (Lazy Injection) {id="solution-1-lazy-injection"}
...
### 解決策 2: 共有依存関係の抽出 {id="solution-2-extract-shared-dependency"}
...
### 解決策 3: インターフェースの使用 {id="solution-3-use-an-interface"}
...
### `verify()` によるモジュールの検証 {id="verify-modules-with-verify"}
...
## 一般的なアンチパターン {id="common-anti-patterns"}

### 1. サービスロケータの過剰な使用 {id="1-service-locator-overuse"}
...
### 2. 巨大なモジュール (God Modules) {id="2-god-modules"}
...
### 3. 過剰な限定子 (Qualifiers) {id="3-excessive-qualifiers"}
...
### 5. 隠れた依存関係 {id="5-hidden-dependencies"}
...
## ベストプラクティスのまとめ {id="best-practices-summary"}
