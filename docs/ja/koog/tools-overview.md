...
Koog フレームワークでは、ツールを操作するための以下のワークフローを提供しています：
...
!!! tip
    エージェントの失敗を防ぐために、ツールに適切な [エラーハンドリング](agent-event-handlers.md) を実装していることを確認してください。
...
詳細については、[API リファレンス](api:agents-core::ai.koog.agents.core.agent.session.AIAgentLLMWriteSession) を参照してください。
