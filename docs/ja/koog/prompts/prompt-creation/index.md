...
Koog は、メッセージタイプ、その順序、および内容を制御しながらプロンプトを作成するために、型安全な Kotlin DSL を使用します。

これらのプロンプトにより、複数のメッセージを含む会話履歴を事前に構成したり、マルチモーダルなコンテンツ、例、ツール呼び出し、およびその実行結果を提供したりすることができます。

## 基本構造

`prompt()` 関数は、一意の ID とメッセージのリストを持つ Prompt オブジェクトを作成します。
...
Kotlin DSL は以下のメッセージタイプをサポートしており、それぞれが会話における特定の役割に対応しています。
...
システムメッセージを作成するには、`system()` 関数の引数として文字列を渡します。
...
ユーザーメッセージを作成するには、`user()` 関数の引数として文字列を渡します。
...
アシスタントメッセージを作成するには、`assistant()` 関数の引数として文字列を渡します。
