...
|----------------------|--------------|----------|----------|-----------------------|
| `RetryConfig.DISABLED`     | 1 (リトライなし) | -        | -        | 開発とテスト        |
| `RetryConfig.CONSERVATIVE` | 3            | 2秒      | 30秒     | 通常のプロダクション利用 |
| `RetryConfig.AGGRESSIVE`   | 5            | 500ミリ秒 | 20秒     | 重要な操作          |
| `RetryConfig.PRODUCTION`   | 3            | 1秒      | 20秒     | 推奨されるデフォルト    |
...
!!!note
    ...
    ストリーミングが開始された後は、エラーはすべてそのまま通過します。
