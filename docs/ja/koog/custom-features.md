...
#### グラフベースエージェント専用のインターセプター {id="interceptors-specific-to-graph-based-agents"}
...
機能のロジックが完全なエージェントイベント構造に依存している場合、イベントフィルタリングが予期しない動作を引き起こす可能性があります。これを防ぐには、機能を実装する際にイベントフィルタリングを無効にする必要があります。そのためには、機能の設定で `setEventFilter` をオーバーライドし、機能のインストール時に設定されたカスタムフィルターを無視するようにします。
