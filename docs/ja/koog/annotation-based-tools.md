...
アノテーションベースのツールは、関数を大規模言語モデル（LLM）用のツールとして公開するための宣言的な手法を提供します。
アノテーションを使用することで、あらゆる関数を LLM が理解して利用できるツールに変換できます。

このアプローチは、ツールの説明を手動で実装することなく、既存の機能を LLM に公開する必要がある場合に便利です。
...
`@Tool` アノテーションは、LLM にツールとして公開する関数をマークするために使用されます。
`@Tool` でアノテーションされた関数は、`ToolSet` インターフェースを実装するオブジェクトからリフレクションによって収集されます。詳細は、[ToolSet インターフェースの実装](#implement-the-toolset-interface) を参照してください。
...
関数をツールとしてマークするには、`ToolSet` インターフェースを実装するクラス内の関数に `@Tool` アノテーションを適用します。
<!--- INCLUDE
...
`@LLMDescription` アノテーションは、コード要素（クラス、関数、パラメータなど）に関する説明情報を LLM に提供します。
...
クラスに関数を追加し、`@Tool` アノテーションを付けてツールとして公開します。
...
* **デフォルト値を文書化する**: パラメータにデフォルト値がある場合は、説明の中でその旨を記述してください。
...
- すべてのツール関数に `@Tool` アノテーションが付いているか。
- ツール関数が適切な戻り値の型を持っているか（シンプルにするために `String` が推奨されます）。
...
- `@LLMDescription` アノテーションをより具体的で明確なものに改善する。
- 必要に応じて、説明の中に例を含める。
- 説明の中でパラメータの制約を指定する（例：「正の数である必要があります」）。
- 説明全体で一貫した用語を使用する。
