...
| [Alibaba](https://www.alibabacloud.com/en?_p_lc=1) ([DashScope](https://dashscope.aliyuncs.com/) OpenAI互換クライアント)                                                            | 大規模なコンテキストと費用対効果の高いQwenモデル。                                                                                 |
| [Ollama](https://ollama.com/)                                                                                                                               | プライバシー、ローカル開発、オフライン運用、APIコストなし。                                                               |

以下の表は、KoogがサポートするLLM機能と、これらの機能がどのプロバイダーのモデルで提供されているかを示しています。

| <div style="width:115px">LLM機能</div> | OpenAI                       | Anthropic                 | Google                                  | DeepSeek | OpenRouter       | Amazon Bedrock   | Mistral                   | Alibaba (DashScope OpenAI互換クライアント) | Ollama (ローカルモデル) |
|:--------------------------------------|:-----------------------------|:--------------------------|:----------------------------------------|:---------|:-----------------|:-----------------|:--------------------------|:---------------------------------------------|:----------------------|
| サポートされる入力                               | テキスト、画像、音声、ドキュメント | テキスト、画像、ドキュメント[^1] | テキスト、画像、音声、動画、ドキュメント[^1] | テキスト     | モデルによる | モデルによる | テキスト、画像、ドキュメント[^1] | テキスト、画像、音声、動画[^1]                | テキスト、画像[^1]       |
| レスポンスストリーミング                            | ✓                            | ✓                         | ✓                                       | ✓        | ✓                | ✓                | ✓                         | ✓                                            | ✓                     |
| ツール                                         | ✓                            | ✓                         | ✓                                       | ✓        | ✓                | ✓[^1]            | ✓                         | ✓                                            | ✓                     |
| ツール選択                                   | ✓                            | ✓                         | ✓                                       | ✓        | ✓                | ✓[^1]            | ✓                         | ✓                                            | –                     |
...
*   特定のプロバイダーと直接対話するための**LLMクライアント**を使用する。
    各クライアントは`LLMClient`インターフェースを実装し、プロバイダーの認証、
    リクエストのフォーマット、レスポンスのパースを処理します。
...
*   1つまたは複数のLLMクライアントをラップし、そのライフサイクルを管理し、プロバイダー間のインターフェースを統一する、より高レベルの抽象化である**プロンプトエグゼキューター**を使用する。
    プロバイダー間を切り替えることができ、オプションで、対応するクライアントを使用して設定されたプロバイダーとLLMにフォールバックできます。
...
プロンプトエグゼキューターは、1つまたは複数の`LLMClient`の上に高レベルのレイヤーを提供します。
クライアントのライフサイクルを管理し、プロバイダー全体で統一されたインターフェースを公開します。
マルチプロバイダー設定では、プロバイダー間でリクエストをルーティングし、中核となるリクエストが必要な場合に、オプションで指定されたクライアントにフォールバックできます。独自のエグゼキューターを作成することも、事前定義されたものを使用することもできます。シングルプロバイダーとマルチプロバイダーの両方のオプションが利用可能です。
