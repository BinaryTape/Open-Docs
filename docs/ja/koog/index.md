...
-   :material-database-search:{ .lg .middle } [**知識の検索**](embeddings.md)
    ...
    [ベクトル埋め込み](embeddings.md)、[ランク付けされたドキュメントストレージ](ranked-document-storage.md)、[共有エージェントメモリ](features/agent-memory.md)を使用して、会話をまたいで知識を保持および取得する
...
-   :material-timeline-text:{ .lg .middle } [**長期メモリ**](features/long-term-memory.md)
