...
## 設定と初期化 {id="configuration-and-initialization"}

### テスト依存関係のセットアップ {id="setting-up-test-dependencies"}
...
### LLMレスポンスのモック化 {id="mocking-llm-responses"}
...
### ツールコールのモック化 {id="mocking-tool-calls"}
...
### テストモードの有効化 {id="enabling-testing-mode"}
...
## 高度なテスト {id="advanced-testing"}

### グラフ構造のテスト {id="testing-the-graph-structure"}
...
#### 基本構造のテスト {id="basic-structure-testing"}
...
#### ツール実行ノードのテスト {id="testing-tool-run-nodes"}
...
#### 高度なノードテスト {id="advanced-node-testing"}
...
#### 条件付きルーティングのテスト {id="testing-conditional-routing"}
...
#### 高度なエッジテスト {id="advanced-edge-testing"}
...
## 完全なテスト例 {id="complete-testing-example"}
...
#### 特定のツールレスポンスをモック化するにはどうすればよいですか？ {id="how-do-i-mock-a-specific-tool-response"}
...
#### 複雑なグラフ構造をテストするにはどうすればよいですか？ {id="how-can-i-test-complex-graph-structures"}
...
#### 入力に基づいて異なるLLMレスポンスをシミュレートするにはどうすればよいですか？ {id="how-do-i-simulate-different-llm-responses-based-on-input"}
...
#### モックエグゼキューターが常にデフォルトレスポンスを返す {id="mock-executor-always-returns-the-default-response"}

パターンマッチングが正しいか確認してください。パターンは大文字と小文字を区別し、指定された通りに正確に一致する必要があります。

#### ツールコールがインターセプトされない {id="tool-calls-are-not-being-intercepted"}
...
#### グラフのアサーションが失敗する {id="graph-assertions-are-failing"}
