## サブグラフの作成と設定 {id="creating-and-configuring-subgraphs"}

以下のセクションでは、エージェント・ワークフロー用のサブグラフ作成におけるコードテンプレートと一般的なパターンを紹介します。

### 基本的なサブグラフの作成 {id="basic-subgraph-creation"}
...
### サブグラフでのツールの設定 {id="configuring-tools-in-a-subgraph"}
...
## 高度なサブグラフの手法 {id="advanced-subgraph-techniques"}

### マルチパート戦略 {id="multi-part-strategies"}
...
### ツールが利用できない {id="tools-not-available"}

サブグラフでツールが利用できない場合:

- ツールがツール・レジストリに正しく登録されているか確認してください。

### サブグラフが定義された、期待通りの順序で実行されない {id="subgraphs-not-running-in-the-defined-and-expected-order"}
