...
## LLMセッションの理解 {id="understanding-llm-sessions"}
...
### セッションのライフサイクル {id="session-lifecycle"}
...
## LLMセッションの操作 {id="working-with-llm-sessions"}

### セッションの作成 {id="creating-sessions"}

セッションは、`AIAgentLLMContext` クラスのメソッドを使用して作成されます：
=== "Kotlin"
...
### セッションのスコープとスレッド安全性 {id="session-scope-and-thread-safety"}
...
### セッションプロパティへのアクセス {id="accessing-session-properties"}
...
## LLMリクエストの送信 {id="making-llm-requests"}

### 基本的なリクエストメソッド {id="basic-request-methods"}
...
### リクエストの仕組み {id="how-requests-work"}
...
### ツールを使用したリクエストメソッド {id="request-methods-with-tools"}
...
### 構造化リクエストとストリーミングリクエスト {id="structured-and-streaming-requests"}
...
3. `requestLLMStreaming()`: LLMに対してストリーミングリクエストを送信し、レスポンスチャンクの flow を返します。ストリーミングの詳細については、[ストリーミングAPI](streaming-api.md) のページを参照してください。
...
## 会話履歴の管理 {id="managing-conversation-history"}

### プロンプトの更新 {id="updating-the-prompt"}
...
### レスポンス時の履歴の自動更新 {id="automatic-history-update-on-response"}
...
## セッションでのツールの実行 {id="running-tools-in-sessions"}

### ツールの呼び出し {id="calling-tools"}
...
### ツールの並列実行 {id="parallel-tool-runs"}
...
### セッションが既に閉じられている (Session already closed) {id="session-already-closed"}
...
### 履歴が大きすぎる (History too large) {id="history-too-large"}
...
### ツールが見つからない (Tool not found) {id="tool-not-found"}
