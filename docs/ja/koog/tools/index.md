...
### 利用可能なツールのタイプ {id="available-tool-types"}
...
### エージェントへのツールの受け渡し {id="passing-tools-to-an-agent"}
...
#### ツールの並列呼び出し {id="parallel-tool-calls"}
...
#### ノードからのツールの呼び出し {id="calling-tools-from-nodes"}
...
## エージェントをツールとして使用する {id="using-agents-as-tools"}
...
### エージェントからツールへの変換 {id="converting-agents-to-tools"}
...
### 他のエージェントでエージェントツールを使用する {id="using-agent-tools-in-other-agents"}
...
### エージェントツールの実行 {id="agent-tool-execution"}
...
### エージェントをツールとして使用する利点 {id="benefits-of-agents-as-tools"}
