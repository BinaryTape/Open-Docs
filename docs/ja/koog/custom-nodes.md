...
カスタムノードの実装は、入力データに対して基本的なロジックを実行して出力を返す単純なものから、引数を受け取り、実行間で状態を保持するより複雑なものまで多岐にわたります。
...
カスタムノードを作成する別の方法として、`node` 関数を呼び出す `AIAgentSubgraphBuilderBase` の拡張関数を定義する方法もあります。
