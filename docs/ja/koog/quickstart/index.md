...
??? tip "ナイトリービルド"
    ...
    その後、Koog の依存関係を目的のナイトリーバージョンに更新します。ナイトリーバージョンは以下のパターンに従います：
...
=== "OpenAI"
    ...
    [OpenAI API キー](https://platform.openai.com/api-keys)を取得し、環境変数 `OPENAI_API_KEY` に割り当てます。
...
=== "Anthropic"
    ...
    [Anthropic API キー](https://console.anthropic.com/settings/keys)を取得し、環境変数 `ANTHROPIC_API_KEY` に割り当てます。
...
=== "Google"
    ...
    [Gemini API キー](https://aistudio.google.com/app/api-keys)を取得し、環境変数 `GOOGLE_API_KEY` に割り当てます。
...
=== "DeepSeek"
    ...
    [DeepSeek API キー](https://platform.deepseek.com/api_keys)を取得し、環境変数 `DEEPSEEK_API_KEY` に割り当てます。
...
=== "OpenRouter"
    ...
    [OpenRouter API キー](https://openrouter.ai/keys)を取得し、環境変数 `OPENROUTER_API_KEY` に割り当てます。
...
=== "Bedrock"
    ...
    [Amazon Bedrock API キーを生成](https://docs.aws.amazon.com/bedrock/latest/userguide/api-keys.html)し、環境変数 `BEDROCK_API_KEY` に割り当てます。
...
=== "Mistral"
    ...
    [Mistral API キー](https://console.mistral.ai/api-keys)を取得し、環境変数 `MISTRAL_API_KEY` に割り当てます。
...
=== "Ollama"
    ...
    [Ollama のドキュメント](https://docs.ollama.com/quickstart)の記載に従って、Ollama でローカル LLM を実行します。

## 最初の Koog エージェントの作成

=== "OpenAI"

    以下の例では、OpenAI API を介して [`GPT-4o`](https://platform.openai.com/docs/models/gpt-4o) モデルを使用するシンプルな Koog エージェントを作成して実行します。
    ...
    fun main() = runBlocking {
        // OPENAI_API_KEY 環境変数から OpenAI API キーを取得する
...
=== "Anthropic"
    ...
    以下の例では、Anthropic API を介して [`Claude Opus 4.1`](https://www.anthropic.com/news/claude-opus-4-1) モデルを使用するシンプルな Koog エージェントを作成して実行します。
    ...
    ```kotlin
    fun main() = runBlocking {
        // ANTHROPIC_API_KEY 環境変数から Anthropic API キーを取得する
...
=== "Google"
    ...
    以下の例では、Gemini API を介して [`Gemini 2.5 Pro`](https://cloud.google.com/vertex-ai/generative-ai/docs/models/gemini/2-5-pro) モデルを使用するシンプルな Koog エージェントを作成して実行します。
    ...
    ```kotlin
    fun main() = runBlocking {
        // GOOGLE_API_KEY 環境変数から Gemini API キーを取得する
...
=== "DeepSeek"
    ...
    以下の例では、DeepSeek API を介して `deepseek-chat` モデルを使用するシンプルな Koog エージェントを作成して実行します。
    ...
    ```kotlin
    fun main() = runBlocking {
        // DEEPSEEK_API_KEY 環境変数から DeepSeek API キーを取得する
        ...
        // エージェントを作成する
        val agent = AIAgent(
            // LLM クライアントを使用してプロンプトエグゼキューターを作成する
...
=== "OpenRouter"
    ...
    以下の例では、OpenRouter API を介して [`GPT-4o`](https://openrouter.ai/openai/gpt-4o) モデルを使用するシンプルな Koog エージェントを作成して実行します。
    ...
    ```kotlin
    fun main() = runBlocking {
        // OPENROUTER_API_KEY 環境変数から OpenRouter API キーを取得する
...
=== "Bedrock"
    ...
    以下の例では、Bedrock API を介して [`Claude Sonnet 4.5`](https://www.anthropic.com/news/claude-sonnet-4-5) モデルを使用するシンプルな Koog エージェントを作成して実行します。
    ...
    ```kotlin
    fun main() = runBlocking {
        // BEDROCK_API_KEY 環境変数から Bedrock API キーを取得する
...
=== "Mistral"
    ...
    以下の例では、Mistral AI API を介して [`Mistral Medium 3.1`](https://docs.mistral.ai/models/mistral-medium-3-1-25-08) モデルを使用するシンプルな Koog エージェントを作成して実行します。
    ...
    ```kotlin
    fun main() = runBlocking {
        // MISTRAL_API_KEY 環境変数から Mistral AI API キーを取得する
...
=== "Ollama"
    ...
    以下の例では、Ollama を介してローカルで実行されている [`llama3.2`](https://ollama.com/library/llama3.2) モデルを使用するシンプルな Koog エージェントを作成して実行します。
...
- [メモリ機能を持つチャットエージェントの構築](chat-agent.md) — 以前のメッセージを記憶する会話型エージェントを作成する
- [エージェントの構築](../agents/index.md)について詳しく学ぶ