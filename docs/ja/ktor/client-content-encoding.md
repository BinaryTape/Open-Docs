...
以下の[例](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-content-encoding)は、指定された品質値を使用してクライアントで`deflate`および`gzip`エンコーダーを有効にする方法を示しています：
...
必要に応じて、`ContentEncoder`インターフェースを実装してカスタムエンコーダーを作成し、それを`customEncoder`関数に渡すことができます。
