...
### 追加
-   [PostgreSQL Dialect] Postgres の `numeric`/`integer`/`biginteger` 型マッピングを修正 (#5994 by @griffio)
-   [Compiler] `CAST` が必要な場合に、ソースファイルの位置を含めるようにコンパイラのエラーメッセージを改善 (#5979 by @griffio)
-   [PostgreSQL Dialect] Postgres `JSON` オペレーターのパス抽出のサポートを追加 (#5971 by @griffio)
-   [SQLite Dialect] 共通テーブル式 (`Common Table Expressions`) を使用した `MATERIALIZED` クエリプランナーヒントの `Sqlite 3.35` サポートを追加 (#5961 by @griffio)
-   [PostgreSQL Dialect] 共通テーブル式 (`Common Table Expressions`) を使用した `MATERIALIZED` クエリプランナーヒントのサポートを追加 (#5961 by @griffio)
-   [PostgreSQL Dialect] Postgres `JSON` 集約 `FILTER` のサポートを追加 (#5957 by @griffio)
-   [PostgreSQL Dialect] Postgres `Enum` のサポートを追加 (#5935 by @griffio)
-   [PostgreSQL Dialect] Postgres `Triggers` の限定的なサポートを追加 (#5932 by @griffio)
-   [PostgreSQL Dialect] `SQL` 式が `JSON` として解析できるかをチェックする述語を追加 (#5843 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql Comment On` ステートメントに対する限定的なサポートを追加 (#5808 by @griffio)
-   [MySQL Dialect] インデックス可視性オプションのサポートを追加 (#5785 by @orenkislev-faire)
-   [PostgreSql Dialect] `TSQUERY` データ型のサポートを追加 (#5779 by @griffio)
-   [Gradle Plugin] モジュール追加時のバージョンカタログ (`version catalogs`) のサポートを追加 (#5755 by @DRSchlaubi)
...
-   [Compiler] コンストラクタ参照 (`constructor references`) を使用して、デフォルトで生成されるクエリを簡素化 (#5814 by @jonapoul)

### 修正
-   [Compiler] 共通テーブル式 (`Common Table Expression`) を含む `View` を使用する際のスタックオーバーフロー (`stack overflow`) を修正 (#5928 by @griffio)
-   [Gradle Plugin] `SqlDelight` ツールウィンドウ (`tool window`) で「新しい接続 (`New Connection`)」を追加する際のクラッシュを修正 (#5906 by @griffio)
-   [IntelliJ Plugin] コピー・トゥ・SQLiteガターアクション (`copy-to-sqlite gutter action`) におけるスレッド関連のクラッシュを回避 (#5901 by @griffio)
-   [IntelliJ Plugin] `CREATE INDEX` および `CREATE VIEW` スキーマステートメント (`schema statements`) 使用時の `PostgreSql` ダイアレクトの修正 (#5772 by @griffio)
-   [Compiler] 列参照時の `FTS` スタックオーバーフロー (`stack overflow`) を修正 (#5896 by @griffio)
-   [Compiler] `WITH RECURSIVE` スタックオーバーフロー (`stack overflow`) を修正 (#5892 by @griffio)
-   [Compiler] `INSERT|UPDATE|DELETE RETURNING` ステートメントの通知 (`Notify`) を修正 (#5851 by @griffio)
-   [Compiler] `Long` を返すトランザクションブロック (`transaction blocks`) の非同期結果型 (`async result type`) を修正 (#5836 by @griffio)
-   [Compiler] `SQL` パラメータバインディング (`SQL parameter binding`) の複雑度 (`complexity`) を `O(n²)` から `O(n)` に最適化 (#5898 by @chenf7)
-   [SQLite Dialect] `Sqlite 3.18` で不足していた関数を修正 (#5759 by @griffio)
...
### 追加
-   [WASM Driver] `wasmJs` の `web worker driver` へのサポートを追加 (#5534 by @IlyaGulya)
-   [PostgreSQL Dialect] `PostgreSql UnNest Array to rows` をサポート (#5673 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql TSRANGE/TSTZRANGE` のサポート (#5297 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql Right Full Join` のサポート (#5086 by @griffio)
-   [PostgreSQL Dialect] `temporal types` からの `Postrgesql extract` をサポート (#5273 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql array contains operators` のサポート (#4933 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql drop constraint` をサポート (#5288 by @griffio)
-   [PostgreSQL Dialect] `Postgresql type casting` をサポート (#5089 by @griffio)
-   [PostgreSQL Dialect] サブクエリ (`subquery`) 用の `PostgreSql lateral join operator` をサポート (#5122 by @griffio)
...
-   [MySQL Dialect] `MySql` ダイアレクトの `RENAME INDEX` をサポート (#5212 by @orenkislev-faire)
-   [JSON Extension] `json table function` にエイリアスを追加 (#5372 by @griffio)

### 変更
-   [Compiler] 生成されるクエリファイルが、シンプルなミューテーター (`mutators`) の行数 (`row counts`) を返すように変更 (#4578 by @MariusVolkhart)
-   [Native Driver] `NativeSqlDatabase.kt` を更新し、`DELETE`、`INSERT`、および `UPDATE` ステートメントの読み取り専用フラグ (`readonly flag`) を変更 (#5680 by @griffio)
-   [PostgreSQL Dialect] `PgInterval` を `String` に変更 (#5403 by @griffio)
-   [PostgreSQL Dialect] `SqlDelight` モジュールが `PostgreSql` 拡張を実装 (`implement PostgreSql extensions`) できるようにサポート (#5677 by @griffio)

### 修正
-   [Compiler] 修正: 結果を伴うグループステートメント (`group statements`) 実行時にクエリに通知するよう修正 (#5006 by @vitorhugods)
-   [Compiler] `SqlDelightModule` 型リゾルバー (`type resolver`) を修正 (#5625 by @griffio)
-   [Compiler] `5501` の挿入オブジェクトエスケープ列 (`insert object escaped column`) を修正 (#5503 by @griffio)
-   [Compiler] コンパイラ: エラーメッセージ (`error message`) を改善し、パスリンク (`path links`) が正しい行 (`line`) と文字位置 (`char position`) でクリック可能 (`clickable`) になるように修正 (#5604 by @vanniktech)
-   [Compiler] `issue 5298` を修正: キーワードをテーブル名として使用できるように修正
-   [Compiler] 名前付き実行 (`named executes`) を修正し、テストを追加
-   [Compiler] 初期化ステートメント (`initialization statements`) のソート時に外部キーテーブル制約 (`foreign key table constraints`) を考慮 (#5325 by @TheMrMilchmann)
-   [Compiler] タブ (`tabs`) が関係する場合にエラー下線 (`error underlines`) を適切に揃えるよう修正 (#5224 by @drewd)
-   [JDBC Driver] トランザクション終了時の `connectionManager` のメモリリーク (`memory leak`) を修正
-   [JDBC Driver] ドキュメントに記載されているように、SQLite のマイグレーションをトランザクション内で実行するよう修正 (#5218 by @morki)
-   [JDBC Driver] トランザクションコミット/ロールバック (`commit / rollback`) 後の接続リーク (`leaking connections`) を修正 (#5205 by @morki)
-   [Gradle Plugin] `DriverInitializer` を `GenerateSchemaTask` の前に実行するように修正 (#5562 by @nwagu)
-   [Runtime] 実際のドライバーが `Async` の場合に `LogSqliteDriver` でクラッシュする問題を修正 (#5723 by @edenman)
-   [Runtime] `StringBuilder` の容量 (`capacity`) を修正 (#5192 by @janbina)
-   [PostgreSQL Dialect] `PostgreSql create or replace view` を修正 (#5407 by @griffio)
-   [PostgreSQL Dialect] `Postgresql to_json` を修正 (#5606 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql numeric resolver` を修正 (#5399 by @griffio)
-   [PostgreSQL Dialect] `sqlite windows function` を修正 (#2799 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql SELECT DISTINCT ON` を修正 (#5345 by @griffio)
-   [PostgreSQL Dialect] `alter table add column if not exists` を修正 (#5309 by @griffio)
-   [PostgreSQL Dialect] `Postgresql async bind parameter` を修正 (#5313 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql boolean literals` を修正 (#5262 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql window functions` を修正 (#5155 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql isNull isNotNull types` を修正 (#5173 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql select distinct` を修正 (#5172 by @griffio)
-   [Paging Extension] ページングの初期ロード時のリフレッシュ (`paging refresh initial load`) を修正 (#5615 by @evant)
-   [Paging Extension] `MacOS native targets` を追加 (#5324 by @vitorhugods)
...
### 追加
-   [PostgreSQL Dialect] PostgreSQL の `STRING_AGG` 関数を追加 (#4950 by @anddani)
-   [PostgreSQL Dialect] `pg` ダイアレクトに `SET` ステートメントを追加 (#4927 by @de-luca)
-   [PostgreSQL Dialect] `PostgreSql alter column sequence parameters` を追加 (#4916 by @griffio)
-   [PostgreSQL Dialect] `INSERT` ステートメントの `postgresql alter column default` サポートを追加 (#4912 by @griffio)
-   [PostgreSQL Dialect] `PostgreSql alter sequence` と `drop sequence` を追加 (#4920 by @griffio)
-   [PostgreSQL Dialect] 追加の `Postgres Regex function definitions` を追加 (#5025 by @MariusVolkhart)
-   [PostgreSQL Dialect] `GIN` の文法 (`grammar`) を追加 (#5027 by @griffio)

### 変更
-   [IDE Plugin] 最低バージョン (`Minimum version`) を `2023.1 / Android Studio Iguana` に引き上げ
-   [Compiler] `encapsulatingType` での型の `null` 許容の上書き (`override the type nullability`) を許可 (#4882 by @eygraber)
-   [Compiler] `SELECT *` の列名 (`column names`) をインライン化 (`Inline`)
-   [Gradle Plugin] `processIsolation` に切り替え (#5068 by @nwagu)
-   [Android Runtime] Android の `minSDK` を `21` に引き上げ (#5094 by @hfhbd)
-   [Drivers] ダイアレクト作者 (`dialect authors`) 向けに `JDBC/R2DBC` ステートメントメソッドをさらに公開 (`Expose more JDBC/R2DBC statement methods`) (#5098 by @hfhbd)

### 修正
-   [PostgreSQL Dialect] `postgresql alter table alter column` を修正 (#4868 by @griffio)
-   [PostgreSQL Dialect] `4448` のテーブルモデル (`table model`) のインポート不足 (`missing import`) を修正 (#4885 by @griffio)
-   [PostgreSQL Dialect] `4932` の `postgresql default constraint functions` を修正 (#4934 by @griffio)
-   [PostgreSQL Dialect] `4879` のマイグレーション中の `alter table rename column` での `postgresql class-cast error` を修正 (#4880 by @griffio)
-   [PostgreSQL Dialect] `4474 PostgreSql create extension` を修正 (#4541 by @griffio)
-   [PostgreSQL Dialect] `5018 PostgreSql add Primary Key not nullable types` を修正 (#5020 by @griffio)
-   [PostgreSQL Dialect] `4703 aggregate expressions` を修正 (#5071 by @griffio)
-   [PostgreSQL Dialect] `5028 PostgreSql json` を修正 (#5030 by @griffio)
-   [PostgreSQL Dialect] `5040 PostgreSql json operators` を修正 (#5041 by @griffio)
-   [PostgreSQL Dialect] `5040` の `json operator binding` を修正 (#5100 by @griffio)
-   [PostgreSQL Dialect] `5082 tsvector` を修正 (#5104 by @griffio)
-   [PostgreSQL Dialect] `5032 PostgreSql UPDATE FROM statement` での `column adjacency` を修正 (#5035 by @griffio)
-   [SQLite Dialect] `4897 sqlite alter table rename column` を修正 (#4899 by @griffio)
...
### 追加
-   [Compiler] `SELECT` 実行時に複数列式 (`multi-column-expr`) をサポート (#4453 by @Adriel-M)
-   [PostgreSQL Dialect] `PostgreSQL CREATE INDEX CONCURRENTLY` のサポートを追加 (#4531 by @griffio)
-   [PostgreSQL Dialect] `PostgreSQL CTEs auxiliary statements` が互いを参照 (`reference each other`) できるように許可 (#4493 by @griffio)
-   [PostgreSQL Dialect] `binary expr` と `sum` の `PostgreSQL types` のサポートを追加 (#4539 by @Adriel-M)
-   [PostgreSQL Dialect] `PostgreSQL SELECT DISTINCT ON` 構文のサポートを追加 (#4584 by @griffio)
-   [PostgreSQL Dialect] `SELECT` ステートメントにおける `PostgreSQL JSON functions` サポートを追加 (#4590 by @MariusVolkhart)
-   [PostgreSQL Dialect] `generate_series PostgreSQL function` を追加 (#4717 by @griffio)
-   [PostgreSQL Dialect] 追加の `Postgres String function definitions` を追加 (#4752 by @MariusVolkhart)
-   [PostgreSQL Dialect] `min` および `max aggregate functions` に `DATE PostgreSQL type` を追加 (#4816 by @anddani)
-   [PostgreSQL Dialect] `SqlBinaryExpr` に `PostgreSql temporal types` を追加 (#4657 by @griffio)
-   [PostgreSQL Dialect] `postgres` ダイアレクトに `TRUNCATE` を追加 (#4817 by @de-luca)
-   [SQLite 3.35 Dialect] 順序通りに評価 (`evaluated in order`) される複数の `ON CONFLICT` 句 (`clauses`) を許可 (#4551 by @griffio)
-   [JDBC Driver] より快適な `SQL` 編集 (`pleasant SQL editing`) のために `Language annotations` を追加 (#4602 by @MariusVolkhart)
-   [Native Driver] ネイティブドライバー (`Native-driver`): `linuxArm64` のサポートを追加 (#4792 by @hfhbd)
-   [Android Driver] `AndroidSqliteDriver` に `windowSizeBytes parameter` を追加 (#4804 by @BoD)
-   [Paging3 Extension] 機能: `OffsetQueryPagingSource` に `initialOffset` を追加 (#4802 by @MohamadJaara)

### 変更
-   [Compiler] 適切な場合に `Kotlin types` を優先 (`Prefer Kotlin types where appropriate`) するよう変更 (#4517 by @eygraber)
-   [Compiler] `value type insert` を行う場合、常に列名 (`column names`) を含める (`always include`) ように変更 (#4864)
-   [PostgreSQL Dialect] `PostgreSQL` ダイアレクトから実験的ステータス (`experimental status`) を削除 (#4443 by @hfhbd)
-   [PostgreSQL Dialect] `PostgreSQL types` のドキュメント (`docs`) を更新 (#4569 by @MariusVolkhart)
-   [R2DBC Driver] PostgreSQL で整数データ型 (`integer data types`) を扱う際のパフォーマンス (`performance`) を最適化 (`Optimize`) (#4588 by @MariusVolkhart)

### 削除
-   [SQLite Javascript Driver] `sqljs-driver` を削除 (#4613, #4670 by @dellisd)

### 修正
-   [Compiler] 戻り値があり、パラメータのないグループ化されたステートメント (`grouped statements`) のコンパイル (`compilation`) を修正 (#4699 by @griffio)
-   [Compiler] `SqlBinaryExpr` で引数をバインド (`Bind arguments`) するように修正 (#4604 by @griffio)
...
-   [Gradle Plugin] `verifyMigrationTask Gradle task` のドキュメント (`documentation`) を修正 (#4713 by @joshfriend)
-   [Gradle Plugin] ユーザーがデータベースを検証する前にデータベースを生成するのに役立つ `Gradle task output message` を追加 (#4684 by @jingwei99)
-   [PostgreSQL Dialect] `PostgreSQL columns` の複数回の名前変更 (`renaming`) を修正 (#4566 by @griffio)
-   [PostgreSQL Dialect] `4714 postgresql alter column nullability` を修正 (#4831 by @griffio)
-   [PostgreSQL Dialect] `4837 alter table alter column` を修正 (#4846 by @griffio)
-   [PostgreSQL Dialect] `4501 PostgreSql sequence` を修正 (#4528 by @griffio)
-   [SQLite Dialect] `JSON binary operator` が列式 (`column expression`) で使用できるように許可 (#4776 by @eygraber)
-   [SQLite Dialect] 名前で複数の列が見つかった場合の `Update From false positive` を修正 (#4777 by @eygraber)
-   [Native Driver] 名前付きインメモリデータベース (`named in-memory databases`) をサポート (#4662 by @05nelsonm)
-   [Native Driver] クエリリスナーコレクション (`query listener collection`) のスレッドセーフティ (`thread safety`) を保証 (#4567 by @kpgalligan)
-   [JDBC Driver] `ConnectionManager` における接続リーク (`connection leak`) を修正 (#4589 by @MariusVolkhart)
-   [JDBC Driver] `ConnectionManager` 型を選択する際の `JdbcSqliteDriver url parsing` を修正 (#4656 by @05nelsonm)

## [2.0.0] - 2023-07-26
[2.0.0]: https://github.com/sqldelight/sqldelight/releases/tag/2.0.0

### 追加
-   [MySQL Dialect] MySQL: `IF expression` での `timestamp/bigint` のサポート (#4329 by @shellderp)
-   [MySQL Dialect] MySQL: `now` を追加 (#4431 by @hfhbd)
...
### 変更
-   [Sqlite Driver] `JdbcSqliteDriver` のスキーママイグレーション (`schema migrations`) の使用を簡素化 (`Simplify`) (#3737 by @morki)
-   [R2DBC Driver] 真の非同期 `R2DBC cursor` (#4387 by @hfhbd)
...
-   [Compiler] 仮想テーブル (`virtual table`) をインターフェース型 (`interface type`) として使用しないように修正 (#4427 by @hfhbd)

## [2.0.0-rc02] - 2023-06-27
[2.0.0-rc02]: https://github.com/sqldelight/sqldelight/releases/tag/2.0.0-rc02

### 追加
-   [MySQL Dialect] 小文字の日付型 (`lowercase date types`) と日付型の最小/最大値 (`min and max on date types`) のサポート (#4243 by @shellderp)
-   [MySQL Dialect] `binary expr` と `sum` の `mysql types` をサポート (#4254 by @shellderp)
-   [MySQL Dialect] 表示幅なしの符号なし整数 (`unsigned ints without display width`) をサポート (#4306 by @shellderp)
-   [MySQL Dialect] `LOCK IN SHARED MODE` をサポート
-   [PostgreSQL Dialect] `min max` に `boolean` と `Timestamp` を追加 (#4245 by @griffio)
-   [PostgreSQL Dialect] Postgres: ウィンドウ関数 (`window function`) サポートを追加 (#4283 by @hfhbd)
-   [Runtime] `runtime` に `linuxArm64`、`androidNative`、`watchosDeviceArm` ターゲットを追加 (#4258 by @hfhbd)
-   [Paging Extension] ペイジング拡張 (`paging extension`) に `linux` と `mingw x64` ターゲットを追加 (#4280 by @chippman)

### 変更
-   [Gradle Plugin] Android `API 34` の自動ダイアレクトサポート (`automatic dialect support`) を追加 (#4251)
-   [Paging Extension] `QueryPagingSource` で `SuspendingTransacter` をサポート (#4292 by @daio)
-   [Runtime] `addListener api` を改善 (#4244 by @hfhbd)
-   [Runtime] マイグレーションバージョン (`migration version`) に `Long` を使用するように変更 (#4297 by @hfhbd)

### 修正
-   [Gradle Plugin] 生成されたソース (`generated source`) の安定した出力パス (`stable output path`) を使用するように修正 (#4269 by @joshfriend)
-   [Gradle Plugin] `Gradle tweaks` を修正 (#4222 by @3flex)
...
-   [PostgreSQL Dialect] `gen_random_uuid()` のサポートとテストを追加 (#3855 by @davidwheeler123)
-   [PostgreSQL Dialect] `Alter table add constraint postgres` をサポート (#4116 by @griffio)
-   [PostgreSQL Dialect] `Alter table add constraint check` をサポート (#4120 by @griffio)
-   [PostgreSQL Dialect] `postgreSql character length functions` を追加 (#4121 by @griffio)
-   [PostgreSQL Dialect] `postgreSql column default interval` を追加 (#4142 by @griffio)
-   [PostgreSQL Dialect] `postgreSql interval column result` を追加 (#4152 by @griffio)
-   [PostgreSQL Dialect] `postgreSql Alter Column` を追加 (#4165 by @griffio)
-   [PostgreSQL Dialect] PostgreSQL: `date_part` を追加 (#4198 by @hfhbd)
-   [MySQL Dialect] `sql char length functions` を追加 (#4134 by @griffio)
...
-   [Gradle Plugin] 型安全なプロジェクトアクセッサ (`typesafe project accessors`) を有効化 (`Enable`) (#4005 by @hfhbd)

### 変更
-   [Gradle Plugin] `ServiceLoader` メカニズムによる `VerifyMigrationTask` への `DriverInitializer` の登録 (`registering DriverInitializer`) を許可 (#3986 by @C2H6O)
-   [Gradle Plugin] 明示的なコンパイラ環境 (`explicit compiler env`) を作成 (#4079 by @hfhbd)
-   [JS Driver] `web worker driver` を別のアーティファクト (`separate artifact`) に分割 (`Split`)
-   [JS Driver] `JsWorkerSqlCursor` を公開しない (`Don't expose`) ように修正 (#3874 by @hfhbd)
...
-   [Runtime] 非推奨のターゲット (`deprecated targets`) を削除 (#4149 by @hfhbd)
...
-   [Gradle Plugin] 小規模な `Gradle plugin QoL improvements` を修正 (#3930 by @zacsweers)
...
-   [SQLite Dialect] `SQLite 3.33` サポート (`UPDATE FROM`) を追加 (by @eygraber)
-   [PostgreSQL Dialect] Postgres で `UPDATE FROM` をサポート (by @eygraber)

### 変更
-   [RDBC Driver] 接続 (`connection`) を公開 (`Expose`) (by @hfhbd)
...
-   [Gradle Plugin] `IntelliJ` のみをシェード (`Only shade Intellij`) するように変更 (by @hfhbd)
-   [Gradle Plugin] `Kotlin 1.8.0-Beta` をサポートし、マルチバージョン `Kotlin` テスト (`multi version Kotlin test`) を追加 (by @hfhbd)

### 修正
-   [RDBC Driver] `javaObjectType` を使用するように変更 (by @hfhbd)
-   [RDBC Driver] `bindStatement` におけるプリミティブな `NULL` 値 (`primitive null values`) を修正 (by @hfhbd)
-   [RDBC Driver] `R2DBC 1.0` をサポート (by @hfhbd)
-   [PostgreSQL Dialect] Postgres: 型パラメータなしの配列 (`Array without type parameter`) を修正 (by @hfhbd)
-   [IDE Plugin] `intellij` を `221.6008.13` にバンプ (`Bump intellij to 221.6008.13`) (by @hfhbd)
-   [Compiler] 純粋なビュー (`pure views`) からの再帰的な `origin table` を解決 (`Resolve recursive origin table`) (by @hfhbd)
-   [Compiler] テーブル外部キー句 (`table foreign key clause`) から値クラス (`value classes`) を使用 (`Use value classes from table foreign key clause`) (by @hfhbd)
-   [Compiler] `SelectQueryGenerator` が括弧なしのバインド式 (`bind expression without parenthesis`) をサポートするように修正 (by @bellatoris)
-   [Compiler] トランザクション使用時に `${name}Indexes` 変数の重複生成 (`duplicate generation`) を修正 (by @sachera)
...
### 追加
-   [HSQL Dialect] HSQL: `Insert` で生成された列 (`generated columns`) に `DEFAULT` を使用することをサポート (#3372 by @hfhbd)
-   [PostgreSQL Dialect] PostgreSQL: `INSERT` で生成された列 (`generated columns`) に `DEFAULT` を使用することをサポート (#3373 by @hfhbd)
-   [PostgreSQL Dialect] PostgreSQL に `NOW()` を追加 (#3403 by @hfhbd)
-   [PostgreSQL Dialect] PostgreSQL に `NOT` 演算子 (`operator`) を追加 (#3504 by @hfhbd)
...
-   [Native Driver] `NativeSqliteDriver` の `DatabaseConfiguration` 作成にフック (`hook into`) するコールバック (`callback`) を追加 (#3512 by @svenjacobs)
...
-   [Async Runtime] `await*` を上位クラス `ExecutableQuery` に移動 (#3524 by @hfhbd)
-   [Coroutines Extensions] `flow` 拡張のデフォルトパラメータ (`default params`) を削除 (#3489)

### 修正
-   [Gradle Plugin] Kotlin `1.7.20` にアップデート (#3542 by @zacsweers)
-   [R2DBC Driver] 常に値を送信しない `R2DBC changes` を採用 (#3525 by @hfhbd)
-   [HSQL Dialect] HSQL で `sqlite VerifyMigrationTask` が失敗する問題を修正 (#3380 by @hfhbd)
-   [Gradle Plugin] タスクをレイジー設定 `API` (`lazy configuration API`) を使用するように変換 (by @3flex)
...
-   [Compiler] `AS` を修正 (#3370 by @hfhbd)
-   [Compiler] `getObject` メソッドが実際の型の自動埋め込み (`automatic filling of the actual type`) をサポートするように修正 (#3401 by @robxyy)
-   [Compiler] 非同期グループ化 `returning` ステートメント (`async grouped returning statements`) のコード生成 (`codegen`) を修正 (#3411)
-   [Compiler] 可能であればバインドパラメータ (`bind parameter`) の `Kotlin` 型を推論 (`Infer the Kotlin type`) し、そうでなければより良いエラーメッセージ (`better error message`) で失敗するように修正 (#3413 by @hfhbd)
-   [Compiler] `ABS("foo")` を許可しないように修正 (#3430 by @hfhbd)
-   [Compiler] 他のパラメータから `kotlin type` を推論 (`inferring kotlin type`) するサポートを追加 (#3431 by @hfhbd)
-   [Compiler] 常にデータベースの実装 (`database implementation`) を作成するように修正 (#3540 by @hfhbd)
-   [Compiler] `javaDoc` を緩和し、カスタムマッパー関数 (`custom mapper function`) にも追加するように修正 (#3554 @hfhbd)
-   [Compiler] バインディング (`binding`) の `DEFAULT` を修正 (by @hfhbd)
...
-   [Runtime] 非同期ドライバー (`async drivers`) のサポートを追加 (#3168 by @dellisd)
-   [Native Driver] 新しい `kotlin native memory model` をサポート (#3177 by @kpgalligan)
-   [JS Driver] `SqlJs workers` 用のドライバー (`driver`) を追加 (#3203 by @dellisd)
...
-   [PostgreSQL Dialect] `date_trunc` を追加 (#3295 by @hfhbd)
...
-   [Runtime] サードパーティの実装 (`3rd party implementations`) をサポートするためにダイアレクト (`dialect`) とリゾルバー (`resolver`) を公開 (`Open`) (#3232 by @hfhbd)
-   [Compiler] 失敗レポート (`failure reports`) にコンパイル (`compile`) に使用されたダイアレクト (`dialect`) を含める (`Include`) ように変更 (#3086)
-   [Compiler] 未使用のアダプター (`unused adapters`) をスキップ (`Skip`) (#3162 by @eygraber)
-   [Compiler] `PrepareStatement` で `0` ベースのインデックス (`zero based index`) を使用するように変更 (#3269 by @hfhbd)
...
-   [Gradle Plugin] Gradle Verify Task: データベースファイルが欠落 (`missing database file`) している場合にエラーをスロー (`Throw when missing database file`) するように修正 (#3126 by @vanniktech)

### 修正
-   [Gradle Plugin] Gradle プラグインの軽微なクリーンアップと調整 (`Minor cleanups and tweaks`) を修正 (#3171 by @3flex)
-   [Gradle Plugin] 生成されたディレクトリ (`generated directory`) に `AGP` 文字列を使用しないように変更
-   [Gradle Plugin] `AGP namespace attribute` を使用するように修正 (#3220)
-   [Gradle Plugin] `kotlin-stdlib` を Gradle プラグインのランタイム依存関係 (`runtime dependency`) として追加しないように修正 (#3245 by @mbonnin)
-   [Gradle Plugin] マルチプラットフォーム設定 (`multiplatform configuration`) を簡素化 (`Simplify`) (#3246 by @mbonnin)
-   [Gradle Plugin] `js only projects` をサポート (#3310 by @hfhbd)
...
-   [Compiler] 空のクエリファイル (`empty query files`) を生成しない (`Don't generate empty query files`) ように修正 (#3300 by @hfhbd)
-   [Compiler] 疑問符のみのアダプター (`adapter with question mark only`) を修正 (#3314 by @hfhbd)
-   [PostgreSQL Dialect] Postgres の主キー列 (`primary key columns`) は常に `non-null` になるように修正 (#3092)
-   [PostgreSQL Dialect] 複数のテーブルで同じ名前のコピー (`copy with same name`) を修正 (#3297 by @hfhbd)
-   [SQLite 3.35 Dialect] 変更されたテーブル (`altered table`) からインデックス付き列 (`indexed column`) を削除 (`dropping`) する場合にのみエラーを表示するように修正 (#3158 by @eygraber)
...
-   [Runtime] `SqlCursor` を接続にアクセスするクリティカルセクション (`critical section accessing the connection`) に限定 (`Confine`) するように変更 (#2123 by @andersio)
...
-   [Gradle Plugin] `Gradle type safe project accessors` のショートカット (`shortcut`) を追加 (by @hfhbd)
-   [Compiler] ダイアレクト (`dialect`) に基づいてコード生成 (`codegen`) をカスタマイズ (`Customize`) (by @MariusVolkhart)
-   [JDBC Driver] `JdbcDriver` に共通型 (`common types`) を追加 (by @MariusVolkhart)
...
-   [Compiler] 生成されたデータクラス (`generated data classes`) のカスタム `toString` 生成を削除 (by @PaulWoitaschek)
-   [JS Driver] `sql.js` 依存関係 (`dependency`) を `sqljs-driver` から削除 (by @dellisd)
...
### 修正
-   [Runtime] リスナリスト (`listener list`) をフラット化 (`Flatten`) し、割り当て (`allocations`) とポインタチェイス (`pointer chasing`) を削減 (by @andersio)
...
-   [Gradle Plugin] ランタイム (`runtime`) 追加時に `js/android` プラグインを明示的にサポート (`Explicitly support`) (by @ZacSweers)
-   [Gradle Plugin] マイグレーション (`migrations`) からスキーマ (`schemas`) を導出 (`derviving`) せずにマイグレーション出力タスク (`migration output task`) を登録 (`Register`) するように修正 (#2744 by @kevincianfarini)
...
-   [Gradle Plugin] コード生成 (`generating code`) 時にファイルをソート (`Sort files`) し、出力が冪等 (`idempotent outputs`) であることを保証 (by @ZacSweers)
-   [Compiler] ファイルを反復処理 (`iterating files`) するための高速な `API` (`faster APIs`) を使用し、`PSI` グラフ全体 (`entire PSI graph`) を探索 (`explore`) しないように修正
-   [Compiler] `select function parameters` にキーワードマングリング (`keyword mangling`) を追加 (#2759 by @aperfilyev)
-   [Compiler] マイグレーションアダプター (`migration adapter`) の `packageName` を修正 (by @hfhbd)
-   [Compiler] 型 (`types`) の代わりにプロパティ (`properties`) にアノテーション (`annotations`) を出力 (`Emit`) するように修正 (#2798 by @aperfilyev)
-   [Compiler] `Query subtype` に渡す前に引数をソート (`Sort arguments`) するように修正 (#2379 by @aperfilyev)

## [1.5.3] - 2021-11-23
[1.5.3]: https://github.com/sqldelight/sqldelight/releases/tag/1.5.3

### 追加
-   [JDBC Driver] サードパーティドライバー実装 (`3rd party driver implementations`) のために `JdbcDriver` を公開 (`Open`) (#2672 by @hfhbd)
-   [MySQL Dialect] 時間増分 (`time increments`) 用の不足している関数 (`missing functions`) を追加 (#2671 by @sdoward)
-   [Coroutines Extension] コルーチン拡張 (`coroutines-extensions`) に `M1` ターゲット (`M1 targets`) を追加 (by @PhilipDukhov)

### 変更
-   [Paging3 Extension] `sqldelight-android-paging3` を `AAR` ではなく `JAR` として配布 (`Distribute`) するように変更 (#2634 by @julioromano)
...
### 修正
-   [Compiler] 重複する配列パラメータ (`duplicate array parameters`) の変数 (`variables`) を抽出しない (`Don't extract`) ように修正 (by @aperfilyev)
-   [Gradle Plugin] `kotlin.mpp.enableCompatibilityMetadataVariant` を追加 (#2628 by @martinbonnin)
...
### 追加
-   [Gradle Plugin] `HMPP` サポート (#2548 by @martinbonnin)
...
-   [SQLite Driver] `mingwX86` ターゲットを追加 (#2558 by @enginegl)
-   [SQLite Driver] `M1` ターゲットを追加
-   [SQLite Driver] `linuxX64` サポートを追加 (#2456 by @chippmann)
-   [MySQL Dialect] `MySQL` に `ROW_COUNT` 関数を追加 (#2523)
-   [PostgreSQL Dialect] postgres の列名変更 (`rename`)、列削除 (`drop column`) をサポート (by @pabl0rg)
...
-   [Runtime] `SqlDriver` を `AfterVersion` のパラメータとして提供 (#2534, 2614 by @ahmedre)

### 変更
-   [Gradle Plugin] Gradle `7.0` を明示的に要求 (`explicitly require`) するように変更 (#2572 by @martinbonnin)
-   [Gradle Plugin] `VerifyMigrationTask` が `Gradle` の最新チェック (`up-to-date checks`) をサポートするように変更 (#2533 by @3flex)
...
-   [Gradle Plugin] `generateDatabaseInterface task` の依存関係 (`dependency`) を潜在的なコンシューマ (`potential consumers`) に伝播 (`Propagate`) するように修正 (#2518 by @martinbonnin)

## [1.5.1] - 2021-07-16
[1.5.1]: https://github.com/sqldelight/sqldelight/releases/tag/1.5.1

### 追加
-   [PostgreSQL Dialect] PostgreSQL `JSONB` と `ON Conflict Do Nothing` をサポート (by @satook)
-   [PostgreSQL Dialect] `PostgreSQL ON CONFLICT (column, ...) DO UPDATE` のサポートを追加 (by @satook)
-   [MySQL Dialect] `MySQL generated columns` をサポート (by @JGulbronson)
...
### 変更
-   [Native Driver] フリーズ (`freezing`) と共有可能データ構造 (`shareable data structures`) を可能な限り回避 (`avoiding`) することで、ネイティブトランザクション (`native transaction`) のパフォーマンス (`performance`) を改善 (by @andersio)
-   [Paging 3] Paging3 バージョンを `3.0.0 stable` にバンプ (`Bump`)
-   [JS Driver] `sql.js` を `1.5.0` にアップグレード

### 修正
-   [JDBC SQLite Driver] `ThreadLocal` をクリアする前に接続 (`connection`) で `close()` を呼び出す (#2444 by @hannesstruss)
-   [RX extensions] サブスクリプション/破棄競合リーク (`subscription / disposal race leak`) を修正 (#2403 by @pyricau)
-   [Coroutines extension] 通知 (`notifying`) する前にクエリリスナー (`query listener`) を登録 (`register`) することを保証
-   [Compiler] 一貫した `kotlin output file` を持つよう `notifyQueries` をソート (`Sort notifyQueries`) (by @thomascjy)
-   [Compiler] `select query class properties` に `@JvmField` アノテーションを付けない (`Don't annotate`) (by @eygraber)
...
### 追加
-   [SQLite Javascript Driver] `sqljs-driver publication` を有効化 (#1667 by @dellisd)
-   [Paging3 Extension] `Android Paging 3 Library` の拡張 (#1786 by @kevincianfarini)
-   [MySQL Dialect] `mysql's ON DUPLICATE KEY UPDATE` 競合解決 (`conflict resolution`) のサポートを追加 (by @rharter)
-   [SQLite Dialect] `SQLite offsets()` のコンパイラサポート (`compiler support`) を追加 (by @qjroberts)
...
### 修正
-   [Compiler] ラムダ (`lambdas`) の前の `NBSP` を修正 (by @oldergod)
...
-   [SQLite Dialect] `UPSERT` 通知 (`notifications`) は `BEFORE/AFTER UPDATE` トリガー (`triggers`) を考慮すべき (#2198 by @andersio)
...
-   [Gradle Plugin] `WorkerApi` を使用したデータベース生成 (`generating a database`) での競合状態 (`Race condition`) (#2062 by @stephanenicolas)
-   [Gradle Plugin] `classLoaderIsolation` がカスタム `jdbc` の使用 (`custom jdbc usage`) を妨げる (`prevents`) (#2048 by @benasher44)
-   [Gradle Plugin] 欠落している `packageName` エラーメッセージ (`error message`) を改善 (by @vanniktech)
...
-   [Gradle Plugin] `Gradle plugin` で `kotlin-native-utils` に依存しない (`Do not depend on`) (by @ilmat192)
...
### 修正
-   [Compiler] `ORDER BY` 句で列名 (`column name`) をバインド (`binding`) する際にエラーを報告 (`Report an error`) (#1187 by @eygraber)
...
-   [Compiler] クエリラッパー (`query wrappers`) で多数の名前シャドウ警告 (`name shadowed warnings`) (#1946 by @eygraber)
...
-   [Gradle Plugin] `SqlDeslight gradle plugin` は `Configuration Caching (CoCa)` をサポートしていない (`doesn't support`) (#1947 by @stephanenicolas)
-   [SQLite JDBC Driver] `SQLException`: `database in auto-commit mode` (#1832)
-   [Coroutines Extension] `coroutines-extensions` の `IR backend` を修正 (#1918 by @dellisd)

## [1.4.3] - 2020-09-04
[1.4.3]: https://github.com/sqldelight/sqldelight/releases/tag/1.4.3

### 追加
-   [MySQL Dialect] `MySQL last_insert_id function` のサポートを追加 (by @lawkai)
-   [PostgreSQL Dialect] `SERIAL` データ型 (`data type`) をサポート (by @veyndan & @felipecsl)
-   [PostgreSQL Dialect] `PostgreSQL RETURNING` をサポート (by @veyndan)

### 修正
-   [MySQL Dialect] `MySQL AUTO_INCREMENT` をデフォルト値 (`default value`) を持つものとして扱う (#1823)
-   [Compiler] `Upsert statement compiler error` を修正 (#1809 by @eygraber)
-   [Compiler] 無効な `Kotlin` が生成される (`invalid Kotlin being generated`) 問題を修正 (#1925 by @eygraber)
...
-   [Gradle Plugin] `generateSqlDelightInterface Gradle task` を追加 (by @vanniktech)
-   [Gradle Plugin] `verifySqlDelightMigration Gradle task` を追加 (by @vanniktech)
...
-   [MySQL Dialect] `mySqlFunctionType()` に `minute` を追加 (by @maaxgr)
...
### 修正
-   [Compiler] 列定義ルール (`column defintion rule`) とテーブルインターフェースジェネレータ (`table interface generator`) にオプションの `javadoc` を追加 (#1224 by @endanke)
-   [SQLite Dialect] `sqlite fts5 auxiliary functions highlight, snippet, and bm25` のサポートを追加 (by @drampelt)
-   [MySQL Dialect] `MySQL bit data type` をサポート
-   [MySQL Dialect] `MySQL binary literals` をサポート
-   [PostgreSQL Dialect] `sql-psi` から `SERIAL` を公開 (by @veyndan)
-   [PostgreSQL Dialect] `BOOLEAN data type` を追加 (by @veyndan)
-   [PostgreSQL Dialect] `NULL column constraint` を追加 (by @veyndan)
-   [HSQL Dialect] `HSQL` に `AUTO_INCREMENT` サポートを追加 (by @rharter)
...
-   [SQLite Dialect] `alter table rename column` のサポート (#1505 by @angusholder)
...
### 変更
-   [Documentation] ドキュメントウェブサイト (`documentation website`) の大幅な刷新 (`Overhaul`) (by @saket)
-   [Gradle Plugin] サポートされていないダイアレクト (`unsupported dialect`) のエラーメッセージ (`error message`) を改善 (by @veyndan)
...
-   [Compiler] ディレクトリ名 (`directory name`) のサニタイズ (`sanitizing`) における不整合 (`inconsistency`) を修正 (by @ZacSweers)
...
-   [Compiler] クォーティング (`quoting`) を修正 (#1525 by @angusholder)
...
-   [Native Driver] `execute` で `transactionPool` を使用するようにする (by @benasher44)
