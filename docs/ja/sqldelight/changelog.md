...
-   [PostgreSQL ダイアレクト] 追加の Postgres Regex 関数定義 (`Postgres Regex function definitions`) を追加 (#5025 by @MariusVolkhart)
...
-   [PostgreSQL ダイアレクト] 追加の Postgres String 関数定義 (`Postgres String function definitions`) を追加 (#4752 by @MariusVolkhart)
...
### 変更
-   [コンパイラ] 適切な場合に Kotlin 型 (`Kotlin types`) を優先 (`Prefer Kotlin types where appropriate`) するよう変更 (#4517 by @eygraber)
...
### 変更
-   [Gradle プラグイン] Android API 34 (`Android API 34`) の自動ダイアレクトサポート (`automatic dialect support`) を追加 (#4251)
...
-   [Gradle プラグイン] Gradle の微調整 (`Gradle tweaks`) (#4222 by @3flex)
...
### 修正
-   [R2DBC Driver] R2DBC: ドライバーのクローズを待機 (`Await closing the driver`) (#4139 by @hfhbd)
...
-   [IDE プラグイン] Java ホーム (`java home`) が不足している場合、利用可能な JDK (`available jdk`) を使用する (#3925 by @aperfilyev)
...
-   [IDE プラグイン] IntelliJ 2023.1 (`IntelliJ 2023.1`) のサポートを追加 (#4037 by @madisp)
...
これは Kotlin 1.8 および IntelliJ 2021 以降の互換性リリースであり、JDK 17 をサポートします。
...
-   Paging 3 拡張 API は、カウントに `int` 型のみを許可するように変更されました。
-   コルーチン拡張は、デフォルトではなくディスパッチャを渡す必要があります。
-   ダイアレクトとドライバクラスは `final` であり、代わりに委譲を使用してください。
...
-   [R2DBC Driver] 常に値を送信しない R2DBC の変更 (`Adopt R2DBC changes which do not always send a value`) を採用 (#3525 by @hfhbd)
...
-   [IDE プラグイン] `UnusedQueryInspection` - `ArrayIndexOutOfBoundsException` を修正 (#3427 by @vanniktech)
...
-   [IDE プラグイン] Android Studio DB インスペクター (`android studio DB inspector`) のサポートを追加 (#3107 by @aperfilyev)
-   [ランタイム] 非同期ドライバー (`async drivers`) のサポートを追加 (#3168 by @dellisd)
-   [ネイティブドライバー] 新しい Kotlin Native メモリモデル (`new kotlin native memory model`) をサポート (#3177 by @kpgalligan)
...
-   [Gradle プラグイン] AGP 名前空間属性 (`AGP namespace attribute`) を使用 (#3220)
-   [Gradle プラグイン] Gradle プラグインのランタイム依存関係として `kotlin-stdlib` を追加しない (`Do not add kotlin-stdlib as a runtime dependency of the Gradle plugin`) (#3245 by @mbonnin)
-   [Gradle プラグイン] マルチプラットフォーム設定を簡素化 (`Simplify the multiplatform configuration`) (#3246 by @mbonnin)
-   [Gradle プラグイン] JS 専用プロジェクトをサポート (`Support js only projects`) (#3310 by @hfhbd)
-   [IDE プラグイン] Gradle tooling API に Java ホーム (`java home`) を使用 (#3078)
...
-   [MySQL] MySQL の `TRUNCATE` ステートメントをサポート
-   [MySQL] MySQL で JSON 固有のオペレーターをサポート (by @eygraber)
-   [MySQL] MySQL の `INTERVAL` をサポート (#2969 by @eygraber)
...
### 修正
-   [コンパイラ] 異なるテーブルの同じアダプター型が 2.0 アルファでコンパイルエラーを引き起こす問題を修正
...
-   [JS Driver] `sqljs-driver` から `sql.js` 依存関係を削除 (by @dellisd)
...
-   [コルーチン拡張] コルーチン拡張機能に M1 ターゲットを追加 (by @PhilipDukhov)
...
-   [PostgreSQL ダイアレクト] PostgreSQL の `GENERATED` 列の文法を追加
...
-   [PostgreSQL ダイアレクト] PostgreSQL の `ON CONFLICT (column, ...) DO UPDATE` のサポートを追加 (by @satook)
-   [MySQL ダイアレクト] MySQL の生成列をサポート (by @JGulbronson)
...
-   [Paging3 拡張] Android Paging 3 ライブラリの拡張機能 (#1786 by @kevincianfarini)
-   [MySQL ダイアレクト] MySQL の `ON DUPLICATE KEY UPDATE` 競合解決のサポートを追加 (by @rharter)
-   [SQLite ダイアレクト] SQLite の `offsets()` のコンパイラサポートを追加 (by @qjroberts)
...
-   [JDBC ドライバー] JDBC ドライバーは `autoCommit` が `true` であると仮定する (#2041)
...
-   [IDE プラグイン] `GoTo` 中に Kotlin の解決が失敗した場合、`sqldelight` ファイルに移動しようとしない
...
-   [Gradle プラグイン] `SqlDeslight` Gradle プラグインが Configuration Caching (CoCa) をサポートしない (#1947 by @stephanenicolas)
-   [SQLite JDBC Driver] `SQLException: database in auto-commit mode` (#1832)
-   [コルーチン拡張] コルーチン拡張機能の IR バックエンドを修正 (#1918 by @dellisd)
...
-   [PostgreSQL ダイアレクト] PostgreSQL の `RETURNING` をサポート (by @veyndan)

### 修正
-   [MySQL ダイアレクト] MySQL の `AUTO_INCREMENT` をデフォルト値を持つものとして扱う (#1823)
...
-   [MySQL ダイアレクト] MySQL の `bit` データ型をサポート
-   [MySQL ダイアレクト] MySQL のバイナリリテラルをサポート
...
*   新規: 生成されたコードが `Support SQLite` ライブラリのみを使用するように更新されました。すべてのクエリは、生の文字列ではなくステートメントオブジェクトを生成するようになりました。
...
*   修正: IntelliJ でパラメータ化された型をクリックできるようになりました
