...
-   [IDE Plugin] `IDEA 2023.3` で `BugSnag` の初期化に失敗 (`fails to init`) (by @aperfilyev)
...
-   [コンパイラ] グループ化されたステートメント (`grouped statements`) の結果式 (`result expression`) を修正 (#4378)
-   [コンパイラ] 仮想テーブル (`virtual table`) をインターフェース型 (`interface type`) として使用しないように修正 (#4427 by @hfhbd)

## [2.0.0-rc02] - 2023-06-27
[2.0.0-rc02]: https://github.com/sqldelight/sqldelight/releases/tag/2.0.0-rc02

### 追加
-   [MySQL ダイアレクト] 小文字の日付型 (`lowercase date types`) と日付型の最小/最大値 (`min and max on date types`) のサポート (#4243 by @shellderp)
-   [MySQL ダイアレクト] `binary expr` と `sum` の `mysql types` をサポート (#4254 by @shellderp)
-   [MySQL ダイアレクト] 表示幅なしの符号なし整数 (`unsigned ints without display width`) をサポート (#4306 by @shellderp)
-   [MySQL ダイアレクト] `LOCK IN SHARED MODE` をサポート
-   [PostgreSQL ダイアレクト] `min max` に `boolean` と `Timestamp` を追加 (#4245 by @griffio)
-   [PostgreSQL ダイアレクト] Postgres: ウィンドウ関数 (`window function`) サポートを追加 (#4283 by @hfhbd)
-   [ランタイム] `runtime` に `linuxArm64`、`androidNative`、`watchosDeviceArm` ターゲットを追加 (#4258 by @hfhbd)
-   [ページング拡張] ペイジング拡張 (`paging extension`) に `linux` と `mingw x64` ターゲットを追加 (#4280 by @chippman)

### 変更
-   [Gradle プラグイン] Android `API 34` の自動ダイアレクトサポート (`automatic dialect support`) を追加 (#4251)
-   [ページング拡張] `QueryPagingSource` で `SuspendingTransacter` をサポート (#4292 by @daio)
-   [ランタイム] `addListener api` を改善 (#4244 by @hfhbd)
-   [ランタイム] マイグレーションバージョン (`migration version`) に `Long` を使用するように変更 (#4297 by @hfhbd)

### 修正
-   [Gradle プラグイン] 生成されたソース (`generated source`) の安定した出力パス (`stable output path`) を使用するように修正 (#4269 by @joshfriend)
-   [Gradle プラグイン] `Gradle tweaks` を修正 (#4222 by @3flex)

## [2.0.0-rc01] - 2023-05-29
[2.0.0-rc01]: https://github.com/sqldelight/sqldelight/releases/tag/2.0.0-rc01

### 追加
-   [Paging] ペイジング拡張 (`paging extensions`) に `js browser` ターゲットを追加 (#3843 by @sproctor)
...
-   [PostgreSQL ダイアレクト] `Alter table add constraint postgres` をサポート (#4116 by @griffio)
-   [PostgreSQL ダイアレクト] `Alter table add constraint check` をサポート (#4120 by @griffio)
-   [PostgreSQL ダイアレクト] `postgreSql character length functions` を追加 (#4121 by @griffio)
-   [PostgreSQL ダイアレクト] `postgreSql column default interval` を追加 (#4142 by @griffio)
-   [PostgreSQL ダイアレクト] `postgreSql interval column result` を追加 (#4152 by @griffio)
-   [PostgreSQL ダイアレクト] `postgreSql Alter Column` を追加 (#4165 by @griffio)
-   [PostgreSQL ダイアレクト] PostgreSQL: `date_part` を追加 (#4198 by @hfhbd)
-   [MySQL ダイアレクト] `sql char length functions` を追加 (#4134 by @griffio)
-   [IDE Plugin] `sqldelight directory suggestions` を追加 (#3976 by @aperfilyev)
-   [IDE Plugin] プロジェクトツリー (`project tree`) のミドルパッケージ (`middle packages`) を圧縮 (`Compact`) (#3992 by @aperfilyev)
-   [IDE Plugin] `join clause completion` を追加 (#4086 by @aperfilyev)
-   [IDE Plugin] `Create view intention` と `live template` を追加 (#4074 by @aperfilyev)
-   [IDE Plugin] `DELETE` または `UPDATE` 内の `WHERE` 句の欠落 (`missing WHERE`) について警告 (`Warn`) (#4058 by @aperfilyev)
-   [Gradle プラグイン] 型安全なプロジェクトアクセッサ (`typesafe project accessors`) を有効化 (`Enable`) (#4005 by @hfhbd)

### 変更
-   [Gradle プラグイン] `ServiceLoader` メカニズムによる `VerifyMigrationTask` への `DriverInitializer` の登録 (`registering DriverInitializer`) を許可 (#3986 by @C2H6O)
-   [Gradle プラグイン] 明示的なコンパイラ環境 (`explicit compiler env`) を作成 (#4079 by @hfhbd)
-   [JS ドライバー] `web worker driver` を別のアーティファクト (`separate artifact`) に分割 (`Split`)
-   [JS ドライバー] `JsWorkerSqlCursor` を公開しない (`Don't expose`) ように修正 (#3874 by @hfhbd)
-   [JS ドライバー] `sqljs driver` の公開 (`publication`) を無効化 (`Disable`) (#4108)
-   [ランタイム] 同期ドライバー (`synchronous drivers`) が同期スキーマ初期化子 (`synchronous schema initializer`) を必要とすることを強制 (`Enforce`) (#4013)
-   [ランタイム] カーソル (`Cursors`) の非同期サポート (`async support`) を改善 (#4102)
-   [ランタイム] 非推奨のターゲット (`deprecated targets`) を削除 (#4149 by @hfhbd)
-   [ランタイム] 古い `MM` のサポートを削除 (#4148 by @hfhbd)

### 修正
-   [R2DBC ドライバー] R2DBC: ドライバーのクローズ (`closing the driver`) を待機 (`Await`) するように修正 (#4139 by @hfhbd)
-   [コンパイラ] マイグレーションからの `PRAGMAs` を `database create(SqlDriver)` に含めるように修正 (#3845 by @MariusVolkhart)
-   [コンパイラ] `RETURNING` 句のコード生成 (`codegen`) を修正 (#3872 by @MariusVolkhart)
-   [コンパイラ] 仮想テーブル (`virtual tables`) の型を生成しない (`Dont generate types`) ように修正 (#4015)
-   [Gradle プラグイン] 小規模な `Gradle plugin QoL improvements` を修正 (#3930 by @zacsweers)
-   [IDE Plugin] 未解決の Kotlin 型 (`unresolved kotlin types`) を修正 (#3924 by @aperfilyev)
-   [IDE Plugin] ワイルドカード展開インテンション (`expand wildcard intention`) がクオリファイア (`qualifier`) で機能するように修正 (#3979 by @aperfilyev)
-   [IDE Plugin] `java home` が欠落している場合、利用可能な `jdk` を使用するように修正 (#3925 by @aperfilyev)
...
-   [IDE Plugin] `IntelliJ 2023.1` のサポートを追加 (#4037 by @madisp)
-   [IDE Plugin] 列名 (`column rename`) の名前変更時に名前付き引数 (`named argument`) の使用を名前変更するように修正 (#4027 by @aperfilyev)
-   [IDE Plugin] マイグレーション追加ポップアップ (`add migration popup`) を修正 (#4105 by @aperfilyev)
-   [IDE Plugin] マイグレーションファイル (`migration files`) での `SchemaNeedsMigrationInspection` を無効化 (`Disable`) (#4106 by @aperfilyev)
-   [IDE Plugin] 型名 (`type name`) の代わりに `sql column name` をマイグレーション生成 (`migration generation`) に使用するように修正 (#4112 by @aperfilyev)

## [2.0.0-alpha05] - 2023-01-20
[2.0.0-alpha05]: https://github.com/sqldelight/sqldelight/releases/tag/2.0.0-alpha05

### 追加
-   [Paging] マルチプラットフォームページング拡張 (by @jeffdgr8)
-   [ランタイム] `Listener` インターフェースに `fun` 修飾子 (`fun modifier`) を追加。
-   [SQLite ダイアレクト] `SQLite 3.33` サポート (`UPDATE FROM`) を追加 (by @eygraber)
-   [PostgreSQL ダイアレクト] Postgres で `UPDATE FROM` をサポート (by @eygraber)

### 変更
-   [R2DBC ドライバー] 接続 (`connection`) を公開 (`Expose`) (by @hfhbd)
-   [ランタイム] マイグレーションコールバック (`migration callbacks`) をメインの `migrate` 関数に移動 (`Move migration callbacks into main migrate fun`)
-   [Gradle プラグイン] ダウンストリームプロジェクト (`downstream projects`) から設定 (`Configurations`) を非表示 (`Hide`) に
-   [Gradle プラグイン] `IntelliJ` のみをシェード (`Only shade Intellij`) するように変更 (by @hfhbd)
-   [Gradle プラグイン] `Kotlin 1.8.0-Beta` をサポートし、マルチバージョン `Kotlin` テスト (`multi version Kotlin test`) を追加 (by @hfhbd)

### 修正
-   [R2DBC ドライバー] `javaObjectType` を使用するように変更 (by @hfhbd)
-   [R2DBC ドライバー] `bindStatement` におけるプリミティブな `NULL` 値 (`primitive null values`) を修正 (by @hfhbd)
-   [R2DBC ドライバー] `R2DBC 1.0` をサポート (by @hfhbd)
-   [PostgreSQL ダイアレクト] Postgres: 型パラメータなしの配列 (`Array without type parameter`) を修正 (by @hfhbd)
-   [IDE Plugin] `intellij` を `221.6008.13` にバンプ (`Bump intellij to 221.6008.13`) (by @hfhbd)
-   [コンパイラ] 純粋なビュー (`pure views`) からの再帰的な `origin table` を解決 (`Resolve recursive origin table`) (by @hfhbd)
-   [コンパイラ] テーブル外部キー句 (`table foreign key clause`) から値クラス (`value classes`) を使用 (`Use value classes from table foreign key clause`) (by @hfhbd)
-   [コンパイラ] `SelectQueryGenerator` が括弧なしのバインド式 (`bind expression without parenthesis`) をサポートするように修正 (by @bellatoris)
-   [コンパイラ] トランザクション使用時に `${name}Indexes` 変数の重複生成 (`duplicate generation`) を修正 (by @sachera)
...
### 追加
-   [HSQL ダイアレクト] HSQL: `Insert` で生成された列 (`generated columns`) に `DEFAULT` を使用することをサポート (#3372 by @hfhbd)
-   [PostgreSQL ダイアレクト] PostgreSQL: `INSERT` で生成された列 (`generated columns`) に `DEFAULT` を使用することをサポート (#3373 by @hfhbd)
-   [PostgreSQL ダイアレクト] PostgreSQL に `NOW()` を追加 (#3403 by @hfhbd)
-   [PostgreSQL ダイアレクト] PostgreSQL に `NOT` 演算子 (`operator`) を追加 (#3504 by @hfhbd)
-   [Paging] `*QueryPagingSource` に `CoroutineContext` を渡せるように許可 (#3384)
-   [Gradle プラグイン] ダイアレクト (`dialects`) のバージョンカタログ (`version catalog`) サポートを改善 (#3435)
-   [ネイティブドライバー] `NativeSqliteDriver` の `DatabaseConfiguration` 作成にフック (`hook into`) するコールバック (`callback`) を追加 (#3512 by @svenjacobs)
...
-   [非同期ランタイム] `await*` を上位クラス `ExecutableQuery` に移動 (#3524 by @hfhbd)
-   [コルーチン拡張] `flow` 拡張のデフォルトパラメータ (`default params`) を削除 (#3489)

### 修正
-   [Gradle プラグイン] Kotlin `1.7.20` にアップデート (#3542 by @zacsweers)
-   [R2DBC ドライバー] 常に値を送信しない `R2DBC changes` を採用 (#3525 by @hfhbd)
-   [HSQL ダイアレクト] HSQL で `sqlite VerifyMigrationTask` が失敗する問題を修正 (#3380 by @hfhbd)
-   [Gradle プラグイン] タスクをレイジー設定 `API` (`lazy configuration API`) を使用するように変換 (by @3flex)
-   [Gradle プラグイン] Kotlin `1.7.20` での `NPEs` を回避 (#3398 by @ZacSweers)
-   [Gradle プラグイン] `squash migrations task` の説明 (`description`) を修正 (#3449)
...
-   [コンパイラ] 拡張関数 (`extension function`) を持つカスタムダイアレクト (`Custom dialect`) がインポート (`imports`) を作成しない問題を修正 (#3338 by @hfhbd)
-   [コンパイラ] `CodeBlock.of("${CodeBlock.toString()}")` のエスケープ (`escaping`) を修正 (#3340 by @hfhbd)
-   [コンパイラ] マイグレーション (`migrations`) で非同期実行ステートメント (`async execute statements`) を待機 (`Await`) するように修正 (#3352)
-   [コンパイラ] `AS` を修正 (#3370 by @hfhbd)
-   [コンパイラ] `getObject` メソッドが実際の型の自動埋め込み (`automatic filling of the actual type`) をサポートするように修正 (#3401 by @robxyy)
-   [コンパイラ] 非同期グループ化 `returning` ステートメント (`async grouped returning statements`) のコード生成 (`codegen`) を修正 (#3411)
-   [コンパイラ] 可能であればバインドパラメータ (`bind parameter`) の `Kotlin` 型を推論 (`Infer the Kotlin type`) し、そうでなければより良いエラーメッセージ (`better error message`) で失敗するように修正 (#3413 by @hfhbd)
-   [コンパイラ] `ABS("foo")` を許可しないように修正 (#3430 by @hfhbd)
-   [コンパイラ] 他のパラメータから `kotlin type` を推論 (`inferring kotlin type`) するサポートを追加 (#3431 by @hfhbd)
-   [コンパイラ] 常にデータベースの実装 (`database implementation`) を作成するように修正 (#3540 by @hfhbd)
-   [コンパイラ] `javaDoc` を緩和し、カスタムマッパー関数 (`custom mapper function`) にも追加するように修正 (#3554 @hfhbd)
-   [コンパイラ] バインディング (`binding`) の `DEFAULT` を修正 (by @hfhbd)
...
### 追加
-   [IDE Plugin] 実行中のデータベースに対して `SQLite`、`MySQL`、および `PostgreSQL` コマンドを実行するサポートを追加 (#2718 by @aperfilyev)
-   [IDE Plugin] `Android Studio DB inspector` のサポートを追加 (#3107 by @aperfilyev)
-   [ランタイム] 非同期ドライバー (`async drivers`) のサポートを追加 (#3168 by @dellisd)
-   [ネイティブドライバー] 新しい `kotlin native memory model` をサポート (#3177 by @kpgalligan)
-   [JS ドライバー] `SqlJs workers` 用のドライバー (`driver`) を追加 (#3203 by @dellisd)
-   [Gradle プラグイン] `SQLDelight` タスクのクラスパス (`classpath`) を公開 (`Expose`)
-   [Gradle プラグイン] マイグレーションをスカッシュ (`squashing migrations`) するための `gradle task` を追加
-   [Gradle プラグイン] マイグレーションチェック (`migration checks`) 中にスキーマ定義 (`schema definitions`) を無視 (`ignore`) するフラグ (`flag`) を追加
-   [MySQL ダイアレクト] MySQL で `FOR SHARE` と `FOR UPDATE` をサポート (#3098)
-   [MySQL ダイアレクト] MySQL のインデックスヒント (`index hints`) をサポート (#3099)
-   [PostgreSQL ダイアレクト] `date_trunc` を追加 (#3295 by @hfhbd)
-   [JSON 拡張] `JSON table functions` をサポート (#3090)

### 変更
-   [ランタイム] ドライバーなしの `AfterVersion` 型を削除 (#3091)
-   [ランタイム] `Schema` 型をトップレベル (`top-level`) に移動
-   [ランタイム] サードパーティの実装 (`3rd party implementations`) をサポートするためにダイアレクト (`dialect`) とリゾルバー (`resolver`) を公開 (`Open`) (#3232 by @hfhbd)
-   [コンパイラ] 失敗レポート (`failure reports`) にコンパイル (`compile`) に使用されたダイアレクト (`dialect`) を含める (`Include`) ように変更 (#3086)
-   [コンパイラ] 未使用のアダプター (`unused adapters`) をスキップ (`Skip`) (#3162 by @eygraber)
-   [コンパイラ] `PrepareStatement` で `0` ベースのインデックス (`zero based index`) を使用するように変更 (#3269 by @hfhbd)
-   [Gradle プラグイン] ダイアレクト (`dialect`) も文字列 (`string`) ではなく、適切な `gradle` 依存関係 (`proper gradle dependency`) にする (`make the dialect a proper gradle dependency`) ように変更 (#3085)
-   [Gradle プラグイン] Gradle Verify Task: データベースファイルが欠落 (`missing database file`) している場合にエラーをスロー (`Throw when missing database file`) するように修正 (#3126 by @vanniktech)

### 修正
-   [Gradle プラグイン] Gradle プラグインの軽微なクリーンアップと調整 (`Minor cleanups and tweaks`) を修正 (#3171 by @3flex)
-   [Gradle プラグイン] 生成されたディレクトリ (`generated directory`) に `AGP` 文字列を使用しないように変更
-   [Gradle プラグイン] `AGP namespace attribute` を使用するように修正 (#3220)
-   [Gradle プラグイン] `kotlin-stdlib` を Gradle プラグインのランタイム依存関係 (`runtime dependency`) として追加しないように修正 (#3245 by @mbonnin)
-   [Gradle プラグイン] マルチプラットフォーム設定 (`multiplatform configuration`) を簡素化 (`Simplify`) (#3246 by @mbonnin)
-   [Gradle プラグイン] `js only projects` をサポート (#3310 by @hfhbd)
...
-   [コンパイラ] `RETURNING` ステートメントを持つ更新 (`updates`) と削除 (`deletes`) がクエリを実行 (`execute queries`) するように保証 (#3084)
-   [コンパイラ] 複合 `SELECT` で引数型 (`argument types`) を正しく推論 (`Correctly infer`) するように修正 (#3096)
-   [コンパイラ] 共通テーブル (`Common tables`) はデータクラス (`data classes`) を生成しないため、それらを返さないように修正 (#3097)
-   [コンパイラ] 最上位のマイグレーションファイル (`top migration file`) をより速く見つける (`Find the top migration file faster`) ように修正 (#3108)
-   [コンパイラ] パイプ演算子 (`pipe operator`) で `null` 許容性 (`nullability`) を適切に継承 (`Properly inherit`)
-   [コンパイラ] `iif ANSI SQL function` をサポート
-   [コンパイラ] 空のクエリファイル (`empty query files`) を生成しない (`Don't generate empty query files`) ように修正 (#3300 by @hfhbd)
-   [コンパイラ] 疑問符のみのアダプター (`adapter with question mark only`) を修正 (#3314 by @hfhbd)
-   [PostgreSQL ダイアレクト] Postgres の主キー列 (`primary key columns`) は常に `non-null` になるように修正 (#3092)
-   [PostgreSQL ダイアレクト] 複数のテーブルで同じ名前のコピー (`copy with same name`) を修正 (#3297 by @hfhbd)
-   [SQLite 3.35 ダイアレクト] 変更されたテーブル (`altered table`) からインデックス付き列 (`indexed column`) を削除 (`dropping`) する場合にのみエラーを表示するように修正 (#3158 by @eygraber)
...
### 追加
-   [コンパイラ] グループ化されたステートメント (`grouped statement`) の最後に `returning` をサポート
-   [コンパイラ] ダイアレクトモジュール (`dialect modules`) によるコンパイラ拡張 (`compiler extensions`) をサポートし、`SQLite JSON` 拡張を追加 (#1379, #2087)
-   [コンパイラ] 値 (`value`) を返す `PRAGMA` ステートメント (`PRAGMA statements`) をサポート (#1106)
-   [コンパイラ] マークされた列 (`marked columns`) の値型 (`value types`) 生成をサポート
-   [コンパイラ] 楽観的ロック (`optimistic locks`) とバリデーション (`validation`) のサポートを追加 (#1952)
-   [コンパイラ] マルチ更新ステートメント (`multi-update statements`) をサポート
...
-   [MySQL] `Mysql` で `json specific operators` をサポート (by @eygraber)
-   [MySQL] `MySql INTERVAL` をサポート (#2969 by @eygraber)
-   [HSQL] `HSQL Window functionality` を追加
-   [SQLite] `WHERE` 句で `nullable parameters` の等価チェック (`equality checks`) を置き換えない (`Don't replace`) ように修正 (#1490 by @eygraber)
-   [SQLite] `Sqlite 3.35 returning statements` をサポート (#1490 by @eygraber)
-   [SQLite] `GENERATED` 句をサポート
-   [SQLite] `Sqlite 3.38 dialect` のサポートを追加 (by @eygraber)

### 変更
-   [コンパイラ] 生成されたコード (`generated code`) を少しクリーンアップ (`Clean up`)
-   [コンパイラ] グループ化されたステートメント (`grouped statements`) でのテーブルパラメータ (`table parameters`) の使用を禁止 (`Forbid usage`) (#1822)
-   [コンパイラ] グループ化されたクエリ (`grouped queries`) をトランザクション内 (`inside a transaction`) に入れるように変更 (#2785)
-   [ランタイム] ドライバーの `execute` メソッドから更新された行数 (`updated row count`) を返すように変更
-   [ランタイム] `SqlCursor` を接続にアクセスするクリティカルセクション (`critical section accessing the connection`) に限定 (`Confine`) するように変更 (#2123 by @andersio)
-   [Gradle プラグイン] マイグレーション (`migrations`) のスキーマ定義 (`schema definitions`) を比較 (`Compare`) するように変更 (#841)
-   [PostgreSQL] `PG` での二重引用符 (`double quotes`) を禁止 (`Disallow`) するように変更
-   [MySQL] `MySQL` での `==` の使用時にエラーを出す (`Error on usage of ==`) ように変更 (#2673)

### 修正
-   [コンパイラ] `2.0 alpha` で異なるテーブルからの同じアダプター型 (`Same adapter type from different tables`) がコンパイルエラー (`compilation error`) を引き起こす問題を修正
-   [コンパイラ] `upsert` ステートメント (`upsert statement`) のコンパイル問題 (`Problem compiling`) (#2791)
-   [コンパイラ] 複数の一致 (`multiple matches`) がある場合、クエリ結果 (`Query result`) が `SELECT` 内のテーブル (`tables in the select`) を使用するように修正 (#1874, #2313)
-   [コンパイラ] `INSTEAD OF` トリガー (`trigger`) を持つビュー (`view`) の更新をサポート (#1018)
-   [コンパイラ] 関数名 (`function names`) における `from` と `for` のサポート
-   [コンパイラ] 関数式 (`function expressions`) における `SEPARATOR` キーワードを許可
-   [コンパイラ] `ORDER BY` でエイリアス化されたテーブル (`aliased table`) の `ROWID` にアクセスできない問題を修正
-   [コンパイラ] `MySQL` の `HAVING` 句でエイリアス化された列名 (`aliased column name`) が認識されない問題を修正
-   [コンパイラ] 誤った「`Multiple columns found`」エラーを修正
-   [コンパイラ] `PRAGMA locking_mode = EXCLUSIVE;` を設定できない問題を修正
...
### 追加
-   [IDE Plugin] 基本的な推奨マイグレーション (`Basic suggested migration`) (by @aperfilyev)
-   [IDE Plugin] インポートヒントアクション (`import hint action`) を追加 (by @aperfilyev)
-   [IDE Plugin] `kotlin class completion` を追加 (by @aperfilyev)
-   [Gradle プラグイン] `Gradle type safe project accessors` のショートカット (`shortcut`) を追加 (by @hfhbd)
-   [コンパイラ] ダイアレクト (`dialect`) に基づいてコード生成 (`codegen`) をカスタマイズ (`Customize`) (by @MariusVolkhart)
-   [JDBC ドライバー] `JdbcDriver` に共通型 (`common types`) を追加 (by @MariusVolkhart)
-   [SQLite] `sqlite 3.35` のサポートを追加 (by @eygraber)
-   [SQLite] `ALTER TABLE DROP COLUMN` のサポートを追加 (by @eygraber)
-   [SQLite] `Sqlite 3.30 dialect` のサポートを追加 (by @eygraber)
-   [SQLite] `sqlite` で `NULLS FIRST/LAST` をサポート (by @eygraber)
-   [HSQL] 生成された句 (`generated clause`) の `HSQL` サポートを追加 (by @MariusVolkhart)
-   [HSQL] `HSQL` での名前付きパラメータ (`named parameters`) のサポートを追加 (by @MariusVolkhart)
-   [HSQL] `HSQL insert query` をカスタマイズ (`Customize`) (by @MariusVolkhart)

### 変更
-   [全体] パッケージ名 (`Package name`) が `com.squareup.sqldelight` から `app.cash.sqldelight` に変更されました。
-   [ランタイム] ダイアレクト (`dialects`) を独自の隔離された `gradle modules` に移動 (`Move`)
-   [ランタイム] ドライバーによって実装されるクエリ通知 (`driver-implemented query notifications`) に切り替え (`Switch`)。
-   [ランタイム] デフォルトの列アダプター (`default column adapters`) を別のモジュール (`separate module`) に抽出 (`Extract`) (#2056, #2060)
-   [コンパイラ] 各モジュールで再実行 (`redoing`) する代わりに、モジュールがクエリ実装 (`queries implementations`) を生成 (`generate`) するように変更
-   [コンパイラ] 生成されたデータクラス (`generated data classes`) のカスタム `toString` 生成を削除 (by @PaulWoitaschek)
-   [JS ドライバー] `sql.js` 依存関係 (`dependency`) を `sqljs-driver` から削除 (by @dellisd)
-   [Paging] `android paging 2 extension` を削除
-   [IDE Plugin] `SQLDelight` が同期中 (`syncing`) にエディタバナー (`editor banner`) を追加 (#2511)
-   [IDE Plugin] サポートされる `IntelliJ` の最小バージョン (`Minimum supported IntelliJ version`) は `2021.1`

### 修正
-   [ランタイム] リスナリスト (`listener list`) をフラット化 (`Flatten`) し、割り当て (`allocations`) とポインタチェイス (`pointer chasing`) を削減 (by @andersio)
-   [IDE Plugin] エラーメッセージ (`error message`) を修正し、エラー箇所にジャンプ (`jumping to error`) できるように修正 (by @hfhbd)
...
-   [IDE Plugin] 古い `IntelliJ` バージョンとの互換性 (`compatibility`) を改善 (by @3flex)
-   [IDE Plugin] 高速なアノテーション `API` (`faster annotation API`) を使用
-   [Gradle プラグイン] ランタイム (`runtime`) 追加時に `js/android` プラグインを明示的にサポート (`Explicitly support`) (by @ZacSweers)
-   [Gradle プラグイン] マイグレーション (`migrations`) からスキーマ (`schemas`) を導出 (`derviving`) せずにマイグレーション出力タスク (`migration output task`) を登録 (`Register`) するように修正 (#2744 by @kevincianfarini)
-   [Gradle プラグイン] マイグレーションタスク (`migration task`) がクラッシュ (`crashes`) した場合、クラッシュしたファイル (`file it crashed running`) をログに出力 (`print the file it crashed running`) するように修正
-   [Gradle プラグイン] コード生成 (`generating code`) 時にファイルをソート (`Sort files`) し、出力が冪等 (`idempotent outputs`) であることを保証 (by @ZacSweers)
-   [コンパイラ] ファイルを反復処理 (`iterating files`) するための高速な `API` (`faster APIs`) を使用し、`PSI` グラフ全体 (`entire PSI graph`) を探索 (`explore`) しないように修正
-   [コンパイラ] `select function parameters` にキーワードマングリング (`keyword mangling`) を追加 (#2759 by @aperfilyev)
-   [コンパイラ] マイグレーションアダプター (`migration adapter`) の `packageName` を修正 (by @hfhbd)
-   [コンパイラ] 型 (`types`) の代わりにプロパティ (`properties`) にアノテーション (`annotations`) を出力 (`Emit`) するように修正 (#2798 by @aperfilyev)
-   [コンパイラ] `Query subtype` に渡す前に引数をソート (`Sort arguments`) するように修正 (#2379 by @aperfilyev)

## [1.5.3] - 2021-11-23
[1.5.3]: https://github.com/sqldelight/sqldelight/releases/tag/1.5.3

### 追加
-   [JDBC ドライバー] サードパーティドライバー実装 (`3rd party driver implementations`) のために `JdbcDriver` を公開 (`Open`) (#2672 by @hfhbd)
-   [MySQL ダイアレクト] 時間増分 (`time increments`) 用の不足している関数 (`missing functions`) を追加 (#2671 by @sdoward)
-   [コルーチン拡張] コルーチン拡張 (`coroutines-extensions`) に `M1` ターゲット (`M1 targets`) を追加 (by @PhilipDukhov)

### 変更
-   [Paging3 拡張] `sqldelight-android-paging3` を `AAR` ではなく `JAR` として配布 (`Distribute`) するように変更 (#2634 by @julioromano)
...
### 修正
-   [コンパイラ] 重複する配列パラメータ (`duplicate array parameters`) の変数 (`variables`) を抽出しない (`Don't extract`) ように修正 (by @aperfilyev)
...
### 追加
-   [Gradle プラグイン] `HMPP` サポート (#2548 by @martinbonnin)
-   [IDE Plugin] `NULL` 比較インスペクション (`NULL comparison inspection`) を追加 (by @aperfilyev)
-   [IDE Plugin] インスペクションサプレッサー (`inspection suppressor`) を追加 (#2519 by @aperfilyev)
-   [IDE Plugin] 混合された名前付き/位置パラメータインスペクション (`Mixed named and positional parameters inspection`) を追加 (by @aperfilyev)
-   [SQLite ドライバー] `mingwX86` ターゲットを追加 (#2558 by @enginegl)
-   [SQLite ドライバー] `M1` ターゲットを追加
-   [SQLite ドライバー] `linuxX64` サポートを追加 (#2456 by @chippmann)
-   [MySQL ダイアレクト] `MySQL` に `ROW_COUNT` 関数を追加 (#2523)
-   [PostgreSQL ダイアレクト] postgres の列名変更 (`rename`)、列削除 (`drop column`) をサポート (by @pabl0rg)
-   [PostgreSQL ダイアレクト] `PostgreSQL` 文法が `CITEXT` を認識しない問題を修正
-   [PostgreSQL ダイアレクト] `TIMESTAMP WITH TIME ZONE` と `TIMESTAMPTZ` を含める
-   [PostgreSQL ダイアレクト] `PostgreSQL GENERATED columns` の文法 (`grammar`) を追加
-   [ランタイム] `SqlDriver` を `AfterVersion` のパラメータとして提供 (#2534, 2614 by @ahmedre)

### 変更
-   [Gradle プラグイン] Gradle `7.0` を明示的に要求 (`explicitly require`) するように変更 (#2572 by @martinbonnin)
-   [Gradle プラグイン] `VerifyMigrationTask` が `Gradle` の最新チェック (`up-to-date checks`) をサポートするように変更 (#2533 by @3flex)
-   [IDE Plugin] `null` 許容型 (`nullable type`) と非 `null` 許容型 (`non-nullable type`) を結合する際に、「`Join compares two columns of different types`」という警告を出さないように修正 (#2550 by @pchmielowski)
-   [IDE Plugin] 列型 (`column type`) の小文字の「`as`」に対するエラーを明確化 (`Clarify the error`) (by @aperfilyev)
...
-   [IDE Plugin] `JavaTypeMixin` の `npe` を修正 (#2603 by @aperfilyev)
-   [IDE Plugin] `MismatchJoinColumnInspection` の `IndexOutOfBoundsException` を修正 (#2602 by @aperfilyev)
-   [IDE Plugin] `UnusedColumnInspection` の説明 (`description`) を追加 (#2600 by @aperfilyev)
-   [IDE Plugin] `PsiElement.generatedVirtualFiles` を読み取りアクション (`read action`) でラップ (`Wrap`) するように修正 (#2599 by @aperfilyev)
-   [IDE Plugin] 不要な `nonnull` キャスト (`cast`) を削除 (#2596)
-   [IDE Plugin] 使用箇所の検索 (`find usages`) で `null` を適切に処理するように修正 (#2595)
-   [IDE Plugin] `Android` 向けの生成されたファイル (`generated files`) の `IDE` 自動補完 (`autocomplete`) を修正 (#2573 by @martinbonnin)
-   [IDE Plugin] `SqlDelightGotoDeclarationHandler` の `npe` を修正 (by @aperfilyev)
-   [IDE Plugin] `INSERT` ステートメント内の引数で `Kotlin` キーワードをマングリング (`Mangle kotlin keywords`) するように修正 (#2433 by @aperfilyev)
-   [IDE Plugin] `SqlDelightFoldingBuilder` の `npe` を修正 (#2382 by @aperfilyev)
-   [IDE Plugin] `CopyPasteProcessor` での `ClassCastException` をキャッチ (`Catch`) するように修正 (#2369 by @aperfilyev)
-   [IDE Plugin] ライブテンプレート (`live template`) の更新を修正 (by @IliasRedissi)
-   [IDE Plugin] インテンションアクション (`intention actions`) に説明 (`descriptions`) を追加 (#2489 by @aperfilyev)
-   [IDE Plugin] テーブルが見つからない場合 (`table is not found`) の `CreateTriggerMixin` での例外 (`exception`) を修正 (by @aperfilyev)
-   [コンパイラ] テーブル作成ステートメント (`table creation statemenets`) をトポロジカルにソート (`Topologically sort`)
-   [コンパイラ] ディレクトリ (`directories`) に対する `forDatabaseFiles` コールバックの呼び出しを停止 (`Stop invoking`) するように修正 (#2532)
-   [Gradle プラグイン] `generateDatabaseInterface task` の依存関係 (`dependency`) を潜在的なコンシューマ (`potential consumers`) に伝播 (`Propagate`) するように修正 (#2518 by @martinbonnin)

## [1.5.1] - 2021-07-16
[1.5.1]: https://github.com/sqldelight/sqldelight/releases/tag/1.5.1

### 追加
-   [PostgreSQL ダイアレクト] PostgreSQL `JSONB` と `ON Conflict Do Nothing` をサポート (by @satook)
-   [PostgreSQL ダイアレクト] `PostgreSQL ON CONFLICT (column, ...) DO UPDATE` のサポートを追加 (by @satook)
-   [MySQL ダイアレクト] `MySQL generated columns` をサポート (by @JGulbronson)
-   [ネイティブドライバー] `watchosX64` サポートを追加
-   [IDE Plugin] パラメータ型 (`parameter types`) とアノテーション (`annotations`) を追加 (by @aperfilyev)
-   [IDE Plugin] 「`select all`」クエリ生成アクション (`query generation action`) を追加 (by @aperfilyev)
-   [IDE Plugin] 自動補完 (`autocomplete`) に列型 (`column types`) を表示 (by @aperfilyev)
-   [IDE Plugin] 自動補完 (`autocomplete`) にアイコン (`icons`) を追加 (by @aperfilyev)
-   [IDE Plugin] 主キー (`primary key`) による「`select by primary key`」クエリ生成アクション (`query generation action`) を追加 (by @aperfilyev)
-   [IDE Plugin] 「`insert into`」クエリ生成アクション (`query generation action`) を追加 (by @aperfilyev)
-   [IDE Plugin] 列名 (`column names`)、ステートメント識別子 (`stmt identifiers`)、関数名 (`function names`) のハイライト (`highlighting`) を追加 (by @aperfilyev)
-   [IDE Plugin] 残りのクエリ生成アクション (`remaining query generation actions`) を追加 (#489 by @aperfilyev)
-   [IDE Plugin] `insert-stmt` からパラメータヒント (`parameter hints`) を表示 (by @aperfilyev)
-   [IDE Plugin] テーブルエイリアスインテンションアクション (`Table alias intention action`) を追加 (by @aperfilyev)
-   [IDE Plugin] 列名修飾インテンション (`Qualify column name intention`) を追加 (by @aperfilyev)
-   [IDE Plugin] `kotlin property` の宣言へ移動 (`Go to declaration`) を追加 (by @aperfilyev)

### 変更
-   [ネイティブドライバー] フリーズ (`freezing`) と共有可能データ構造 (`shareable data structures`) を可能な限り回避 (`avoiding`) することで、ネイティブトランザクション (`native transaction`) のパフォーマンス (`performance`) を改善 (by @andersio)
-   [Paging 3] Paging3 バージョンを `3.0.0 stable` にバンプ (`Bump`)
-   [JS ドライバー] `sql.js` を `1.5.0` にアップグレード

### 修正
-   [JDBC SQLite ドライバー] `ThreadLocal` をクリアする前に接続 (`connection`) で `close()` を呼び出す (#2444 by @hannesstruss)
-   [RX 拡張] サブスクリプション/破棄競合リーク (`subscription / disposal race leak`) を修正 (#2403 by @pyricau)
-   [コルーチン拡張] 通知 (`notifying`) する前にクエリリスナー (`query listener`) を登録 (`register`) することを保証
-   [コンパイラ] 一貫した `kotlin output file` を持つよう `notifyQueries` をソート (`Sort notifyQueries`) (by @thomascjy)
-   [コンパイラ] `select query class properties` に `@JvmField` アノテーションを付けない (`Don't annotate`) (by @eygraber)
-   [IDE Plugin] インポートオプティマイザ (`import optimizer`) を修正 (#2350 by @aperfilyev)
-   [IDE Plugin] 未使用列の検査 (`unused column inspection`) を修正 (by @aperfilyev)
-   [IDE Plugin] インポート検査 (`import inspection`) とクラスアノテーター (`class annotator`) にネストされたクラス (`nested classes`) のサポートを追加 (by @aperfilyev)
-   [IDE Plugin] `CopyPasteProcessor` の `npe` を修正 (#2363 by @aperfilyev)
-   [IDE Plugin] `InlayParameterHintsProvider` でのクラッシュ (`crash`) を修正 (#2359 by @aperfilyev)
-   [IDE Plugin] `create table stmt` に任意のテキスト (`any text`) をコピーペースト (`copy-pasting`) する際の空白行 (`blank lines`) の挿入 (`insertion`) を修正 (#2431 by @aperfilyev)

## [1.5.0] - 2021-04-23
[1.5.0]: https://github.com/sqldelight/sqldelight/releases/tag/1.5.0

### 追加
-   [SQLite Javascript ドライバー] `sqljs-driver publication` を有効化 (#1667 by @dellisd)
-   [Paging3 拡張] `Android Paging 3 Library` の拡張 (#1786 by @kevincianfarini)
-   [MySQL ダイアレクト] `mysql's ON DUPLICATE KEY UPDATE` 競合解決 (`conflict resolution`) のサポートを追加 (by @rharter)
-   [SQLite ダイアレクト] `SQLite offsets()` のコンパイラサポート (`compiler support`) を追加 (by @qjroberts)
-   [IDE Plugin] 未知の型 (`unknown type`) のインポートクイックフィックス (`import quick fix`) を追加 (#683 by @aperfilyev)
-   [IDE Plugin] 未使用インポートの検査 (`unused import inspection`) を追加 (#1161 by @aperfilyev)
-   [IDE Plugin] 未使用クエリの検査 (`unused query inspection`) を追加 (by @aperfilyev)
-   [IDE Plugin] 未使用列の検査 (`unused column inspection`) を追加 (#569 by @aperfilyev)
-   [IDE Plugin] コピー/ペースト (`copy/paste`) 時にインポートを自動的に追加 (`Automatically bring imports`) (#684 by @aperfilyev)
-   [IDE Plugin] `gradle/intellij plugin versions` 間に非互換性 (`incompatibilities`) がある場合にバルーン (`balloon`) を表示
-   [IDE Plugin] `Insert Into ... VALUES(?)` パラメータヒント (`parameter hints`) を追加 (#506 by @aperfilyev)
-   [IDE Plugin] インラインパラメータヒント (`Inline parameter hints`) を追加 (by @aperfilyev)
-   [ランタイム] コールバック (`callbacks`) 付きでマイグレーションを実行 (`running migrations`) するためのランタイム `API` (`API in the runtime`) を含む (#1844)

### 変更
-   [コンパイラ] 「`IS NOT NULL`」クエリ (`queries`) をスマートキャスト (`Smart cast`) (#867)
-   [コンパイラ] 実行時に失敗する可能性のあるキーワード (`keywords that will fail at runtime`) から保護 (`Protect against`) (#1471, #1629)
-   [Gradle プラグイン] `gradle plugin` のサイズを `60MB` から `13MB` に削減。
-   [Gradle プラグイン] `android variants` を適切にサポートし、`KMM target-specific sql` のサポートを削除 (#1039)
-   [Gradle プラグイン] `minsdk` に基づいて最小の `sqlite` バージョンを選択 (`Pick a minimum sqlite version based on minsdk`) (#1684)
-   [ネイティブドライバー] ネイティブドライバ接続プール (`connection pool`) とパフォーマンス (`performance`) の更新

### 修正
-   [コンパイラ] ラムダ (`lambdas`) の前の `NBSP` を修正 (by @oldergod)
-   [コンパイラ] 生成された `bind*` および `cursor.get*` ステートメント (`statements`) での互換性のない型 (`incompatible types`) を修正
-   [コンパイラ] `SQL clause` はアダプトされた型 (`adapted type`) を保持 (`persist`) すべき (#2067)
-   [コンパイラ] `NULL` キーワードのみの列 (`Column with only NULL keyword`) は `null` 許容であるべき
-   [コンパイラ] 型アノテーション (`type annotations`) 付きマッパーラムダ (`mapper lambda`) を生成しない (`Dont generate`) (#1957)
-   [コンパイラ] カスタムクエリ (`custom queries`) が衝突 (`clash`) する場合、ファイル名 (`file name`) を追加のパッケージサフィックス (`additional package suffix`) として使用 (#1057, #1278)
-   [コンパイラ] 外部キーカスケード (`foreign key cascades`) によってクエリリスナー (`query listeners`) が通知 (`notified`) されることを保証 (#1325, #1485)
-   [コンパイラ] 同じ型の `2` つを結合 (`unioning two of the same type`) する場合、テーブル型 (`table type`) を返す (#1342)
-   [コンパイラ] `ifnull` と `coalesce` のパラメータ (`params`) は `null` 許容にできる (`can be nullable`) ことを保証 (#1263)
-   [コンパイラ] クエリによって課せられる式 (`expressions`) の `null` 許容性 (`nullability`) を正しく使用
-   [MySQL ダイアレクト] `MySQL if statements` をサポート
-   [PostgreSQL ダイアレクト] `PostgreSQL` で `NUMERIC` と `DECIMAL` を `Double` として取得 (#2118)
-   [SQLite ダイアレクト] `UPSERT` 通知 (`notifications`) は `BEFORE/AFTER UPDATE` トリガー (`triggers`) を考慮すべき (#2198 by @andersio)
-   [SQLite ドライバー] インメモリでない限り (`unless we are in memory`)、`SqliteDriver` でスレッド (`threads`) に複数の接続 (`multiple connections`) を使用 (#1832)
-   [JDBC ドライバー] `JDBC Driver` は `autoCommit` が `true` であると仮定 (`assumes autoCommit is true`) (#2041)
-   [JDBC ドライバー] 例外発生時 (`on exception`) に接続 (`connections`) を閉じる (`close`) ことを保証 (#2306)
-   [IDE Plugin] パスセパレータ (`path separator`) のバグにより `Windows` で `GoToDeclaration/FindUsages` が破損する (`broken`) のを修正 (#2054 by @angusholder)
...
-   [IDE Plugin] 「ワイルドカード展開インテンション (`expand * intention`)」が追加のプロジェクション (`additional projections`) で機能するように修正 (#2173 by @aperfilyev)
...
-   [Gradle プラグイン] `WorkerApi` を使用したデータベース生成 (`generating a database`) での競合状態 (`Race condition`) (#2062 by @stephanenicolas)
-   [Gradle プラグイン] `classLoaderIsolation` がカスタム `jdbc` の使用 (`custom jdbc usage`) を妨げる (`prevents`) (#2048 by @benasher44)
-   [Gradle プラグイン] 欠落している `packageName` エラーメッセージ (`error message`) を改善 (by @vanniktech)
-   [Gradle プラグイン] `SQLDelight` が `IntelliJ` の依存関係 (`dependencies`) をビルドスクリプトクラスパス (`buildscript class path`) に漏らす (`bleeds`) (#1998)
-   [Gradle プラグイン] `Gradle build caching` を修正 (#2075)
-   [Gradle プラグイン] `Gradle plugin` で `kotlin-native-utils` に依存しない (`Do not depend on`) (by @ilmat192)
-   [Gradle プラグイン] マイグレーションファイル (`migration files`) のみの場合もデータベース (`database`) を書き込む (`write`) (#2094)
-   [Gradle プラグイン] ダイヤモンド依存関係 (`diamond dependencies`) が最終コンパイルユニット (`final compilation unit`) で一度だけ選択 (`picked up once`) されることを保証 (#1455)

また、今回のリリースで `SQLDelight` インフラ (`infrastructure`) を改善するために多大な貢献をした @3flex 氏に心から感謝します。

## [1.4.4] - 2020-10-08
[1.4.4]: https://github.com/sqldelight/sqldelight/releases/tag/1.4.4

### 追加
-   [PostgreSQL ダイアレクト] `WITH` 句内のデータ変更ステートメント (`data-modifying statements`) をサポート
-   [PostgreSQL ダイアレクト] `substring` 関数をサポート
-   [Gradle プラグイン] `SQLDelight` コンパイル中 (`compilation`) にマイグレーション (`migrations`) を検証 (`validating`) するための `verifyMigrations` フラグ (`flag`) を追加 (#1872)

### 変更
-   [コンパイラ] 非 `SQLite` ダイアレクト (`non-SQLite dialects`) で `SQLite` 固有の関数 (`SQLite specific functions`) を不明 (`unknown`) としてフラグ付け (`Flag`)
-   [Gradle プラグイン] `sqldelight plugin` が適用 (`applied`) されているがデータベースが設定されていない (`no databases are configured`) 場合に警告 (`warning`) を表示 (#1421)

### 修正
-   [コンパイラ] `ORDER BY` 句で列名 (`column name`) をバインド (`binding`) する際にエラーを報告 (`Report an error`) (#1187 by @eygraber)
-   [コンパイラ] `db interface` 生成時にレジストリ警告 (`Registry warnings`) が表示される (#1792)
-   [コンパイラ] `CASE` ステートメント (`case statement`) の誤った型推論 (`Incorrect type inference`) (#1811)
-   [コンパイラ] バージョンがないマイグレーションファイル (`migration files with no version`) に対してより良いエラー (`better errors`) を提供 (#2006)
-   [コンパイラ] `ColumnAdapter` の一部のデータベース型 (`database type`) をマーシャル (`marshal`) するために必要なデータベース型が不正 (#2012)
-   [コンパイラ] `CAST` の `Null` 許容性 (`Nullability`) (#1261)
-   [コンパイラ] クエリラッパー (`query wrappers`) で多数の名前シャドウ警告 (`name shadowed warnings`) (#1946 by @eygraber)
-   [コンパイラ] 生成されたコード (`Generated code`) が完全修飾名 (`full qualifier names`) を使用している (#1939)
...
-   [Gradle プラグイン] `SQLDelight` が `IntelliJ` の依存関係 (`dependencies`) をビルドスクリプトクラスパス (`buildscript class path`) に漏らす (`bleeds`) (#1998)
-   [Gradle プラグイン] `*.sq` ファイルにメソッドドキュメント (`method doc`) を追加する際に「`JavadocIntegrationKt not found`」コンパイルエラー (`compilation error`) (#1982)
-   [Gradle プラグイン] `SqlDeslight gradle plugin` は `Configuration Caching (CoCa)` をサポートしていない (`doesn't support`) (#1947 by @stephanenicolas)
-   [SQLite JDBC ドライバー] `SQLException`: `database in auto-commit mode` (#1832)
-   [コルーチン拡張] `coroutines-extensions` の `IR backend` を修正 (#1918 by @dellisd)

## [1.4.3] - 2020-09-04
[1.4.3]: https://github.com/sqldelight/sqldelight/releases/tag/1.4.3

### 追加
-   [MySQL ダイアレクト] `MySQL last_insert_id function` のサポートを追加 (by @lawkai)
-   [PostgreSQL ダイアレクト] `SERIAL` データ型 (`data type`) をサポート (by @veyndan & @felipecsl)
-   [PostgreSQL ダイアレクト] `PostgreSQL RETURNING` をサポート (by @veyndan)

### 修正
-   [MySQL ダイアレクト] `MySQL AUTO_INCREMENT` をデフォルト値 (`default value`) を持つものとして扱う (#1823)
-   [コンパイラ] `Upsert statement compiler error` を修正 (#1809 by @eygraber)
-   [コンパイラ] 無効な `Kotlin` が生成される (`invalid Kotlin being generated`) 問題を修正 (#1925 by @eygraber)
-   [コンパイラ] 未知の関数 (`unknown functions`) に対してより良いエラーメッセージ (`better error message`) を提供 (#1843)
-   [コンパイラ] `instr` の `2` 番目のパラメータの型 (`type for the second parameter`) として `string` を公開
...
### 追加
-   [ランタイム] 新しい `JS IR backend` をサポート
-   [Gradle プラグイン] `generateSqlDelightInterface Gradle task` を追加 (by @vanniktech)
-   [Gradle プラグイン] `verifySqlDelightMigration Gradle task` を追加 (by @vanniktech)
...
### 変更
-   [Gradle プラグイン] `AGP` 依存関係 (`dependency`) を `compileOnly` に (#1362)

### 修正
-   [コンパイラ] 列定義ルール (`column defintion rule`) とテーブルインターフェースジェネレータ (`table interface generator`) にオプションの `javadoc` を追加 (#1224 by @endanke)
-   [SQLite ダイアレクト] `sqlite fts5 auxiliary functions highlight, snippet, and bm25` のサポートを追加 (by @drampelt)
-   [MySQL ダイアレクト] `MySQL bit data type` をサポート
-   [MySQL ダイアレクト] `MySQL binary literals` をサポート
-   [PostgreSQL ダイアレクト] `sql-psi` から `SERIAL` を公開 (by @veyndan)
-   [PostgreSQL ダイアレクト] `BOOLEAN data type` を追加 (by @veyndan)
-   [PostgreSQL ダイアレクト] `NULL column constraint` を追加 (by @veyndan)
...
### 追加
-   [MySQL ダイアレクト] MySQL サポート (by @JGulbronson & @veyndan)
-   [PostgreSQL ダイアレクト] 実験的な `PostgreSQL` サポート (by @veyndan)
-   [HSQL ダイアレクト] 実験的な `H2` サポート (by @MariusVolkhart)
-   [SQLite ダイアレクト] `SQLite FTS5` サポート (by @benasher44 & @jpalawaga)
-   [SQLite ダイアレクト] `alter table rename column` のサポート (#1505 by @angusholder)
-   [IDE] マイグレーション (`.sqm`) ファイルの `IDE` サポート
-   [IDE] 組み込みの `SQL Live Templates` を模倣した `SQLDelight Live Templates` を追加 (#1154 by @veyndan)
-   [IDE] 新しい `SqlDelight file action` を追加 (#42 by @romtsn)
-   [ランタイム] 結果を返すトランザクション (`transactions that return results`) 用の `transactionWithReturn API`
-   [コンパイラ] `.sq` ファイルで複数の `SQL` ステートメント (`SQL statements`) をグループ化 (`grouping`) する構文
-   [コンパイラ] マイグレーションファイル (`migration files`) からスキーマ (`schemas`) を生成 (`generating`) するサポート
-   [Gradle プラグイン] マイグレーションファイル (`migration files`) を有効な `sql` として出力 (`outputting`) するタスクを追加

### 変更
-   [ドキュメント] ドキュメントウェブサイト (`documentation website`) の大幅な刷新 (`Overhaul`) (by @saket)
-   [Gradle プラグイン] サポートされていないダイアレクト (`unsupported dialect`) のエラーメッセージ (`error message`) を改善 (by @veyndan)
-   [IDE] ダイアレクト (`dialect`) に基づいてファイルアイコン (`file icon`) を動的に変更 (by @veyndan)
-   [JDBC ドライバー] `javax.sql.DataSource` から `JdbcDriver` コンストラクタ (`constructor`) を公開 (#1614)

### 修正
-   [コンパイラ] テーブルの `Javadoc` をサポートし、`1` ファイル内の複数の `Javadoc` を修正 (#1224)
-   [コンパイラ] 合成列 (`synthesized columns`) に値 (`value`) を挿入 (`inserting`) できるようにする (#1351)
-   [コンパイラ] ディレクトリ名 (`directory name`) のサニタイズ (`sanitizing`) における不整合 (`inconsistency`) を修正 (by @ZacSweers)
-   [コンパイラ] 合成列 (`Synthesized columns`) は結合全体 (`across joins`) で `null` 許容性 (`nullability`) を保持 (`retain`) すべき (#1656)
-   [コンパイラ] `DELETE` キーワードに `DELETE` ステートメント (`statement`) を固定 (`Pin`) (#1643)
-   [コンパイラ] クォーティング (`quoting`) を修正 (#1525 by @angusholder)
-   [コンパイラ] `BETWEEN` 演算子 (`operator`) が式 (`expressions`) に適切に再帰 (`properly recurse`) するように修正 (#1279)
-   [コンパイラ] インデックス作成時 (`creating an index`) にテーブル/列が欠落している場合 (`missing table/column`)、より良いエラー (`better error`) を提供 (#1372)
-   [コンパイラ] 結合制約 (`join constraints`) で外部クエリ (`outer querys`) のプロジェクション (`projection`) を使用できるようにする (#1346)
-   [ネイティブドライバー] `execute` で `transactionPool` を使用するようにする (by @benasher44)
-   [JDBC ドライバー] `SQLite` の代わりに `jdbc transaction APIs` を使用する (#1693)
...
-   [ページング拡張] リークする `DataSource` (`leaky DataSource`) を修正 (#1628)
-   [Gradle プラグイン] スキーマ生成時 (`generating a schema`)、出力 `db file` (`output db file`) がすでに存在する場合、それを削除 (`delete`) する (#1645)
-   [Gradle プラグイン] ギャップ (`gaps`) がある場合、マイグレーション検証 (`migration validation`) を失敗させる
-   [Gradle プラグイン] 設定したファイルインデックス (`file index`) を明示的に使用 (`Explicitly use`) (#1644)
...
*   新規: [コンパイラ] `#1009 mysql` ダイアレクトの実験的サポート (`Experimental support`)。
*   新規: [コンパイラ] `#1436 sqlite:3.24` ダイアレクトと `upsert` のサポート。
*   新規: [JDBC ドライバー] `sqlite jvm driver` から `JDBC` ドライバー (`JDBC driver`) を分割 (`Split out`)。
*   修正: [コンパイラ] `#1199` あらゆる長さのラムダ (`lambdas of any length`) をサポート。
*   修正: [コンパイラ] `#1610 avg()` の戻り値の型 (`return type`) を `null` 許容に修正。
...
*   新規: [ランタイム] `Windows (mingW)`、`tvOS`、`watchOS`、および `macOS` アーキテクチャ (`architectures`) のサポート。
*   修正: [コンパイラ] `sum()` の戻り値の型 (`Return type`) は `null` 許容であるべき。
*   修正: [Paging] 競合状態 (`race conditions`) を避けるため、`Transacter` を `QueryDataSourceFactory` に渡す。
*   修正: [IntelliJ プラグイン] ファイルのパッケージ名 (`package name`) を検索 (`looking for a file's package name`) する際に依存関係 (`dependencies`) を検索しない。
...
*   修正: [コンパイラ] `#1469 Query` の `SynetheticAccessor` 作成。
*   修正: [JVM ドライバー] メモリリーク (`memory leak`) を修正。
...
*   修正: [コンパイル] `#1405 query` が `FTS table` を結合 (`joined`) している場合、プロジェクト (`project`) をビルドできない。
...
*   修正: [コンパイラ] 各クエリ (`query`) に対して一意の `ID` が決定論的に生成される (`deterministically generated`)。
*   修正: [コンパイラ] トランザクション (`transaction`) が完了 (`complete`) したときにのみリスニングクエリ (`listening queries`) に通知 (`notify`) する。
*   修正: [JVM ドライバー] `#1370 JdbcSqliteDriver` ユーザーに `DB URL` の提供 (`supply a DB URL`) を強制する。
...
*   新規: [ランタイム] `#1267 Logging driver decorator` を追加。
*   修正: [コンパイラ] `#1254` `2^16` 文字より長い文字列リテラル (`string literals`) を分割 (`Split`)。
...
*   修正: [コンパイラ] 配列型 (`Array types`) が適切に出力される。

## [1.1.0] - 2019-02-27
[1.1.0]: https://github.com/sqldelight/sqldelight/releases/tag/1.1.0

*   新規: [Gradle] `#502 schema module dependencies` の指定を許可。
*   改善: [コンパイラ] `#1111` テーブルエラー (`Table errors`) が他のエラー (`other errors`) より前にソート (`sorted`) される。
*   修正: [コンパイラ] `#1225 REAL literals` の正しい型 (`correct type`) を返す。
*   修正: [コンパイラ] `#1218 docid` がトリガー (`triggers`) を通じて伝播 (`propagates`) する。

## [1.0.3] - 2019-01-30
[1.0.3]: https://github.com/sqldelight/sqldelight/releases/tag/1.0.3

*   改善: [ランタイム] `#1195 Native Driver/Runtime Arm32` を改善。
*   改善: [ランタイム] `#1190 Query` 型からマッパー (`mapper`) を公開 (`Expose`) する。

## [1.0.2] - 2019-01-26
[1.0.2]: https://github.com/sqldelight/sqldelight/releases/tag/1.0.2

*   修正: [Gradle プラグイン] `kotlin 1.3.20` にアップデート。
*   修正: [ランタイム] トランザクション (`Transactions`) が例外 (`exceptions`) を飲み込まない (`swallow`) ように。

## [1.0.1] - 2019-01-21
[1.0.1]: https://github.com/sqldelight/sqldelight/releases/tag/1.0.1

*   改善: [ネイティブドライバー] `DatabaseConfiguration` にディレクトリ名 (`directory name`) を渡すことを許可。
*   改善: [コンパイラ] `#1173` パッケージのないファイル (`Files without a package`) はコンパイルに失敗 (`fail compilation`) する。
...
*   修正: [コンパイラ] `#1167 UNION` と `SELECT` を含む複雑な `SQL` ステートメント (`complex SQL statements`) を解析 (`parse`) しようとすると例外 (`exception`) をスロー (`Throws`) する。
...
*   新規: Select クエリ (`Select queries`) は、文字列定数 (`string constants`) ではなく `SqlDelightStatement` ファクトリ (`factory`) として公開 (`exposed`) されるようになりました。
