...
ルート公開物は、すべてのターゲット固有の公開物を参照するエントリポイントとして機能します。これにはメタデータアーティファクトが含まれており、個別のプラットフォームアーティファクトの期待されるURLや座標（coordinates）など、他の公開物への参照を含めることで、適切な依存関係の解決を保証します。
