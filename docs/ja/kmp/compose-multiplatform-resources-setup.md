...
スクリプト（書字系）やその他の BCP 47 サブタグを指定するには、`b+` プレフィックスを使用し、その後に続くサブタグのハイフン（`-`）をすべて `+` 記号に置き換えます。例：`values-b+zh+Hans`、`values-b+zh+Hant`。
...
この曖昧さを避けるために、スクリプト固有のリソースを持つすべての言語に対して、スクリプトなしのディレクトリを追加してください。

```
commonMain/composeResources/
├── values/                 # デフォルトリソース
├── values-zh/              # スクリプトなしの `zh` 用リソース
├── values-b+zh+Hans/
└── values-b+zh+Hant/
```
