...
`dependsOn`関係は、ソースセット階層として知られるツリーのような構造を作成します。以下は、`android`、`iosArm64`（iPhoneデバイス）、および`iosSimulatorArm64`（Apple Silicon Mac用のiPhoneシミュレーター）を含むモバイル開発の典型的なプロジェクトの例です。
...
2.  上記の_ソースセット → マルチプラットフォームライブラリ_の依存関係、例えば`commonMain`から`org.jetbrians.kotlinx:kotlinx-coroutines-core:1.7.3`のようなものは、依存関係解決の中間状態を表します。解決の最終状態は常に_ソースセット → ソースセット_の依存関係で表されます。
