...
* `main` ブランチで CocoaPods を、`spm-import` ブランチで SwiftPM を使用している以下のサンプルプロジェクトを確認してください：
  * [Firebase サンプル](https://github.com/Kotlin/kmp-with-cocoapods-multitarget-xcode-sample)
...
> [用意されているスキル](https://github.com/Kotlin/kmp-cocoapods-to-spm-migration/blob/master/SKILL.md)を使用して、CocoaPods から SwiftPM への移行をお好みの AI エージェントに任せることができます。
...
`build.gradle.kts` ファイルを更新するには、SwiftPM インポートのページの指示に従ってください：

1. [Kotlin Multiplatform Gradle プラグインのバージョンを **%spmImport%** に変更する](multiplatform-spm-import.md#set-up-environment)
2. [CocoaPods プラグインを無効にしたり CocoaPods 依存関係を削除したりせずに、必要な SwiftPM 依存関係を追加する](multiplatform-spm-import.md#add-and-call-swiftpm-dependencies)

例えば、`FirebaseAnalytics` pod を使用している場合：

1. Kotlin Multiplatform Gradle プラグインをバージョン **%spmImport%** を使用するように[設定](multiplatform-spm-import.md#set-up-environment)していることを確認してください。
