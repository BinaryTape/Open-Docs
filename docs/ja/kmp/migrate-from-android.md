...
Googleは長期間にわたりComposeを進化させ、充実させてきました。一般的なシナリオについては[Jetpack Compose移行ガイド](https://developer.android.com/develop/ui/compose/migrate)を確認してください。
...
同様に、`:core:domain-testing` モジュールも簡単にマルチプラットフォームに更新できます。
...
Jetcasterの例では、ステップバイステップで追いやすい、より明確な変更の順序を選択しました。
