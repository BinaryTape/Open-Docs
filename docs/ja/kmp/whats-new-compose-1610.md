...
## 破壊的変更：新しい Compose コンパイラ Gradle プラグイン {id="breaking-change-new-compose-compiler-gradle-plugin"}
...
#### マルチプラットフォームリソース用の設定 DSL {id="configuration-dsl-for-multiplatform-resources"}
...
#### リソース URI 生成用の公開関数 {id="public-function-for-generating-resource-uri"}
...
#### 文字列リソースの複数形対応 {id="plurals-for-string-resources"}
...
#### 3文字のロケールのサポート {id="support-three-letter-locales"}
...
### BasicTextField2 の基本サポート {id="basic-support-of-basictextfield2"}
...
### DialogWindow 用の alwaysOnTop フラグ {id="alwaysontop-flag-for-dialogwindow"}
...
### iOS 17 以上での選択コンテナの拡大鏡 {id="selection-container-magnifier-for-ios-17-and-higher"}
...
### ダイアログ中央配置のためのソフトウェアキーボードインセット {id="software-keyboard-inset-for-dialog-centering"}
...
### Kotlin/Wasm サポートが Alpha に {id="kotlin-wasm-support-in-alpha"}
...
### macOS 最小バージョンの変更が可能に {id="possibility-to-modify-the-macos-minimum-version"}
...
### 既知の問題：MissingResourceException {id="known-issue-missingresourceexception"}
