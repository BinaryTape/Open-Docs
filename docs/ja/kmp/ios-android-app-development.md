...
クロスプラットフォーム開発は、もはや妥協ではなく戦略的な選択です。今日のクロスプラットフォーム技術を掘り下げる前に、なぜそれらが[iOSとAndroidの両方向けに構築するチーム](build-ios-android-app.topic)にとって大きな転換点（ゲームチェンジャー）となったのかを振り返ってみましょう。

[![Kotlin Multiplatformを見つける](discover-kmp.svg){width="700" style="block"}](https://www.jetbrains.com/kotlin-multiplatform/)

## iOSとAndroidを個別に開発する際にチームが直面する11の苦痛 {id="11-pains-teams-face-when-developing-for-ios-and-android-separately"}
...
## 救世主としてのクロスプラットフォーム開発 {id="cross-platform-development-to-the-rescue"}

クロスプラットフォームのモバイル開発は、プラットフォーム間でコードを共有することにより、iOSおよびAndroidアプリでの重複作業を削減します。アプローチによって、トレードオフ、柔軟性、およびネイティブ統合の度合いが異なります。詳細な導入については、[クロスプラットフォームモバイル開発とは何か](cross-platform-mobile-development.topic)の概要をご覧ください。

### Webベースおよびハイブリッドソリューション {id="web-based-and-hybrid-solutions"}
...
### クロスプラットフォームフレームワーク {id="cross-platform-frameworks"}
...
トレードオフは、ネイティブプラットフォームの上にある追加の抽象化レイヤーです。OSのバージョンが進化するにつれて、このレイヤーは新たな失敗点、ライブラリ品質のばらつき、およびネイティブAPIやパフォーマンスが重要な機能を統合する際の余分な複雑さを導入する可能性があります。広く使用されているオプションの詳細については、[最も人気のあるクロスプラットフォームアプリ開発フレームワーク](cross-platform-frameworks.topic)の概要をご覧ください。

### Kotlin Multiplatform：Compose MultiplatformによるコードとUIの共有 {id="kotlin-multiplatform-shared-code-and-uis-with-compose-multiplatform"}
...
### Kotlin Multiplatformが特に役立つシナリオ {id="scenarios-where-kotlin-multiplatform-is-particularly-helpful"}
...
## iOSおよびAndroidプロジェクトに適切なクロスプラットフォーム技術を選択する方法 {id="how-to-choose-the-right-cross-platform-technology-for-your-ios-and-android-project"}

### 主要な要件を特定する {id="identify-your-primary-requirements"}
...
### チームの現在の能力を考慮する {id="consider-your-team-s-current-competencies"}
...
### エコシステムを評価する {id="evaluate-the-ecosystem"}
...
これらの基準を評価することで、将来の開発の重みで停滞したり崩壊したりするソリューションを選択することを防げます。例えば、Flutter開発者は[pub.dev](http://pub.dev)の豊富なエコシステムの恩恵を受け、Kotlinチームは[klibs.io](http://klibs.io)の共有ライブラリを利用できます。
...
### ネイティブAPIとの相互作用を評価する {id="evaluate-interaction-with-native-apis"}
...
### パフォーマンスベンチマークを確認する {id="check-performance-benchmarks"}
...
### 総所有コストを評価する {id="assess-the-cost-of-ownership"}
...
### 実世界のケーススタディを確認する {id="review-real-world-case-studies"}
...
### 長寿を確保するために支援組織を考慮する {id="consider-the-supporting-organization-to-ensure-longevity"}
