...
`library`モジュールは、共有コードとプラットフォーム固有の実装を収容できるように構成されています。以下は、メインソースコード（`src`）の内容の内訳です。
...
* **`commonTest`, `androidUnitTest`, `iosTest`, `jvmTest`, `linuxX64Test`:** それぞれ、共有コードである`commonMain`のテスト、および各プラットフォーム固有のテストが含まれます。
...
[`expect`/`actual`メカニズム](multiplatform-connect-to-apis.md#expected-and-actual-functions-and-properties)を使用して、[Kotlin/Wasm](https://kotlinlang.org/docs/wasm-overview.html)プラットフォームのサポートを構成します。`firstElement`と`secondElement`プロパティに対してプラットフォーム固有の機能を実装できます。
...
5. `fibiprops.wasm.kt`ファイルに以下のコードを追加します。
    ...
    このコードは、`firstElement`を`3`、`secondElement`を`5`として`actual`値を定義し、Wasm固有の実装を設定します。
...
公開されたライブラリを確認するには、ファイルエクスプローラーまたはターミナルを使用して、ユーザーホームディレクトリ内の`.m2\repository\io\github\kotlin\library\1.0.0\`に移動します。

## 別のプロジェクトに依存関係としてライブラリを追加する

マルチプラットフォームライブラリをローカルのMavenリポジトリに公開した後、同じマシン上の他のKotlinプロジェクトで使用できます。

コンシューマープロジェクト（利用側プロジェクト）の`build.gradle.kts`ファイルで、公開されたライブラリへの依存関係を追加します。
...
`repositories{}`ブロックは、Gradleに対してローカルのMavenリポジトリからライブラリを解決し、共有コードで利用可能にするよう指示します。

`implementation`の依存関係は、ライブラリの`build.gradle.kts`ファイルで指定されたグループとバージョンで構成されます。
...
コンシューマープロジェクトを同期して、ライブラリの使用を開始しましょう！

## 次のステップ

マルチプラットフォーム開発をさらに深く探究することをお勧めします：

* [Maven Centralへのライブラリの公開](multiplatform-publish-libraries.md)
...
* ![YouTube](youtube.svg){width=25}{type="joined"} **Kotlin YouTube チャンネル**: 購読して [Kotlinマルチプラットフォームに関するビデオ](https://www.youtube.com/playlist?list=PLlFc5cFwUnmy_oVc9YQzjasSNoAk4hk_C)を視聴する