...
* マルチプラットフォームプロジェクトを手動で設定してiOSフレームワークを作成し、Xcodeプロジェクトでその最新バージョンを取得するようにします。
  この設定は、通常、Kotlin MultiplatformウィザードまたはAndroid Studio用Kotlin Multiplatformプラグインが行います。
  [フレームワークをiOSプロジェクトに接続する](multiplatform-integrate-in-existing-app.md#configure-the-ios-project-to-use-a-kmp-framework)を参照して、Xcodeでフレームワークを直接追加する方法を学んでください。
