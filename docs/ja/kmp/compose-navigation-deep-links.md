...
## Operating System でディープリンクスキーマを登録する {id="register-deep-links-schemas-in-the-operating-system"}
...
## デスティネーションにディープリンクを割り当てる {id="assign-deep-links-to-destinations"}
...
### 一般的なディープリンクの URI パターン {id="general-uri-patterns-for-deep-links"}
...
### ルート型に対して生成された URI パターン {id="generated-uri-pattern-for-a-route-type"}
...
### デスティネーションにディープリンクを追加する例 {id="example-of-adding-deep-links-to-a-destination"}
...
## 受信したディープリンクを処理する {id="handle-received-deep-links"}
...
### URI リスナーを持つシングルトンを宣言する {id="declare-a-singleton-with-a-uri-listener"}
...
### シングルトンへのプラットフォーム固有の呼び出しを実装する {id="implement-platform-specific-calls-to-the-singleton"}
...
### リスナーをセットアップする {id="set-up-the-listener"}
