...
## 明示的なコンポーザビリティを優先する {id="prefer-explicit-composability"}
...
## 拡張関数とプロパティを使用する {id="use-extension-functions-and-properties"}
...
## 引数として Boolean 型を使用することを避ける {id="avoid-using-the-boolean-type-as-an-argument"}
...
## 数値型を適切に使用する {id="use-numeric-types-appropriately"}
