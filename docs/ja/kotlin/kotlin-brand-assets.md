...
ロゴの使用に関しては、特に以下の制限事項に注意してください：

* マークをテキストから切り離さないでください。要素を入れ替えないでください。
* ロゴの透明度を変更しないでください。
* ロゴにアウトライン（縁取り）を付けないでください。
* ロゴをサードパーティの色で塗り直さないでください。
* テキストを変更しないでください。
* 複雑な背景の上にロゴを置かないでください。明るい背景の前にロゴを配置しないでください。
...
2. 画像の右上のロゴに、ユーザーグループの所在地の短縮名を追加します。背景パターンをカスタム画像に置き換えたい場合は、背景のパターン画像をクリックし、「画像を置換」を選択してから「パソコンからアップロード」または他のソースを選択してください。
