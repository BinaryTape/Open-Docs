...
Kiluaは、宣言的なUIコンポーデントを作成し、その状態を管理するためのモジュール式APIを提供します。また、一般的なWebアプリケーションのユースケース向けの、すぐに使えるコンポーネントセットも含まれています。
...
[Kotlin React](https://github.com/JetBrains/kotlin-wrappers/blob/master/docs/guide/react.md)は、ReactのエコシステムとKotlinの型安全性および表現力を組み合わせた、React用のKotlinラッパーです。
