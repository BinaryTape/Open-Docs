...
1. [アプリケーションを実行します](#build-and-run-the-application)。
2. 名前を入力します。
...
3. [アプリケーションを再度実行します](#build-and-run-the-application)。
...
> `nodejs` 環境を使用している場合は、[プロジェクトのビルドと実行](#build-and-run-the-project) セクションに進んでください。
> 
> {style="note"}
