...
* [言語バージョン](compiler-reference.md#language-version-version)は、ライブラリを直接使用するコードをどのKotlinコンパイラバージョンでコンパイルできるかを決定します。
* [APIバージョン](compiler-reference.md#api-version-version)は、実行時に必要な最小のKotlin標準ライブラリバージョンを決定します。

ほとんどの場合、同じ言語バージョンとAPIバージョンを使用します。

ライブラリに新しい言語バージョンを設定すると、利用者はより新しいKotlinコンパイラバージョンを使用する必要があります：

* JVMでは、利用者は以前の言語バージョン以降の任意のコンパイラバージョンを使用できます。たとえば、ライブラリが言語バージョン2.2を使用している場合、利用者はコンパイラバージョン2.1.x、2.2.x以降を使用できます。
* その他のプラットフォームでは、利用者はライブラリに設定された言語バージョンと同じか、それ以降のコンパイラバージョンを使用できます。この要件により、コンパイラをすぐにアップグレードできない利用者が、ライブラリの新しいバージョンへのアップグレードを遅らせる可能性があります。

また、利用者は、ライブラリに設定されたAPIバージョンと少なくとも同じ新しさのKotlin標準ライブラリバージョンを提供する必要があります。
GradleやIDEプラグインのように、実行環境が利用者の制御外にあり、より新しい標準ライブラリバージョンを提供することが困難な場合、アップグレードがより難しくなる可能性があります。
