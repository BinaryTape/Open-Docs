...
1. 利用可能な Gradle の _run_ タスクのいずれかを呼び出して、プロジェクトを実行します。例えば `jsBrowserDevelopmentRun` を使用します。
   [Kotlin/JS の実行](running-kotlin-js.md#run-the-browser-target) についての詳細を参照してください。
