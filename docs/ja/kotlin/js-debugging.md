...
1. 利用可能な Gradle の `run` タスクのいずれかを呼び出して、プロジェクトを実行します。例えば、マルチプラットフォームプロジェクトの場合は `browserDevelopmentRun` や `jsBrowserDevelopmentRun` を使用します。
...
[IntelliJ IDEA Ultimate](https://www.jetbrains.com/idea/) は、開発中のコードをデバッグするための強力なツールセットを提供しています。
