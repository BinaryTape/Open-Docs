...
Kotlin 1.5.20 には、1.5.0 の新機能で発見された問題の修正が含まれており、さまざまなツール改善も提供されています。
...
Kotlin 1.5.20 では、JVM プラットフォームで以下のアップデートが適用されます。
...
コンパイラオプションの追加方法については、[Gradle](gradle-compiler-options.md)、[Maven](maven-compile-package.md#specify-compiler-options)、および[コマンドラインコンパイラ](compiler-reference.md#compiler-options)を参照してください。
