...
## デフォルトで正しい動作をするようにする {id="do-the-right-thing-by-default"}
...
## 拡張の機会を提供する {id="allow-opportunities-for-extension"}
...
## 不要または無効な拡張を防止する {id="prevent-unwanted-and-invalid-extensions"}
...
## 可変状態の露出を避ける {id="avoid-exposing-mutable-state"}
...
## 入力と状態を検証する {id="validate-inputs-and-state"}
