[//]: # (title: イディオマティックなKotlinでのAdvent of Codeパズル)
...
* [このGitHubテンプレート](https://github.com/kotlin-hands-on/advent-of-code-kotlin-template)を使用してプロジェクトを作成してください。
...
[Kotlin Advent of Codeテンプレート](https://github.com/kotlin-hands-on/advent-of-code-kotlin-template)と、Kotlinでの文字列やコレクションを扱う便利な関数（[`maxOf()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/max-of.html)や[`sumOf()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/sum-of.html)など）について学びます。拡張関数がソリューションをきれいに構成するのにどのように役立つかをご覧ください。
...
Kotlinの`Char`型に対する操作を理解し、`Pair`型と`to`コンストラクタがパターンマッチングとどのようにうまく機能するかをご覧ください。[`compareTo()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-comparable/compare-to.html)関数を使用して独自のオブジェクトを順序付ける方法を理解します。
...
ファクトリ関数を使ったより複雑なオブジェクトの構築、正規表現の使用方法、および両端キュー[`ArrayDeque`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/-array-deque/)型について学びます。
...
入力処理、リストのイテレーション、マップを構築するさまざまな方法、そしてコードを簡素化するための[`let`](scope-functions.md#let)関数の使用法を探ります。
...
文字列ユーティリティ関数、正規表現、コレクションに対する操作、そして[`let`](scope-functions.md#let)関数が式を変換するのにいかに役立つかを探ります。
...
[`when`](control-flow.md#when-expressions-and-statements)式を適用し、入力を検証するさまざまな方法を探ります。ユーティリティ関数、範囲の操作、セットのメンバーシップの確認、特定の正規表現への一致などです。
