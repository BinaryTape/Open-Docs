...
### OSGi サポートのあるプロジェクトでの Kotlin アーティファクトのソースダウンロード {id="download-sources-for-kotlin-artifacts-in-projects-with-osgi-support"}
...
### 新しいデフォルトの kapt プラグイン {id="new-default-kapt-plugin"}
...
### Lombok コンパイラプラグイン: `@SuperBuilder` のサポートと `@Builder` のアップデート {id="lombok-compiler-plugin-support-for-superbuilder-and-updates-on-builder"}
...
## Kotlin Multiplatform: Gradle の Application プラグインを置き換える新しい DSL {id="kotlin-multiplatform-new-dsl-to-replace-gradle-s-application-plugin"}
...
### Xcode 16.3 のサポート {id="support-for-xcode-16-3"}
...
### 新しいインライン化の最適化 {id="new-inlining-optimization"}
<primary-label ref="experimental-opt-in"/>
...
### カスタムフォーマッタがデフォルトで有効に {id="custom-formatters-enabled-by-default"}
...
### Kotlin/Wasm コードデバッグのための DWARF サポート {id="support-for-dwarf-to-debug-kotlin-wasm-code"}
...
```
### Kotlin/Wasm および Kotlin/JS プロパティの Provider API への移行 {id="migration-to-provider-api-for-kotlin-wasm-and-kotlin-js-properties"}
...
### Gradle の Isolated Projects と互換性のある Kotlin Gradle プラグイン {id="kotlin-gradle-plugins-compatible-with-gradle-s-isolated-projects"}
<primary-label ref="experimental-opt-in"/>
...
### カスタム Gradle パブリケーションバリアントの追加サポート {id="support-for-adding-custom-gradle-publication-variants"}
<primary-label ref="experimental-opt-in"/>
...
### 共通アトミック型 {id="common-atomic-types"}
<primary-label ref="experimental-opt-in"/>
...
### UUID の解析、フォーマット、および比較可能性の変更 {id="changes-in-uuid-parsing-formatting-and-comparability"}
<primary-label ref="experimental-opt-in"/>
...
### 新しい時間追跡機能 {id="new-time-tracking-functionality"}
<primary-label ref="experimental-opt-in"/>
...
### オープンな `@Composable` 関数におけるデフォルト値を持つパラメータのサポート {id="support-for-parameters-with-default-values-in-open-composable-functions"}
...
### final なオーバーライド関数が再開可能 (restartable) になることを許可 {id="final-overridden-functions-are-allowed-to-be-restartable"}
...
### `ComposableSingletons` が公開 API から削除 {id="composablesingletons-removed-from-public-api"}
...
### ソース情報がデフォルトで含まれるように {id="source-information-included-by-default"}
...
### 刷新および新規ページ {id="revamped-and-new-pages"}
...
## Kotlin 2.1.20 へのアップデート方法 {id="how-to-update-to-kotlin-2-1-20"}
