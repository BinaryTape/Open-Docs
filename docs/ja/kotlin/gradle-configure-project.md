...
Kotlin/JVM の `KotlinJvmCompile`/`KotlinCompile` タスク of `destinationDirectory` の場所をオーバーライドする場合は、ビルドスクリプトを更新してください。JAR ファイル内で `sourceSets.main.kotlin.classesDirectories` を `sourceSets.main.outputs` に明示的に追加する必要があります。
