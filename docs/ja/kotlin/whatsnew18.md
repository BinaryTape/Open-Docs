...
静的リンクを使用している既存의 プロジェクトを Kotlin 1.8.0 にアップグレードする（またはリンクタイプを明示的に変更する）場合、プロジェクトの実行時にエラーが発生することがあります。これを修正するには、Xcode プロジェクトを閉じ、Podfile のあるディレクトリで `pod install` を実行してください。
