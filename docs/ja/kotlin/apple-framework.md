...
2. `build.gradle(.kts)` Gradleビルドファイルを以下のように更新します：
    ...
    Kotlin/Nativeは、iOS向けに `iosArm64`、`iosX64`、`iosSimulatorArm64` ターゲットをサポートしており、macOS向けに `macosArm64` ターゲットをサポートしています。そのため、`iosArm64()` をターゲットプラットフォームに応じたGradle関数に置き換えることができます：
