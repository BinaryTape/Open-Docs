...
| `Long`   | 64          | -9,223,372_036,854,775,808 (-2<sup>63</sup>) | 9,223,372,036,854,775,807 (2<sup>63</sup> - 1) |
...
JVMが数値を格納する方法によっては、小さな（バイトサイズ程度の）数値に対してデフォルトで使用されるキャッシュのために、コードが直感に反する挙動を示すことがあります。

JVMは数値を `int` や `double` などのプリミティブ型として格納します。
[ジェネリック型](generics.md)を使用する場合や、`Int?` のような Null 許容の数値参照を作成する場合、数値は Java の `Integer` や `Double` などのクラスにボックス化（boxed）されます。

JVMは、`-128` から `127` までの数値を表す `Integer` やその他のオブジェクトに対して [メモリ最適化手法](https://docs.oracle.com/javase/specs/jls/se22/html/jls-5.html#jls-5.1.7) を適用します。
そのようなオブジェクトに対するすべての Null 許容の参照は、同じキャッシュされたオブジェクトを参照します。
例えば、以下のコードの Null 許容オブジェクトは [参照の等価性](equality.md#referential-equality) が認められます。
...
この範囲外の数値の場合、Null 許容オブジェクトは異なりますが、[構造の等価性](equality.md#structural-equality) は保たれます。
...
このため、Kotlinではボックス化可能な数値やリテラルに対して参照の等価性を使用すると、`"Identity equality for arguments of types ... and ... is prohibited."`（...型と...型の引数に対する同一性の等価比較は禁止されています）というメッセージで警告が表示されます。
`Int`、`Short`、`Long`、`Byte` 型（および `Char` と `Boolean`）を比較する場合は、一貫した結果を得るために構造の等価性チェックを使用してください。
...
Kotlinは整数に対して一連の *ビット演算* を提供しています。これらは数値の表現形式であるビットを直接操作します。
