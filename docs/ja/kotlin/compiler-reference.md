...
詳細は [Maven](maven-compile-package.md#specify-compiler-options) を参照してください。
...
実験的な[アノテーションの `all` 使用箇所ターゲット](annotations.md#all-meta-target)を有効にします：
...
新しい実験的な[アノテーション使用箇所ターゲットのデフォルトルール](annotations.md#defaults-when-no-use-site-targets-are-specified)を有効にします：
...
生成される JVM バイトコードのターゲットバージョンを指定します。可能な値は `1.8`, `9`, `10`, ..., `25` です。
