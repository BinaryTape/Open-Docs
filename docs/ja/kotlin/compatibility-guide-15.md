...
後者は、この削除がコードの移行をできるだけスムーズにするために、事前に十分に周知されるべきであると述べています。
...
>
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 では、シグネチャ多相呼び出しにおけるスプレッド演算子 (`*`) の使用が禁止されます。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 では、それらのクラスから見えない抽象メンバー（internal/package-private）を含む非抽象クラスが禁止されます。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 では、JVMで非具体化型パラメータに基づく配列を具体化型引数として使用することが禁止されます。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 では、プライマリコンストラクタに委譲しないセカンダリ列挙型コンストラクタが禁止されます。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 では、プライベートなインライン関数から匿名型を公開することが禁止されます。
>
> **Deprecation cycle**:
>
> - < 1.5: 問題のあるコンストラクタに対して警告を導入
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 では、SAM変換を伴う引数の後に非スプレッド配列を渡すことが禁止されます。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 では、catchブロックで例外のパラメータ名を省略するために使用されるアンダースコアシンボル (`_`) への参照が禁止されます。
>
> **Deprecation cycle**:
>
...
>
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: Kotlin 1.5 から、SAM（単一抽象メソッド）変換の実装戦略が、匿名クラスの生成から `invokedynamic` JVM命令の使用に変更されます。
>
> **Deprecation cycle**:
>
> - 1.5: SAM変換の実装戦略を変更
...
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: Kotlin 1.5 では、Kotlin/JVM コンパイラに[IRベースのバックエンド](https://blog.jetbrains.com/kotlin/2021/02/the-jvm-backend-is-in-beta-let-s-make-it-stable-together/)がデフォルトで使用されます。以前の言語バージョンでは、古いバックエンドが引き続きデフォルトで使用されます。
>
...
> プロジェクトの設定ファイルに以下の行を追加して、一時的に1.5以前の振る舞いに戻すことができます。
>
> Gradle の場合:
>
> <tabs>
...
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: バージョン 1.5 から、Kotlin は[IRベースのバックエンド](https://blog.jetbrains.com/kotlin/2021/02/the-jvm-backend-is-in-beta-let-s-make-it-stable-together/)を
...
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: Kotlin 1.5 から、Kotlin コンパイラはデリゲート式でジェネリック呼び出しを含むデリゲートプロパティに対して null 可能性アサーションを発行します。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 では、型安全性を向上させるために、`contains`、`indexOf`、`assertEquals` のような無意味な引数を持つ呼び出しが禁止されます。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: Kotlin 1.5 では、名前付き可変長引数を持つ呼び出しにおける引数実行の順序が変更されます。
>
> **Deprecation cycle**:
>
> - < 1.5: 問題のあるコンストラクタに対して警告を導入
...
> **Issue**: [KT-42064](https://youtrack.com/issue/KT-42064)
>
> **Component**: Kotlin/JVM
>
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: Kotlin 1.5 では、演算子呼び出しにおけるパラメータのデフォルト値が使用されます。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: Kotlin 1.5 では、通常のプログレッションが空の場合、forループで空の逆順プログレッションを生成します。
>
> **Deprecation cycle**:
>
> - < 1.5: 以前の振る舞い（詳細については Issue を参照）
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 から、Charから数値型への変換が非推奨になります。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: Kotlin 1.5 から、`Char.equals` は大文字・小文字を区別しない比較において、まず文字の大文字化された変種が等しいかどうかを比較し、次にそれらの大文字化された変種の小文字化された変種（文字自体とは対照的に）が等しいかどうかを比較することで改善されます。
>
> **Deprecation cycle**:
>
> - < 1.5: 以前の振る舞い（詳細については Issue を参照）
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.5 から、`String.toUpperCase()` のようなデフォルトロケールに依存する大文字・小文字変換関数が非推奨になります。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: コレクションの`min`および`max`関数の戻り値の型は、1.6で非Nullableに変更されます。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: ソース互換性
>
> **Short summary**: Kotlin 1.4 で `WARNING` レベルで非推奨とされた浮動小数点型から `Short` および `Byte` への変換は、Kotlin 1.5.0 以降エラーになります。
>
> **Deprecation cycle**:
>
...
> **Incompatible change type**: 振る舞い互換性
>
> **Short summary**: 異なるテストフレームワーク用の相互排他的な`kotlin-test`の複数のバリアントが、推移的な依存関係によってもたらされた場合、プロジェクト内に存在し得ました。1.5.0 以降、Gradleは異なるテストフレームワーク用の相互排他的な`kotlin-test`バリアントの混在を許可しません。
>
> **Deprecation cycle**:
>
