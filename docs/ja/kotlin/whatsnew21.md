...
* [マルチダラー文字列補間：文字列リテラル内の `$` の扱いの改善](#multi-dollar-string-interpolation)
...
| `ARRAY_EQUALITY_OPERATOR_CAN_BE_REPLACED_WITH_EQUALS` | `arrayOf("").contentEquals(arrayOf(""))` の代わりに `arrayOf("") == arrayOf("")` が使用されている |
| `REDUNDANT_CALL_OF_CONVERSION_METHOD` | `42` の代わりに `42.toInt()` が使用されている |
| `USELESS_CALL_ON_NOT_NULL` | `""` の代わりに `"".orEmpty()` が使用されている |
...
| `REDUNDANT_SETTER_PARAMETER_TYPE` | `set(value)` の代わりに `set(value: Int)` が使用されている |
...
| `REDUNDANT_RETURN_UNIT_TYPE` | `fun foo() {}` の代わりに `fun foo(): Unit {}` が使用されている |
...
現在、Android をターゲットとする Kotlin Multiplatform プロジェクトでは `androidTarget` オプションを使用することを推奨しています。
...
これは、ターゲットが CI パイプラインで定期的にテストされ、コンパイルおよび実行が可能であることが保証されていることを意味します。また、このターゲットについては、コンパイラリリース間でのソースおよびバイナリ互換性も提供されます。
...
* [Kotlin Multiplatform 概要](multiplatform.topic) ページの更新 – Kotlin Multiplatform の要点や一般的なユースケースを確認できます。
