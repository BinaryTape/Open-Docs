...
もう一つの逆転関数である [`asReversed()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/as-reversed.html) は、同じコレクションインスタンスの逆順のビュー（view）を返します。そのため、元のリストが変更されない予定であれば、`reversed()` よりも軽量で好ましい場合があります。
