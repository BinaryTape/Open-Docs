...
## プロジェクトのセットアップ {id="setting-up-your-project"}
...
## バインディング (Bindings) {id="bindings"}

### 基本的な相互運用型 {id="basic-interop-types"}
...
### バインディングへのポインタの渡し方 {id="pass-pointers-to-bindings"}
...
### スコープローカルなポインタ {id="scope-local-pointers"}
...
### 構造体の値渡しと受け取り {id="pass-and-receive-structs-by-value"}
...
#### コールバックへのユーザーデータの渡し方 {id="pass-user-data-to-callbacks"}
...
### オブジェクトのピン留め (Object pinning) {id="object-pinning"}
