...
-   *ソース*: ソース非互換の変更とは、これまで問題なく（エラーや警告なしに）コンパイルできていたコードが、コンパイルできなくなる変更です。
-   *バイナリ*: 2つのバイナリ成果物がバイナリ互換であるとは、それらを交換してもロードエラーやリンクエラーが発生しない場合を指します。
-   *振る舞い*: ある変更が振る舞い非互換であるとは、同じプログラムが変更適用前後で異なる振る舞いを示す場合を指します。
