...
## サポートされているワークフロー {id="supported-workflows"}

Kotlin AI スキルは、さまざまな Kotlin 特有のシナリオで使用できます。 
以下の例は、AI スキルが役立つタスクの一部を示しています。

### Java ソースファイルを Kotlin に変換する {id="convert-java-source-files-to-kotlin"}
...
### Android アプリを含むマルチプラットフォームプロジェクトを AGP 9 を使用するように移行する {id="migrate-multiplatform-projects-with-android-apps-to-use-agp-9"}
...
### マルチプラットフォームプロジェクトを CocoaPods から SwiftPM 依存関係に移行する {id="migrate-multiplatform-projects-from-cocoapods-to-swiftpm-dependencies"}
