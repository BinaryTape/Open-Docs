...
## 在您开始之前 {id="before-you-start"}

在开始迁移之前，请完成以下步骤。

### 验证支持的版本 {id="verify-supported-versions"}
...
### 启用迁移帮助程序 {id="enable-migration-helpers"}
...
### 同步您的项目与 Gradle {id="sync-your-project-with-gradle"}
...
#### 外部文档链接 {id="external-documentation-links"}
...
#### 其他文件的输出目录 {id="output-directory-for-additional-files"}
...
### 在子项目之间共享 Dokka 配置 {id="share-dokka-configuration-across-subprojects"}
...
#### 不带约定插件的多模块项目 {id="multi-module-projects-without-convention-plugins"}
...
##### 将约定插件应用于您的子项目 {id="apply-the-convention-plugin-to-your-subprojects"}
...
#### 带有约定插件的多模块项目 {id="multi-module-projects-with-convention-plugins"}
...
### 更新多模块项目中的文档聚合 {id="update-documentation-aggregation-in-multi-module-projects"}
...
### 更改聚合文档的目录 {id="change-directory-of-aggregated-documentation"}
...
#### 还原到 DGP v1 目录行为 {id="revert-to-the-dgp-v1-directory-behavior"}
...
### 使用更新后的任务生成文档 {id="generate-documentation-with-the-updated-task"}
...
### 选择文档输出格式 {id="select-documentation-output-format"}
...
### 处理弃用和移除项 {id="address-deprecations-and-removals"}
...
## 完成迁移 {id="finalize-your-migration"}

完成项目迁移后，请执行以下步骤以结束工作并提高性能。

### 设置选择性加入标志 {id="set-the-opt-in-flag"}
