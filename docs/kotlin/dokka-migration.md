...
此属性将启用带迁移帮助程序的 DGP v2 插件。当构建脚本引用 DGP v1 中已不再提供的任务时，这些帮助程序可以防止编译错误。
...
此外，使用 DGP v2 的[辅助函数](https://github.com/Kotlin/dokka/blob/v2.1.0/dokka-runners/dokka-gradle-plugin/src/main/kotlin/engine/parameters/HasConfigurableVisibilityModifiers.kt#L14-L16)来添加要记录的可见性：
...
配置源链接，以便从生成的文档导航到远程仓库中相应的源代码。在 `dokkaSourceSets.main{}` 代码块中进行此配置。
