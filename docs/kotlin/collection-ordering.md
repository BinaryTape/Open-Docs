...
要为集合排序定义自定义顺序，您可以提供自己的 `Comparator`。为此，请调用 [`.sortedWith()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/sorted-with.html) 扩展函数并传入您的 `Comparator`。使用此函数，按字符串长度排序如下所示：
...
如果元素按指定顺序排列或者元素少于两个，这些扩展函数将返回 `true`。一旦发现乱序对，它们将返回 `false` 并停止检查。

对于没有保证迭代顺序的集合（例如 `HashSet`），结果在不同调用之间可能会有所不同。这同样适用于不产生一致顺序元素的序列。要使不同调用的结果相同，请仅在具有保证迭代顺序的集合（如 `List`）上使用这些函数。

在检查 `Double` 和 `Float` 值时，这些函数将 `NaN` 视为大于任何其他值，并将 `-0.0` 视为小于 `0.0`。此外，`.isSortedBy()` 和 `.isSortedByDescending()` 函数将 `null` 选择器结果视为小于任何非 null 值。

当您在序列上调用这些函数时，该操作是终端操作。它会消耗序列以生成 `Boolean` 值，而不是返回另一个序列。

> 这些排序顺序函数也适用于数组、原始类型数组和无符号数组。无符号数组及其操作是[实验性功能](components-stability.md#stability-levels-explained)，需要使用 `@ExperimentalUnsignedTypes` 注解进行启用。
> 
{style="note"}

下面是使用 `.isSorted()` 和 `.isSortedBy()` 函数检查排序顺序的示例：
...
`.reversed()` 扩展函数返回一个包含元素副本的新集合。因此，如果您以后更改原始集合，这不会影响之前获得的 `.reversed()` 结果。
