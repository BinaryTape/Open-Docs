...
### 通过 invokedynamic 进行字符串串联 {id="string-concatenation-via-invokedynamic"}
...
### 支持 JSpecify 为 null 性注解 {id="support-for-jspecify-nullness-annotations"}
...
### 支持在包含 Kotlin 和 Java 代码的模块中调用 Java 的 Lombok 生成的方法 {id="support-for-calling-java-s-lombok-generated-methods-within-modules-that-have-kotlin-and-java-code"}
...
### 选择性地将 KDoc 注释导出到生成的 Objective-C 头文件 {id="opt-in-export-of-kdoc-comments-to-generated-objective-c-headers"}
...
### 提高了单个数组内部 Array.copyInto() 的性能 {id="improved-performance-of-array-copyinto-inside-one-array"}
...
### JS IR 后端的迁移指南 {id="migration-guide-for-the-js-ir-backend"}
...
### kapt 中注解处理器类加载器的缓存 {id="caching-for-annotation-processors-classloaders-in-kapt"}
...
### 弃用 kotlin.parallel.tasks.in.project 构建属性 {id="deprecation-of-the-kotlin-parallel-tasks-in-project-build-property"}
...
### Kotlin/Native 和 Kotlin/JS 的 Char.digitToInt() 支持所有 Unicode 数字 {id="support-for-all-unicode-digits-in-char-digittoint-in-kotlin-native-and-kotlin-js"}
...
### 跨平台统一 Char.isLowerCase()/isUpperCase() 实现 {id="unification-of-char-islowercase-isuppercase-implementations-across-platforms"}
