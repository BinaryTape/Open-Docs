...
* **任务 #1**  
  在高层次上描述你构思的 Kotlin 编译器模糊测试器架构。

  > 你可以在以后的任务中完善和迭代此设计。  
  > 如果可能，请记录你的更改并在回答中进行解释。
  >
  > 你可以使用 AI 工具，但请说明你如何使用它们以及如何确保结果的质量。
  >
  {style="note"}

* **任务 #2**  
  基于任务 1 中的架构，哪些组件对可插拔性最重要，为什么？

* **任务 #3**  
  基于任务 1 中的架构，你是否看到了使用 LLM 或 AI 的机会？如果有，在何处以及如何使用？

* **任务 #4**  
  你将如何测试模糊测试器本身？回测（back-testing）是否可行？如果可行，如何进行？

* **任务 #5** (加分项)  
  从任务 1 的架构中选择一个组件，并更详细地描述如何实现它。  
  例如，可以使用哪些工具、库或算法？
...
* **任务 #1**  
  复刻 [swift-java](https://github.com/swiftlang/swift-java) 仓库，并为其扩展一个生成 Kotlin 源代码的新目标。

  > **要求：**
  >  * 添加一个 CLI 选项（例如 `--lang kotlin-jvm` 或 `--emit kotlin-jvm`）
  >  * 为最小子集生成 Kotlin：
  >     * 顶层 Swift 函数
  >     * 类型：`Int`/`Int32`、`Bool`、`Double`、`String`、`Void` → Kotlin `Int`、`Boolean`、`Double`、`String`、`Unit`
  >  * 添加测试
  >  * 添加关于如何运行该项目的说明
  >
  > **生成的 Kotlin 代码应包括：**
  >  * 带有映射名称和类型的顶层函数声明
  >  * 存根（stub）主体（例如 `TODO("Not implemented")`）
  >
  > **添加一份简短的 `DESIGN.md` (或 `NOTES.md`)，描述：**
  >  * 权衡以及故意跳过的部分
  >  * 更完整的解决方案会是什么样子
  >  * 如果有更多时间，你接下来的打算
  > 
...
* **任务 #2** (可选)  
  扩展之前的任务，使生成的 Kotlin 代码可以使用现有的 JNI 或 FFM 运行时进行调用。
