...
<web-summary>了解如何从 Java 字符串迁移到 Kotlin 字符串。本指南涵盖了 Java StringBuilder、字符串串联与拆分、多行字符串、流以及其他主题。</web-summary>
...
在 Kotlin 中，在变量名前使用美元符号 (`$`) 即可将该变量的值插值到您的字符串中：
...
在 Kotlin 中，使用 [buildString()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/build-string.html) —— 这是一个[内联函数](inline-functions.md)，它接收用于构造字符串的逻辑作为 lambda 表达式实参：
...
在底层，`buildString` 使用与 Java 相同的 `StringBuilder` 类，您可以在 [lambda表达式](lambdas.md#function-literals-with-receiver) 内部通过隐式的 `this` 访问它。
...
在 Kotlin 中，使用 [joinToString()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/join-to-string.html) 函数，Kotlin 为每个 `List` 都定义了该函数：
...
Kotlin 提供了内联函数 [ifBlank()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/if-blank.html)，它接受默认值作为实参：
...
在 Kotlin 中，使用 [removeSurrounding()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/remove-surrounding.html) 函数并指定字符串分隔符 `##`：
...
在 Kotlin 中，您可以使用 [Regex](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/-regex/) 类，它简化了正则表达式的使用。
...
在 Kotlin 中，使用 Kotlin 函数 [split()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/split.html)，它接受可变数量的分隔符作为输入参数：
...
在 Kotlin 中，您可以使用 [substringAfter()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/substring-after.html) 函数，无需计算您想要获取其后子字符串的字符索引：
...
有一点需要记住：如果您打印一个多行字符串，且三引号位于下一行，则会多出一个空行：
...
![Java 15 多行输出](java-15-multiline-output.png){width=700}
...
![Kotlin 多行输出](kotlin-multiline-output.png){width=700}
