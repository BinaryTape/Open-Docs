...
您还可以将多行输入读取为单个字符串。
使用 [`joinToString()`](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin.sequences/join-to-string.html) 函数来合并这些行：
...
> 使用 [`readlnOrNull()`](#handle-standard-input-safely) 代替 `readln()` 来读取输入直到输入末尾，而不会抛出异常。
> 
{style="note"}
