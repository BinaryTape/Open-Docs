...
```kotlin
fun jsTypeOf(o: Any): String {
    return js(getTypeof() + " o") // 此处会报错
}

fun getTypeof() = "typeof"
```

> 由于 JavaScript 代码是由 Kotlin 编译器解析的，因此可能并非所有的 ECMAScript 特性都受支持。在这种情况下，你可能会遇到编译错误。
> 
{style="note"}

请注意，调用 `js()` 会返回一个 [`dynamic`](dynamic-type.md) 类型的结果，它在编译时不提供类型安全性。
