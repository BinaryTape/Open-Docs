[//]: # (title: Kotlin 2.2.0 的最新变化)

<web-summary>阅读 Kotlin 2.2.0 发布说明，涵盖新的语言功能、Kotlin Multiplatform、JVM、Native、JS 和 Wasm 的更新，以及对 Gradle 和 Maven 的构建工具支持。</web-summary>

_[发布日期：2025 年 6 月 23 日](releases.md#release-history)_

Kotlin 2.2.0 正式发布！以下是主要亮点：

* **语言**：预览版中的新语言功能，包括 [上下文参数](#preview-of-context-parameters)。
  几个[此前处于实验性的功能现已稳定](#stable-features-guard-conditions-non-local-break-and-continue-and-multi-dollar-interpolation)，
  例如守卫条件、非局部 break 与 continue 以及多美元符插值。
...
* **标准库**：[稳定的 Base64 与 HexFormat API](#stable-base64-encoding-and-decoding)。
* **文档**：我们的 [文档调查已开放](https://surveys.jetbrains.com/s3/Kotlin-Docs-2025)，并且 [Kotlin 文档已进行显著改进](#documentation-updates)。

你也可以观看此视频，了解 Kotlin 语言演进团队对新功能的讨论并回答相关问题：

<video src="https://www.youtube.com/watch?v=jne3923lWtw" title="Kotlin 2.2.0 的最新变化"/>
...
你不需要更新 IDE 中的 Kotlin 插件。
你只需在构建脚本中将 [Kotlin 版本更改](configure-build-for-eap.md#adjust-the-kotlin-version) 为 2.2.0。

详情请参阅 [更新至新版本](releases.md#update-to-a-new-kotlin-version)。

## 语言

此版本将守卫条件、非局部 `break` 与 `continue` 以及多美元符插值 [晋升](#stable-features-guard-conditions-non-local-break-and-continue-and-multi-dollar-interpolation) 为 [稳定版](components-stability.md#stability-levels-explained)。此外，还引入了多个功能的预览版，例如 [上下文参数](#preview-of-context-parameters) 和 [上下文相关解析](#preview-of-context-sensitive-resolution)。
...
通过上下文参数，你不需要手动传递在多组函数调用中共享且很少更改的值，例如服务或依赖项。
...
主要区别在于上下文参数不会作为接收器引入到函数体中。因此，你需要使用上下文参数的名称来访问其成员，而不像上下文接收器那样上下文是隐式可用的。

Kotlin 中的上下文参数代表了在通过简化依赖注入、改进 DSL 设计和作用域操作来管理依赖项方面的重大改进。欲了解更多信息，请参阅该功能的 [KEEP](https://github.com/Kotlin/KEEP/blob/context-parameters/proposals/context-parameters.md)。

#### 如何声明上下文参数

你可以使用 `context` 关键字后跟参数列表来为属性和函数声明上下文参数，每个参数的形式为 `name: Type`。以下是一个依赖于 `UserService` 接口的示例：

```kotlin
// UserService 定义了上下文中需要的依赖项
...
此功能计划在未来的 Kotlin 版本中稳定并改进。
如果你能在我们的问题跟踪器 [YouTrack](https://youtrack.jetbrains.com/issue/KT-10468/Context-Parameters-expanding-extension-receivers-to-work-with-scopes) 上提供反馈，我们将不胜感激。

### 上下文相关解析预览
<primary-label ref="experimental-general"/> 

Kotlin 2.2.0 引入了上下文相关解析的预览实现。

你可以在此视频中找到该功能的概述：

<video src="https://www.youtube.com/v/aF8RYQrJI8Q" title="Kotlin 2.2.0 中的上下文相关解析"/>

以前，即使可以从上下文中推断出类型，你也必须写出枚举项或密封类成员的全名。例如：
...
现在，通过上下文相关解析，你可以在已知预期类型的上下文中省略类型名称：
...
> 上下文相关解析不适用于函数、带有参数的属性或带有接收器的扩展属性。
>
{style="note"}

要在项目中试用上下文相关解析，请在命令行中使用以下编译器选项：
...
我们计划在未来的 Kotlin 版本中稳定并改进此功能，欢迎在我们的问题跟踪器 [YouTrack](https://youtrack.jetbrains.com/issue/KT-16768/Context-sensitive-resolution) 上提供反馈。

### 注解使用点目标功能预览
<primary-label ref="experimental-general"/>

Kotlin 2.2.0 引入了几个功能，使处理注解使用点目标（use-site targets）更加方便。

#### 针对属性的 `@all` 元目标
<primary-label ref="experimental-general"/>

Kotlin 允许你将注解附加到声明的特定部分，这被称为 [使用点目标](annotations.md#annotation-use-site-targets)。然而，单独为每个目标添加注解非常复杂且容易出错：
...
为了简化这一点，Kotlin 引入了新的针对属性的 `@all` 元目标。此功能告诉编译器将注解应用于属性的所有相关部分。当你使用它时，`@all` 会尝试将注解应用于：
...
* **`RECORD_COMPONENT`**：如果类是 `@JvmRecord`，则注解应用于 [Java record 组件](#improved-support-for-annotating-jvm-records)。此行为模仿了 Java 处理 record 组件上注解的方式。
...
你可以对主构造函数内部或外部的任何属性使用 `@all` 元目标。但是，你不能在 [多个注解](https://kotlinlang.org/spec/syntax-and-grammar.html#grammar-rule-annotation) 中使用 `@all` 元目标。
...
此功能目前处于预览阶段。请向我们的问题跟踪器 [YouTrack](https://kotl.in/issue) 报告任何问题。有关 `@all` 元目标的更多信息，请阅读此 [KEEP](https://github.com/Kotlin/KEEP/blob/master/proposals/annotation-target-in-properties.md) 提案。

#### 使用点注解目标的新默认规则
<primary-label ref="experimental-general"/>

Kotlin 2.2.0 引入了新的默认规则，用于将注解传播到形参、字段和属性。以前默认情况下注解仅应用于 `param`、`property` 或 `field` 中的一个，现在的默认值更符合对注解的预期。

如果有多个适用目标，将按如下方式选择一个或多个：

* 如果构造函数形参目标 (`param`) 适用，则使用它。
* 如果属性目标 (`property`) 适用，则使用它。
* 如果字段目标 (`field`) 适用而 `property` 不适用，则使用 `field`。

如果有多个目标，且 `param`、`property` 或 `field` 都不适用，则该注解会导致错误。
...
* 在特定情况下，显式定义所需的目标，例如，使用 `@param:Annotation` 代替 `@Annotation`。
...
此功能目前处于预览阶段。请向我们的问题跟踪器 [YouTrack](https://kotl.in/issue) 报告任何问题。有关使用点注解目标新默认规则的更多信息，请阅读此 [KEEP](https://github.com/Kotlin/KEEP/blob/master/proposals/annotation-target-in-properties.md) 提案。

### 支持嵌套类型别名
<primary-label ref="beta"/>

Kotlin 2.2.0 增加了对在其他声明中定义类型别名的支持。

你可以在此视频中找到该功能的概述：

<video src="https://www.youtube.com/v/1W6d45IOwWk" title="Kotlin 2.2.0 中的嵌套类型别名"/>

以前，你只能在 Kotlin 文件的顶层声明 [类型别名](type-aliases.md)。这意味着即使是内部或特定领域的类型别名也必须存在于使用它们的类之外。

从 2.2.0 开始，你可以在其他声明中定义类型别名，只要它们不从外部类捕获类型形参：
...
嵌套类型别名通过改进封装、减少包级杂乱和简化内部实现，使代码更加简洁且易于维护。
...
嵌套类型别名目前处于 [Beta](components-stability.md#stability-levels-explained) 阶段。请向我们的问题跟踪器 [YouTrack](https://kotl.in/issue) 报告任何问题。有关此功能的更多信息，请阅读此 [KEEP](https://github.com/Kotlin/KEEP/blob/master/proposals/nested-typealias.md) 提案。

### 稳定功能：守卫条件、非局部 `break` 和 `continue` 以及多美元符插值

在 Kotlin 2.1.0 中，引入了几个预览版的新语言功能。我们很高兴地宣布，以下语言功能在此版本中已达到 [稳定版](components-stability.md#stability-levels-explained)：

* [带有主体的 `when` 中的守卫条件](control-flow.md#guard-conditions-in-when-expressions)
* [非局部 `break` 与 `continue`](inline-functions.md#break-and-continue)
* [多美元符插值：改进了字符串文字中 `$` 的处理](strings.md#multi-dollar-string-interpolation)
...
此功能默认启用，但仅在满足以下 _所有_ 条件时才在 Kotlin/Native 默认内存分配器中可用：
...
为了更轻松地检查库版本之间的二进制兼容性，我们正在尝试将 [二进制兼容性验证器 (binary compatibility validator)](https://github.com/Kotlin/binary-compatibility-validator) 的功能移动到 Kotlin Gradle 插件 (KGP) 中。你可以在练手项目中试用它，但我们还不建议在生产环境中使用它。
...
有关此属性及其选项的更多信息，请参阅 Gradle 有关 [自定义日志格式 (Customizing log format)](https://docs.gradle.org/current/userguide/command_line_interface.html#sec:command_line_customizing_log_format) 的文档。
...
### 默认启用 `OptimizeNonSkippingGroups` 功能标志

从 Kotlin 2.2.0 开始，`OptimizeNonSkippingGroups` 功能标志默认启用。此优化通过删除为不可跳过的可组合函数生成的组调用来提高运行时性能。它不应导致运行时任何可观察的行为变化。
...
* Kotlin 脚本编写（scripting）仍然是 Kotlin 生态系统的重要组成部分，但我们正专注于特定的使用场景，如自定义脚本，以及 `gradle.kts` 和 `main.kts` 脚本，以提供更好的体验。欲了解更多信息，请参阅我们更新后的 [博客文章](https://blog.jetbrains.com/kotlin/2024/11/state-of-kotlin-scripting-2024/)。因此，Kotlin 2.2.0 弃用了以下支持：
...
* 弃用的 `kotlin-android-extensions` 插件在 [Kotlin 2.2.0 中被移除](compatibility-guide-22.md#deprecate-kotlin-android-extensions-plugin)。请使用 `kotlin-parcelize` 插件生成 `Parcelable` 实现，并使用 Android Jetpack 的 [视图绑定 (view bindings)](https://developer.android.com/topic/libraries/view-binding) 代替合成视图（synthetic views）。
