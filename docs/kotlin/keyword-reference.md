 ...
 * `in`
     ...
     - 用作中缀运算符，检查一个值是否属于[范围](ranges.md)、集合或其他[定义了 'contains' 方法](operator-overloading.md#in-operator)的实体。
     - 在 [when 表达式](control-flow.md#when-expressions-and-statements)中用于相同目的。
     - 将类型形参标记为[逆变](generics.md#declaration-site-variance)。
 * `!in`
     - 用作运算符，检查一个值是否**不**属于[范围](ranges.md)、集合或其他[定义了 'contains' 方法](operator-overloading.md#in-operator)的实体。
     - 在 [when 表达式](control-flow.md#when-expressions-and-statements)中用于相同目的。
 * `interface` 声明一个[接口](interfaces.md)。
 * `is`
     - 检查[某个值是否具有特定类型](typecasts.md#is-and-is-operators)。
     - 在 [when 表达式](control-flow.md#when-expressions-and-statements)中用于相同目的。
 * `!is`
     - 检查[某个值是否**不**具有特定类型](typecasts.md#is-and-is-operators)。
     - 在 [when 表达式](control-flow.md#when-expressions-and-statements)中用于相同目的。
 ...
 * `super`
     ...
     - [在次构造函数中调用超类构造函数](classes.md#inheritance)。
 * `this`
     - 引用[当前接收者](this-expressions.md)。
     - [在次构造函数中调用相同类的另一个构造函数](classes.md#constructors-and-initializer-blocks)。
 ...
 * `while` 开始一个 [while 循环](control-flow.md#while-loops)（带有前置条件的循环）。

## 软关键字

以下标记在适用的上下文中充当关键字，但在其他上下文中可用作标识符：
 ...
 * `field` 用作[注解使用处目标](annotations.md#annotation-use-site-targets)。
 * `file` 用作[注解使用处目标](annotations.md#annotation-use-site-targets)。
 * `finally` 开始一个[当 try 块退出时始终执行](exceptions.md)的代码块。
 * `get`
     - 声明[属性的 getter](properties.md)。
     - 用作[注解使用处目标](annotations.md#annotation-use-site-targets)。
 * `import` [将另一个软件包的声明导入到当前文件](packages.md)。
...
* `value` 与 `class` 关键字配合声明一个[内联类](inline-classes.md)。
* `where` 指定[泛型类型形参的约束](generics.md#upper-bounds)。

## 修饰符关键字

以下标记在声明的修饰符列表中充当关键字，但在其他上下文中可用作标识符：

 * `abstract` 将类或成员标记为[抽象](classes.md#abstract-classes)。
 * `actual` 表示[多平台项目](https://kotlinlang.org/docs/multiplatform/multiplatform-expect-actual.html)中的平台相关实现。
 ...
 * `crossinline` 禁止[在传递给内联函数的 lambda 中进行非局部返回](inline-functions.md#returns)。
 * `data` 指示编译器[为类生成规范成员](data-classes.md)。
 * `enum` 声明一个[枚举](enum-classes.md)。
 * `expect` 将声明标记为[平台相关](https://kotlinlang.org/docs/multiplatform/multiplatform-expect-actual.html)，并期待在平台模块中实现。
 * `external` 将声明标记为在 Kotlin 之外实现（通过 [JNI](java-interop.md#using-jni-with-kotlin) 访问或在 [JavaScript](js-interop.md#external-modifier) 中实现）。
 * `final` 禁止[重写成员](inheritance.md#overriding-methods)。
 * `infix` 允许使用[中缀表示法](functions.md#infix-notation)调用函数。
 * `inline` 告知编译器[在调用处内联函数及传递给它的 lambda](inline-functions.md)。
 * `inner` 允许从[嵌套类](nested-classes.md)中引用外部类实例。
 * `internal` 将声明标记为[在当前模块中可见](visibility-modifiers.md)。
 * `lateinit` 允许[在构造函数之外初始化非空属性](properties.md#late-initialized-properties-and-variables)。
 * `noinline` 关闭[传递给内联函数的 lambda 的内联](inline-functions.md#noinline)。
 * `open` 允许[类被继承或成员被重写](classes.md#inheritance)。
 * `operator` 将函数标记为[重载运算符或实现约定](operator-overloading.md)。
 * `out` 将类型形参标记为[协变](generics.md#declaration-site-variance)。
 * `override` 将成员标记为[对超类成员的重写](inheritance.md#overriding-methods)。
 ...
 * `sealed` 声明一个[密封类](sealed-classes.md)（受限继承的类）。
 * `suspend` 将函数或 lambda 标记为挂起（可用作[协程](coroutines-overview.md)）。
 * `tailrec` 将函数标记为[尾递归](functions.md#tail-recursive-functions)（允许编译器将递归替换为迭代）。
 * `vararg` 允许[为某个形参传递可变数量的实参](functions.md#variable-number-of-arguments-varargs)。

## 特殊标识符

以下标识符由编译器在特定上下文定义，在其他上下文中可用作普通标识符：
 ...
 * `=`
     - 赋值运算符。
     - 用于指定[形参默认值](functions.md#parameters-with-default-values)。
 * `+=`、`-=`、`*=`、`/=`、`%=` - [增强赋值运算符](operator-overloading.md#augmented-assignments)。
 * `++`、`--` - [递增与递减运算符](operator-overloading.md#increments-and-decrements)。
 * `&&`、`||`、`!` - 逻辑“与”、“或”、“非”运算符（对于按位操作，请改用相应的[中缀函数](numbers.md#operations-on-numbers)）。
 * `==`、`!=` - [相等运算符](operator-overloading.md#equality-and-inequality-operators)（对于非基本类型，会转换为对 `equals()` 的调用）。
 * `===`、`!==` - [引用相等运算符](equality.md#referential-equality)。
 * `<`、`>`、`<=`、`>=` - [比较运算符](operator-overloading.md#comparison-operators)（对于非基本类型，会转换为对 `compareTo()` 的调用）。
 * `[`、`]` - [索引访问运算符](operator-overloading.md#indexed-access-operator)（会转换为对 `get` 和 `set` 的调用）。
 * `!!` [断言一个表达式是非空的](null-safety.md#not-null-assertion-operator)。
 ...
 * `->`
     - 分隔 [lambda表达式](lambdas.md#lambda-expression-syntax)的形参和主体。
     - 在[函数类型](lambdas.md#function-types)中分隔形参和返回值类型声明。
 ...
 * `_`
     - 在 [lambda表达式](lambdas.md#underscore-for-unused-variables)中替代未使用的形参。
