...
随着 Kotlin 语言和生态系统的不断演进，Kotlin 编译器也在不断发展。第一步是在之前引入了共享逻辑的新 JVM 与 JS IR (中间表示) 后端，这简化了针对不同平台目标的生成代码。现在，其演进的下一个阶段带来了被称为 K2 的新前端。
...
fun signalCheck(signalStatus: Any) {
    if (signalStatus is Postponed || signalStatus is Declined) {
        ...
        // 未解析的引用 (Unresolved reference) 错误。signal() 函数只有
        // 在另一次类型检查后才能成功调用：
...
在 Kotlin 2.0.0 之前，编译器不理解对象类型在使用了自增或自减运算符后可能会发生变化。由于编译器无法准确跟踪对象类型，您的代码可能会导致未解析引用错误。在 Kotlin 2.0.0 中，这已被修复：
...
在 Kotlin 2.0.0 中，我们对新 Kotlin K2 编译器的实现包括了对编译方案的重新设计，以确保公共源集和平台源集之间的严格分离。当您使用[预期和实际函数](https://kotlinlang.org/docs/multiplatform/multiplatform-expect-actual.html#expected-and-actual-functions)时，这种变化最为明显。以前，公共代码中的函数调用可能会解析为平台代码中的函数。例如：
...
除了提高各平台行为的一致性外，我们还努力修复了 IntelliJ IDEA 或 Android Studio 与编译器之间存在行为冲突的情况。例如，当您使用[预期和实际类](https://kotlinlang.org/docs/multiplatform/multiplatform-expect-actual.html#expected-and-actual-classes)时，会发生以下情况：
...
由于 K2 编译器的新架构，我们改变了处理不可访问泛型类型的方式。通常，您绝不应在代码中依赖不可访问的泛型类型，因为这表示项目构建配置中存在配置错误，导致编译器无法访问编译所需的信息。在 Kotlin 2.0.0 中，您不能声明或调用具有不可访问泛型类型的函数文字，也不能使用具有不可访问泛型类型实参的泛型类型。此限制有助于您避免稍后在代码中出现编译器错误。
...
除了当函数文字包含不可访问泛型类型的实参时会触发错误外，当一个类型具有不可访问的泛型类型实参时也会发生错误。
...
> 预期类和实际类处于 [Beta](components-stability.md#stability-levels-explained) 阶段。
...
使用之前的编译器，预期的非抽象类可以在不[重写函数](inheritance.md#overriding-rules)的情况下继承抽象函数。由于编译器可以同时访问公共代码和平台代码，因此编译器可以查看抽象函数在实际类中是否有相应的重写和定义。
...
在您的公共代码中，如果您有一个名为 `PlatformFileSystem` 的预期非抽象类继承自 `FileSystem` 类，则 `PlatformFileSystem` 类会继承抽象函数 `listFiles()`。但是，在 Kotlin 中，非抽象类中不能有抽象函数。要使 `listFiles()` 函数变为非抽象，您必须将其声明为不带 `abstract` 关键字的 override：
...
| [KT-62507](https://youtrack.jetbrains.com/issue/KT-62507) | K2：禁止在公共代码中为预期/实际顶层属性进行智能转换 |
...
| [KT-58042](https://youtrack.jetbrains.com/issue/KT-58042) | 如果等效的 Getter 不可见，即使重写的声明可见，也要使合成属性不可见 |
...
| [KT-57422](https://youtrack.jetbrains.com/issue/KT-57422) | K2：禁止在属性 Getter 上使用使用处 'get' 目标的注解 |
