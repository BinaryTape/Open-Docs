...
### 弃用在投影接收器上使用合成 setter {id="deprecate-use-of-a-synthetic-setter-on-a-projected-receiver"}
...
### 修正调用在 Java 子类中重载且带有内联类参数的函数时的修饰 (mangling) {id="correct-mangling-when-calling-functions-with-inline-class-parameters-that-are-overloaded-in-a-java-subclass"}
...
### 修正逆变捕获类型的类型近似算法 {id="correct-type-approximation-algorithm-for-contravariant-captured-types"}
...
### 禁止在属性初始化之前访问属性值 {id="prohibit-accessing-property-value-before-property-initialization"}
...
### 当导入的同名类存在歧义时报告错误 {id="report-error-when-there-s-ambiguity-in-imported-classes-with-the-same-name"}
...
### 默认通过 invokedynamic 和 LambdaMetafactory 生成 Kotlin lambda {id="generate-kotlin-lambdas-via-invokedynamic-and-lambdametafactory-by-default"}
...
### 当需要表达式时禁止仅带有一个分支的 if 条件 {id="forbid-if-condition-with-one-branch-when-an-expression-is-required"}
...
### 禁止通过传递泛型类型的星投影来违反自上界 (self upper bounds) {id="prohibit-violation-of-self-upper-bounds-by-passing-a-star-projection-of-a-generic-type"}
...
### 对私有内联函数返回值类型中的匿名类型进行近似 {id="approximate-anonymous-types-in-private-inline-functions-return-type"}
...
### 更改重载解析行为，优先处理局部扩展函数调用，而非局部函数类型属性的 invoke 约定 {id="change-overload-resolution-behavior-to-prioritize-local-extension-function-calls-over-invoke-conventions-of-local-functional-type-properties"}
...
### 当由于二进制依赖项中基类型的更改而发生继承成员冲突时报告错误 {id="report-error-when-an-inherited-member-conflict-occurs-due-to-a-change-in-a-supertype-from-binary-dependency"}
...
### 忽略不变类型参数上的 @UnsafeVariance 注解 {id="ignore-unsafevariance-annotations-on-parameters-in-invariant-types"}
...
### 更改对伴生对象成员的调用外引用的类型 {id="change-type-for-out-of-call-references-to-a-companion-object-s-member"}
...
### 禁止通过非私有内联函数暴露匿名类型 {id="prohibit-exposure-of-anonymous-types-from-private-inline-functions"}
...
### 对 while 循环中断 (break) 后不合理的智能转换报告错误 {id="report-error-for-an-unsound-smart-cast-after-a-while-loop-break"}
...
### 当交集类型的变量被赋予一个不是该交集类型子类型的值时报告错误 {id="report-error-when-a-variable-of-an-intersection-type-is-assigned-a-value-that-is-not-a-subtype-of-that-intersection-type"}
...
### 当使用 SAM 构造函数构建的接口包含需要选择使用的函数时，要求选择使用 (opt-in) {id="require-opt-in-when-an-interface-constructed-with-a-sam-constructor-contains-a-method-that-requires-an-opt-in"}
...
### 禁止在 typealias 构造函数中违反上界 {id="prohibit-upper-bound-violation-in-typealias-constructors"}
...
### 使解构变量的真实类型与指定的显式类型保持一致 {id="make-the-real-type-of-a-destructuring-variable-consistent-with-the-explicit-type-when-specified"}
...
### 当调用的构造函数具有需要选择使用的默认值参数类型时，要求选择使用 (opt-in) {id="require-opt-in-when-calling-a-constructor-that-has-parameter-types-with-default-values-that-require-an-opt-in"}
...
### 在同一作用域级别下，同名的属性和枚举项之间报告歧义 {id="report-ambiguity-between-a-property-and-an-enum-entry-with-the-same-name-at-the-same-scope-level"}
...
### 更改限定符解析行为，优先选择伴生属性而非枚举项 {id="change-qualifier-resolution-behavior-to-prefer-companion-property-over-enum-entry"}
...
### 像以脱糖形式编写的那样解析 invoke 调用接收器类型和 invoke 函数类型 {id="resolve-invoke-call-receiver-type-and-the-invoke-function-type-as-if-written-in-desugared-form"}
...
### 禁止通过非私有内联函数暴露私有类成员 {id="prohibit-exposing-private-class-members-through-non-private-inline-functions"}
...
### 修正投影泛型类型中绝对非空类型的为 null 性 {id="correct-nullability-of-definitely-non-null-types-in-projected-generic-types"}
...
### 更改前缀自增的推断类型以匹配 getter 的返回值类型，而非 inc() 运算符的返回值类型 {id="change-inferred-type-of-prefix-increment-to-match-getter-s-return-type-instead-of-inc-operator-s-return-type"}
...
### 从基类中声明的泛型内部类继承内部类时强制执行边界检查 {id="enforce-bound-checks-when-inheriting-inner-classes-from-generic-inner-classes-declared-in-superclasses"}
...
### 当预期类型是带有函数类型参数的函数类型时，禁止分配具有 SAM 类型的可调用引用 {id="forbid-assigning-callable-references-with-sam-types-when-the-expected-type-is-a-function-type-with-a-function-type-parameter"}
...
### 在伴生对象上的注解解析中考虑伴生对象作用域 {id="consider-companion-object-scope-for-annotation-resolution-on-companion-objects"}
...
### 更改安全调用与约定运算符组合的评估语义 {id="change-evaluation-semantics-for-combination-of-safe-calls-and-convention-operators"}
...
### 要求具有支持字段和自定义 setter 的属性立即初始化 {id="require-properties-with-backing-field-and-a-custom-setter-to-be-immediately-initialized"}
...
### 禁止在 invoke 运算符约定调用中对任意表达式进行 Unit 转换 {id="prohibit-unit-conversion-on-arbitrary-expressions-in-invoke-operator-convention-call"}
...
### 当通过安全调用访问字段时，禁止向非空 Java 字段分配可空值 {id="forbid-nullable-assignment-to-non-null-java-field-when-the-field-is-accessed-with-a-safe-call"}
...
### 重写包含原始类型 (raw-type) 参数的 Java 方法时需要星投影类型 {id="require-star-projected-type-when-overriding-a-java-method-containing-a-raw-type-parameter"}
...
### 当 V 具有伴生对象时更改 (V)::foo 引用解析 {id="change-v-foo-reference-resolution-when-v-has-a-companion"}
...
### 禁止在有效公开的内联函数中隐式访问非公开 API {id="forbid-implicit-non-public-api-access-in-effectively-public-inline-functions"}
...
### 禁止在属性 getter 上使用使用处 get 注解 {id="prohibit-use-site-get-annotations-on-property-getters"}
...
### 防止在构建器推断 lambda 函数中将类型参数隐式推断为上界 {id="prevent-implicit-inference-of-type-parameters-into-upper-bounds-in-builder-inference-lambda-functions"}
...
### 在公共签名中近似局部类型时保留为 null 性 {id="keep-nullability-when-approximating-local-types-in-public-signatures"}
...
### 为了智能转换的目的，移除对 false && ... 和 false || ... 的特殊处理 {id="remove-special-handling-for-false-and-false-for-the-purposes-of-smart-casting"}
...
### 禁止在枚举中使用内联 open 函数 {id="forbid-inline-open-functions-in-enums"}
...
### Gradle 中的可见性变更 {id="visibility-changes-in-gradle"}
...
### 弃用 kotlinOptions DSL {id="deprecate-kotlinoptions-dsl"}
...
### 弃用 KotlinCompilation DSL 中的 compilerOptions {id="deprecate-compileroptions-in-kotlincompilation-dsl"}
...
### 弃用处理 CInteropProcess 的旧方式 {id="deprecate-old-ways-of-cinteropprocess-handling"}
...
### 移除 kotlin.useK2 Gradle 属性 {id="remove-kotlin-usek2-gradle-property"}
...
### 移除 outputFile JavaScript 编译器选项 {id="remove-outputfile-javascript-compiler-option"}
