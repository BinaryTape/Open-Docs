...
//sampleStart
    ...
    println("${duration.inWholeMinutes} 分钟内有 ${duration.inWholeSeconds} 秒")
//sampleEnd
}
```
{validate="false"}
...
* **比较数组、序列和任意可迭代对象的容器内容**
  ...
  新增了一组重载的 `assertContentEquals()` 函数，用于为未实现 [结构相等 (structural equality)](equality.md#structural-equality) 的不同集合比较内容：
...
* **`assertTrue()`、`assertFalse()`、`expect()` 函数现在是内联的**
  ...
  从现在起，您可以将这些函数用作内联函数，因此可以在 lambda 表达式内部调用 [挂起函数 (suspend functions)](composing-suspending-functions.md)：
...
Kotlin 1.5.0 是一个 [特性版本 (feature release)](kotlin-evolution-principles.md#language-and-tooling-releases)，因此可能会为语言带来不兼容的变化。在 [Kotlin 1.5 兼容性指南](compatibility-guide-15.md) 中可以找到此类变化的详细列表。