...
3. 在 IDE 中运行 `linkDebugShared<YourTargetName>` Gradle 任务，或在终端中使用控制台命令（本例中如下所示）来构建库：

   ```bash
   ./gradlew linkDebugSharedMacosArm64
   ```

该构建会在 `build/bin/<yourTargetName>/debugShared` 目录下生成库，其中包含以下文件：
...
### 服务运行时函数 {id="service-runtime-functions"}
