...
> 关于无符号整数类型，请参见 [unsigned-integer-types.md](unsigned-integer-types.md)。
>
{style="tip"}
...
JVM 对表示 `−128` 到 `127` 之间数字的 `Integer` 及其他对象应用了[内存优化技术](https://docs.oracle.com/javase/specs/jls/se22/html/jls-5.html#jls-5.1.7)。所有对此类对象的可空引用都指向相同的缓存对象。例如，以下代码中的可空对象是[引用相等的](#referential-equality)：
...
对于超出此区间的数字，可空对象是不同的，但却是[结构相等的](#structural-equality)：
