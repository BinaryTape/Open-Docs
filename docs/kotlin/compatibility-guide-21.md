...
-   _源代码_：源代码不兼容的变更会阻止之前能够正常编译（无错误或警告）的代码继续编译。
-   _二进制_：如果互相替换两个二进制构件不会导致加载或链接错误，则称它们是二进制兼容的。
-   _行为_：如果同一个程序在应用变更前后表现出不同的行为，则称此变更是行为不兼容的。
...
> -   2.1.0：报告语言版本 1.6 和 1.7 的警告；将警告提升为错误，适用于语言版本 1.4 和 1.5
...
> **不兼容变更类型**：源代码
>
> **简要概述**：现在禁止通过可见性较低的类型形参界限暴露类型，以此解决类型可见性规则中的不一致问题。此变更确保了类型形参的界限遵循与类相同的可见性规则，从而防止了诸如 JVM 中 IR 验证错误之类的问题。
>
> **弃用周期**：
>
