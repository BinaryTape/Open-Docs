...
在本教程中，您将学习如何创建 Kotlin/Native 应用程序。请选择最适合您的工具，并使用以下方式创建您的应用程序：

*   **[IDE](#in-ide)**。在这里，您可以从版本控制系统克隆项目模板并在 IntelliJ IDEA 中使用它。
*   **[Gradle 构建系统](#using-gradle)**。为了更好地理解内部工作原理，请手动为您的项目创建构建文件。
*   **[命令行工具](#using-the-command-line-compiler)**。您可以使用 Kotlin/Native 编译器，它作为标准 Kotlin 发行版的一部分提供，并直接在命令行工具中创建应用程序。

    控制台编译可能看起来简单直接，但对于包含数百个文件和库的大型项目来说，它无法很好地扩展。对于此类项目，我们建议使用 IDE 或构建系统。

使用 Kotlin/Native，您可以为[不同的目标](native-target-support.md)编译，包括 Linux、macOS 和 Windows。虽然可以进行跨平台编译（这意味着使用一个平台为另一个平台编译），但在本教程中，您将以与编译平台相同的平台为目标平台。

> 如果您使用的是 Mac 并且想要创建和运行 macOS 或其他 Apple 目标平台的应用程序，您还需要先安装 [Xcode Command Line Tools](https://developer.apple.com/download/)，启动它并接受许可条款。
>
{style="note"}

## 在 IDE 中

在本节中，您将学习如何使用 IntelliJ IDEA 创建 Kotlin/Native 应用程序。您可以同时使用 Community Edition 和 Ultimate Edition。
...
3.  打开 `gradle/libs.versions.toml` 文件，它是项目依赖项的版本目录。要创建 Kotlin/Native 应用程序，您需要 Kotlin Multiplatform Gradle 插件，它的版本与 Kotlin 相同。请确保您使用的是最新 Kotlin 版本：
...
有关这些设置的更多信息，请参见 [Multiplatform Gradle DSL 参考](https://kotlinlang.org/docs/multiplatform/multiplatform-dsl-reference.html)。
...
IntelliJ IDEA 使用 Gradle 任务运行代码并将结果输出到 **Run** 标签页中：
...
您可以[配置 IntelliJ IDEA](https://www.jetbrains.com/help/idea/compiling-applications.html#auto-build) 以自动构建您的项目：

1.  转到 **Settings | Build, Execution, Deployment | Compiler**。
2.  在 **Compiler** 页面，选择 **Build project automatically**。
3.  应用更改。

现在，当您更改类文件或保存文件（<shortcut>Ctrl + S</shortcut>/<shortcut>Cmd + S</shortcut>）时，IntelliJ IDEA 会自动执行项目的增量构建。

### 更新应用程序

让我们为您的应用程序添加一个特性，使其能够计算您姓名中的字母数量：

1.  在 `Main.kt` 文件中，添加代码以读取输入。使用 [`readln()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/readln.html) 函数读取输入值并将其赋值给 `name` 变量：
...
2.  要使用 Gradle 运行此应用，请在 `build.gradle.kts` 文件中将 `System.in` 指定为要使用的输入，并加载 Gradle 更改：
...
3.  消除空格并计算字母：
    ...
    *   使用作用域函数 [`let`](scope-functions.md#let) 在对象上下文内运行函数。
    *   通过添加美元符号并在花括号中包裹它 – `${it.length}`，使用[字符串模板](strings.md#string-templates)将您的姓名长度插入到字符串中。`it` 是 [lambda 实参](coding-conventions.md#lambda-parameters)的默认名称。
...
在本节中，您将学习如何使用 [Gradle](https://gradle.org) 手动创建 Kotlin/Native 应用程序。它是 Kotlin/Native 和 Kotlin Multiplatform 项目的默认构建系统，也常用于 Java、Android 及其他生态系统。

构建 Kotlin/Native 项目时，Kotlin Gradle 插件会下载以下构件：

*   主要的 Kotlin/Native 捆绑包，包括 `konanc`、`cinterop` 和 `jsinterop` 等不同工具。默认情况下，Kotlin/Native 捆绑包作为简单的 Gradle 依赖项从 [Maven Central](https://repo1.maven.org/maven2/org/jetbrains/kotlin/kotlin-native-prebuilt/) 仓库下载。
*   `konanc` 本身所需的依赖项，例如 `llvm`。它们使用自定义逻辑从 JetBrains CDN 下载。

您可以在 Gradle 构建脚本的 `repositories {}` 代码块中更改主捆绑包的下载来源。
...
2.  创建一个空项目目录。在其中，创建一个 `build.gradle(.kts)` 文件并包含以下内容：
...
*   学习如何[为实际的 Kotlin/Native 项目编写 Gradle 构建脚本](https://kotlinlang.org/docs/multiplatform/multiplatform-dsl-reference.html)。
