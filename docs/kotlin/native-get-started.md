...
在本教程中，你将学习如何创建 Kotlin/Native 应用程序。选择最适合你的工具，并使用以下方式创建应用：

* **[IDE](#in-ide)**。在这里，你可以从版本控制系统中克隆项目模板并在 IntelliJ IDEA 中使用。
* **[Gradle 构建系统](#using-gradle)**。为了更好地理解底层原理，请手动为你的项目创建构建文件。
* **[命令行工具](#using-the-command-line-compiler)**。你可以使用 Kotlin/Native 编译器（它是标准 Kotlin 分发的一部分），并在命令行工具中直接创建应用。

  控制台编译看起来简单直接，但对于拥有数百个文件和库的大型项目来说，它的扩展性并不好。对于此类项目，我们建议使用 IDE 或构建系统。

使用 Kotlin/Native，你可以为[不同的目标](native-target-support.md)进行编译，包括 Linux、macOS 和 Windows。虽然支持跨平台编译（即使用一个平台为另一个平台进行编译），但在本教程中，你将针对正在编译的同一平台。

> 如果你使用的是 Mac 并希望为 macOS 或其他 Apple 目标创建并运行应用程序，你还需要先安装 [Xcode 命令行工具](https://developer.apple.com/download/)，启动它并先接受许可条款。
>
{style="note"}

## 在 IDE 中

在本节中，你将学习如何使用 IntelliJ IDEA 创建 Kotlin/Native 应用程序。你可以使用 Community Edition 或 Ultimate Edition。
...
3. 打开 `gradle/libs.versions.toml` 文件，这是项目依赖项的版本目录。要创建 Kotlin/Native 应用程序，你需要 Kotlin Multiplatform Gradle 插件，其版本与 Kotlin 相同。确保使用最新的 Kotlin 版本：
...
4. 按照建议重新加载 Gradle 文件：
   ...
   ![加载 Gradle 更改按钮](load-gradle-changes.png){width=295}
...
![应用程序输出](native-output-gutter-1.png){width=331}

首次运行后，IDE 会在顶部创建相应的运行配置：

![Gradle 运行配置](native-run-config.png){width=503}

> IntelliJ IDEA Ultimate 用户可以安装 [Native Debugging Support](https://plugins.jetbrains.com/plugin/12775-native-debugging-support) 插件，该插件允许调试已编译的原生可执行文件，并为导入的 Kotlin/Native 项目自动创建运行配置。
...
现在，当你对类文件进行更改或保存文件 (<shortcut>Ctrl + S</shortcut>/<shortcut>Cmd + S</shortcut>) 时，IntelliJ IDEA 会自动执行项目的增量构建。

### 更新应用程序

让我们为应用程序添加一个功能，以便它可以计算你名字中的字母数量：
...
3. 去除空格并计算字母数量：
   ...
   * 使用[字符串模板](strings.md#string-templates)，通过添加美元符号并将其包裹在花括号中——`${it.length}`，将名字长度插入字符串。`it` 是 [lambda 形参](coding-conventions.md#lambda-parameters) 的默认名称。
...
5. 输入你的名字并查看结果：
   ...
   ![应用程序输出](native-output-gutter-2.png){width=422}
...
4. 输入你的名字并查看结果：
   ...
   ![应用程序输出](native-output-gutter-3.png){width=422}
...
2. 创建一个空的项目目录。在其中创建一个包含以下内容的 `build.gradle(.kts)` 文件：
   ...
   你可以使用不同的[目标名称](native-target-support.md)，例如 `macosArm64`、`iosArm64`、`linuxArm64` 和 `mingwX64` 来定义编译代码的目标。这些目标名称可以选择性地将平台名称作为形参，在本例中为 `native`。平台名称用于在项目中生成源路径和任务名称。
...
按照约定，所有源代码都位于 `src/<target name>[Main|Test]/kotlin` 目录中，其中 `Main` 用于源代码，`Test` 用于测试。`<target name>` 对应于构建文件中指定的评估目标平台（在本例中为 `native`）。
