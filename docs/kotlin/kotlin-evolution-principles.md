...
Kotlin 编程语言的所有更改均由 [首席语言设计师](https://kotlinfoundation.org/structure/)（目前为 Michail Zarečenskij）监督。首席设计师在所有与语言演进相关的事项上拥有最终决定权。此外，针对完全稳定组件的不兼容更改必须由 [Kotlin Foundation](https://kotlinfoundation.org/structure/) 指定的 [语言委员会](https://kotlinfoundation.org/structure/) 批准（目前成员包括 Jeffrey van Gogh、Werner Dietl 和 Michail Zarečenskij）。
