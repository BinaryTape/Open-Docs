...
在发布语言和工具的稳定版本之前，我们会发布许多名为 EAP（即“Early Access Preview”，抢先体验预览）的预览构建，以便我们更快地迭代并收集社区反馈。语言版本的 EAP 通常会生成稍后会被稳定编译器拒绝的二进制文件，以确保二进制格式中可能存在的错误不会超出预览期。最终的 Release Candidate（发布候选版）通常不受此限制。要了解更多信息，请参阅[参与 Kotlin 抢先体验计划](eap.md)。
