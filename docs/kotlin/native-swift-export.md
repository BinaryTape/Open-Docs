...
该功能目前处于[实验性](components-stability.md#stability-levels-explained)阶段，尚未准备好用于生产环境。如需试用，请在您的 Kotlin 项目中[配置构建文件](#configure-kotlin-project)，并[设置 Xcode](#configure-xcode-project) 以集成 Swift 导出。
...
* 当模块在 Gradle 坐标中具有相同名称时（例如 SQLDelight 的 Runtime 模块和 Compose Runtime 模块），Swift 导出将失效 ([KT-80185](https://youtrack.jetbrains.com/issue/KT-80185))。
...
| Kotlin | Swift | 说明 |
|------------------------|--------------------------------|-------------------------|
| `class` | `class` | [说明](#classes) |
| `object` | 带有 `shared` 属性的 `class` | [说明](#objects) |
| `enum class` | `enum` | [说明](#enums) |
| `typealias` | `typealias` | [说明](#type-aliases) |
| 函数 | 函数 | [说明](#functions) |
| 属性 | 属性 | [说明](#properties) |
| 构造函数 | 初始值设定项 | [说明](#constructors) |
| 软件包 | 嵌套枚举 | [说明](#packages) |
| `Boolean` | `Bool` | |
...
| `Any` | `KotlinBase` 类 | |
| `Unit` | `Void` | |
| `Nothing` | `Never` | [说明](#kotlin-nothing) |
...
> 目前对带有 `suspend`、`inline` 和 `operator` 关键字的函数支持有限。
> 通常不支持泛型类型。
>
{style="note"}
