...
*sam-with-receiver* 编译器插件会将带有注解的 Java “单一抽象方法” (SAM) 接口方法的第一个形参在 Kotlin 中转换为 receiver。这种转换仅在 SAM 接口作为 Kotlin lambda 传递时生效，无论是对于 SAM 适配器还是 SAM 构造函数（详见 [SAM 转换文档](java-interop.md#sam-conversions)）。
