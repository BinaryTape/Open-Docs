...
### 外部声明 (External declarations) {id="external-declarations"}
...
#### 外部接口 (External interfaces) {id="external-interfaces"}
...
#### 外部对象 (External objects) {id="external-objects"}
...
#### 外部类型层次结构 (External type hierarchy) {id="external-type-hierarchy"}
...
## Kotlin/Wasm 与 Kotlin/JS 互操作性的差异 {id="kotlin-wasm-and-kotlin-js-interoperability-differences"}
