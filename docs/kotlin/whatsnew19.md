...
* [用于开区间（open-ended ranges）的稳定 `..<` 运算符](#stable-operator-for-open-ended-ranges)
...
我们引入了一个 C 互操作（C interop）编译器选项，允许您使用隐式整数转换。经过
...
从 Kotlin 1.9.0 开始，新的 Android 源集（source set）布局成为默认。它取代了之前的目录命名架构，后者在多方面令人困惑。新布局具有多项优势：
...
Kotlin 1.9.0 支持在多平台库中使用 [Gradle 配置缓存](https://docs.gradle.org/current/userguide/configuration_cache.html)。如果您是库作者，现在就可以从改进的构建性能中获益。
...
其他类型作为不透明引用（opaque references）在没有转换的情况下传递，导致 JavaScript 和 Kotlin 
子类型（subtyping）之间不一致。
...
* [弃用外部枚举（external enum）](#deprecation-of-external-enum)
...
Kotlin Gradle 插件现在为选择性加入（opt-ins）和编译器的渐进模式提供了新属性。
...
在 1.9.0 之前，[kapt 编译器插件](kapt.md)通过请求 Kotlin 编译任务的配置实例来导致任务被过早创建（eager task creation）。此行为已在 Kotlin 1.9.0 中修复。如果您在 `build.gradle.kts` 文件中使用了默认配置，那么您的设置不受此更改的影响。
...
自 1.3.50 起，我们提供了新的时间测量 API 预览。API 的时长（duration）部分在 1.6.0 中已变为稳定。在 1.9.0 中，
剩余的时间测量 API 已变为稳定。

旧的时间 API 提供了 `measureTimeMillis` 和 `measureNanoTime` 函数，它们使用起来并不直观。虽然 
清楚它们都以不同的单位测量时间，但不清楚 `measureTimeMillis` 使用的是[挂钟时间](https://en.wikipedia.org/wiki/Elapsed_real_time)来测量时间，而 `measureNanoTime` 使用的是单调（monotonic）时间源。新的时间 API 解决了这些及其他问题，使 API 更加用户友好。
...
在 1.9.0 之前，每个平台都有自己的扩展，可以从正则表达式匹配中按名称获取正则表达式捕获组（capture group）。但是没有通用的函数。在 Kotlin 1.8.0 之前不可能有通用函数， 
...
> 新的 `HexFormat` 类及其相关的扩展函数是[实验性的](components-stability.md#stability-levels-explained)， 
...
Kotlin 1.9.0 是一个[特性版本](kotlin-evolution-principles.md#language-and-tooling-releases)，因此可能会 
