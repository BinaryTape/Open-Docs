...
并非每一段 Kotlin 代码都能编译到所有平台。如果某段代码无法编译到特定平台，Kotlin 编译器会阻止您在通用代码中使用平台特定函数或类。
...
如果您也想将 `greeting.kt` 文件编译为 `.js`，您只需声明 JS 目标。随后 `commonMain` 中的代码会获得一个对应的 `js` 额外标签，指示 Kotlin 生成 `.js` 文件：
...
* **模拟器目标** 用于为在本地机器上启动的 iOS 模拟器生成二进制文件。如果您使用的是 Apple 芯片 Mac 计算机，请选择 `iosSimulatorArm64` 作为模拟器目标。如果您使用的是基于 Intel 的 Mac 计算机，请使用 `iosX64`。

如果您仅声明 `iosArm64` 设备目标，您将无法在本地机器上运行和调试应用程序及测试。

像 `iosArm64Main`、`iosSimulatorArm64Main` 和 `iosX64Main` 这样的平台特定源集通常是空的，因为用于 iOS 设备和模拟器的 Kotlin 代码通常是相同的。您可以仅使用 `iosMain` 中间源集在所有这些目标之间共享代码。

这同样适用于其他非 Mac 的 Apple 目标。例如，如果您有针对 Apple TV 的 `tvosArm64` 设备目标，以及分别针对 Apple 芯片和 Intel 设备上的 Apple TV 模拟器的 `tvosSimulatorArm64` 和 `tvosX64` 模拟器目标，您可以为所有这些目标使用 `tvosMain` 中间源集。
