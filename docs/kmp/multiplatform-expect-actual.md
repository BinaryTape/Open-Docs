...
当您使用在不同目标平台之间共享的中间源集时，也可以声明实际声明。例如，考虑将 `iosMain` 作为在 `iosX64Main`、`iosArm64Main` 和 `iosSimulatorArm64Main` 平台源集之间共享的中间源集。通常只有 `iosMain` 包含实际声明，而平台源集则不包含。然后，Kotlin 编译器将使用这些实际声明为相应平台生成结果代码。
...
但是，在这种情况下，平台源集中的这些额外常量将无法与通用代码中的常量匹配。因此，编译器要求您处理所有额外的情况。
