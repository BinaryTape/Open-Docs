...
<td>Android 应用程序和库</td>
        ...
        <td>
            ...
            <p>每个 Gradle 子项目只能创建一个 Android 目标平台。</p>
        </td>
</tr>
...
*   所有目标平台都可用的 [通用目标平台配置](#common-target-configuration)。
*   目标平台特定的配置。
...
`js {}` 代码块描述 Kotlin/JS 目标平台的配置，而 `wasmJs {}` 代码块描述与 JavaScript 互操作的 Kotlin/Wasm 目标平台的配置。它们可以包含以下两个代码块之一，具体取决于目标平台执行环境：
...
要为项目中的特定目标平台配置编译器选项，请在目标平台代码块中使用 `compilerOptions {}` 代码块：
...
| `enableLanguageFeature` | 启用指定的语言特性。可用值与当前[实验性](https://kotlinlang.org/docs/experimental-features.html)的或在某个时间点作为实验性引入的语言特性相对应。 |
