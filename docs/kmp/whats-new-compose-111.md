[//]: # (title: Compose Multiplatform 1.11.1 的最新变化)
...
### 针对 non-Android 目标的 Shader 包装器 {id="shader-wrapper-for-non-android-targets"}
...
### 最低 Kotlin 版本已提高 {id="minimum-kotlin-version-increased"}

如果您的项目包含原生或 Web 目标，最新功能需要升级到 Kotlin 2.3.10。

### iOS 目标支持的变化 {id="changes-to-ios-target-support"}
...
### Compose UI 测试 v2 {id="compose-ui-tests-v2"}
...
### Skia 更新至 Milestone 144 {id="skia-updated-to-milestone-144"}
...
### 默认启用并发渲染 {id="concurrent-rendering-enabled-by-default"}
...
### Web 目标的滚动性能已提升至与原生 UI 一致 {id="scroll-on-web-targets-brought-in-line-with-native-ui"}
