...
### Kotlin 2.0.0 需要新的 Compose 编译器 Gradle 插件 {id="new-compose-compiler-gradle-plugin-is-required-for-kotlin-2-0-0"}
...
#### 稳定的资源库 {id="stable-resource-library"}

[资源库 API](compose-multiplatform-resources.md) 的主体部分现已被视为稳定。

#### 支持包含 Compose Multiplatform 资源的多模块项目 {id="support-for-multimodule-projects-with-compose-multiplatform-resources"}
...
#### 多平台资源的配置 DSL {id="configuration-dsl-for-multiplatform-resources"}
...
#### 生成资源 URI 的公开函数 {id="public-function-for-generating-resource-uri"}
...
### 实验性导航库 {id="experimental-navigation-library"}
...
### Lifecycle 库 {id="lifecycle-library"}
...
### 支持 Kotlin 2.0.0 {id="support-for-kotlin-2-0-0"}
...
### 对 BasicTextField2 的初步支持 {id="basic-support-of-basictextfield2"}
...
### DialogWindow 的 alwaysOnTop 标志 {id="alwaysontop-flag-for-dialogwindow"}
...
### 针对 iOS 17 及更高版本的选择容器放大镜 {id="selection-container-magnifier-for-ios-17-and-higher"}

iOS 上的 Compose Multiplatform 选择容器现在会模拟原生的放大镜工具。

![iPhone 聊天应用在文字放大镜激活时的屏幕截图](compose-1610-ios-magnifier.png){width=390}

### 用于对话框居中的软件键盘内边距 {id="software-keyboard-inset-for-dialog-centering"}
...
### 基础 IME 键盘支持 {id="basic-ime-keyboard-support"}

Compose Multiplatform 的 Web 目标现在对虚拟 (IME) 键盘提供了基础支持。

## Gradle 插件 {id="gradle-plugin"}

### 能够修改 macOS 最低版本 {id="possibility-to-modify-the-macos-minimum-version"}
...
### 创建支持 ProGuard 的 uber JAR 的选项 {id="option-to-create-uber-jars-with-proguard-support"}
