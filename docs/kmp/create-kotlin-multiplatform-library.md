...
`library` 模块的结构旨在容纳共享代码以及特定平台的实现。以下是其主要源代码 (`src`) 内容的详细说明：
...
通过使用 [`expect`/`actual` 机制](multiplatform-connect-to-apis.md#expected-and-actual-functions-and-properties) 配置对 [Kotlin/Wasm](https://kotlinlang.org/docs/wasm-overview.html) 平台的支持。您可以为 `firstElement` 和 `secondElement` 属性实现平台特定的功能。
...
2. 找到 `mavenPublishing { }` 块并注释掉 `signAllPublications()` 方法，以表明发布仅限本地：
...
要找到已发布的库，请使用文件管理器或终端并导航到用户主目录中的 `.m2\repository\io\github\kotlin\library\1.0.0\`。

## 将库作为依赖项添加到另一个项目中

将多平台库发布到本地 Maven 仓库后，您就可以在同一台机器上的其他 Kotlin 项目中使用它。

在消费者项目的 `build.gradle.kts` 文件中，添加对已发布库的依赖项：
...
`repositories{}` 块告诉 Gradle 从本地 Maven 仓库解析该库，并使其在共享代码中可用。

`implementation` 依赖项由库的 `build.gradle.kts` 文件中指定的 group 和 version 组成。

如果您是将其添加到另一个多平台项目中，可以将其添加到共享源集或特定平台的源集中：
...
* [将您的库发布到 Maven Central](multiplatform-publish-libraries.md)
