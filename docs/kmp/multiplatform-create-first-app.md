...
* `androidApp` 是一个构建为 Android 应用程序的 Kotlin 模块。它使用 Gradle 作为构建系统。
  `androidApp` 模块依赖并像使用常规 Android 库一样使用 shared 模块。
* `iosApp` 是一个构建为 iOS 应用程序的 Xcode 项目。
...
* `sharedLogic` 是多平台模块，包含 Android 和 iOS 应用程序通用的逻辑。
* `sharedUI` 是包含 Compose Multiplatform UI 代码的模块：在此项目中，它仅供 Android 应用使用，
...
_源集 (source set)_ 是一个 Gradle 概念，用于将一组文件在逻辑上分组在一起，每个组都有自己的依赖项。
在 Kotlin Multiplatform 中，共享模块中的不同源集可以针对不同的平台。

`sharedLogic` 模块包含 `androidMain`、`commonMain` 和 `iosMain` 源集。
`commonMain` 源集包含共享的 Kotlin 代码，而平台特定源集包含仅限于每个平台的代码。
...
2. 您可以在 `androidMain` 和 `iosMain` 源集中找到实现该接口的平台特定类：
    ...
    * `AndroidPlatform` 类的 `name` 属性使用了 Android 特定的代码，即 `android.os.Build` 类。此代码被解释为 Kotlin/JVM。如果您尝试在此处访问 JVM 特定的类（如 `java.util.Random`），此代码将可以编译。
    * `IOSPlatform` 类的 `name` 属性使用了 iOS 特定的代码，即 `platform.UIKit.UIDevice` 类。此代码被解释为 Kotlin/Native，这意味着您可以在 Kotlin 中引用 iOS 声明。该代码将成为 iOS 框架的一部分，并在 `iosApp` 模块的 Swift 代码中导入。

3. 每个源集都包含一个 `getPlatform()` 函数。
   它的 `expect` 声明没有函数体，而 `actual` 实现是在平台代码中提供的：
...
在为特定平台生成代码时，Kotlin 编译器会将 `expect` 声明和 `actual` 声明合并为具有正确实现的单一 `getPlatform()` 函数。

这就是为什么 `expect` 和 `actual` 声明需要定义在同一个包中的原因 —— 它们在生成的平台代码中被合并为一个声明。生成的平台代码中对期望 `getPlatform()` 函数的任何调用随后都会引用正确的实际实现。
...
如果您之前创建了可选的 expect/actual 属性，您应该会看到 Android 的问候语中添加了 "[1]"，而 iOS 的问候语中添加了 "[2]"。
...
在 IntelliJ IDEA 中，选择运行配置列表中的 **iosApp**，在运行配置旁边选择一个模拟设备，
然后点击 **Run**（运行）。
...
4. 确保 Bundle Identifier 是唯一的，并且已成功分配 Signing Certificate（签名证书）。
...
使用数据线连接您的 iPhone。如果您已经在 Xcode 中注册了该设备，IntelliJ IDEA 应该会将其显示在运行配置列表中。运行相应的 `iosApp` 配置。
