...
## 破坏性变更：新的 Compose 编译器 Gradle 插件 {id="breaking-change-new-compose-compiler-gradle-plugin"}
...
#### 稳定资源库 {id="stable-resource-library"}

[资源库 API](compose-multiplatform-resources.md) 的大部分内容现已视为稳定。

#### 支持在多模块项目中使用 Compose Multiplatform 资源 {id="support-for-multimodule-projects-with-compose-multiplatform-resources"}
...
#### 用于生成资源 URI 的公共函数 {id="public-function-for-generating-resource-uri"}
...
### BasicTextField2 的初步支持 {id="basic-support-of-basictextfield2"}
...
### iOS 17 及更高版本的选择容器放大镜 {id="selection-container-magnifier-for-ios-17-and-higher"}

iOS 上的 Compose Multiplatform 选择容器现在可以模拟原生放大工具。

![iPhone 聊天应用的屏幕截图，其中文本放大镜处于激活状态](compose-1610-ios-magnifier.png){width=390}

### 用于 Dialog 居中的软键盘边距 {id="software-keyboard-inset-for-dialog-centering"}
...
### Kotlin/Wasm 支持进入 Alpha 阶段 {id="kotlin-wasm-support-in-alpha"}
...
### 创建支持 Proguard 的 uber JAR 选项 {id="option-to-create-uber-jars-with-proguard-support"}
...
### 已知问题：MissingResourceException {id="known-issue-missingresourceexception"}
