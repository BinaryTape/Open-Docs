...
通过本教程，您可以启动并运行一个简单的 Kotlin Multiplatform 应用。

## 设置环境

Kotlin Multiplatform (KMP) 项目需要特定的环境，但大多数要求通过 IDE 中的预检都能清晰地了解。

首先从 IDE 和必要的插件开始：

1. 选择并安装 IDE。
    IntelliJ IDEA 和 Android Studio 都支持 Kotlin Multiplatform，因此您可以使用自己偏好的 IDE。
    
    推荐使用 [JetBrains Toolbox App](https://www.jetbrains.com/toolbox/app/) 来安装 IDE。它可以让您管理多个产品或版本，包括[抢先体验计划](https://www.jetbrains.com/resources/eap/) (EAP) 和 Nightly 版本。
...
3. 如果您尚未安装必要的依赖项，在 IntelliJ IDEA 中安装 Kotlin Multiplatform IDE 插件时也会一并安装（Android Studio 已捆绑所有必要的插件）。
    
4. 如果您尚未设置 `ANDROID_HOME` 环境变量，请配置您的系统以识别它：
...
5. 若要创建 iOS 应用程序，您需要一台安装了 [Xcode](https://apps.apple.com/us/app/xcode/id497799835) 的 macOS 主机。您的 IDE 将在后台运行 Xcode 以构建 iOS 框架。
...
4. 我们建议选择一个版本的 [JetBrains Runtime](https://github.com/JetBrains/JetBrainsRuntime) (JBR) 作为项目的 JDK，因为它提供了重要的修复，特别是能够提高桌面端 KMP 应用的兼容性。每个 IntelliJ IDEA 分发版中都包含相关的 JBR 版本，因此无需额外设置。
5. 选择您希望作为项目一部分的平台：
    * 可以设置所有目标平台使用 Compose Multiplatform，以便从一开始就共享 UI 代码（没有 UI 代码的服务器模块除外）。
    * 对于 iOS，您可以选择以下两种实现之一：
        * 使用 Compose Multiplatform 共享 UI 代码。
        * 使用 SwiftUI 制作完全原生的 UI，并通过共享逻辑连接到 Kotlin 模块。
    * 桌面端目标包含 [Compose Hot Reload](compose-hot-reload.md) 功能，让您在保存代码更改后即可看到 UI 变化。即使您不打算制作桌面应用，可能也想使用桌面版本来加速 UI 代码的编写。

选择完平台后，点击 **Create** 按钮并等待 IDE 生成并导入项目。

![IntelliJ IDEA 向导，默认设置并选择了 Android、iOS、桌面和 Web 平台](idea-wizard-1step.png){width=600}
...
4. 选择您希望作为项目一部分的平台：
    ...
    * 对于 iOS，您可以选择以下两种实现之一： 
      * 使用 Compose Multiplatform 共享 UI 代码。
      * 使用 SwiftUI 制作完全原生的 UI，并通过共享逻辑连接到 Kotlin 模块。  
    * 桌面端目标包含 Beta 版本的 [Compose Hot Reload](compose-hot-reload.md) 功能，让您在更改相应代码后即可看到 UI 变化。即使您不打算制作桌面应用，可能也想使用桌面版本来加速 UI 代码的编写。
5. 项目生成后，我们建议选择一个版本的 [JetBrains Runtime](https://github.com/JetBrains/JetBrainsRuntime) (JBR) 作为项目的 JDK，因为它提供了重要的修复，特别是能够提高桌面端 KMP 应用的兼容性。每个 IntelliJ IDEA 分发版中都包含相关的 JBR 版本，因此无需额外设置。
...
点击右侧边栏或底部栏上的预检图标 ![带飞机的预检图标](ide-preflight-checks.png){width="20"}

在此工具窗口中，您可以查看与这些检查相关的消息、重新运行检查或更改其设置。 

预检命令在 **Search Everywhere** 对话框中也可用。双击 <shortcut>Shift</shortcut> 键并搜索包含“preflight”一词的命令：
...
* 关于处理共享 UI 代码的一系列教程：[创建您的 Compose Multiplatform 应用](compose-multiplatform-create-first-app.md)
* 关于在处理共享代码的同时使用原生 UI 的一系列教程：[创建您的 Kotlin Multiplatform 应用](multiplatform-create-first-app.md)
