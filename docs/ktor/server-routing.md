...
[安装](#install_plugin)路由插件后，你可以在 `routing` 内部调用 [route](https://api.ktor.io/ktor-server-core/io.ktor.server.routing/route.html) 函数来定义路由：
```kotlin
import io.ktor.server.routing.*
import io.ktor.http.*
...
Ktor 还提供了一系列函数，使定义路由处理程序变得更加容易和简洁。例如，你可以使用 [get](https://api.ktor.io/ktor-server-core/io.ktor.server.routing/get.html) 函数替换前面的代码，现在它只需要接收 URL 和处理请求的代码：
...
* **HTTP 动词**
  ...
  选择 HTTP 动词，例如 `GET`、`POST`、`PUT` 等。最方便的方法是使用专用的动词函数，如 `get`、`post`、`put` 等。

* **路径模式**

  指定用于[匹配 URL 路径](#match_url)的路径模式，例如 `/hello`、`/customer/{id}`。你可以将路径模式直接传递给 `get`/`post` 等函数，也可以使用 `route` 函数来对[路由处理程序](#multiple_routes)进行分组并定义[嵌套路由](#nested_routes)。
...
传递给[路由](#define_route)函数（`route`、`get`、`post` 等）的路径模式用于匹配 URL 的 *path*（路径）部分。路径可以包含一系列由斜杠 `/` 字符分隔的路径段。

> 请注意，Ktor 会区分带尾随斜杠和不带尾随斜杠的路径。你可以通过[安装](server-plugins.md#install) `IgnoreTrailingSlash` 插件来更改此行为。

以下是几个路径示例：
* `/hello`  
  包含单个路径段的路径。
* `/order/shipment`  
  包含多个路径段的路径。你可以按原样将此类路径传递给 [route/get/等](#define_route)函数，或者通过[嵌套](#multiple_routes)多个 `route` 函数来组织子路由。
* `/user/{login}`  
  带有 `login` [路径参数](#path_parameter)的路径，其值可以在路由处理程序内部访问。
* `/user/*`  
  带有[通配符](#wildcard)的路径，可匹配任何路径段。
* `/user/{...}`  
  带有[尾随通配符 (tailcard)](#tailcard)的路径，可匹配 URL 路径的所有其余部分。
* `/user/{param...}`  
  包含[带尾随通配符的路径参数](#path_parameter_tailcard)的路径。
* `Regex("/.+/hello")`  
  包含[正则表达式](#regular_expression)的路径，用于匹配路径段，直到并包括最后一次出现的 `/hello`。
...
* `/user/{login?}` 既匹配 `/user/john`，也匹配 `/user`。
   > 请注意，可选路径参数 `{param?}` 只能在路径末尾使用。
   >
   {type="note"}
...
> 如果请求包含查询字符串，`call.parameters` 也会包含该查询字符串的参数。要了解如何在处理程序内部访问查询字符串及其参数，请参阅[查询参数](server-requests.md#query_parameters)。

### 带尾随通配符的路径参数 {id="path_parameter_tailcard"}

带尾随通配符的路径参数 (`{param...}`) 匹配 URL 路径的所有其余部分，并使用 `param` 作为键，将每个路径段的多个值放入参数中。例如，`/user/{param...}` 匹配 `/user/john/settings`。
要在路由处理程序内部访问路径段的值，请使用 `call.parameters.getAll("param")`。对于上述示例，`getAll` 函数将返回一个包含 *john* 和 *settings* 值的数组。
...
#### 在处理程序中访问路径部分 {id="accessing-path-parts-in-handler"}

在正则表达式中，命名组是一种捕获字符串中匹配模式的特定部分并为其分配名称的方法。
语法 `(?<name>pattern)` 用于定义命名组，其中 `name` 是组名，而 `pattern` 是匹配该组的正则表达式模式。
...
无论你如何进行分组，Ktor 还允许你将子路由作为 `route` 函数的参数。这对于定义逻辑上属于其他资源子资源的资源非常有用。
以下示例向我们展示了如何响应针对 `/order/shipment` 的 `GET` 和 `POST` 请求：
...
传递给[路由](#define_route)函数（`route`、`get`、`post` 等）的路径模式用于匹配 URL 的 *path*（路径）部分。路径可以包含由斜杠 `/` 字符分隔的一系列路径段。
...
配置[日志记录](server-logging.md)后，Ktor 会启用路由跟踪，帮助你确定某些路由未被执行的原因。
...
> 要在 [Native 服务器](server-native.md)上启用路由跟踪，请在[运行](server-run.md)应用程序时向 `KTOR_LOG_LEVEL` 环境变量传递 *TRACE* 值。