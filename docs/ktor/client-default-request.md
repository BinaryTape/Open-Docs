...
[`DefaultRequest`](https://api.ktor.io/ktor-client-core/io.ktor.client.plugins/-default-request/index.html) 插件允许您为所有 [请求](client-requests.md) 配置默认参数：指定基础 URL、添加标头、配置查询参数等。

## 添加依赖项 {id="add_dependencies"}

`DefaultRequest` 仅需要 [ktor-client-core](client-dependencies.md) 构件，不需要任何特定的依赖项。

## 安装 DefaultRequest {id="install_plugin"}

要安装 `DefaultRequest`，请在 [客户端配置块](client-create-and-configure.md#configure-client) 中将其传递给 `install` 函数：
...
### 替换现有配置 {id="default_request_replace"}
...
`DefaultRequest` 允许您配置 URL 的基础部分，该部分将与 [请求 URL](client-requests.md#url) 合并。
例如，下面的 `url` 函数为所有请求指定了基础 URL：
...
为避免重复标头，您可以使用 `appendIfNameAbsent`、`appendIfNameAndValueAbsent` 和 `contains` 函数：
...
您可以 [构建带有 Unix 域套接字的单个请求](client-requests.md#specify-a-unix-domain-socket)，
但也可以使用套接字参数配置默认请求。
