...
## 何时使用上下文隔离 {id="when-to-use-context-isolation"}

- **SDK/库开发** —— 你的库在内部使用 Koin
- **避免冲突** —— 宿主应用也可能使用 Koin
- **封装** —— 保持你的 DI 容器为私有

## 创建隔离上下文 {id="creating-an-isolated-context"}
...
## 自定义 KoinComponent {id="custom-koincomponent"}
...
## 测试隔离上下文 {id="testing-isolated-context"}
