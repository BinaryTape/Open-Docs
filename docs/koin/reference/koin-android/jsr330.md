...
### @Singleton 和 @Inject {id="singleton-and-inject"}
...
### 自定义 @Qualifier 注解 {id="custom-qualifier-annotations"}
...
## 从 Hilt 迁移 {id="migration-from-hilt"}

### 之前 (Hilt) {id="before-hilt"}
...
### 之后 (Koin 搭配 JSR-330) {id="after-koin-with-jsr-330"}
...
## 编译器插件支持 {id="compiler-plugin-support"}
