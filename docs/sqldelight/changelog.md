...
- [PostgreSQL 方言] 添加谓词来检测 SQL 表达式是否可解析为 JSON (#5843 by [Griffio][griffio])
...
### 变更
- 开发中的快照现在已发布到 Central Portal 快照版本库：https://central.sonatype.com/repository/maven-snapshots/。
...
- [编译器] 将 SQL 形参绑定的复杂度从 O(n²) 优化到 O(n) (#5898 by [Chen Frenkel][chenf7])
...
- [PostgreSQL 方言] sqlite windows function (#2799 by [Griffio][griffio])
...
- [驱动] 为方言作者暴露更多 JDBC/R2DBC 语句方法 (#5098 by [Philip Wedemann][hfhbd])
...
- [PostgreSQL 方言] 修复 4932 postgresql default constraint functions (#4934 by [Griffio][griffio])
...
- [PostgreSQL 方言] 修复 5018 PostgreSql add Primary Key 不可空类型 (#5020 by [Griffio][griffio])
...
- [IDE 插件] 不将 kotlin 库打包到 intellij 插件中，因为它已经是插件依赖项 (#5126)
...
- [PostgreSQL 方言] 将 PostgreSql temporal types 添加到 SqlBinaryExpr (#4657 by [Griifio][griffio])
...
- [JDBC 驱动] 添加 Language 注解以实现更愉快的 SQL 编辑 (#4602 by [Marius Volkhart][MariusV])
- [原生驱动] 原生驱动：添加对 linuxArm64 的支持 (#4792 by [Philip Wedemann][hfhbd])
- [Android 驱动] 为 AndroidSqliteDriver 添加 windowSizeBytes 形参 (#4804 by [Benoit Lubek][BoD])
- [Paging3 扩展] 特性：为 OffsetQueryPagingSource 添加 initialOffset (#4802 by [Mohamad Jaara][MohamadJaara])
...
- [IDE 插件] 如果已设置，则使用 IDEA Project JDK (#4689 by [Griffio][griffio])
- [IDE 插件] 修复 IDEA 2023.2 及更高版本中的“Unknown element type: TYPE_NAME”错误 (#4727)
- [IDE 插件] 修复了与 2023.2 的一些兼容性问题
...
- [JDBC 驱动] 修复 JdbcSqliteDriver url 解析当选择 ConnectionManager 类型时 (#4656 by [Matthew Nelson][05nelsonm])
...
- [IDE 插件] 允许用户在 gradle tooling 连接失败时显示堆栈跟踪 (#4383)
...
- [IDE 插件] 警告 DELETE 或 UPDATE 中缺少 WHERE 子句 (#4058 by [Alexander Perfilyev][aperfilyev])
...
- [IDE 插件] 忽略编译器在失效状态下的 IDE 运行 (#4016)
- [IDE 插件] 添加对 IntelliJ 2023.1 的支持 (#4037 by [Madis Pink][madisp])
- [IDE 插件] 在列重命名时重命名命名实参使用 (#4027 by [Alexander Perfilyev][aperfilyev])
...
### 变更
- [RDBC 驱动] 暴露连接 (by [Philip Wedemann][hfhbd])
...
### 修复
- [RDBC 驱动] 使用 javaObjectType 代替 (by [Philip Wedemann][hfhbd])
- [RDBC 驱动] 修复 bindStatement 中的原始空值 (by [Philip Wedemann][hfhbd])
- [RDBC 驱动] 支持 R2DBC 1.0 (by [Philip Wedemann][hfhbd])
...
- [编译器] 如果可能，推断绑定形参的 Kotlin 类型，否则抛出更好的错误消息 (#3413 by [Philip Wedemann][hfhbd])
- [编译器] 不允许 ABS("foo") (#3430 by [Philip Wedemann][hfhbd])
- [编译器] 支持从其他形参推断 kotlin 类型 (#3431 by [Philip Wedemann][hfhbd])
...
- [JS 驱动] 为 SqlJs worker 添加了驱动 (#3203 by [Derek Ellis][dellisd])
- [Gradle 插件] 暴露 SQLDelight 任务的类路径
- [Gradle 插件] 添加了一个用于压缩迁移的 gradle 任务
...
- [Gradle 插件] Gradle Verify Task：当数据库文件缺失时抛出异常 (#3126 by [Niklas Baudy][vanniktech])
...
- [IDE 插件] 使用 java home 用于 gradle tooling API (#3078)
...
- [编译器] 正确推断复合选择中的实参类型 (#3096)
- [编译器] Common tables 不生成数据类，因此不返回它们 (#3097)
...
### 新增
- [编译器] 支持在分组语句末尾返回
- [编译器] 通过方言模块支持编译器扩展，并添加一个 SQLite JSON 扩展 (#1379, #2087)
...
- [SQLite] 不在 WHERE 子句中替换可空形参的相等性检测 (#1490 by [Eliezer Graber][eygraber])
...
- [IDE 插件] 确保传递生成的代码在 IDE 中也更新 (#1837)
- [IDE 插件] 更新方言时使索引失效
...
- 原生类型现在必须导入（例如 `INTEGER AS Boolean`，您必须 `import kotlin.Boolean`），一些以前支持的类型现在需要适配器。大多数转换（例如 `IntColumnAdapter` 用于 `Integer AS kotlin.Int`）的原始适配器可在 `app.cash.sqldelight:primitive-adapters:2.0.0-alpha01` 中找到。
...
- [JDBC 驱动] 为 JdbcDriver 添加 common 类型 (by [Marius Volkhart][MariusV])
...
- [HSQL] 自定义 HSQL insert 查询 (by [Marius Volkhart][MariusV])
...
- [编译器] 移除生成的代码块中自定义 toString 的生成。 (by [Paul Woitaschek][PaulWoitaschek])
- [JS 驱动] 移除 sql.js 依赖项从 sqljs-driver (by [Derek Ellis][dellisd])
...
- [IDE 插件] 高亮 import 关键字 (by [Alexander Perfilyev][aperfilyev])
...
- [IDE 插件] 除非发生 gradle 同步，否则不初始化文件索引
...
- [编译器] 添加关键字名字修饰到 select 函数形参 (#2759 by [Alexander Perfilyev][aperfilyev])
- [编译器] 修复 migration adapter 的 packageName (by [Philip Wedemann][hfhbd])
- [编译器] 在属性而不是类型上发出注解 (#2798 by [Alexander Perfilyev][aperfilyev])
- [编译器] 在传递给 Query 子类型之前对实参进行排序 (#2379 by [Alexander Perfilyev][aperfilyev])
...
- 属性名如果也是软关键字，现在将以 `_` 为后缀。例如 `value` 将被暴露为 `value_`

### 修复
- [编译器] 不为重复数组形参提取变量 (by [Alexander Perfilyev][aperfilyev])
- [Gradle 插件] add kotlin.mpp.enableCompatibilityMetadataVariant. (#2628 by [Martin Bonnin][martinbonnin])
- [IDE 插件] Find usages 处理需要读取操作
...
- [IDE 插件] 混合命名和位置形参检查 (by [Alexander Perfilyev][aperfilyev])
...
- [PostgreSQL 方言] postgres 重命名，删除列 (by [Juan Liska][pabl0rg])
...
- [运行时] 提供 SqlDriver 作为 AfterVersion 的形参 (#2534, 2614 by [Ahmed El-Helw][ahmedre])
...
- [IDE 插件] 当可空类型与非空类型连接时，不警告“Join compares two columns of different types” (#2550 by [Piotr Chmielowski][pchmielowski])
...
- [IDE 插件] 在 insert 语句中对形参中的 Kotlin 关键字进行名字修饰 (#2433 by [Alexander Perfilyev][aperfilyev])
...
- [IDE 插件] 修复 update live template (by [Ilias Redissi][IliasRedissi])
...
- [IDE 插件] 添加形参类型和注解 (by [Alexander Perfilyev][aperfilyev])
...
- [IDE 插件] 添加列名、stmt 标识符、函数名的高亮显示 (by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 添加剩余的查询生成操作 (#489 by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 显示 insert 语句的形参提示 (by [Alexander Perfilyev][aperfilyev])
...
- [JS 驱动] 升级 sql.js 到 1.5.0

### 修复
- [JDBC SQLite 驱动] 在清除 ThreadLocal 之前调用 connection 上的 close() (#2444 by [Hannes Struß][hannesstruss])
- [RX 扩展] 修复订阅/释放竞争泄漏 (#2403 by [Pierre Yves Ricau][pyricau])
- [协程扩展] 确保在通知之前注册查询监听器
- [编译器] 排序 notifyQueries 以获得一致的 kotlin 输出文件 (by [Jiayu Chen][thomascjy])
- [编译器] 不为 select query class 属性注解 @JvmField (by [Eliezer Graber][eygraber])
- [IDE 插件] 修复 import optimizer (#2350 by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 修复未使用列检查 (by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 为 import inspection 和 class annotator 添加嵌套类支持 (by [Alexander Perfilyev][aperfilyev])
...
- [IDE 插件] 修复将任何文本复制粘贴到 create table stmt 时插入空白行的问题 (#2431 by [Alexander Perfilyev][aperfilyev])

## [1.5.0] - 2021-04-23
### 新增
- [SQLite Javascript 驱动] 启用 sqljs-driver publication (#1667 by [Derek Ellis][dellisd])
...
- [IDE 插件] 为未知类型添加 import quick fix (#683 by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 添加未使用 import inspection (#1161 by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 添加未使用 query inspection (by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 添加未使用 column inspection (#569 by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 复制/粘贴时自动引入 import (#684 by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 当 gradle/intellij 插件版本不兼容时，弹出气泡通知
- [IDE 插件] Insert Into ... VALUES(?) 形参提示 (#506 by [Alexander Perfilyev][aperfilyev])
- [IDE 插件] 内联形参提示 (by [Alexander Perfilyev][aperfilyev])
...
- [Gradle 插件] 正确支持 android 变体，并移除对 KMM target-specific sql 的支持 (#1039)
...
- [编译器] 不生成带类型注解的 mapper lambda (#1957)
- [编译器] 如果 custom queries 会冲突，则使用文件名为额外的包后缀 (#1057, #1278)
- [编译器] 确保外键级联导致查询监听器被通知 (#1325, #1485)
- [编译器] 如果联合两种相同类型，则返回表类型 (#1342)
- [编译器] 确保 ifnull 和 coalesce 的形参可空 (#1263)
...
- [JDBC 驱动] JDBC Driver 假定 autoCommit 为 true (#2041)
- [JDBC 驱动] 确保在异常时关闭连接 (#2306)
- [IDE 插件] 修复 Windows 上由于路径分隔符 bug 导致 GoToDeclaration/FindUsages 损坏的问题 (#2054 by [Angus Holder][AngusH])
...
- [IDE 插件] 此外，确保在运行 gradle 操作之前项目未被处置 (#2155)
- [IDE 插件] 可空类型上的算术运算也应是可空 (#1853)
- [IDE 插件] 使“expand * intention”与附加投影一起工作 (#2173 by [Alexander Perfilyev][aperfilyev])
...
- [Gradle 插件] SqlDeslight gradle 插件不支持 Configuration Caching (CoCa)。 (#1947 by [Stéphane Nicolas][stephanenicolas])
- [SQLite JDBC 驱动] SQLException: database in auto-commit mode (#1832)
- [协程扩展] 修复 coroutines-extensions 的 IR 后端 (#1918 by [Derek Ellis][dellisd])
...
### 修复
- [IDE 插件] 使用 gradle tooling API 促进 IDE 和 gradle 之间的数据共享
...
* 修复: [编译器] #1469 Query 的 SynetheticAccessor 创建。
* 修复: [JVM 驱动] 修复内存泄漏。
* 注意: 协程扩展构件要求将 kotlinx bintray maven 版本库添加到您的 buildscript。
...
* 修复: [Gradle] #1348 运行 verify 任务时打印“Could not retrieve functions”。
* 修复: [Compile] #1405 查询包含 FTS 表连接时无法构建项目。
...
* 新增: [运行时] #1267 Logging driver decorator。
* 修复: [编译器] #1254 拆分超过 2^16 个字符的字符串字面量。
* 修复: [Gradle] #1260 在 Multiplatform 项目中，生成的源被识别为 iOS 源。
* 修复: [IDE] #1290 `kotlin.KotlinNullPointerException` 在 `CopyAsSqliteAction.kt:43`。
* 修复: [Gradle] #1268 在最近版本中运行 `linkDebugFrameworkIos*` 任务失败。
...
* 增强: [运行时] #1195 Native Driver/Runtime Arm32。
 ...
 * 新增: 没有实参的 Delete、Update 和 Insert 语句生成编译语句。
 * 修复: 子查询中使用的视图中的 USING 子句不再报错。
 * 修复: 移除生成的 Mapper 上的重复类型。
 * 修复: 子查询可以在检测实参的表达式中使用。
 ...
 * 新增: 为视图名称发出字符串常量。
 * 修复: 需要工厂的视图上的查询现在正确要求这些工厂作为实参。
 * 修复: 验证 insert 实参的数量与指定列的数量匹配。
 ...
 * 新增: 生成的 mappers 和 factory methods 上的 Javadoc。
 ...
 * 新增: 命名语句上的 Javadoc 将出现在生成的 Strings 中。
 * 修复: 生成的 view models 包含可空性注解。
 * 修复: 联合生成的代码具有正确的类型和可空性，以支持所有可能的列。
 * 修复: sum 和 round SQLite 函数在生成的代码中具有正确的类型。
 * 修复: CAST's，内部 selects bug 修复。
 * 修复: CREATE TABLE 语句中的自动补全。
 * 修复: SQLite 关键字可以在包中使用。

## [0.4.2] - 2016-06-16

 * 新增: Marshal 可以从 factory 创建。
 * 修复: IntelliJ 插件生成 factory methods 时泛型顺序正确。
 * 修复: 函数名可以使用任何大小写。
...
## [0.3.0] - 2016-04-26
  ...
  * 修复: 自动补全功能正常。
  * 修复: SQLDelight model files 在 .sq 文件编辑时更新。
  * 移除: 不再支持附加数据库。
 ...
 * 新增: 更新到 Kotlin 1.0 final。
 * 修复: 以非失败方式报告 'sqldelight' 文件夹结构问题。
 * 修复: 禁止名为 `table_name` 的列。它们生成的常量与表名常量冲突。
 * 修复: 确保 IDE 插件立即生成 model classes，无论 `.sq` 文件是否打开。
 * 修复: 支持 IDE 和 Gradle 插件中的 Windows 路径。

## [0.1.2] - 2016-02-13

 * 修复: 移除阻止 Gradle 插件在大多数项目中使用s的代码。
