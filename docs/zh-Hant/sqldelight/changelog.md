...
- [PostgreSQL 方方言] 新增對 Postgres 觸發器的有限支援（#5932 由 @griffio 提供）
...
- [IntelliJ 外掛程式] 避免在 copy-to-sqlite 邊欄操作中發生執行緒相關的當機（#5901 由 @griffio 提供）
...
- [PostgreSQL 方言] 從時間型別提取 Postrgesql 資料（#5273 由 @griffio 提供）
...
### 新增
- [編譯器] 執行 SELECT 時新增對多列運算式的支援（#4453 由 @Adriel-M 提供）
...
- [PostgreSQL 方言] 新增對 PostgreSQL SELECT DISTINCT ON 語法的支援（#4584 由 @griffio 提供）
...
- [Paging3 擴充套件] 特性：為 OffsetQueryPagingSource 新增 initialOffset（#4802 由 @MohamadJaara 提供）
...
- [執行階段] 移除對舊版 MM 的支援（#4148 由 @hfhbd 提供）
...
- [IDE 外掛程式] 確保如果專案未配置，醒目提示存取器不會執行（#2981、#2976）
...
- 現在必須匯入原始型別（例如 `INTEGER AS Boolean` 必須 `import kotlin.Boolean`），一些先前支援的型別現在需要配接器。大多數轉換的原始配接器可在 `app.cash.sqldelight:primitive-adapters:2.0.0-alpha01` 中找到（例如用於 `Integer AS kotlin.Int` 的 `IntColumnAdapter`）。
...
- [IDE 外掛程式] 在 Gradle 同步發生前不初始化檔案索引
...
- [Gradle 外掛程式] 新增執行階段時明確支援 js/android 外掛程式（由 @ZacSweers 提供）
...
- [Gradle 外掛程式] 將 Gradle 外掛程式大小從 60mb 減少到 13mb。
...
- [IDE 外掛程式] 如果 SQLDelight 檔案被移至非 SQLDelight 模組，請勿嘗試產生程式碼
- [IDE 外掛程式] 在 IDE 中忽略程式碼產生錯誤
...
- [IDE 外掛程式] 變更 .sq 檔案時外掛程式未重新產生資料庫介面（#1945）
...
* 修復：[IDE] #1290 CopyAsSqliteAction.kt:43 中的 kotlin.KotlinNullPointerException。
...
* 新功能：Kotlin 多平台支援。
* 新功能：Android、iOS 與 JVM SQLite 驅動程式產物。
* 新功能：交易 API。
 ...
 * 新功能：對 insert、update、delete、index 與 trigger 陳述式所使用的列進行編譯時驗證。
