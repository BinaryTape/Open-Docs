...
- [編譯器] 改進編譯器錯誤訊息，在需要 CAST 時包含原始檔案位置（#5979 由 @griffio 提供）
- [PostgreSQL 方言] 新增對 Postgres JSON 運算子路徑提取的支援（#5971 由 @griffio 提供）
- [SQLite 方言] 為使用通用資料表運算式 (Common Table Expressions) 的 MATERIALIZED 查詢計畫提示新增 SQLite 3.35 支援（#5961 由 @griffio 提供）
...
- [PostgreSQL 方言] 新增對 Postgres 列舉 (Enum) 的支援（#5935 由 @griffio 提供）
...
### 修復
- [編譯器] 修復使用包含通用資料表運算式的檢視 (View) 時的堆疊溢位（#5928 由 @griffio 提供）
...
- [PostgreSQL 方言] PostgreSql 型別轉換 (type casting)（#5089 由 @griffio 提供）
...
- [PostgreSQL 方言] PostgreSql 正規表示式 (Regex) 運算子（#5137 由 @griffio 提供）
...
- [PostgreSQL 方言] SQLite 視窗函式 (window function)（#2799 由 @griffio 提供）
...
- [PostgreSQL 方言] 新增 Postgres 正規表示式 (Regex) 函式定義（#5025 由 @MariusVolkhart 提供）
...
- [PostgreSQL 方言] 修復 5018 PostgreSql 新增主鍵 (Primary Key) 不可為 null 型別（#5020 由 @griffio 提供）
...
- [IDE 外掛程式] 在 2023.2 上使用正確的虛設常式 (stub) 索引鍵進行 Kotlin 型別解析（#4416）
...
- [IDE 外掛程式] 修復展開萬用字元 (wildcard) 意圖使其可與限定詞搭配使用（#3979 由 @aperfilyev 提供）
...
- [Gradle 外掛程式] 對下游專案隱藏組態 (configuration)
...
- 方言 (Dialect) 現在像實際的 Gradle 相依項一樣被引用。
```groovy
sqldelight {
  MyDatabase {
...
- [JS 驅動程式] 為 SqlJs 背景工作執行緒 (worker) 新增驅動程式（#3203 由 @dellisd 提供）
- [Gradle 外掛程式] 公開 SQLDelight 任務的類別路徑 (classpath)
- [Gradle 外掛程式] 為壓縮遷移 (squashing migrations) 新增 Gradle 任務
...
- [IDE 外掛程式] 最佳化檢查器 (inspector)，並使其在遇到預期的例外型別時能安靜地失敗（#3121）
- [IDE 外掛程式] 刪除應為產生目錄的檔案（#3198）
- [IDE 外掛程式] 修復一個非安全運算子 (not-safe operator) 呼叫
- [編譯器] 確保帶有 RETURNING 陳述式的更新與刪除會執行查詢。（#3084）
- [編譯器] 在複合選擇 (compound select) 中正確推論引數型別（#3096）
...
- [編譯器] 支援為標記列產生值型別 (value type)
- [編譯器] 增加對樂觀鎖 (optimistic lock) 與驗證的支援（#1952）
- [編譯器] 支援多重更新 (multi-update) 陳述式
- [PostgreSQL] 支援 PostgreSQL returning 陳述式
- [PostgreSQL] 支援 PostgreSQL 日期型別
- [PostgreSQL] 支援 pg 間隔 (interval)
- [PostgreSQL] 支援 PG 布林 (boolean) 並修復 alter table 上的插入問題
...
- [PostgreSQL] 支援在通用資料表運算式 (common table expression) 內傳回查詢（#2471）
...
- 方言 (Dialect) 現在是相依項，您可以在 Gradle 中指定：
...
- [IDE 外掛程式] 新增匯入提示操作 (import hint action)（由 @aperfilyev 提供）
...
- [HSQL] 為產生的子句 (generated clause) 新增 HSQL 支援（由 @MariusVolkhart 提供）
- [HSQL] 增加對 HSQL 中具名參數 (named parameter) 的支援（由 @MariusVolkhart 提供）
...
- [IDE 外掛程式] 若專案索引尚未初始化，則不要嘗試檢查不相符的列
- [IDE 外掛程式] 若 Gradle 同步發生前，不要初始化檔案索引
- [IDE 外掛程式] 若 Gradle 同步開始，則取消 SQLDelight 匯入
- [IDE 外掛程式] 在執行復原操作的執行緒之外重新產生資料庫
- [IDE 外掛程式] 如果參照無法解析，則使用空白的 Java 型別
...
- [IDE 外掛程式] 從 insert 陳述式顯示參數提示（由 @aperfilyev 提供）
- [IDE 外掛程式] 資料表別名意圖操作（由 @aperfilyev 提供）
- [IDE 外掛程式] 限定列名意圖 (Qualify column name intention)（由 @aperfilyev 提供）
...
- [IDE 外掛程式] 內嵌參數提示 (Inline parameter hints)（由 @aperfilyev 提供）
- [執行階段] 在執行階段中包含一個 API，用於執行帶有回呼的遷移（#1844）

### 變更
- [編譯器] 智慧轉型 (Smart cast) 「IS NOT NULL」 查詢（#867）
- [編譯器] 防止在執行時會失敗的關鍵字使用（#1471、#1629）
- [Gradle 外掛程式] 將 Gradle 外掛程式大小從 60 MB 減少到 13 MB。
- [Gradle 外掛程式] 正確支援 Android 變體 (variant)，並移除對 KMM 目標特定 SQL 的支援（#1039）
...
- [IDE 外掛程式] 若 SQLDelight 檔案被移至非 sqldelight 模組，請勿嘗試程式碼產生
...
- [IDE 外掛程式] 讓 「展開 * 意圖 (expand * intention)」 可與額外的投影 (projection) 配合使用（#2173 由 @aperfilyev 提供）
- [IDE 外掛程式] 若 Kotlin 解析在 GoTo 期間失敗，請勿嘗試前往 SQLDelight 檔案
...
- [IDE 外掛程式] 若無處可移動游標，則不執行任何操作而不是當機（#1994）
...
- [Gradle 外掛程式] SQLDelight Gradle 外掛程式不支援組態快取 (Configuration Caching, CoCa)。（#1947 由 @stephanenicolas 提供）
...
* 修復：[Gradle] #1380 乾淨組建 (clean build) 有時會失敗。
...
* 修復：[Gradle] #1243 在 Gradle 隨需組態 (configure on demand) 下使用 SQLDelight 的修復。
* 修復：[Gradle] #1385 在累加註解處理 (incremental annotation processing) 下使用 SQLDelight 的修復。
* 修復：[Gradle] 允許 Gradle 任務快取 (cache)。
 ...
 * 新功能：陳述式摺疊 (folding) 於 IDE。
 * 新功能：布林型別現在會自動處理。
 * 修復：從程式碼產生中移除已棄用的編組程式 (marshals)。
 ...
 * 修復：原始型別在參數中的如果是可 null 的，則會被裝箱 (boxed)。
