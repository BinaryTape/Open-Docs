...
## 2) 啟動 Playwright MCP 伺服器 {id="2-start-the-playwright-mcp-server"}
...
## 3) 從 Koog 連接並執行 agent {id="3-connect-from-koog-and-run-the-agent"}
...
## 4) 關閉 MCP 處理程序 {id="4-shut-down-the-mcp-process"}
在執行結束時，請務必清理外部處理程序。
