...
在本教學中，我們將使用 Kotlin 中的 **Koog** Agent 構建一個小型銀行助手。
你將學習如何：
- 定義領域模型與範例資料
...
val strategy = strategy<String, String>("banking assistant") {
    ...
    val transferMoney by subgraphWithTask<ClassifiedBankRequest, String>(
        ...
        llmModel = OpenAIModels.Chat.GPT4o  // 對轉帳使用更強大的模型
    ) { request ->
        """
...
$transactionAnalysisPrompt")
    ...
    maxAgentIterations = 50  // 允許複雜的多步操作
)
...
2. 構建將 LLM 與工具結合以完成特定任務的單一目的 Agent
3. 使用策略 (strategies) 與子圖 (subgraphs) 針對複雜工作流程實作圖型 Agent
...
4. 使用者體驗：對於轉帳等關鍵操作，應包含確認步驟
5. 模組化：將關注點分離到不同的工具與 Agent 中，以獲得更好的可維護性