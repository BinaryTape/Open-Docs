...
## 策略：多重工具呼叫（含可選壓縮） {id="strategy-multiple-tool-calls-with-optional-compression"}
...
## 嘗試強制平行呼叫 {id="try-forcing-parallel-calls"}
...
## 使用 Ollama 執行 {id="running-with-ollama"}
