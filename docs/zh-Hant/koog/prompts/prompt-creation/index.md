...
Koog 使用型別安全的 Kotlin DSL 來建立提示 (prompt)，並可控制訊息類型、其順序與內容。

這些提示讓您可以使用多則訊息預先配置對話歷程記錄，提供多模態內容、範例、工具呼叫及其結果。

## 基本結構

`prompt()` 函式會建立一個具有唯一 ID 與訊息清單的 Prompt 物件：
...
Kotlin DSL 支援以下類型的訊息，每一種都對應到對話中的特定角色：
...
要建立助理訊息，請將字串作為引數提供給 `assistant()` 函式：
...
在組建 `system()`、`user()` 或 `assistant()` 訊息時，您可以使用
輔助的 [文字建置函式](api:prompt-model::ai.koog.prompt.text.TextContentBuilder)
進行豐富文字格式化。
...
您也可以使用 [Markdown](api:prompt-markdown::ai.koog.prompt.markdown.markdown) 
與 [XML](api:prompt-xml::ai.koog.prompt.xml.xml) 建置器來以對應的格式新增內容。
...
您可以透過呼叫 `prompt()` 函式並將現有提示作為引數傳入來擴充現有提示：
