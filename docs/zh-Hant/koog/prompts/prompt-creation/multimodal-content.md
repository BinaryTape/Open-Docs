...
- 建立並將 `ContentPart` 物件傳遞給函式，以便對附件參數進行自訂控制。
...
如果您將 URL 或檔案路徑傳遞給附件函式，Koog 會根據副檔名自動建構相應的附件參數。
...
您可以為每個附件建立特定實作的執行個體，配置其參數，並將其傳遞給相應的 `image()`、`audio()`、`video()` 或 `file()` 函式。
