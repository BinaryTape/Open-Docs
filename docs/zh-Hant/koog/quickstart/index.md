...
??? tip "Nightly 組建"
    ...
    來自 develop 分支的 Nightly 組建已發佈至 [JetBrains Grazie Maven](https://packages.jetbrains.team/maven/p/grazi/grazie-platform-public) 存儲庫。
    
    若要使用 Nightly 組建，請將以下存儲庫新增至您的組建組態：
    `https://packages.jetbrains.team/maven/p/grazi/grazie-platform-public`。
    
    接著將您的 Koog 相依性更新至所需的 Nightly 版本。Nightly 版本遵循以下模式：
    `[next-major-version]-develop-[date]-[time]`。
    
    您可以在[此處](https://packages.jetbrains.team/maven/p/grazi/grazie-platform-public/ai/koog/koog-agents/)瀏覽可用的 Nightly 組建。

## 設定 API 金鑰

Koog 需要來自[受支援的 LLM 提供者](../llm-providers.md)的 API 金鑰，或是在本機執行的 LLM。
...
=== "OpenAI"
    ...
    獲取您的 [OpenAI API 金鑰](https://platform.openai.com/api-keys) 並將其指派給 `OPENAI_API_KEY` 環境變數。
...
=== "Anthropic"
    ...
    獲取您的 [Anthropic API 金鑰](https://console.anthropic.com/settings/keys) 並將其指派給 `ANTHROPIC_API_KEY` 環境變數。
...
=== "Google"
    ...
    獲取您的 [Gemini API 金鑰](https://aistudio.google.com/app/api-keys) 並將其指派給 `GOOGLE_API_KEY` 環境變數。
...
=== "DeepSeek"
    ...
    獲取您的 [DeepSeek API 金鑰](https://platform.deepseek.com/api_keys) 並將其指派給 `DEEPSEEK_API_KEY` 環境變數。
...
=== "OpenRouter"
    ...
    獲取您的 [OpenRouter API 金鑰](https://openrouter.ai/keys) 並將其指派給 `OPENROUTER_API_KEY` 環境變數。
...
=== "Mistral"
    ...
    獲取您的 [Mistral API 金鑰](https://console.mistral.ai/api-keys) 並將其指派給 `MISTRAL_API_KEY` 環境變數。
...
=== "OpenAI"
    ...
    以下範例透過 OpenAI API 使用 [`GPT-4o`](https://platform.openai.com/docs/models/gpt-4o) 模型建立並執行一個簡單的 Koog 代理。
    ...
    fun main() = runBlocking {
        // 從 OPENAI_API_KEY 環境變數獲取 OpenAI API 金鑰
...
=== "Anthropic"
    ...
    以下範例透過 Anthropic API 使用 [`Claude Opus 4.1`](https://www.anthropic.com/news/claude-opus-4-1) 模型建立並執行一個簡單的 Koog 代理。
    ...
    ```kotlin
    fun main() = runBlocking {
        // 從 ANTHROPIC_API_KEY 環境變數獲取 Anthropic API 金鑰
...
=== "Google"
    ...
    以下範例透過 Gemini API 使用 [`Gemini 2.5 Pro`](https://cloud.google.com/vertex-ai/generative-ai/docs/models/gemini/2-5-pro) 模型建立並執行一個簡單的 Koog 代理。
    ...
    ```kotlin
    fun main() = runBlocking {
        // 從 GOOGLE_API_KEY 環境變數獲取 Gemini API 金鑰
...
=== "DeepSeek"
    ...
    以下範例透過 DeepSeek API 使用 `deepseek-chat` 模型建立並執行一個簡單的 Koog 代理。
    ...
    ```kotlin
    fun main() = runBlocking {
        // 從 DEEPSEEK_API_KEY 環境變數獲取 DeepSeek API 金鑰
...
=== "OpenRouter"
    ...
    以下範例透過 OpenRouter API 使用 [`GPT-4o`](https://openrouter.ai/openai/gpt-4o) 模型建立並執行一個簡單的 Koog 代理。
    ...
    ```kotlin
    fun main() = runBlocking {
        // 從 OPENROUTER_API_KEY 環境變數獲取 OpenRouter API 金鑰
...
=== "Bedrock"
    ...
    以下範例透過 Bedrock API 使用 [`Claude Sonnet 4.5`](https://www.anthropic.com/news/claude-sonnet-4-5) 模型建立並執行一個簡單的 Koog 代理。
    ...
    ```kotlin
    fun main() = runBlocking {
        // 從 BEDROCK_API_KEY 環境變數獲取 Bedrock API 金鑰
...
=== "Mistral"
    ...
    以下範例透過 Mistral AI API 使用 [`Mistral Medium 3.1`](https://docs.mistral.ai/models/mistral-medium-3-1-25-08) 模型建立並執行一個簡單的 Koog 代理。
    ...
    ```kotlin
    fun main() = runBlocking {
        // 從 MISTRAL_API_KEY 環境變數獲取 Mistral AI API 金鑰
...
=== "Ollama"
    ...
    以下範例使用經由 Ollama 在本機執行的 [`llama3.2`](https://ollama.com/library/llama3.2) 模型建立並執行一個簡單的 Koog 代理。
...
- [建置具有記憶功能的聊天代理](chat-agent.md) — 建立一個能記住先前訊息的對話式代理
- 進一步了解[建置代理](../agents/index.md)