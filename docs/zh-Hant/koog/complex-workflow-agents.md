...
除了基本代理程式之外，`AIAgent` 類別還允許您透過定義自訂策略、工具、設定以及自訂的輸入/輸出類型來建構處理複雜工作流程的代理程式。

!!! tip
    如果您剛接觸 Koog 並想建立最簡單的代理程式，請從 [基本代理程式](basic-agents.md) 開始。
...
- 您擁有來自用於實作 AI 代理程式的 LLM 提供者的有效 API 金鑰。有關所有可用提供者的列表，請參閱 [LLM 提供者](llm-providers.md)。
...
有關所有可用安裝方法，請參閱 [安裝 Koog](getting-started.md#install-koog)。
...
若要了解更多資訊，請參閱 [提示執行器](prompts/prompt-executors.md)。
