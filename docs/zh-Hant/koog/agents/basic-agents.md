...
它接受字串輸入（問題、請求或任務描述），並將此輸入傳送至配置的大型語言模型 (LLM)。
LLM 可能決定呼叫提供的工具。
...
3. **翻譯**：我可以將文字從一種語言翻譯成另一種語言，包括常見的西班牙文、法文、德文、中文等。
...
您可以直接向 Agent 建構函式提供一些 [LLM 參數](../llm-parameters.md#llm-parameter-reference)，
以自訂 LLM 的行為。
...
首先，透過使用 [`@Tool`](https://api.koog.ai/agents/agents-tools/ai.koog.agents.core.tools.annotations/-tool/index.html) 註解標註函式來建立工具：
...
```text
Agent: 您希望我解釋哪個迷因？請從以下選項中選擇：Grumpy Cat、Success Kid 或 Doge。
...
!!! tip
    ...
    除了將模型、溫度、最大迭代次數和其他參數直接傳遞給 Agent 建構函式外，
...
如需更多關於 Koog Agent 功能的資訊，請參閱[功能概覽](../features-overview.md)。
