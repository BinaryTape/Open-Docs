...
一個使用提供的提示詞建置器將訊息新增至 LLM 提示詞的節點。
...
該架構提供了 `transform` 擴充函式，讓您可以建立節點的轉換版本，對其輸出套用轉換。當您需要將節點的輸出轉換為不同的型別或格式，同時保留原始節點的功能時，這非常有用。
...
架構提供了預定義的子圖，封裝了常用的模式和工作流程。這些子圖透過自動處理基礎節點和邊的建立，簡化了複雜代理策略的開發。
