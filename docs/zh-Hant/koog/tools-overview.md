...
在 Agent 程式碼中，有幾種調用工具的方式。建議的方法是使用 Agent context 中提供的方法，而不是直接調用工具，因為這能確保在 Agent 環境中正確處理工具運作。

!!! tip
    確保您已在工具中實作適當的[錯誤處理](agent-event-handlers.md)，以防止 Agent 失敗。
...
如需更多詳情，請參閱 [API 參考資料](api:agents-core::ai.koog.agents.core.agent.session.AIAgentLLMWriteSession)。
