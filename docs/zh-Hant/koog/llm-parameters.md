...
在 Koog 中，`LLMParams` 類別整合了 LLM 參數，並提供了一個一致的介面來配置語言模型行為。您可以透過以下方式使用 LLM 參數：
...
下表提供了 `LLMParams` 類別中包含的 LLM 參數參考，這些參數受 Koog 開箱即用的所有 LLM 供應商支援。有關某些供應商特有的參數列表，請參閱 [特定供應商參數](#provider-specific-parameters)。
