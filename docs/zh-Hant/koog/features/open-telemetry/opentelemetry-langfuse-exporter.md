...
=== "Java"
    ...
    public static void main(String[] args) {
        ...
        var agent = AIAgent.builder()
            ...
            .install(OpenTelemetry.Feature, config ->
                config.addLangfuseExporter(
                    null,           // Langfuse 主機 (回退至 LANGFUSE_HOST)
                    null,           // Public key (回退至 LANGFUSE_PUBLIC_KEY)
                    null,           // Secret key (回退至 LANGFUSE_SECRET_KEY)
                    null,           // Timeout (使用預設值)
                    List.of(
...
- **連線問題**：如果執行自我託管的 Langfuse，請確認您的環境可以連通 `LANGFUSE_HOST`。
