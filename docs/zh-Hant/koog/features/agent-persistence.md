...
- 常見 LLM 參數
- 選取的工具
- `AIAgentStorage` 內容 (執行期間儲存的鍵值對資料)
- 建立時間戳記
...
**僅持久化可序列化的值。**
所使用的序列化程序是透過 `AIAgentConfig` 的 `serializer` 屬性配置的序列化程序。
無法由該序列化程序編碼的值將被靜默跳過，且不會出現在還原後的存儲中。

不可序列化的值在寫入檢查點時會被靜默捨棄。
如果從檢查點還原後遺失了某個值，請確認其型別是否可由配置的序列化程序序列化。

如需更多資訊，請參閱 [序列化 (Serialization)](../serialization.md)。
