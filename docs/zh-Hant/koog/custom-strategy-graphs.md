...
| onToolCalls         | 當 LLM 呼叫一或多個工具時符合的條件。 |
| onTextMessage       | 當 LLM 回覆文字訊息時符合的條件。 |
...
![basic-strategy-graph](basic-strategy-graph.png)
...
對於需要並行執行多個工具的工作流程，可以使用 `nodeExecuteToolsAndGetResults` 節點並設定 `parallel = true`：
