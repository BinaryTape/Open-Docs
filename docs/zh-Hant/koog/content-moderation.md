...
| `categories`     | Map&lt;ModerationCategory, Boolean&gt;               | 是      |            | 審查類別與布林值的對應 Map，指示哪些類別被標記。 |
| `categoryScores` | Map&lt;ModerationCategory, Double&gt;                | 否       | emptyMap() | 審查類別與信心分數（0.0 到 1.0）的對應 Map。                          |
| `categoryAppliedInputTypes` | Map&lt;ModerationCategory, List&lt;InputType&gt;&gt; | 否       | emptyMap()           | 指示哪些輸入類型（`TEXT` 或 `IMAGE`）觸發了各個類別的對應 Map。                |
...
| **S7 – Privacy** (洩露個人資料、肉搜)                                         | **唯一**                                                                            | OpenAI 內容審查中沒有直接的隱私洩漏類別。                                |
| **S8 – Intellectual property**                                                            | **唯一**                                                                            | 版權 / 智慧財產權問題在 OpenAI 中不是一個審查類別。                             |
| **S9 – Indiscriminate weapons**                                                           | `illicit/violent`                                                                     | 製造或部署大規模殺傷性武器的指示屬於暴力非法內容。                          |
