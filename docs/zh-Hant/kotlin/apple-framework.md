...
2. 使用以下內容更新您的 `build.gradle(.kts)` Gradle 建構檔案：
    ...
    Kotlin/Native 支援 iOS 的 `iosArm64` 和 `iosSimulatorArm64` 目標，以及 macOS 的 `macosArm64` 
...
3. 在 IDE 中執行 `linkDebugFramework<YourTargetName>` Gradle 任務，或在終端機中使用主控台指令（例如）來建置框架：

   ```bash
   ./gradlew linkDebugFrameworkIosArm64
   ```

該組建會將框架產生至 `build/bin/<yourTargetName>/debugFramework` 目錄中。

> 您也可以使用通用的 `link<YourTargetName>` Gradle 任務來同時產生框架的 `debug` 和 `release` 變體。
>
{style="tip"}
