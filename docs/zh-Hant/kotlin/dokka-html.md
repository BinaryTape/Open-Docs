...
所有執行器皆支援 HTML 作為輸出格式。要生成 HTML 文件，請根據您的建構工具或執行器遵循以下步驟：
...
| `<@version/>`   | 從配置中獲取的模組版本。如果應用了 [版本控制外掛程式](https://github.com/Kotlin/dokka/tree/%dokkaVersion%/dokka-subprojects/plugin-versioning)，它將被版本導覽器替換。 |