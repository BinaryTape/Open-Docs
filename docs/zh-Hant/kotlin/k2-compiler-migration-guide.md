...
* **增強的 IDE 效能**。從 2025.1 開始，IntelliJ IDEA 使用 K2 模式來分析您的 Kotlin 程式碼，提高了穩定性並提供了效能改進。如需了解更多資訊，請參閱 [IDE 支援](#support-in-ides)。
...
要在 Android Studio 中獲得最佳體驗，請在您的 IDE 中使用 [K2 模式](#support-in-ides)。IntelliJ IDEA 預設使用 K2 模式，因此您無需進行任何更改。
...
在 Android Studio 中，您可以從 2024.1 開始按照以下步驟啟用 K2 模式：

1. 前往 **Settings** | **Languages & Frameworks** | **Kotlin**。
2. 勾選 **Enable K2 mode** 選項。
...
* 對具有 `@NotNull` 的基本型別陣列使用 null 檢查，現在會發出「Unnecessary safe call」（不必要的安全呼叫）或「Comparison with null always false」（與 null 比較始終為 false）警告。
