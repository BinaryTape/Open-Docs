...
新的分配器允許同時擁有個多個獨立的分配空間，這將允許 Kotlin 團隊嘗試不同的頁面佈局，以進一步提高效能。
...
Kotlin 1.9.0 在多平台程式庫中支援 [Gradle 組態快取](https://docs.gradle.org/current/userguide/configuration_cache.html)。如果您是程式庫作者，現在已經可以從改進的建置效能中受益。
