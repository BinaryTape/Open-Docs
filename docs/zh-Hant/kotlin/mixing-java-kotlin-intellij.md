...
3. 在 `<build><plugins>` 區段中，新增 Kotlin 外掛程式：
    ...
    * `<extensions>true</extensions>` 讓 Maven 將 Kotlin 外掛程式整合到建置生命週期中。
    * 自訂執行階段允許 Kotlin 外掛程式先編譯 Kotlin，再編譯 Java。
    * Kotlin 和 Java 程式碼可以透過配置的 `sourceDirs` 目錄相互引用。
    * 當使用帶有擴充功能的 Kotlin Maven 外掛程式時，您不需要在 `<build><pluginManagement>` 區段中準備個別的 `maven-compiler-plugin`。
