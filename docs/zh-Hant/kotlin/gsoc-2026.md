...
* **任務 #1**  
  在高層次上描述您如何構思 Kotlin 編譯器 fuzzer 的架構。

  > 您可以在之後的任務中微調並迭代此設計。  
  > 如果可能的話，請追蹤您的變更並在答案中解釋。
  >
  > 您可以使用 AI 工具，但請說明您如何使用它們以及如何確保結果的品質。
  >
  {style="note"}

* **任務 #2**  
  根據任務 1 的架構，哪些組件對於外掛化最重要，為什麼？

* **任務 #3**  
  根據任務 1 的架構，您是否看到使用 LLM 或 AI 的機會？如果有，在哪裡以及如何使用？

* **任務 #4**  
  您會如何測試 fuzzer 本身？是否可以進行回測（back-testing）？如果可以，如何進行？

* **任務 #5** (加分)  
  從任務 1 的架構中選擇一個組件，並更詳細地描述它可以如何實作。  
  例如，可以使用哪些工具、程式庫或演算法？
...
* **任務 #1**  
  Fork [swift-java](https://github.com/swiftlang/swift-java) 存儲庫，並為其擴展一個產生 Kotlin 原始碼的新目標。

  > **需求：**
  >  * 加入一個 CLI 選項（例如 `--lang kotlin-jvm` 或 `--emit kotlin-jvm`）
  >  * 為最小子集產生 Kotlin：
  >     * 頂層（top-level）Swift 函式
  >     * 型別：`Int`/`Int32`, `Bool`, `Double`, `String`, `Void` → Kotlin `Int`, `Boolean`, `Double`, `String`, `Unit`
  >  * 加入測試
  >  * 加入如何執行專案的說明
  >
  > **產生的 Kotlin 程式碼應包含：**
  >  * 具有對應名稱和型別的頂層函式宣告
  >  * 虛設常式（stub）主體（例如 `TODO("Not implemented")`）
  >
  > **加入一份簡短的 `DESIGN.md` (或 `NOTES.md`) 描述：**
  >  * 權衡和刻意跳過的部分
  >  * 更完整的解決方案會是什麼樣子
  >  * 如果有更多時間，您下一步會做什麼
  > 
...
* **任務 #2** (選修)  
  透過使產生的 Kotlin 程式碼可以使用現有的 JNI 或 FFM 執行時進行呼叫，來擴展前一個任務。
