...
建構器推論是一種特殊的型別推論，在呼叫泛型建構函式時非常有用。它透過使用其 Lambda 引數內其他呼叫的型別資訊，來協助編譯器推論該呼叫的型別引數。
...
[可呼叫參照](reflection.md#callable-references) 指向函數式介面建構函式現在已進入 [穩定版](components-stability.md)。了解如何使用可呼叫參照從具有建構函式的介面 [遷移](fun-interfaces.md#migration-from-an-interface-with-constructor-function-to-a-functional-interface) 到函數式介面。
...
先前，您需要同時安裝 CocoaPods 相依管理員和 `cocoapods-generate` 外掛程式才能使用 CocoaPods，例如，在 Kotlin Multiplatform Mobile 專案中處理 [iOS 相依性](https://kotlinlang.org/docs/multiplatform/multiplatform-ios-dependencies.html#with-cocoapods)。
...
> 下載器會將原生版本和目標作業系統附加到此基礎 URL，以確保下載實際的編譯器分發版。
>
{style="note"}
...
從 Kotlin 1.7.0 開始，`js` 區塊中支援 JavaScript 的 `import` 陳述式，允許您在執行時將套件動態帶入您的應用程式：
...
`matchAt()` 如果找到匹配則回傳匹配結果，否則回傳 `null`：
...
考慮這個匹配城市座標的範例。要獲取正規表示式比對到的群組集合，請使用 [`groups`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/-match-result/groups.html)。比較透過編號（索引）和透過名稱使用 `value` 來擷取群組內容：
...
具名群組參照可用於取代運算式。考慮 [`replace()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/-regex/replace.html) 函式，它將輸入中所有指定的正規表示式出現處替換為取代運算式，以及 [`replaceFirst()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/-regex/replace-first.html) 函式，它僅交換第一個匹配項。
...
* `http` 使用 HTTP(S) 發送建置報告。POST 方法以 JSON 格式發送指標。數據可能會隨版本變更。您可以在 [Kotlin 儲存庫](https://github.com/JetBrains/kotlin/blob/master/libraries/tools/kotlin-gradle-plugin/src/common/kotlin/org/jetbrains/kotlin/gradle/report/data/GradleCompileStatisticsData.kt) 中查看目前版本的發送數據。

分析長時間編譯的建置報告可以幫助您解決兩種常見情況：

* 建置不是增量的。分析原因並修正底層問題。
* 建置是增量的，但耗時過長。嘗試重新組織原始碼文件——拆分大文件、將不同的類別儲存在不同的文件中、重構大型類別、在不同的文件中宣告頂層函式等等。
...
要檢查您的建置使用的是哪個變體，請啟用 [`--info` 記錄層級](https://docs.gradle.org/current/userguide/logging.html#sec:choosing_a_log_level) 並在輸出中尋找以 `Using Kotlin Gradle plugin` 開頭的字串，例如 `Using Kotlin Gradle plugin main variant`。
...
這對於追蹤建置中是否存在未使用的註解處理器非常有用。您可以使用產生的報告來查找觸發不必要註解處理器的模組，並更新模組以防止這種情況。
...
* `kotlinOptions.jdkHome` 編譯器選項在 1.5.30 中被棄用，並在此版本中被移除。如果 Gradle 建置包含此選項，現在會失敗。我們鼓勵您使用 [Java 工具鏈](whatsnew1530.md#support-for-java-toolchains)，該功能自 Kotlin 1.5.30 起受支援。
...
在 Kotlin 1.4.0 中，`kotlin2js` 和 `kotlin-dce-plugin` 外掛程式被棄用，並在此版本中被移除。請使用新的 `org.jetbrains.kotlin.js` 外掛程式來代替 `kotlin2js`。當 Kotlin/JS Gradle 外掛程式配置正確時，無效程式碼消除 (DCE) 即可運作。

在 Kotlin 1.6.0 中，我們將 `KotlinGradleSubplugin` 類別的棄用等級更改為 `ERROR`。開發人員使用此類別來撰寫編譯器外掛程式。在此版本中，[此類別已被移除](https://youtrack.jetbrains.com/issue/KT-48831/)。請改用 `KotlinCompilerPluginSupportPlugin` 類別。
