...
### 全套件範圍的密封類別階層結構 {id="package-wide-sealed-class-hierarchies"}
...
Kotlin/JVM 獲得了許多改進，包括內部改進和面向使用者的改進。以下是其中最值得注意的：
...
### 新的預設 JVM 目標版本：1.8 {id="new-default-jvm-target-1-8"}
...
### 透過 invokedynamic 的 SAM 轉接器 {id="sam-adapters-via-invokedynamic"}
...
### 透過 invokedynamic 的 Lambda {id="lambdas-via-invokedynamic"}
...
### 處理可 null 性註解的改進 {id="improvements-to-handling-nullability-annotations"}
...
### 效能提升 {id="performance-improvements"}
...
### 適用於 IR 編譯器的架構與程式庫 {id="frameworks-and-libraries-for-the-ir-compiler"}
...
### 穩定的無符號整數型別 {id="stable-unsigned-integer-types"}
...
### 穩定的與區域設定無關的用於大寫/小寫文字的 API {id="stable-locale-agnostic-api-for-upper-lowercasing-text"}
...
### 穩定的 Char 到整數轉換 API {id="stable-char-to-integer-conversion-api"}
...
### 用於獲取字元類別的新 API 現在可在多平台程式碼中使用 {id="new-api-for-getting-a-char-category-now-available-in-multiplatform-code"}
...
Kotlin 1.5.0 是一個功能版本，因此可能會為語言帶來不相容的變更。在 [Kotlin 1.5 相容性指南](compatibility-guide-15.md) 中可以找到此類變更的詳細清單。