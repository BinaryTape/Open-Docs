...
Kotlin 在其標準函式庫中僅提供最少量的底層 API，以便讓其他程式庫能夠利用協同程式。與許多具有類似功能的其他語言不同，`async` 和 `await` 在 Kotlin 中並非關鍵字，甚至不屬於其標準函式庫的一部分。此外，Kotlin 的暫停函式 (_suspending function_) 概念為非同步作業提供了比 **future** 和 **promise** 更安全且更不易出錯的抽象化。  
...
* [取消與逾時](coroutines-cancellation.md)
