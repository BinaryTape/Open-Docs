...
在 Kotlin 1.4.0 中，我們對所有組件進行了大量改進，[專注於品質與效能](https://blog.jetbrains.com/kotlin/2020/08/kotlin-1-4-released-with-a-focus-of-quality-and-performance/)。
...
* 暴露於公共 API 的屬性與函式必須有顯式型別規格。這保證了 API 使用者知曉他們所使用的 API 成員型別。

根據您的組態，這些顯式 API 可能會產生錯誤（*strict* 模式）或警告（*warning* 模式）。
...
然後您可以完成專案建立，或者（可選）在下一個畫面上 *設定專案*：

4. *加入/移除* 此專案樣板支援的模組與目標。
5. *設定模組與目標設定*，例如目標 JVM 版本、目標樣板與測試架構。

![Kotlin 專案精靈 - 設定目標](multiplatform-project-2-wn.png)

未來，我們將透過加入更多組態選項與樣板，使 Kotlin 專案精靈變得更加靈活。
...
* [可呼 callable 參照的智慧轉型](#smart-casts-for-callable-references)
...
- 從 Gradle 組態中透過 `cssSupport` 組態 webpack 的 CSS 與樣式載入器。在此處閱讀有關 [使用 CSS 與樣式載入器](js-project-setup.md#css) 的更多資訊。
...
有關如何組態新後端的更詳細資訊，請查看 [Kotlin/JS IR 編譯器文件](js-ir-compiler.md)。
...
完成初始組態，當您向 `cocoapods` 加入新相依性時，只需在 IntelliJ IDEA 中重新匯入專案即可。新相依性將自動加入。不需要額外步驟。
...
- _顯式載入指令碼組態_ 以獲得更好的效能。以前，您對建置指令碼所做的變更會在背景自動載入。為了提高效能，我們在 1.4.0 中停用了自動載入建置指令碼組態。現在，IDE 僅在您明確套用變更時才載入它們。

  在低於 6.0 的 Gradle 版本中，您需要透過點擊編輯器中的 **Load Configuration** 手動載入指令碼組態。

  ![*.gradle.kts – 載入組態](gradle-kts-load-config.png)

  在 Gradle 6.0 及更高版本中，您可以透過點擊 **Load Gradle Changes** 或重新匯入 Gradle 專案來顯式套用變更。
 
  我們在 IntelliJ IDEA 2020.1 與 Gradle 6.0 及以上版本中加入了一個額外的操作 – **Load Script Configurations**，它可以在不更新整個專案的情況下載入對指令碼組態的變更。這比重新匯入整個專案花費的時間少得多。

  ![*.gradle.kts – 載入指令碼變更與載入 Gradle 變更](gradle-kts.png)

  對於新建立的指令碼或首次使用新 Kotlin 外掛程式開啟專案時，您也應該 **Load Script Configurations**。
  
  在 Gradle 6.0 及以上版本中，您現在可以一次載入所有指令碼，而不像之前的實作是個別載入。由於每個請求都需要執行 Gradle 組態階段，對於大型 Gradle 專案來說，這可能會耗費大量資源。 
  
  目前，此類載入僅限於 `build.gradle.kts` 與 `settings.gradle.kts` 檔案（請為相關 [問題](https://github.com/gradle/gradle/issues/12640) 投票）。若要為 `init.gradle.kts` 或套用的 [指令碼外掛程式](https://docs.gradle.org/current/userguide/plugins.html#sec:script_plugins) 啟用醒目提示，請使用舊機制 – 將它們加入獨立指令碼。組態對於該指令碼將在需要時個別載入。您也可以為此類指令碼啟用自動重新載入。
...
- [用於 Kotlin 反射的 Proguard 組態](#proguard-configurations-for-kotlin-reflection)
...
* `NaN`, `NEGATIVE_INFINITY`, 與 `POSITIVE_INFINITY` 在 `Double` 與 `Float` 中現在定義為 `const`，因此您可以將它們用作註解引數。
