...
* **[在 IDE 中](#in-ide)**。在這裡，您可以從版本控制系統複製專案樣板，並在 IntelliJ IDEA 中使用它。
* **[使用 Gradle 建置系統](#using-gradle)**。為了更深入了解底層運作方式，請手動為您的專案建立建置檔案。
* **[使用命令列工具](#using-the-command-line-compiler)**。您可以使用作為標準 Kotlin 發行版一部分提供的 Kotlin/Native 編譯器，並直接在命令列工具中建立應用程式。

主控台編譯看似簡單直接，但對於具有數百個檔案和程式庫的大型專案而言，其擴充性並不好。對於這類專案，我們建議使用 IDE 或建置系統。

藉由 Kotlin/Native，您可以針對 [不同的目標](native-target-support.md) 進行編譯，包括 Linux、macOS 和 Windows。雖然可以進行跨平台編譯（即使用一個平台編譯另一個平台的程式碼），但在本教學中，您將針對您正在編譯的相同平台進行開發。

> 如果您使用 Mac 並且想要建立並執行適用於 macOS 或其他 Apple 目標的應用程式，您還需要先安裝 [Xcode Command Line Tools](https://developer.apple.com/download/)，啟動它並接受授權條款。
>
{style="note"}

## 在 IDE 中

在本節中，您將學習如何使用 IntelliJ IDEA 建立 Kotlin/Native 應用程式。
...
3. 開啟 `gradle/libs.versions.toml` 檔案，這是專案相依性的版本目錄（version catalog）。要建立 Kotlin/Native 應用程式，您需要 Kotlin Multiplatform Gradle 外掛程式，其版本與 Kotlin 相同。請確保您使用的是最新的 Kotlin 版本：
...
![執行應用程式](native-run-gutter.png){width=450}

IntelliJ IDEA 使用 Gradle 任務執行程式碼，並在 **Run** 標籤頁中輸出結果：

![應用程式輸出](native-output-gutter-1.png){width=450}

第一次執行後，IDE 會在頂部建立對應的執行組態：

![Gradle 執行組態](native-run-config.png){width=500}

> 擁有 Ultimate 訂閱的 IntelliJ IDEA 使用者可以安裝 [Native Debugging Support](https://plugins.jetbrains.com/plugin/12775-native-debugging-support) 外掛程式，該外掛程式允許偵錯編譯後的原生可執行檔，並為匯入的 Kotlin/Native 專案自動建立執行組態。
...
現在，當您在類別檔案中進行更改或儲存檔案 (<shortcut>Ctrl + S</shortcut>/<shortcut>Cmd + S</shortcut>) 時，IntelliJ IDEA 會自動對專案進行增量建置。

### 更新應用程式

讓我們為您的應用程式加入一項功能，使其可以計算您名字中的字母數量：
...
2. 要使用 Gradle 執行此應用程式，請在 `build.gradle.kts` 檔案中指定將 `System.in` 作為輸入使用，並載入 Gradle 變更：
...
3. 消除空白字元並計算字母：
   ...
   * 使用作用域函式 [`let`](scope-functions.md#let) 在物件內容中執行該函式。
   * 使用 [字串範本](strings.md#string-templates) 將您的名字長度插入字串中，方法是加入錢字號並用花括號將其括起來 – `${it.length}`。`it` 是 [Lambda 參數](coding-conventions.md#lambda-parameters) 的預設名稱。
