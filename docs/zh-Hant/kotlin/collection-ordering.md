...
為此，請呼叫 [`sortedWith()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/sorted-with.html) 函式並傳入你的 `Comparator`。
使用此函式，按長度排序字串如下所示：
...
另一個反轉函式 [`asReversed()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/as-reversed.html) 傳回相同集合執行個體的反轉檢視，因此如果原始列表不打算更改，它可能比 `reversed()` 更輕量且更值得推薦。
