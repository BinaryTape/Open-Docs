[//]: # (title: Kotlin/JS 快速入門)

本教學將展示如何使用 Kotlin/JavaScript (Kotlin/JS) 建立瀏覽器的 Web 應用程式。
若要建立您的 app，請選擇最適合您工作流程的工具：

* **[IntelliJ IDEA](#create-your-application-in-intellij-idea)**：從版本控制複製專案樣板，並在 IntelliJ IDEA 中進行開發。
* **[Gradle 建置系統](#create-your-application-using-gradle)**：手動為您的專案建立建置檔案，以更深入地了解底層的運作方式。

> 除了針對瀏覽器外，使用 Kotlin/JS 您還可以針對其他環境進行編譯。
...
要建立您的 Kotlin/JS Web 應用程式，您可以使用 [IntelliJ IDEA](https://www.jetbrains.com/idea/download/?section=mac) 的 Community 或 Ultimate 版本。

### 設定環境

1. 下載並安裝最新版本的 [IntelliJ IDEA](https://www.jetbrains.com/idea/)。
2. [設定您的 Kotlin Multiplatform 開發環境](https://kotlinlang.org/docs/multiplatform/quickstart.html#set-up-the-environment)。
...
1. 打開 `kmp-js-wizard/gradle/libs.versions.toml` 檔案。它包含了專案相依性的版本目錄。
...
有關多平台專案 Gradle 配置的更多資訊，請參閱 [Multiplatform Gradle DSL 參考](https://kotlinlang.org/docs/multiplatform/multiplatform-dsl-reference.html)。

### 建置並執行應用程式

1. 打開 `src/jsMain/kotlin/Main.kt` 檔案。

   * `src/jsMain/kotlin/` 目錄包含專案 JavaScript 目標的主要 Kotlin 原始碼檔案。
   * `Main.kt` 檔案包含使用 [`kotlinx.browser`](https://github.com/Kotlin/kotlinx-browser) API 在瀏覽器頁面上呈現 "Hello, Kotlin/JS!" 的程式碼。

2. 點擊 `main()` 函式中的 **Run** 圖示來執行程式碼。
...
在您第一次執行應用程式後，IntelliJ IDEA 會在頂端欄位建立其對應的运行配置 (**jsMain [js]**)：
...
Gradle 可以在您進行變更時自動重新建置您的專案：

1. 在运行配置清單中選擇 **jsMain [js]**，然後點擊 **More Actions** | **Edit**。

    ![Gradle edit run configuration](js-edit-run-config.png){width=500}

2. 在 **執行/偵錯配置** 對話方塊中，於 **Run** 欄位輸入 `jsBrowserDevelopmentRun --continuous`。

    ![Continuous run configuration](js-continuous-run-config.png){width=500}

3. 點擊 **OK**。

現在，當您執行應用程式並進行任何變更時，每當您儲存 (<shortcut>Ctrl + S</shortcut>/<shortcut>Cmd + S</shortcut>) 或變更類別檔案時，Gradle 都會自動對專案進行累進建置並對瀏覽器進行熱重新載入 (hot-reloads)。

### 修改應用程式

修改應用程式以增加一個計算單字字母數量的功能。
...
2. 依照 IDE 的建議匯入 `HTMLInputElement` 相依性。
...
1. 透過定義一個建立段落的[擴充方法](extensions.md#extension-functions)，新增一個文字元素來顯示輸出：
...
* [`let{}` 作用域函式](scope-functions.md#let) 在物件內容中執行該函式。
* [字串範本](strings.md#string-templates) (`${it.length}`) 透過在變數前加上錢字號 (`$`) 並將其括在花括號 (`{}`) 中，將單字的長度插入到字串中。
