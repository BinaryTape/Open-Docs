...
## 支援 Kotlin K2 編譯器外掛程式 {id="support-for-kotlin-k2-compiler-plugins"}
...
### 預覽用於建立開放式範圍的 ..< 運算子 {id="preview-of-the-operator-for-creating-open-ended-ranges"}
...
#### 標準函式庫 API 變更 {id="standard-library-api-changes"}

通用的 Kotlin 標準函式庫中的 `kotlin.ranges` 套件將引入以下新類型和操作：

##### 新的 OpenEndRange&lt;T&gt; 介面 {id="new-openendrange-lt-t-gt-interface"}
...
##### 在現有的可迭代範圍中實作 OpenEndRange {id="implementing-openendrange-in-the-existing-iterable-ranges"}
...
##### 標準型別的 rangeUntil 運算子 {id="rangeuntil-operators-for-the-standard-types"}

將為目前由 `rangeTo` 運算子定義的相同型別和組合提供 `rangeUntil` 運算子。我們以擴充函式的形式提供它們以供原型設計使用，但為了保持一致性，我們計劃在穩定開放式範圍 API 之前將其改為成員。

#### 如何啟用 ..&lt; 運算子 {id="how-to-enable-the-lt-operator"}
...
### 為帶有 data object 的單例和密封類別階層改進字串表示 {id="improved-string-representations-for-singletons-and-sealed-class-hierarchies-with-data-objects"}
...
#### 如何啟用 data object {id="how-to-enable-data-objects"}
...
### 新的建置器型別推論限制 {id="new-builder-type-inference-restrictions"}
...
### 泛型內嵌類別 {id="generic-inline-classes"}
...
### 更多委派屬性的優化案例 {id="more-optimized-cases-of-delegated-properties"}
...
### kapt 虛設常式產生任務支援 JVM IR 後端 {id="support-for-the-jvm-ir-backend-in-kapt-stub-generating-task"}
...
### 新的 Kotlin/Native 記憶體管理員預設啟用 {id="the-new-kotlin-native-memory-manager-enabled-by-default"}
...
#### 從 Swift/Objective-C 呼叫 Kotlin 暫停函式 {id="calling-kotlin-suspending-functions-from-swift-objective-c"}
...
### 自訂 Info.plist 檔案 {id="customizing-the-info-plist-file"}
...
### 目標組態 (Target configuration) {id="target-configuration"}
...
### 原始碼目錄組態 {id="source-directories-configuration"}
...
### 用於 JVM 工具鏈組態的新方法 {id="new-method-for-jvm-toolchain-configuration"}
...
### 翻新並改進的頁面 {id="revamped-and-improved-pages"}
...
### 新增及更新的教學 {id="new-and-updated-tutorials"}
...
### 發佈文件的變更 {id="changes-in-release-documentation"}
...
## 安裝 Kotlin 1.7.20 {id="install-kotlin-1-7-20"}
...
### Kotlin 1.7.20 相容性指南 {id="compatibility-guide-for-kotlin-1-7-20"}
