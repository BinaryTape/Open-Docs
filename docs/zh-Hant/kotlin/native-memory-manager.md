...
GC 會在多個執行緒（包括應用程式執行緒、GC 執行緒以及選用的標記執行緒）上並行處理標記佇列。應用程式執行緒和至少一個 GC 執行緒會參與標記過程。預設情況下，標記階段與應用程式執行緒並行執行，從而減少 GC 暫停時間。您可以透過 [GC 記錄](#monitor-gc-performance)來監控 GC 效能。
...
如果您遇到 CMS 的問題，可以切換回並行標記並行清除（PMCS）設定。若要執行此操作，請在您的 `gradle.properties` 檔案中設定以下[二進位選項](native-binary-options.md)：
