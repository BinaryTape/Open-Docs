[//]: # (title: Kotlin Koans)

Kotlin Koans 是一系列主要為 Java 開發人員設計的練習，旨在幫助您熟悉 Kotlin 語法。
每個練習都是以失敗的單元測試形式建立的，而您的任務是使其通過。

> 您可以在我們的 [GitHub 存儲庫](https://github.com/Kotlin/kotlin-koans-edu)中查看任務內容。
> 
{style="note"}

您可以透過以下任一方式完成 Kotlin Koans 任務：

* 您可以在 [Koans 線上版](https://play.kotlinlang.org/koans)中練習。
* 您可以透過[安裝 JetBrains Academy 外掛程式](https://plugins.jetbrains.com/plugin/10081-jetbrains-academy/docs/install-jetbrains-academy-plugin.html)並[選擇 Kotlin Koans 課程](https://plugins.jetbrains.com/plugin/10081-jetbrains-academy/docs/learner-start-guide.html?section=Kotlin%20Koans)，直接在 IntelliJ IDEA 或 Android Studio 中執行任務。

無論您選擇哪種方式來解決 Koans，您都可以查看每個任務的解決方案：
* 在線上版本中，點擊 **Show answer**。
* 對於 JetBrains Academy 外掛程式，請嘗試先完成任務，如果您的答案不正確，再選擇 **Peek solution**。

我們建議您在實作任務後檢查解決方案，將您的答案與建議的解法進行比較。
請確保您沒有作弊！