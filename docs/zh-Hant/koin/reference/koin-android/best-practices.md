...
### 避免 Activity/Fragment 洩漏 {id="avoid-activity-fragment-leaks"}
...
### 正確關閉作用域 {id="close-scopes-properly"}
...
### 清除長生命週期物件中的參考 {id="clear-references-in-long-lived-objects"}
...
## Android 偵錯 {id="android-debugging"}

### 啟用 Android 記錄器 {id="enable-android-logger"}
...
### 在偵錯組建中驗證模組 {id="verify-modules-in-debug-builds"}
...
### 用於偵錯的作用域回呼 {id="scope-callbacks-for-debugging"}
...
## 安全性最佳實務 {id="security-best-practices"}

### 不要在模組中儲存機密資訊 {id="don-t-store-secrets-in-modules"}
...
## 從 Dagger/Hilt 遷移 {id="migration-from-dagger-hilt"}
