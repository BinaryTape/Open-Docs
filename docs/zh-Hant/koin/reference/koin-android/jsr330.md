...
### @Singleton 與 @Inject {id="singleton-and-inject"}
...
### 自訂 @Qualifier 註解 {id="custom-qualifier-annotations"}
...
## 從 Hilt 遷移 {id="migration-from-hilt"}

### 遷移前 (Hilt) {id="before-hilt"}
...
### 遷移後 (搭配 JSR-330 的 Koin) {id="after-koin-with-jsr-330"}
...
## 編譯器外掛程式支援 {id="compiler-plugin-support"}
