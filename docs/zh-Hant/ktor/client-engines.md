...
2.  將
    ...
    ```
3.  若要設定引擎，請在 `engine {}` 區塊中設定 [
    `AndroidEngineConfig`](https://api.ktor.io/ktor-client-android/io.ktor.client.engine.android/-android-engine-config/index.html) 中的屬性：
    ```kotlin
    import io.ktor.client.*
    import io.ktor.client.engine.android.*
    import java.net.Proxy
...
2.  將
    ...
    ```
3.  若要設定引擎，請在 `engine {}` 區塊中設定 [
    `OkHttpConfig`](https://api.ktor.io/ktor-client-okhttp/io.ktor.client.engine.okhttp/-ok-http-config/index.html) 中的屬性：
    ```kotlin
    import io.ktor.client.*
    import io.ktor.client.engine.okhttp.*
...
2.  將 `Darwin` 類別作為引數傳遞給 `HttpClient` 建構函式：
    ...
    ```
3.  使用 `engine {}` 區塊並透過 [
    `DarwinClientEngineConfig`](https://api.ktor.io/ktor-client-darwin/io.ktor.client.engine.darwin/-darwin-client-engine-config/index.html) 設定引擎。
...
2.  將 `WinHttp` 類別作為引數傳遞給 `HttpClient` 建構函式：
    ...
    ```
3.  使用 `engine {}` 區塊並透過 [
    `WinHttpClientEngineConfig`](https://api.ktor.io/ktor-client-winhttp/io.ktor.client.engine.winhttp/-winhttp-client-engine-config/index.html) 設定引擎。
