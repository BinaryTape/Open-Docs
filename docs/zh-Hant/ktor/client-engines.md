...
[Ktor HTTP 用戶端](client-create-and-configure.md) 可用於不同的平台，包括 JVM、[Android](https://kotlinlang.org/docs/android-overview.html)、[JavaScript](https://kotlinlang.org/docs/js-overview.html) 和 [Native](https://kotlinlang.org/docs/native-overview.html)。特定平台可能需要特定的引擎來處理網路請求。例如，您可以為 JVM 應用程式使用 `Apache` 或 `Jetty`，為 Android 使用 `OkHttp` 或 `Android`，為針對 Kotlin/Native 的桌面應用程式使用 `Curl` 等。不同的引擎可能具有特定的功能並提供不同的配置選項。
...
_* 若要在較舊的 Android 版本上使用 CIO 引擎，您需要啟用 [Java 8 API 語法糖降級](https://developer.android.com/studio/write/java8-support)。_
...
您還需要考慮以下會影響通用用戶端配置及特定外掛程式使用的限制：
- 如果引擎支援 HTTP/2，您可以透過自訂引擎配置來啟用它（請參閱 [Java](#java) 引擎的範例）。
- 若要在 Ktor 用戶端中配置 [SSL](client-ssl.md)，您需要自訂所選引擎的配置。
- 某些引擎不支援[代理](client-proxy.md#supported_engines)。
- [Logging](client-logging.md) 外掛程式為不同平台提供不同的記錄器類型。
- [HttpTimeout](client-timeout.md#limitations) 外掛程式對特定引擎有一些限制。

## 新增引擎依賴 {id="dependencies"}

除了 `ktor-client-core` Artifact 之外，Ktor 用戶端還需要為每個引擎新增特定的依賴。對於每個支援的平台，您可以在相應的章節中查看可用的引擎和所需的依賴：
* [JVM](#jvm)
* [JVM 與 Android](#jvm-android)
* [JavaScript](#js)
* [Native](#native)

> 對於不同的引擎，Ktor 提供了帶有諸如 `-jvm` 或 `-js` 後綴的平台專用 Artifact，例如 `ktor-client-cio-jvm`。Gradle 會解析適用於指定平台的 Artifact，而 Maven 不支援此功能。這意味著對於 Maven，您需要手動新增平台專用後綴。
>
{type="note"}

## 使用指定引擎建立用戶端 {id="create"}
若要使用特定引擎建立 HTTP 用戶端，請將引擎類別作為參數傳遞給 [HttpClient](https://api.ktor.io/ktor-client/ktor-client-core/io.ktor.client/-http-client/index.html) 建構函式。例如，您可以如下建立一個使用 `CIO` 引擎的用戶端：
```kotlin
```
{src="snippets/_misc_client/CioCreate.kt"}

### 預設引擎 {id="default"}
如果您在呼叫 `HttpClient` 建構函式時不帶參數，用戶端將根據[建置腳本中新增的 Artifact](#dependencies) 自動選擇引擎。
```kotlin
```
{src="snippets/_misc_client/DefaultEngineCreate.kt"}

這對於多平台專案很有用。例如，對於同時針對 [Android 和 iOS](client-create-multiplatform-application.md) 的專案，您可以將 [Android](#jvm-android) 依賴新增到 `androidMain` 來源集，並將 [Darwin](#darwin) 依賴新增到 `iosMain` 來源集。必要的依賴將在編譯時被選取。

## 配置引擎 {id="configure"}
您可以使用 `engine` 方法來配置引擎。所有引擎都共用 [HttpClientEngineConfig](https://api.ktor.io/ktor-client/ktor-client-core/io.ktor.client.engine/-http-client-engine-config/index.html) 暴露的幾個通用屬性，例如：

```kotlin
```
{src="snippets/_misc_client/BasicEngineConfigExample.kt" interpolate-variables="true" disable-links="false"}

若要了解如何配置特定引擎，請參閱下面的相應章節。

## JVM {id="jvm"}
在本節中，我們將介紹適用於 JVM 的引擎。

### Apache {id="apache"}
`Apache` 引擎支援 HTTP/1.1 並提供多種配置選項。如果您需要 HTTP/2 支援，也可以使用 `Apache5` 引擎，其預設啟用 HTTP/2。
...
在本節中，我們將介紹適用於 JVM/Android 的引擎及其配置。

### Android {id="android"}
`Android` 引擎針對 Android，並可透過以下方式配置：
...
## Native {id="native"}
在本節中，我們將介紹如何配置針對 [Kotlin/Native](https://kotlinlang.org/docs/native-overview.html) 的引擎。

> 在 Kotlin/Native 專案中使用 Ktor 需要[新記憶體管理器](https://kotlinlang.org/docs/native-memory-manager.html)，該管理器從 Kotlin 1.7.20 開始預設啟用。
>
{id="newmm-note"}

### Darwin {id="darwin"}
`Darwin` 引擎針對[基於 Darwin](https://en.wikipedia.org/wiki/Darwin_(operating_system)) 的作業系統（例如 macOS、iOS、tvOS 等），並在底層使用 `NSURLSession`。若要使用 `Darwin` 引擎，請遵循以下步驟：
...
3. 若要配置引擎，請將 [DarwinClientEngineConfig](https://api.ktor.io/ktor-client/ktor-client-darwin/io.ktor.client.engine.darwin/-darwin-client-engine-config/index.html) 暴露的設定傳遞給 `engine` 函式。
   例如，您可以使用 `configureRequest` 函式來存取 `NSMutableURLRequest`，或使用 `configureSession` 來自訂會話配置。以下程式碼片段展示了如何使用 `configureRequest`：
   ```kotlin
   ```
   {src="snippets/client-engine-darwin/src/nativeMain/kotlin/Main.kt" include-lines="8-14"}

   您可以在此處找到完整的範例：[client-engine-darwin](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-engine-darwin)。

### WinHttp {id="winhttp"}
`WinHttp` 引擎針對基於 Windows 的作業系統。若要使用 `WinHttp` 引擎，請遵循以下步驟：
...
3. 若要配置引擎，請將 [WinHttpClientEngineConfig](https://api.ktor.io/ktor-client/ktor-client-winhttp/io.ktor.client.engine.winhttp/-winhttp-client-engine-config/index.html) 暴露的設定傳遞給 `engine` 函式。
   ...
   您可以在此處找到完整的範例：[client-engine-winhttp](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-engine-winhttp)。

### Curl {id="curl"}

對於桌面平台，Ktor 也提供了 `Curl` 引擎。此引擎支援以下平台：`linuxX64`、`macosX64`、`macosArm64`、`mingwX64`。若要使用 `Curl` 引擎，請遵循以下步驟：
...
1. 安裝 [libcurl 函式庫](https://curl.se/libcurl/)。
   ...
   > 在 Windows 上，您可能需要考慮 [MinGW/MSYS2](FAQ.topic#native-curl) 的 `curl` 二進位檔。
...
4. 若要配置引擎，請將 `CurlClientEngineConfig` 暴露的設定傳遞給 `engine` 方法。
   ...
   您可以在此處找到完整的範例：[client-engine-curl](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-engine-curl)。

## JVM、Android 與 Native {id="jvm-android-native"}

### CIO {id="cio"}
CIO 是一個完全非同步、基於協程的引擎，可在 JVM、Android 和 Native 平台上使用。目前它僅支援 HTTP/1.x。若要使用它，請遵循以下步驟：
...
`Js` 引擎可用於 [JavaScript 專案](https://kotlinlang.org/docs/js-overview.html)。此引擎對瀏覽器應用程式使用 `fetch API`，對 Node.js 使用 `node-fetch`。若要使用它，請遵循以下步驟：
...
2. 將 `Js` 類別作為參數傳遞給 `HttpClient` 建構函式：
   ...
   您也可以呼叫 `JsClient` 函式來獲取 `Js` 引擎的單例：
   ```kotlin
   import io.ktor.client.engine.js.*
...
您可以在此處找到完整的範例：[client-engine-js](https://github.com/ktorio/ktor-documentation/tree/%ktor_version%/codeSnippets/snippets/client-engine-js)。
...
讓我們使用在 [](client-create-multiplatform-application.md) 教學中建立的專案來展示如何實現這一點：

1. 開啟 `shared/src/commonMain/kotlin/com/example/kmmktor/Platform.kt` 檔案並新增一個頂層的 `httpClient` 函式，它接受一個用戶端配置並返回 `HttpClient`：
   ```kotlin
...
2. 開啟 `shared/src/androidMain/kotlin/com/example/kmmktor/Platform.kt` 並為 Android 模組新增 `httpClient` 函式的實際宣告：
   ...
   ```
   此範例展示了如何配置 [OkHttp](#okhttp) 引擎，但您也可以使用[Android 支援的](#jvm-android) 其他引擎。
