...
Ktor 提供了用於開發自訂[外掛程式](server-plugins.md)的 API，這些外掛程式可以實作常見功能並在多個應用程式中重複使用。此 API 允許您攔截不同的[管線](#pipelines)階段，以向請求/響應處理中添加自訂邏輯。例如，您可以攔截 `Monitoring` 階段來記錄傳入的請求或收集指標。
...
自訂外掛程式的類別應具有一個實作 `BaseApplicationPlugin` 或 `BaseRouteScopedPlugin` 介面的伴生物件。`BaseApplicationPlugin` 介面接受三個型別參數：
- 此外掛程式相容的管線型別。
...
- `ApplicationReceivePipeline.Transform` 和 `ApplicationSendPipeline.Transform`：允許您獲取和[轉換](#transform)從客戶端接收的資料，並在傳回資料之前進行轉換。
