...
[上一節](#first-plugin) 展示了如何建立一個外掛，該外掛會為每個回應附加預定義的自訂標頭。讓我們使此外掛更有用，並提供一個設定，用於傳遞任何自訂標頭名稱和值：
...
2.  若要在外掛中使用此設定，請將設定類別參考傳遞給 `createClientPlugin`：
...
還有 `on(...)` 處理常式，它允許您呼叫特定的掛鉤，這對於處理呼叫的其他階段可能很有用。
下表列出了所有處理常式及其執行順序：
