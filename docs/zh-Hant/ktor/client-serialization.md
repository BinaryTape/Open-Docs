...
請注意，特定格式的序列化器需要額外的構件。例如，kotlinx.serialization 需要用於 JSON 的 `ktor-serialization-kotlinx-json` 依賴項。根據所包含的構件，Ktor 會自動選擇預設序列化器。如果需要，您可以明確[指定序列化器](#configure_serializer)並進行配置。
