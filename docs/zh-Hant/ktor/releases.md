...
<tr>
<td>3.3.3</td><td>November 26, 2025</td><td>
<p>
一個修補程式版本，新增對 Jetty Client 的 HTTP/2 over cleartext (h2c) 支援，改進日誌記錄和 OpenAPI 生成，並修正了引擎、SSE 處理、重複回應、HTTP/2 標頭和用戶端快取中的錯誤。
</p>
...
<p>
一個修補程式版本，新增對 Darwin 的 SOCKS 代理支援，優化 WebRTC 用戶端目標和 Java 代理處理，並修正了 HTTP retries、OpenAPI、caching 和 Android 上的 Netty 中的多個問題。
</p>
...
<p>
一個修補程式版本，將 Kotlin 更新至 2.2.20，並修正了多個問題，包括 Content-Length 解析錯誤、<code>ClientSSESession</code> 缺少序列化器、Netty 設定和關閉錯誤，並增加了對在 bootJar 中提供靜態資源的支援。
</p>
...
<p>
一個修補程式版本，引入了對 YAML 設定處理、DI 解析和 Wasm/JS 穩定性的改進，以及對 multipart 解析、CIO <code>100 Continue</code> 回應格式、<code>ByteReadChannel</code> 中的無限讀取迴圈，以及伺服器關閉問題的修正。
</p>
...
<p>
一個修補程式版本，改進了 SSE 欄位序列化順序，並解決了多個問題，包括 CORS preflight 處理、測試應用程式串流、設定反序列化錯誤，以及跨平台缺少標頭的問題——包括來自 3.2.1 影響 wasmJs 和 Darwin 目標的迴歸。
</p>
...
<p>
一個修補程式版本，包括對時間 API、模板化和發布的改進，以及對外掛程式行為、Netty、OkHttp 和 3.2.0 中引入的啟動問題的關鍵錯誤修正。
</p>
...
<p>
一個次要版本，引入了型別化設定反序列化、新的依賴注入和 HTMX 模組、Gradle 版本目錄支援和 suspend 模組支援。欲了解更多資訊，請參閱 <Links href="/ktor/whats-new-320" summary="undefined">Ktor 3.2.0 中的新功能</Links>。
</p>
...
一個修補程式版本，包括性能改進，例如更快的位元組操作和 multipart 處理，以及更安全的 token refresh 處理。它還修正了指標中的記憶體問題，改進了標頭行為，並解決了 WebSockets、OkHttp、Apache5 和 Netty 中的 bugs，此外還更新了 JTE 以支援 Kotlin 2.1.0。
</p>
...
一個修補程式版本，將 Kotlin 更新至 2.1.20，並修正了各種問題，包括 Base64 解碼、auth token 清除、Android 伺服器啟動錯誤、WebSocket 標頭格式和 SSE session 取消。
</p>
...
一個修補程式版本，改進了日誌記錄並修正了 WebSocket 逾時處理。它修正了多個 bugs，包括 HTTP 快取不一致、表單資料複製錯誤、gzip 處理崩潰以及導致 segment pool 損壞的併發問題。
</p>
...
一個次要版本，引入了各種 SSE 功能以及擴展的 CIO 引擎和 WebSocket 支援。它增強了平台相容性、日誌記錄和身份驗證，同時修正了與位元組通道處理、HTTP 請求失敗和併發問題相關的關鍵錯誤。
</p>
...
一個修補程式版本，包含各種錯誤修正，包括修正 <code>browserProductionWebpack</code> 中的建置錯誤、gzipped 內容處理和 <code>FormFieldLimit</code> 設定覆寫。此版本還包括核心性能改進和正確的測試應用程式關閉。
</p>
...
一個修補程式版本，解決了與回應損壞、截斷的內容、連線處理和不正確的標頭相關的多個錯誤修正，以及擴展的二進位編碼支援和 Android 的性能增強。
</p>
...
一個修補程式版本，包含錯誤修正、安全修補程式和改進，包括新增對 <code>watchosDeviceArm64</code> 目標的支援。
</p>
...
一個修補程式版本，包括用戶端和伺服器日誌記錄的改進，以及各種錯誤修正。
</p>
...
一個主要版本，包含改進和錯誤修正，包括新增對 Android Native 目標的支援。有關破壞性變更的更多資訊，請參閱 <Links href="/ktor/migrating-3" summary="undefined">遷移指南</Links>。
</p>
...
一個主要 release candidate，包含各種帶有破壞性變更、錯誤修正和功能，例如針對 XML 的多平台支援。
</p>
...
一個主要 release candidate，包含顯著改進和錯誤修正。此更新增強了向後相容性並具有擴展的 <code>staticZip</code> 支援。
</p>
...
一個主要 pre-release version，包含各種改進和錯誤修正，包括 SSE 支援改進和適用於 Kotlin/Wasm 的 Ktor 用戶端。
</p>
...
一個修補程式版本，包括用於將 socket 逾時應用於測試用戶端引擎的錯誤修正。
</p>
...
一個修補程式版本，包括 CallLogging 和 SSE 伺服器外掛程式的各種錯誤修正，改進的 Android 用戶端日誌記錄等。
</p>
...
一個修補程式版本，包括 ContentNegotiation 用戶端外掛程式的錯誤修正，以及增加了對透過 HTTP 傳送安全 Cookie 的支援。
</p>
...
<p>
一個修補程式版本，包括 ContentNegotiation、WebSockets 中的錯誤修正，以及 Native Server 中的記憶體使用量。
</p>
...
<p>
一個主要 pre-release version，包含各種改進和錯誤修正，包括用戶端和伺服器 SSE 支援。
</p>
...
<p>
一個修補程式版本，包括對 <code>linuxArm64</code> 的用戶端和伺服器支援以及各種錯誤修正。
</p>
...
<p>
一個功能發行版本，增加了對多個設定檔、路由中的正規表達式模式等的支援。
</p>
...
<p>
一個修補程式版本，包含 HTTP 用戶端、路由和內容協商中的各種錯誤修正。
</p>
...
<p>
一個修補程式版本，包括針對 OAuth2 的多平台功能以及各種錯誤修正。
</p>
...
<p>
一個多功能發行版本，包括 Swagger UI 託管、新的外掛程式 API、針對 Sessions 的多平台支援等。欲了解更多資訊，請參閱 <Links href="/ktor/migration-to-22x" summary="undefined">從 2.0.x 遷移到 2.2.x</Links> 指南。
</p>
...
<p>
一個修補程式版本，包括路由、測試引擎和 Ktor 用戶端中的錯誤修正。
</p>
...
<p>
一個修補程式版本，包含 Ktor 用戶端和伺服器中的各種錯誤修正。
</p>
