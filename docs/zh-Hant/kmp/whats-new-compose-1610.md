...
## 重大變更：新的 Compose 編譯器 Gradle 外掛程式 {id="breaking-change-new-compose-compiler-gradle-plugin"}
...
#### 穩定資源程式庫 {id="stable-resource-library"}

[資源程式庫 API](compose-multiplatform-resources.md) 的大部分內容現在已被視為穩定。

#### 支援多模組專案與 Compose Multiplatform 資源 {id="support-for-multimodule-projects-with-compose-multiplatform-resources"}
...
#### 多平台資源的配置 DSL {id="configuration-dsl-for-multiplatform-resources"}
...
#### 用於產生資源 URI 的公開函式 {id="public-function-for-generating-resource-uri"}
...
### 實驗性導覽程式庫 {id="experimental-navigation-library"}
...
### 支援 Kotlin 2.0.0 {id="support-for-kotlin-2-0-0"}
...
### BasicTextField2 的基礎支援 {id="basic-support-of-basictextfield2"}
...
### DialogWindow 的 alwaysOnTop 旗標 {id="alwaysontop-flag-for-dialogwindow"}
...
### 無障礙功能支援改進 {id="accessibility-support-improvements"}
...
### iOS 17 及更高版本的選取容器放大鏡 {id="selection-container-magnifier-for-ios-17-and-higher"}

iOS 上的 Compose Multiplatform 選取容器現在會模擬原生的放大工具。

![iPhone 聊天應用程式在文字放大鏡啟動時的螢幕截圖](compose-1610-ios-magnifier.png){width=390}

### 用於 Dialog 居中的軟體鍵盤內嵌 (inset) {id="software-keyboard-inset-for-dialog-centering"}
...
### Alpha 階段的 Kotlin/Wasm 支援 {id="kotlin-wasm-support-in-alpha"}
...
### 基礎 IME 鍵盤支援 {id="basic-ime-keyboard-support"}

Compose Multiplatform 的 Web 目標現在對虛擬 (IME) 鍵盤提供基礎支援。

## Gradle 外掛程式 {id="gradle-plugin"}

### 可以修改 macOS 最低版本 {id="possibility-to-modify-the-macos-minimum-version"}
...
### 支援建立具有 Proguard 支援的 uber JAR {id="option-to-create-uber-jars-with-proguard-support"}
...
### 已知問題：MissingResourceException {id="known-issue-missingresourceexception"}
