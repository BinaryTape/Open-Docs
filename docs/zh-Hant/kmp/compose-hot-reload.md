...
> 從 Compose Multiplatform 1.10.0 開始，您不再需要單獨設定 Compose Hot Reload 外掛程式，因為它已[捆綁](whats-new-compose-110.md#compose-hot-reload-integration)並預設為包含桌面目標的專案啟用。然而，您仍然可以明確宣告 Compose Hot Reload 外掛程式以使用特定版本。
>
{style="note"}

1.  在您的專案中，使用最新版本的 Compose Hot Reload 更新版本目錄（請參閱 [Releases](https://github.com/JetBrains/compose-hot-reload/releases)）。
...
2.  在您的父專案的 `build.gradle.kts` (`ComposeDemo/build.gradle.kts`) 中，將以下程式碼新增到您的 `plugins {}` 區塊：
    ```kotlin
    plugins {
