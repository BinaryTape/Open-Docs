...
### Kotlin 2.0.0 需要新的 Compose 編譯器 Gradle 外掛程式 {id="new-compose-compiler-gradle-plugin-is-required-for-kotlin-2-0-0"}
...
#### 穩定的資源程式庫 {id="stable-resource-library"}

[資源程式庫 API](compose-multiplatform-resources.md) 的大部分內容現在已被視為穩定。

#### 支援具備 Compose Multiplatform 資源的多模組專案 {id="support-for-multimodule-projects-with-compose-multiplatform-resources"}
...
#### 用於產生資源 URI 的公有函式 {id="public-function-for-generating-resource-uri"}
...
### 無障礙支援改進 {id="accessibility-support-improvements"}
...
### 用於對話方塊居中的軟體鍵盤內距 {id="software-keyboard-inset-for-dialog-centering"}
...
### 建立支援 ProGuard 的 uber JAR 選項 {id="option-to-create-uber-jars-with-proguard-support"}
