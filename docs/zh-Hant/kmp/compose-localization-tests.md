...
## 在不同平台上測試地區 {id="testing-locales-on-different-platforms"}
...
## 關鍵測試場景 {id="key-testing-scenarios"}

### 自訂地區 {id="custom-locale"}

* 以程式化方式覆寫地區。
* 斷言 UI 元件、格式化字串和配置能針對所選地區正確調整，包括處理從右到左 (RTL) 的文字（如果適用）。

### 預設資源 {id="default-resources"}
...
### 特定地區案例 {id="locale-specific-cases"}
