...
### 我可以使用 Kotlin Multiplatform 共享 UI 程式碼嗎？ {id="can-i-share-ui-code-using-kotlin-multiplatform"}
...
### 我應該在哪個 IDE 中開發我的跨平台應用程式？ {id="in-which-ide-should-i-work-on-my-cross-platform-app"}
...
### 我有一個現有的 Android 應用程式。該如何將其遷移到 Kotlin Multiplatform？ {id="i-have-an-existing-android-application-how-can-i-migrate-it-to-kotlin-multiplatform"}
...
### 在哪裡可以找到完整的範例來嘗試？ {id="where-can-i-get-complete-examples-to-play-with"}

這裡有[實際範例清單](multiplatform-samples.md)。

### 在哪裡可以找到實際的 Kotlin Multiplatform 應用程式清單？哪些公司在生產環境中使用 KMP？ {id="where-can-i-find-a-list-of-real-life-kotlin-multiplatform-applications-what-companies-use-kmp-in-production"}

查看我們的[案例研究清單](https://kotlinlang.org/case-studies/?type=multiplatform)，向其他已經在生產環境中採用 Kotlin Multiplatform 的公司學習。

### 哪些作業系統可以配合 Kotlin Multiplatform 使用？ {id="which-operating-systems-can-work-with-kotlin-multiplatform"}
...
### 什麼是 Kotlin/Native，它與 Kotlin Multiplatform 有什麼關係？ {id="what-is-kotlin-native-and-how-does-it-relate-to-kotlin-multiplatform"}
...
### 如何加快我的 Kotlin Multiplatform 模組在原生平台（iOS、macOS、Linux）上的編譯速度？ {id="how-can-i-speed-up-my-kotlin-multiplatform-module-compilation-for-native-platforms-ios-macos-linux"}
...
### 它與 Android 的 Jetpack Compose 有什麼關係？ {id="how-does-it-relate-to-jetpack-compose-for-android"}
...
### 我可以在哪些平台之間共享 UI？ {id="between-which-platforms-can-i-share-my-ui"}
...
### 我應該使用什麼 IDE 來建置 Compose Multiplatform 應用程式？ {id="what-ide-should-i-use-for-building-apps-with-compose-multiplatform"}
...
### 我可以嘗試展示應用程式嗎？在哪裡可以找到它？ {id="can-i-play-with-a-demo-application-where-can-i-find-it"}

您可以嘗試我們的[範例](multiplatform-samples.md)。

### Compose Multiplatform 是否附帶小工具？ {id="does-compose-multiplatform-come-with-widgets"}
...
### 我可以在現有的 Kotlin Multiplatform 應用程式中共享 UI 嗎？ {id="can-i-share-the-ui-in-my-existing-kotlin-multiplatform-app"}
...
### 我有一個現有的 Android 應用程式使用 Jetpack Compose。我應該做些什麼來將它遷移到其他平台？ {id="i-have-an-existing-android-application-that-uses-jetpack-compose-what-should-i-do-to-migrate-it-to-other-platforms"}
...
<!-- Need to revise
### 當我的行動作業系統更新並引入新的平台功能時會發生什麼事？ {id="what-happens-when-my-mobile-os-updates-and-introduces-new-platform-capabilities"}
...
### 當我的行動作業系統更新並更改系統元件的視覺風格或行為時會發生什麼事？ {id="what-happens-when-my-mobile-os-updates-and-changes-the-visual-style-of-the-system-components-or-their-behavior"}

作業系統更新後，您的 UI 將保持不變，因為所有元件都是在畫布（canvas）上繪製的。如果您在畫面中嵌入了原生 iOS 元件，更新可能會影響它們的外觀。

## 未來計畫 {id="future-plans"}

### Kotlin Multiplatform 的演進計畫是什麼？ {id="what-are-the-plans-for-the-kotlin-multiplatform-evolution"}
...
### Compose Multiplatform 何時會達到穩定版？ {id="when-will-compose-multiplatform-become-stable"}
...
### 關於 Kotlin 和 Compose Multiplatform 對 Web 目標的未來支援如何？ {id="what-about-future-support-for-web-targets-in-kotlin-and-compose-multiplatform"}
...
### 有任何改進多平台開發工具的計畫嗎？ {id="are-there-any-plans-to-improve-tooling-for-multiplatform-development"}

是的，我們敏銳地意識到目前多平台工具面臨的挑戰，並正積極在多個領域進行增強。

### 你們會提供 Swift 互通性嗎？ {id="are-you-going-to-provide-swift-interoperability"}
