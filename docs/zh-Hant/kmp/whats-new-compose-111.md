[//]: # (title: Compose Multiplatform 1.11.1 的新功能)
...
### 非 Android 目標的著色器（Shader）包裝類別 {id="shader-wrapper-for-non-android-targets"}
...
### iOS 目標支援變更 {id="changes-to-ios-target-support"}
...
### Compose UI 測試 v2 {id="compose-ui-tests-v2"}
...
### 預設啟用並行渲染 {id="concurrent-rendering-enabled-by-default"}
...
### Web 目標上的捲動已與原生 UI 保持一致 {id="scroll-on-web-targets-brought-in-line-with-native-ui"}
