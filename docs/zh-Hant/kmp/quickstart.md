...
透過本教學，您可以建立並執行一個簡單的 Kotlin Multiplatform 應用程式。

## 設定環境

Kotlin Multiplatform (KMP) 專案需要特定的環境，但大多數需求透過 IDE 中的預先檢查 (preflight checks) 即可明確了解。

首先準備 IDE 和必要的 外掛程式：

1. 選擇並安裝 IDE。
    IntelliJ IDEA 與 Android Studio 均支援 Kotlin Multiplatform，因此您可以使用偏好的 IDE。
...
2. 安裝 [Kotlin Multiplatform IDE 外掛程式](https://plugins.jetbrains.com/plugin/14936-kotlin-multiplatform)
    （請勿與 Kotlin Multiplatform Gradle 外掛程式 混淆）。
    
3. 為 IntelliJ IDEA 安裝 Kotlin Multiplatform IDE 外掛程式 時，若尚未安裝所有必要的相依性，系統也會一併安裝（Android Studio 已內建所有必要的 外掛程式）。
    
4. 若您尚未設定 `ANDROID_HOME` 環境變數，請配置您的系統以識別它：
...
5. 選擇您希望作為專案一部分的平台：
    * 所有目標平台都可以設定為使用 Compose Multiplatform，從一開始就共用 UI 程式碼（不包含 UI 程式碼的 server 模組除外）。
    * 對於 iOS，您可以選擇兩種實作方式之一：
        * 使用 Compose Multiplatform 共用 UI 程式碼。
        * 使用 SwiftUI 製作完全原生的 UI，並連接到包含共用邏輯的 Kotlin 模組。
    * 桌面版目標包含 [Compose Hot Reload](compose-hot-reload.md) 功能，讓您在儲存程式碼變更時能立即看到 UI 的變化。
      即使您不打算製作桌面應用程式，也可以使用桌面版本來加速 UI 程式碼的編寫。

完成平台選擇後，點擊 **Create** 按鈕，並等待 IDE 產生並匯入專案。
...
4. 選擇您希望作為專案一部分的平台：
    ...
    * 對於 iOS，您可以選擇兩種實作方式之一： 
      * 使用 Compose Multiplatform 共用 UI 程式碼。
      * 使用 SwiftUI 製作完全原生的 UI，並連接到包含共用邏輯的 Kotlin 模組。  
    * 桌面版目標包含 Beta 版的 [Compose Hot Reload](compose-hot-reload.md) 功能，讓您在修改對應程式碼時能立即看到 UI 的變化。
      即使您不打算製作桌面應用程式，也可以使用桌面版本來加速 UI 程式碼的編寫。
5. 產生專案後，我們建議選擇一個版本的 [JetBrains Runtime](https://github.com/JetBrains/JetBrainsRuntime) (JBR) 作為專案的 JDK，因為它提供了重要的修正，特別是改善了桌面版 KMP 應用程式的相容性。
   每個 IntelliJ IDEA 發行版本中都包含相關版本的 JBR，因此無需額外設定。

完成平台選擇後，點擊 **Finish** 按鈕，並等待 IDE 產生並匯入專案。
...
* 關於處理共用 UI 程式碼的一系列 教學：[建立您的 Compose Multiplatform 應用程式](compose-multiplatform-create-first-app.md)
* 關於在原生 UI 中處理共用程式碼的一系列 教學：[建立您的 Kotlin Multiplatform 應用程式](multiplatform-create-first-app.md)
