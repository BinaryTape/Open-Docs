...
[Amper](https://amper.org) 是由 JetBrains 建立的新工具，旨在協助您設定專案的建置、封裝、發佈等。透過 Amper，您可以花更少的時間處理建置系統，進而專注於解決真正的業務挑戰。
...
透過 Amper，您可以設定平台特定的應用程式和共用的 Kotlin 程式庫。這些內容會使用特殊的宣告式 DSL，在 `module.yaml` 指令清單檔案中宣告為模組。
