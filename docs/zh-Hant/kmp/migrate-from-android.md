...
在原始的 Android Jetcaster 範例中，存在僅限 Java 的呼叫，例如 `Objects.hash()` 與 `Uri.encode()`，以及對 `java.time` 封裝（package）的大量使用。
...
很難提前識別所有此類情況，因此請準備好在遷移過程中尋找替代品或重寫程式碼。這就是為什麼我們展示如何以盡可能小的步驟從一個工作狀態移動到另一個狀態。這樣，當許多部分同時變動時，單一問題就不會耽誤您的進度。
...
為了順利過渡到 Compose Multiplatform，您的 UI 程式碼應該已經使用 Compose 編寫。如果您目前正在使用 View，您需要在新典範中並使用新架構重寫該程式碼。
顯然，如果提前完成這項工作會更容易。

Google 長期以來一直致力於推進與豐富 Compose。請查看 [Jetpack Compose 遷移指南](https://developer.android.com/develop/ui/compose/migrate)以獲取常見場景的協助。
...
但在 Jetcaster 範例中，最阻礙我們直接將程式碼通用化的是 `java.time` 封裝。
