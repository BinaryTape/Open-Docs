...
## 發佈至本機 Maven 存儲庫 {id="publishing-to-a-local-maven-repository"}
...
## 發佈結構 {id="structure-of-publications"}
...
### 目標特定與根發佈 {id="target-specific-and-root-publications"}
...
### 發佈完整的程式庫 {id="publishing-a-complete-library"}
...
## 主機需求 {id="host-requirements"}

Kotlin/Native 支援交叉編譯，允許任何主機產生必要的 `.klib` 構件。然而，仍有一些限制需要注意。

### Apple 目標的編譯 {id="compilation-for-apple-targets"}
...
### 重複發佈 {id="duplicating-publications"}

為了避免在存儲庫中重複發佈，請從單一主機發佈所有構件。例如，Maven Central 明確禁止重複發佈，如果建立重複發佈，程序將會失敗。

## 發佈 Android 程式庫 {id="publish-an-android-library"}
...
## 停用原始碼發佈 {id="disable-sources-publication"}
