...
無需手動調整它們的版本。
請記住，使用任何產品的早期體驗體計劃 (EAP) 版本仍可能不穩定。
 ...
 * 僅在以至少 Kotlin 2.1.0 編譯的情況下，才依賴於以 Compose Multiplatform 為基礎的庫，
 * 對於針對支援快速發展平台（如 iOS 和 Web）的專案，請升級至 Kotlin **2.2.20**。
 
作為在所有相依性更新之前的回溯相容性問題規避方法，您可以透過在 `gradle.properties` 檔案中加入 `kotlin.native.cacheKind=none` 來關閉 Gradle 快取。
這將確保與舊庫的相容性，但會增加編譯時間。
