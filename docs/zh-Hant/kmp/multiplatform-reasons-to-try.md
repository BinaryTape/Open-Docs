...
百度團隊決定嘗試 Kotlin Multiplatform，首先從統一數據層開始：數據模型、RESTful API 請求、JSON 資料解析和快取邏輯。

接著，他們決定採用 Model-View-Intent (MVI) 使用者介面模式，這讓您可以使用 Kotlin Multiplatform 統一介面邏輯。他們還共用了底層數據、處理邏輯和 UI 處理邏輯。
...
2) [共用邏輯同時保留原生 UI](multiplatform-create-first-app.md)：如果平台特定的視覺行為或 UX 還原度是首要任務，您可以選擇僅共用數據和商業邏輯。透過這種結構，每個平台都保留其原生 UI 層，同時受益於共同且一致的邏輯實作。這種方法非常適合希望減少重複工作而不改變現有 UI 工作流程的團隊。
...
如您所見，Kotlin Multiplatform 已經成功被許多大型公司用於構建具有原生外觀 UI 的高性能跨平台應用程式，有效地在它們之間重用程式碼，同時維持原生程式設計的優勢。