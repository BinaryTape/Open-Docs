...
4. 在 **New Project** 視窗中指定以下欄位：
    ...
    * **JDK**: Amazon Corretto version 17
        > 為了讓你稍後新增的其中一個測試能成功執行，需要此 JDK 版本。
        >
        {style="note"}
...
在此目錄中，建立一個 Kotlin 檔案 `Grep.kt`，並包含以下函式：
...
1. 在 `shared/build.gradle.kts` 檔案中，檢查是否已有對 `kotlin.test` 程式庫的相依性：
...
2. `commonTest` 原始碼集儲存所有共通測試。你需要在專案中建立一個同名的目錄：
    ...
    1. 以右鍵點擊 `shared/src` 目錄，然後選擇 **New | Directory**。IDE 會顯示選項清單。
...
2. 在此目錄中，建立 `CurrentRuntime.kt` 檔案並使用以下實作進行更新：
...
3. 在 `shared/src` 目錄中建立測試目錄：
 
   1. 以右鍵點擊 `shared/src` 目錄，然後選擇 **New | Directory**。IDE 會顯示選項清單。
   2. 開始輸入 `androidUnitTest/kotlin` 路徑以縮小選擇範圍，然後從清單中選擇它：

   ![建立 Android 測試目錄](create-android-test-dir.png){width=350}

4. 在 `kotlin` 目錄中，建立一個新的 `org.kmp.testing` 封裝。
5. 在此封裝中，建立 `AndroidRuntimeTest.kt` 檔案並使用以下 Android 測試進行更新：
   ...
   > 如果你在本教學開始時選擇了不同的 JDK 版本，你可能需要更改 `name` 和 `version` 才能使測試成功執行。
   > 
   {style="note"}
...
3. 在 `shared/src` 目錄中建立一個新目錄：
   ...
   ![建立 iOS 測試目錄](create-ios-test-dir.png){width=350}
...
執行測試時，除了 IDE 中的輸出外，還會產生 HTML 報告。你可以在 `shared/build/reports/tests` 目錄中找到它們：
