...
`library` 模組的結構旨在容納共享程式碼以及平台特定的實作。以下是其主要原始碼 (`src`) 內容的分解：
...
透過使用 [`expect`/`actual` 機制](multiplatform-connect-to-apis.md#expected-and-actual-functions-and-properties)來配置對 [Kotlin/Wasm](https://kotlinlang.org/docs/wasm-overview.html) 平台的支援。您可以為 `firstElement` 與 `secondElement` 屬性實作平台特定的功能。
...
2. 找到 `mavenPublishing { }` 區塊，並註解掉 `signAllPublications()` 方法，以表示該發佈僅限本機：
...
要找到您發佈的程式庫，請使用您的檔案管理員或終端，並導覽至使用者家目錄中的 `.m2\repository\io\github\kotlin\library\1.0.0\`。

## 將您的程式庫作為相依性新增至另一個專案中

將多平台程式庫發佈到本機 Maven 存儲庫後，您就可以在同一部電腦上的其他 Kotlin 專案中使用它。

在您取用端專案的 `build.gradle.kts` 檔案中，新增對已發佈程式庫的相依性：
...
`repositories{}` 區塊告訴 Gradle 從本機 Maven 存儲庫解析該程式庫，並使其在共享程式碼中可用。

`implementation` 相依性由您程式庫在 `build.gradle.kts` 檔案中指定的群組與版本組成。

如果您要將其新增至另一個多平台專案，可以將其新增至共享或平台特定的原始碼集：
